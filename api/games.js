...
function sanitizeGameForDb(game = {}) {
  ...
  for (const [key, value] of Object.entries(src)) {
    if (GAME_DB_COLUMNS.has(key)) out[key] = value;
  }
  return out;
}
...
module.exports = async (req, res) => {
  try {
    ...
    if (req.method === 'POST' || req.method === 'PUT') {
      ...
      const dbGame = sanitizeGameForDb(game);
      let { data, error } = await supabase.from('games').upsert(dbGame, { onConflict:'slug' }).select().single();
      // Eski Supabase schema çalıştırılmadıysa bilinmeyen kolon yüzünden panelin bozulmasını engelle.
      if (error && /column|schema cache|upcoming_start|thumbnail|could not find/i.test(String(error.message||''))) {
        const retry = sanitizeGameForDb(game);
        delete retry.upcoming_start;
        delete retry.thumbnail;
