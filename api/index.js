...
async function fetchRawgDetails(slug, key){
  ...
  const data = await response.json(); console.log('YT_DEBUG',{playlistId,hasKey:!!key,url,data});
  ...
  if(key){
    ...
    for(let page=0; page<20; page++){
      ...
      if(!response.ok){
        ...
        throw new Error(`YouTube API hata verdi. Status=${response.status} Cevap=${errText}`);
      }
      const data = await response.json(); console.log('YT_DEBUG',{playlistId,hasKey:!!key,url,data});
