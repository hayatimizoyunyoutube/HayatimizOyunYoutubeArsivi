...
const FEATURE_CATALOG = [
  ...
  { key:'game_auto_meta_fetch', title:'Oyun adından tür, etiket ve açıklama otomatik çekme', group:'Siteye Gelmesi Gerekenler', next:'Oyun düzenleme formunda otomatik meta yenile butonu ekle', target:'Yönetim Paneli > Oyunlar', description:'Oyun adından tür, etiket ve kapak önerisi doldurur.' },
  { key:'feature_edit_delete', title:'Akıllı özelliklerde düzenleme ve silme sistemi', group:'Eklenen Özellikler', next:'Özellik geçmişi ve geri alma ekranı ekle', target:'Yönetim Paneli > Özellik Planı', description:'Özellik kartlarında düzenle/sil işlemlerini açar.' },
  { key:'apply_refresh_flow', title:'Siteye Uygula + Siteyi Yenile akışını aktif et', group:'Eklenen Özellikler', next:'Uygulama sonrası kontrol raporu göster', target:'Yönetim Paneli > Özellik Planı', description:'Özellik uygulandıktan sonra panel verilerini yeniler.' },
...
async function rawgMeta(title){
  const rawgKey = process.env.RAWG_API_KEY || '';
  if(!rawgKey) throw new Error('RAWG_API_KEY yok.');
  const url = `https://api.rawg.io/api/games?key=${encodeURIComponent(rawgKey)}&search=${encodeURIComponent(title)}&page_size=1`;
  const response = await fetch(url, { headers:{ 'Accept':'application/json' } });
  if(!response.ok) throw new Error(`RAWG HTTP ${response.status}`);
  const data = await response.json();
  const game = data?.results?.[0];
  if(!game) throw new Error('RAWG sonucu yok.');
  const genres = (game.genres || []).map(g=>g.name).slice(0,2).join(' / ') || 'Genel / Hikaye';
  const tags = (game.tags || []).map(t=>t.name).slice(0,5).join(', ') || (game.genres || []).map(g=>g.name).join(', ') || 'oyun, seri';
  const cover = game.background_image || '';
  const score = game.rating ? Math.min(10, Number(game.rating) * 2).toFixed(1) : 8.5;
  return { genre:genres, tags, cover, score:Number(score), source:'rawg' };
}
async function bestMeta(title){
  try{ return await rawgMeta(title); }
  catch(e){ const [genre,tags,cover] = localGameMeta(title); return { genre, tags, cover, score:8.5, source:'fallback' }; }
}
...
export default async function handler(req, res){
  ...
  try{
    if(action === 'health') return json(res, 200, { ok:true, version:'v2.1.4.5' });
    ...
    if(action === 'planner-edit'){
      ...
      if(id && !id.startsWith('local-')){
        await supabase(`site_admin_planner?id=eq.${encodeURIComponent(id)}`, { method:'PATCH', body: JSON.stringify({ title, updated_at:new Date().toISOString() }) }).catch(()=>{});
      }else if(key){
        await supabase(`site_admin_planner?feature_key=eq.${encodeURIComponent(key)}`, { method:'PATCH', body: JSON.stringify({ title, updated_at:new Date().toISOString() }) }).catch(()=>{});
      }
      return json(res, 200, { ok:true });
    }
    ...
    if(action === 'planner-delete'){
      ...
      if(key){
        await supabase(`site_features?key=eq.${encodeURIComponent(key)}`, { method:'PATCH', body: JSON.stringify({ enabled:false, updated_at:new Date().toISOString() }) }).catch(()=>{});
        await supabase(`site_admin_planner?feature_key=eq.${encodeURIComponent(key)}`, { method:'DELETE', headers:{ Prefer:'return=minimal' } }).catch(()=>{});
      }
      if(id && !id.startsWith('local-')){
        await supabase(`site_admin_planner?id=eq.${encodeURIComponent(id)}`, { method:'DELETE', headers:{ Prefer:'return=minimal' } }).catch(()=>{});
      }
      return json(res, 200, { ok:true });
    }

    if(action === 'game-auto-meta'){
      await requireStaff(body.adminToken);
      const title = String(body.title || '').trim();
      if(!title) throw new Error('Oyun adı gerekli.');
      const meta = await bestMeta(title);
      return json(res, 200, { ok:true, ...meta });
    }

    if(action === 'games-auto-cover'){
      await requireStaff(body.adminToken);
      const games = await supabase('games?select=id,title,genre,status,episode_count,score,cover_url,tags,created_at,updated_at&order=created_at.desc', { method:'GET' }).catch(()=>[]);
      let changed = 0;
      for(const game of games || []){
        if(game.cover_url) continue;
        const meta = await bestMeta(game.title || game.name || 'oyun');
        if(meta.cover){
          changed += 1;
          await supabase(`games?id=eq.${encodeURIComponent(game.id)}`, { method:'PATCH', body: JSON.stringify({ cover_url:meta.cover, genre:game.genre || meta.genre, tags:game.tags || meta.tags, updated_at:new Date().toISOString() }) }).catch(()=>{});
        }
      }
      const rows = await supabase('games?select=id,title,genre,status,episode_count,score,cover_url,tags,created_at,updated_at&order=created_at.desc', { method:'GET' }).catch(()=>games || []);
      return json(res, 200, { ok:true, changed, games:(rows || []).map(cleanGame) });
    }
