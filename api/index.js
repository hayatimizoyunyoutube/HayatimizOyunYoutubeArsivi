...
export default async function handler(req, res){
  ...
  try{
    ...
    if(action === 'runtime-config-upsert'){
      await requireStaff(body.adminToken);
      const key = String(body.key || '').trim();
      if(!key) throw new Error('Runtime ayar anahtarı gerekli.');
      ...
      return json(res, 200, { ok:true, key, value, row:Array.isArray(rows) ? rows[0] : null });
    }
    ...
    if(action === 'settings-set' || action === 'maintenance-save' || action === 'runtime-config-set'){
      ...
      rowsPayload.push({ key:'schema_version', value:{ version:'v2.5.4', note:'Bakım Modu V2 ve Yayın Takvimi Kalıcı Kayıt', updated_at:now }, updated_at:now });
    ...
    if(action === 'score-check'){
      ...
      if(!clean.length){ const known = ho247f8ApiKnownScore(title); if(known) return json(res, 200, { ok:true, score:known.score, averageScore:known.score, sources:[{source:known.source, score:known.score}], message:'Katalog puanı bulundu.', version:'v2.4.7 FIX8' }); return json(res, 200, { ok:false, score:'', averageScore:'', sources:[], message:'Steam/Google/Epic/Ubisoft puanı bulunamadı.' }); }
      const avg = clean.reduce((a,b)=>a+Number(b.score),0)/clean.length;
      return json(res, 200, { ok:true, score:Number(avg.toFixed(1)), averageScore:Number(avg.toFixed(1)), sources:clean.slice(0,8), version:'v2.4.7' });
    }
