...
export default async function handler(req, res){
  ...
  try{
    if(action === 'health') return json(res, 200, { ok:true, version:'v2.1.2' });
    ...
    if(action === 'games-list'){
      const rows = await supabase('games?select=id,title,genre,status,episode_count,score,cover_url,tags,release_date,rawg_slug,series_name,playlist_url,description,created_at,updated_at&order=created_at.desc', { method:'GET' }).catch(()=>[]);
