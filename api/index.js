...
export default async function handler(req, res){
  ...
  try{
    ...
    if(action === 'calendar-events-upsert'){
      ...
      return json(res, 200, { ok:true, event:{ id:row.id, title:row.title, date:row.event_date, time:row.event_time, type:row.event_type, gameId:row.game_id, gameTitle:row.game_title, episodeNumber:row.episode_number, episodeTitle:row.episode_title, cover:row.cover_url, note:row.note } });
    }
