...
export default async function handler(req, res){
  ...
  try{
    if(action === 'health') return json(res, 200, { ok:true, version:'v2.0.4' });
    ...
    if(action === 'runtime-config-upsert' || action === 'settings-set' || action === 'maintenance-save' || action === 'runtime-config-set'){
      ...
      const rows = await supabase('site_runtime_config?on_conflict=key', {
        ...
        body: JSON.stringify([
          ...
          { key:'schema_version', value:{ version:'v2.0.4', note:'Dolu eski taban geri dönüş', updated_at:now }, updated_at:now }
        ])
      });
