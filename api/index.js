...
function requireAdmin(body) {
  if (!verifyAdminToken(body.adminToken)) throw new Error('Admin yetkisi gerekiyor. Admin Girişi ekranından tekrar giriş yap.');
}
...
async function supabaseFetch(path, options = {}) {
  ...
  if (!response.ok) {
    ...
    if (details.includes('schema cache') || details.includes('PGRST205') || details.includes('Could not find the table')) {
      throw new Error('Supabase tabloyu schema cache içinde göremiyor. supabase/RUN-FIRST-v213-fix3-authority-maintenance.sql dosyasını çalıştır, 20-30 saniye bekle, sonra Vercel redeploy yap.');
    }
...
export default async function handler(req, res) {
  ...
  if (action === 'health' && req.method === 'GET') {
    return json(res, 200, {
      ok: true,
      version: 'v2.1.3-fix-3-authority-maintenance',
  ...
  try {
    ...
    if (action === 'admin-login') {
      ...
      return json(res, 200, { ok: true, session: { name: 'Admin', email: 'admin@hayatimizoyun.local', role: 'admin', adminToken: createAdminToken('admin@hayatimizoyun.local') } });
    }
    ...
    if (action === 'login') {
      ...
      if (session.role === 'admin') session.adminToken = createAdminToken(user.email);
    ...
    if (action === 'user-set-role') {
      requireAdmin(body);
      const role = ['user', 'admin', 'moderator'].includes(String(body.role)) ? String(body.role) : 'user';
    ...
    if (action === 'user-ban') {
      requireAdmin(body);
      await supabaseFetch(`site_users?id=eq.${encodeURIComponent(body.userId)}`, { method: 'PATCH', body: JSON.stringify({ is_active: false, banned_at: new Date().toISOString(), ban_reason: String(body.reason || 'Admin panelinden banlandı').slice(0, 200), updated_at: new Date().toISOString() }) });
      return json(res, 200, { ok: true, users: await listUsers() });
    }

    if (action === 'user-unban') {
      requireAdmin(body);
      await supabaseFetch(`site_users?id=eq.${encodeURIComponent(body.userId)}`, { method: 'PATCH', body: JSON.stringify({ is_active: true, banned_at: null, ban_reason: null, updated_at: new Date().toISOString() }) });
      return json(res, 200, { ok: true, users: await listUsers() });
    }
