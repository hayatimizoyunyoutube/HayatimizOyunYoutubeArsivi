...
export default async function handler(req, res){
  ...
  try{
    ...
    if(action === 'settings-set' || action === 'maintenance-save' || action === 'runtime-config-set'){
      ...
      const maintenance = body.maintenance || body.value || { enabled:false, message:'Hayatımız Oyun kısa süreli bakımda.', eta:'' };
      const now = new Date().toISOString();
      const rows = await supabase('site_runtime_config?on_conflict=key', { method:'POST', headers:{ Prefer:'resolution=merge-duplicates,return=representation' }, body: JSON.stringify([{ key:'maintenance_mode', value:maintenance, updated_at:now }, { key:'schema_version', value:{ version:'v2.4.8 FIX2', note:'Bakım modu Supabase değerini koruma ve 6 kapak önerisi sistemi', updated_at:now }, updated_at:now }]) });
      return json(res, 200, { ok:true, maintenance, rows });
    }
