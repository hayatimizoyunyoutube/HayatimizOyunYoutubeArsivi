...
function gamePayload(game={}, existing={}){
  ...
  const seriesName=String(game.seriesName || game.series_name || existing.series_name || '');
  const collectionName=String(game.collectionName || game.collection_name || seriesName || genre || 'Genel Koleksiyon');
...
export default async function handler(req, res){
  ...
  try{
    if(action === 'health') return json(res, 200, { ok:true, version:'v2.1.2', status:'Public Yayın Öncesi Stabilite aktif' });
    ...
    if(action === 'steam-meta-lite'){
      ...
      return json(res, 200, { ok:true, steam:{ ...steam, releaseDate:steamDate, released:steamDate }, source:'Steam güvenli kontrol', version:'v2.1.2' });
    }
    ...
    if(action === 'settings-get'){
      ...
      const value = { ...raw, enabled: raw.enabled === true, updatedAt: raw.updatedAt || raw.updated_at || row?.updated_at || new Date(0).toISOString(), updated_at: raw.updated_at || raw.updatedAt || row?.updated_at || new Date(0).toISOString() };
    ...
    if(action === 'runtime-config-upsert' || action === 'settings-set' || action === 'maintenance-save' || action === 'runtime-config-set'){
      ...
      const value = key === 'maintenance_mode' ? { ...rawValue, enabled: rawValue.enabled === true, adminBypass: rawValue.adminBypass !== false, updatedAt:now, updated_at:now } : rawValue;
      const rows = await supabase('site_runtime_config?on_conflict=key', {
        method:'POST',
        headers:{ Prefer:'resolution=merge-duplicates,return=representation' },
        body: JSON.stringify([
          { key, value, updated_at:now },
          { key:'schema_version', value:{ version:'v2.1.2', note:'Public yayın öncesi stabilite ve status güncellemesi', updated_at:now }, updated_at:now }
        ])
      });
