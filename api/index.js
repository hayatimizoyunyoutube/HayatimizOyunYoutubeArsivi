...
export default async function handler(req, res){
  ...
  try{
    if(action === 'health') return json(res, 200, { ok:true, version:'v2.1.8', status:'Yetki Paneli ve Kullanıcı Yönetimi aktif' });
    ...
    if(action === 'steam-meta-lite'){
      ...
      return json(res, 200, { ok:true, steam:{ ...steam, releaseDate:steamDate, released:steamDate }, source:'Steam güvenli kontrol', version:'v2.1.8' });
    }
    ...
    if(action === 'user-role-set'){
      ...
      if(email){
        await supabase('site_authority_assignments?on_conflict=email', {
          method:'POST',
          headers:{ Prefer:'resolution=merge-duplicates,return=representation' },
          body: JSON.stringify([{ email, display_name:displayName || rows?.[0]?.full_name || email.split('@')[0], role_code:role, is_active:role !== 'banned', note:'Yönetim panelinden yetki verildi', created_by:'v2.1.8-authority-panel', updated_at:now }])
        }).catch(()=>{});
        await supabase('site_user_role_audit', {
          method:'POST',
          body: JSON.stringify([{ target_email:email, new_role:role, changed_by:'v2.1.8-authority-panel', source:'yonetim-paneli', note:'Yönetim panelinden rol/yetki kaydedildi.', metadata:{ userId, displayName } }])
        }).catch(()=>{});
      }
    ...
    if(action === 'runtime-config-upsert' || action === 'settings-set' || action === 'maintenance-save' || action === 'runtime-config-set'){
      ...
      const rows = await supabase('site_runtime_config?on_conflict=key', {
        ...
        body: JSON.stringify([
          ...
          { key:'schema_version', value:{ version:'v2.1.8', note:'Profesyonel ana sayfa final cila ve status güncellemesi', updated_at:now }, updated_at:now }
        ])
      });
    ...
    if(action === 'admin-data-health'){
      ...
      await supabase('site_status_logs', { method:'POST', body: JSON.stringify([{ status:'ok', scope:'admin-data-health', message:'v2.1.8 admin veri sağlığı kontrol edildi.', details:health }]) }).catch(()=>{});
