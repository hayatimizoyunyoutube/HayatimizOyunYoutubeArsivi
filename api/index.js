...
export default async function handler(req, res){
  ...
  try{
    if(action === 'health') return json(res, 200, { ok:true, version:'v2.2.0 FIX 7' });
    ...
    if(action === 'game-requests-list'){
      await requireStaff(body.adminToken);
      const rows = await supabase('site_game_requests?select=id,game_title,series_name,requester_email,note,status,created_at,updated_at&order=created_at.desc', { method:'GET' }).catch(()=>[]);
    ...
    if(action === 'bug-reports-list'){
      await requireStaff(body.adminToken);
      const rows = await supabase('site_bug_reports?select=id,title,page_name,reporter_email,description,status,created_at,updated_at&order=created_at.desc', { method:'GET' }).catch(()=>[]);
