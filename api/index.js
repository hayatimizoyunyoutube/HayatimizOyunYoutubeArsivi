...
async function fetchRawgMeta(title){
  ...
  for(const game of (data?.results || []).slice(0,5)){
    ...
    candidates.push({
      ...
      rawg_slug:game.slug || '',
      description:localTurkishStory(game.name || title, genres) + (details?.description ? `\n\nRAWG notu: ${details.description.slice(0, 420)}${details.description.length>420?'...':''}` : '')
    });
  }
...
async function fetchYoutubePlaylistItems(playlistUrl){
  ...
  if(!key) return { count:0, episodes:[] };
  ...
  for(let page=0; page<20; page++){
    ...
    (data.items || []).forEach((item, i)=>{
      ...
      if(!videoId || String(sn.title || '').toLowerCase().includes('deleted video') || String(sn.title || '').toLowerCase().includes('private video')) return;
      const idx = episodes.length + 1;
      episodes.push({
        id:`yt-${videoId}`,
        number:idx,
        title:sn.title || `${idx}. Bölüm`,
...
export default async function handler(req, res){
  ...
  try{
    if(action === 'health') return json(res, 200, { ok:true, version:'v2.1.6 FIX' });
    ...
    if(action === 'game-story'){
      ...
      return json(res, 200, { ok:true, story: meta?.description || localTurkishStory(title, meta?.genre || '') });
    }
