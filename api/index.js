...
export default async function handler(req, res){
  ...
  try{
    ...
    if(action === 'ai-feature-apply'){
      ...
      const payload = { version:String(f.version || ''), feature_key:String(f.key || ''), title:String(f.title || ''), target:String(f.target || ''), description:String(f.description || ''), next_feature:String(f.next || ''), actor_email:user.email, created_at:new Date().toISOString() };
