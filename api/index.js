...
export default async function handler(req, res){
  ...
  try{
    ...
    if(action === 'bug-reports-list'){
      await requireStaff(body.adminToken);
      const rows = await supabase('site_bug_reports?select=id,title,page_name,reporter_email,description,status,admin_note,solution_note,created_at,updated_at&order=created_at.desc', { method:'GET' }).catch(()=>[]);
