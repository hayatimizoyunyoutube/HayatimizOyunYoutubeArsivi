...
export default async function handler(req, res){
  ...
  try{
    if(action === 'health') return json(res, 200, { ok:true, version:'v2.5.4 FIX5' });
    ...
    if(action === 'runtime-config-upsert' || action === 'settings-set' || action === 'maintenance-save' || action === 'runtime-config-set'){
      ...
      const rows = await supabase('site_runtime_config?on_conflict=key', {
        ...
        body: JSON.stringify([
          ...
          { key:'schema_version', value:{ version:'v2.5.4 FIX5', note:'Açılış öncesi son kontrol', updated_at:now }, updated_at:now }
        ])
      });
    ...
    if(action === 'score-check'){
      ...
      if(!clean.length){ const known = ho247f8ApiKnownScore(title); if(known) return json(res, 200, { ok:true, score:known.score, averageScore:known.score, sources:[{source:known.source, score:known.score}], message:'Katalog puanı bulundu.', version:'v2.5.4 FIX5' }); return json(res, 200, { ok:false, score:'', averageScore:'', sources:[], message:'Steam/Google/Epic/Ubisoft puanı bulunamadı.' }); }
      const avg = clean.reduce((a,b)=>a+Number(b.score),0)/clean.length;
      return json(res, 200, { ok:true, score:Number(avg.toFixed(1)), averageScore:Number(avg.toFixed(1)), sources:clean.slice(0,8), version:'v2.5.4 FIX5' });
    }
    ...
    if(action === 'multi-store-check'){
      ...
      return json(res, 200, { ok:true, title, result:{ ...result, title, matchScore:82, message:`${result.source} için resmi arama bağlantısı hazırlandı. Sonuçtan kapak ve çıkış tarihini manuel doğrulayabilirsin.` }, version:'v2.5.4 FIX5' });
    }
