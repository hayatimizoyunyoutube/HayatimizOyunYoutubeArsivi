  ...
  try{
    ...
    if(action === 'settings-set' || action === 'maintenance-save' || action === 'runtime-config-set'){
      ...
      if(body.key){
        ...
        if(key === 'maintenance_mode' && body.maintenance && typeof body.maintenance === 'object'){
          rowsPayload[0].value = body.maintenance;
        }
      }else if(body.maintenance){
        rowsPayload.push({ key:'maintenance_mode', value:body.maintenance, updated_at:now });
      }else{
        rowsPayload.push({ key:'maintenance_mode', value:body.value || { enabled:false, message:'Hayatımız Oyun kısa süreli bakımda.', eta:'' }, updated_at:now });
      }

      rowsPayload.push({
        key:'schema_version',
        value:{ version:'v2.5.3 FIX1', note:'Bakım modu ve güncelleme notları stabil ayar kaydı', updated_at:now },
        updated_at:now
      });
