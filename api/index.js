...
async function fetchYoutubePlaylistItemsNoKey(playlistUrl){
  ...
  const titleMatches = [...html.matchAll(/"title":\{"runs":\[\{"text":"([^"]+)"\}\]/g)].map(m=>cleanYoutubeTitle(m[1]));
  for(let i=0;i<videoMatches.length;i++){
    const videoId = videoMatches[i];
    const title = titleMatches[i] || `${i+1}. Bölüm`;
    rows.push({ videoId, title, thumbnail:`https://i.ytimg.com/vi/${videoId}/mqdefault.jpg` });
  }
...
export default async function handler(req, res){
  ...
  try{
    if(action === 'health') return json(res, 200, { ok:true, version:'v2.1.6 FIX 2' });
