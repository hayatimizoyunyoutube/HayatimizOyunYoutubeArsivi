...
export default async function handler(req, res){
  ...
  try{
    if(action === 'health') return json(res, 200, { ok:true, version:'v2.4.0 FIX 10' });
    ...
    if(action === 'ai-feature-apply'){
      const user = await requireStaff(body.adminToken);
      const f = body.feature || {};
      const payload = { version:String(f.version || ''), feature_key:String(f.key || ''), title:String(f.title || ''), target:String(f.target || ''), description:String(f.description || ''), next_feature:String(f.next || ''), actor_email:user.email, github_status:String(f.githubStatus || 'pakete işlendi'), vercel_status:String(f.vercelStatus || 'redeploy gerekli'), supabase_status:String(f.supabaseStatus || 'kaydedildi'), deployment_note:String(f.deploymentNote || 'GitHub push ve Vercel redeploy sonrası canlıya alınır.'), sql_patch:String(f.sqlPatch || ''), created_at:new Date().toISOString() };
      await supabase('site_ai_feature_applications', { method:'POST', body: JSON.stringify([payload]) }).catch(()=>{});
      await supabase('site_update_notes', { method:'POST', body: JSON.stringify([{ version:payload.version, title:payload.title, summary:payload.description, note:`AI Özellik Merkezi üzerinden uygulandı. Hedef: ${payload.target}. Sıradaki öneri: ${payload.next_feature}`, status:'published', created_at:new Date().toISOString() }]) }).catch(()=>{});
      if(payload.next_feature){ await supabase('site_admin_planner', { method:'POST', body: JSON.stringify([{ group_name:'Gelecek Versiyon Özellikleri', title:payload.next_feature, status:'plan', feature_key:`ai-${Date.now()}` }]) }).catch(()=>{}); }
      return json(res, 200, { ok:true, feature:payload });
    }



    if(action === 'ai-sql-draft-save'){
      const user = await requireStaff(body.adminToken);
      const d = body.draft || {};
      const payload = { feature_key:String(d.featureKey || d.feature_key || ''), title:String(d.title || ''), sql_text:String(d.sqlText || d.sql_text || ''), status:String(d.status || 'taslak'), actor_email:user.email, created_at:new Date().toISOString() };
      await supabase('site_ai_sql_drafts', { method:'POST', body: JSON.stringify([payload]) }).catch(()=>{});
      return json(res, 200, { ok:true, draft:payload });
    }

    if(action === 'ai-sql-draft-update'){
      await requireStaff(body.adminToken);
      const id = String(body.id || '');
      const patch = body.patch || {};
      if(id){ await supabase(`site_ai_sql_drafts?id=eq.${encodeURIComponent(id)}`, { method:'PATCH', body: JSON.stringify({ ...patch, updated_at:new Date().toISOString() }) }).catch(()=>{}); }
      return json(res, 200, { ok:true });
    }

    if(action === 'deploy-checklist-save'){
      const user = await requireStaff(body.adminToken);
      const steps = Array.isArray(body.steps) ? body.steps : [];
      for(const step of steps){
        const payload = { step_key:String(step.key || ''), label:String(step.label || ''), done:!!step.done, note:String(step.note || ''), actor_email:user.email, updated_at:new Date().toISOString() };
        await supabase('site_deploy_checklist', { method:'POST', body: JSON.stringify([payload]) }).catch(()=>{});
      }
      return json(res, 200, { ok:true, saved:steps.length });
    }
    ...
    if(action === 'ai-sql-syntax-check'){
      await requireStaff(body.adminToken);
      const sql = String(body.sql || '');
      const issues = [];
      if(!sql.trim()) issues.push('SQL boş.');
      if(sql.trim() && !/[;]\s*$/.test(sql.trim())) issues.push('SQL sonunda noktalı virgül eksik olabilir.');
      if(/\b(drop|truncate)\s+table\b/i.test(sql)) issues.push('DROP/TRUNCATE TABLE riskli komuttur.');
      if(/\bdelete\s+from\b/i.test(sql) && !/\bwhere\b/i.test(sql)) issues.push('WHERE olmayan DELETE risklidir.');
      const open = (sql.match(/\(/g)||[]).length;
      const close = (sql.match(/\)/g)||[]).length;
      if(open !== close) issues.push('Parantez sayısı eşleşmiyor.');
      return json(res, 200, { ok:true, valid:issues.length===0, issues });
    }

    if(action === 'deploy-error-diagnose'){
      const text = String(body.text || '').toLowerCase();
      const issues = [];
      if(text.includes('404') || text.includes('not_found')) issues.push('Vercel 404: Root Directory, dist output ve index.html fallback kontrol edilmeli.');
      if(text.includes('package.json')) issues.push('package.json ana dizinde değil veya yanlış klasör deploy ediliyor.');
      if(text.includes('build')) issues.push('Build log içinde npm run build hatası kontrol edilmeli.');
      if(text.includes('supabase')) issues.push('Supabase ENV veya schema.sql son sürüm kontrol edilmeli.');
      if(!issues.length) issues.push('Genel tanı: GitHub ana dizinde package.json, index.html, vercel.json, src, api, public ve supabase bulunduğunu doğrula.');
      return json(res, 200, { ok:true, issues });
    }


    if(action === 'redeploy-trigger'){
      const webhook = String(body.webhookUrl || process.env.VERCEL_DEPLOY_HOOK_URL || '').trim();
      let triggerStatus = 'webhook_yok';
      let triggerMessage = 'Deploy Hook URL yok. GitHub push sonrası Vercel otomatik deploy beklenir.';
      if(webhook){
        try{
          await fetch(webhook, { method:'POST' });
          triggerStatus = 'triggered';
          triggerMessage = 'Vercel Deploy Hook tetiklendi.';
        }catch(error){
          triggerStatus = 'failed';
          triggerMessage = error.message || String(error);
        }
      }
      await supabase('site_redeploy_requests', { method:'POST', body: JSON.stringify([{ provider:'vercel', status:triggerStatus, message:triggerMessage, version:String(body.version || ''), created_at:new Date().toISOString() }]) }).catch(()=>{});
      return json(res, 200, { ok:true, status:triggerStatus, message:triggerMessage });
    }

    if(action === 'deploy-event-save'){
      const event = body.event || {};
      const payload = { provider:String(event.provider || 'manual'), status:String(event.status || 'info'), message:String(event.message || ''), version:String(event.version || ''), created_at:new Date().toISOString() };
      await supabase('site_deploy_events', { method:'POST', body: JSON.stringify([payload]) }).catch(()=>{});
      return json(res, 200, { ok:true, event:payload });
    }

    if(action === 'schema-feedback-add'){
      const feedback = body.feedback || {};
      const payload = { version:String(feedback.version || ''), source:String(feedback.source || 'admin_panel'), message:String(feedback.message || ''), status:'new', created_at:new Date().toISOString() };
      await supabase('site_schema_feedback', { method:'POST', body: JSON.stringify([payload]) }).catch(()=>{});
      return json(res, 200, { ok:true, feedback:payload });
    }

    if(action === 'ai-feature-registry-save'){
      const features = Array.isArray(body.features) ? body.features : [];
      const version = String(body.version || '');
      const rows = features.map(f => ({ feature_key:String(f.key || ''), title:String(f.title || ''), target:String(f.target || ''), table_name:String(f.table || ''), status:String(f.status || 'tanındı'), version, updated_at:new Date().toISOString() })).filter(x=>x.feature_key);
      if(rows.length){ await supabase('site_ai_feature_registry?on_conflict=feature_key', { method:'POST', headers:{ Prefer:'resolution=merge-duplicates,return=representation' }, body: JSON.stringify(rows) }).catch(()=>{}); }
    ...
    if(action === 'auto-fix-request-add'){
      const payload = { version:String(body.version || 'v2.4.0 FIX 10'), source:String(body.source || 'admin_panel'), error_text:String(body.errorText || body.error_text || ''), diagnosis:body.diagnosis || [], status:String(body.status || 'new'), fixed_files:String(body.fixedFiles || body.fixed_files || ''), created_at:new Date().toISOString(), updated_at:new Date().toISOString() };
