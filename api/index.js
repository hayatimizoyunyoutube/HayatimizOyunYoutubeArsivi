...
async function fetchRawgMeta(title){
  ...
  if(!key) return null;
  const url = `https://api.rawg.io/api/games?key=${encodeURIComponent(key)}&search=${encodeURIComponent(title)}&page_size=1&search_precise=true`;
  const response = await fetch(url, { headers:{ 'User-Agent':'Hayatimiz-Oyun-Archive' } });
  if(!response.ok) return null;
  const data = await response.json();
  const game = data?.results?.[0];
  if(!game) return null;
  const genres = Array.isArray(game.genres) ? game.genres.map(g=>g.name).filter(Boolean).join(', ') : '';
  return { title:game.name || title, genre:genres || 'Genel', released:normalizeRawgDate(game.released), score:Number(game.rating || 0) ? Math.min(10, Number(game.rating) * 2).toFixed(1) : 8.5, cover:game.background_image || '', rawg_slug:game.slug || '' };
}
...
export default async function handler(req, res){
  ...
  try{
    if(action === 'health') return json(res, 200, { ok:true, version:'v2.1.5' });
    ...
    if(action === 'game-meta'){
      ...
      return json(res, 200, { ok:true, meta:{ ...fallback, ...(rawg || {}), releaseDate:(rawg?.released || fallback.released || '') } });
    }

    if(action === 'games-list'){
      const rows = await supabase('games?select=id,title,genre,status,episode_count,score,cover_url,tags,release_date,rawg_slug,created_at,updated_at&order=created_at.desc', { method:'GET' }).catch(()=>[]);
    ...
    if(action === 'games-update'){
      ...
      const patch = {
        ...
        rawg_slug:String(game.rawg_slug || ''),
