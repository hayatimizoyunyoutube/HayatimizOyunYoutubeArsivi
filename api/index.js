...
export default async function handler(req, res){
  ...
  try{
    if(action === 'health') return json(res, 200, { ok:true, version:'v2.5.6 Temiz Çalışan' });
    ...
    if(action === 'settings-set' || action === 'maintenance-save' || action === 'runtime-config-set'){
      await requireStaff(body.adminToken);
      const now = new Date().toISOString();
      const rowsPayload = [];
      if(body.key){
        const key = String(body.key || '').trim();
        if(!key) throw new Error('Ayar anahtarı gerekli.');
        const value = key === 'maintenance_mode' && body.maintenance ? body.maintenance : (body.value ?? {});
        rowsPayload.push({ key, value, updated_at:now });
      }else if(body.maintenance){
        rowsPayload.push({ key:'maintenance_mode', value:body.maintenance, updated_at:now });
      }else{
        rowsPayload.push({ key:'maintenance_mode', value:body.value || { enabled:false, message:'Hayatımız Oyun kısa süreli bakımda.', eta:'', percent:0, progress:0 }, updated_at:now });
      }
      rowsPayload.push({ key:'schema_version', value:{ version:'v2.5.6 Temiz Çalışan', note:'Manuel yayın takvimi ve puan önizleme düzeltmesi', updated_at:now }, updated_at:now });
      const rows = await supabase('site_runtime_config?on_conflict=key', { method:'POST', headers:{ Prefer:'resolution=merge-duplicates,return=representation' }, body: JSON.stringify(rowsPayload) });
      const maintenanceRow = rowsPayload.find(r => r.key === 'maintenance_mode');
      return json(res, 200, { ok:true, key:body.key || 'maintenance_mode', maintenance:maintenanceRow?.value || null, rows });
    }
    ...
    if(action === 'update-note-save'){
      ...
      const payload = {
        ...
        note:String(body.written || body.note || body.summary || '').trim(),
        description:String(body.written || body.description || body.summary || '').trim(),
    ...
    if(action === 'score-check'){
      ...
      if(!clean.length){ const known = ho247f8ApiKnownScore(title); if(known) return json(res, 200, { ok:true, score:known.score, averageScore:known.score, sources:[{source:known.source, score:known.score}], message:'Katalog puanı bulundu.', version:'v2.5.6 Temiz Çalışan' }); return json(res, 200, { ok:false, score:'', averageScore:'', sources:[], message:'Steam/Google/Epic/Ubisoft puanı bulunamadı.' }); }
      const avg = clean.reduce((a,b)=>a+Number(b.score),0)/clean.length;
      return json(res, 200, { ok:true, score:Number(avg.toFixed(1)), averageScore:Number(avg.toFixed(1)), sources:clean.slice(0,8), version:'v2.5.6 Temiz Çalışan' });
    }
    ...
    if(action === 'multi-store-check'){
      ...
      return json(res, 200, { ok:true, title, result:{ ...result, title, matchScore:82, message:`${result.source} için resmi arama bağlantısı hazırlandı. Sonuçtan kapak ve çıkış tarihini manuel doğrulayabilirsin.` }, version:'v2.5.6 Temiz Çalışan' });
    }
...
// v2.4.7 FIX8 - score-check güvenli fallback katalogu
