...
const required = [
  index,
  path.join(assets, 'hayatimiz-app-fix42.js'),
  path.join(assets, 'hayatimiz-app-fix40.js'),
  path.join(assets, 'hayatimiz-app-fix39.js'),
  ...
  path.join(assets, 'hayatimiz-style-fix42.css'),
  path.join(assets, 'hayatimiz-style-fix40.css'),
  path.join(assets, 'hayatimiz-style-fix39.css'),
...
for (const file of jsFiles) {
  ...
  if (!content.includes('HO240F42_INTERNAL_VERSION')) {
    console.error('FIX42 profesyonel temizlik/tarih motoru eksik:', file);
    process.exit(1);
  }
}
const html = fs.readFileSync(index, 'utf8');
if (!html.includes('hayatimiz-app-fix42.js') || !html.includes('hayatimiz-style-fix42.css')) {
  console.error('dist/index.html FIX42 assetlerini göstermiyor.');
  process.exit(1);
}
console.log('FIX42: Hazır dist kontrolü başarılı.');
console.log('FIX42: Profesyonel panel, içerik kontrol ve çıkış tarihi motoru kontrolü başarılı.');
