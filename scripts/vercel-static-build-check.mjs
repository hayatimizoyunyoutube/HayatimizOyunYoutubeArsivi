...
const required = [
  index,
  path.join(assets, 'hayatimiz-app-fix37.js'),
  ...
  path.join(assets, 'hayatimiz-style-fix37.css'),
...
for (const file of jsFiles) {
  ...
  if (!content.includes('HO240F37_INTERNAL_VERSION')) {
    console.error('FIX37 Google/İnternet DLC kapak motoru eksik:', file);
    process.exit(1);
  }
}
const html = fs.readFileSync(index, 'utf8');
if (!html.includes('hayatimiz-app-fix37.js') || !html.includes('hayatimiz-style-fix37.css')) {
  console.error('dist/index.html FIX37 assetlerini göstermiyor.');
  process.exit(1);
}
console.log('FIX37: Hazır dist kontrolü başarılı.');
console.log('FIX37: Google/İnternet DLC kapak havuzu kontrolü başarılı.');
