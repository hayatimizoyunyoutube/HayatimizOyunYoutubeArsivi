...
const required = [
  index,
  path.join(assets, 'hayatimiz-app-fix41.js'),
  ...
  path.join(assets, 'hayatimiz-style-fix41.css'),
...
for (const file of jsFiles) {
  ...
  if (!content.includes('HO240F41_INTERNAL_VERSION')) {
    console.error('FIX41 oyun formu kapak/tarih/tür motoru eksik:', file);
    process.exit(1);
  }
}
const html = fs.readFileSync(index, 'utf8');
if (!html.includes('hayatimiz-app-fix41.js') || !html.includes('hayatimiz-style-fix41.css')) {
  console.error('dist/index.html FIX41 assetlerini göstermiyor.');
  process.exit(1);
}
console.log('FIX41: Hazır dist kontrolü başarılı.');
console.log('FIX41: Oyun formu kapak, çıkış tarihi ve tür butonları kontrolü başarılı.');
