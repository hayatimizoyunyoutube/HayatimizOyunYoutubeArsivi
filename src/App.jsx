...
const VERSION = 'v2.1.3 Fix 4 Admin Navigation';
...
async function loadRuntimeSettings() {
  const response = await fetch('/api?action=settings-get', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' });
  const data = await response.json().catch(() => ({}));
...
const fixNotes = [
  ...
  'v2.1.3 Fix 3: Bakımdayken üst menüde Giriş/Kayıt yerine sadece Admin Girişi görünür; normal kullanıcı siteye geçemez.',
  ...
  return (
    <header className="topbar">
      ...
      <div className="topActions">
        ...
        {!session && maintenanceMode && <button type="button" className="adminAccessBtn" onClick={onOpenAdminLogin}><ShieldCheck size={15}/> Admin Girişi</button>}
...
function AdminUsersPanel({ session, onAction = () => {} }) {
  ...
  const loadUsers = async () => {
    ...
    try {
      ...
      setPanelError(error.message);
      onAction('Kullanıcı listesi alınamadı. Admin girişi veya Supabase schema kontrol edilmeli.');
    } finally {
  ...
  return (
    <section className="sectionBlock userTableBlock">
      ...
      <div className="userTableInfo authorityInfo">
        <article className="adminCard large"><div className="cardHeader"><UserCheck/><strong>Kayıt + Yetki Akışı</strong></div><p>Kayıt Ol formu Supabase public.site_users tablosuna yazar. Admin panelinden kullanıcıyı admin yapabilir, kullanıcıya düşürebilir, banlayabilir, banı kaldırabilir veya silebilirsin.</p><ul><li><CheckCircle2/> Yetkili yapmak için role alanı admin olmalı.</li><li><CheckCircle2/> Ban için is_active false ve banned_at dolu olur.</li><li><CheckCircle2/> Admin işlemleri adminToken ile korunur.</li></ul></article>
        ...
        <article className="adminCard"><div className="cardHeader"><ShieldCheck/><strong>Supabase’ten Manuel Yetki</strong></div><p>Table Editor → public.site_users → kullanıcının role değerini <b>admin</b>, is_active değerini <b>true</b> yap. Sonra kullanıcı çıkış/giriş yapınca admin olur.</p><button type="button" className="ghostBtn" onClick={loadUsers} disabled={loadingUsers}>{loadingUsers ? 'Yükleniyor...' : 'Kullanıcıları Yenile'}</button></article>
      </div>
      {panelError && <div className="authError authorityError"><AlertTriangle size={15}/> {panelError}</div>}
      {!adminToken && <div className="authNote authorityNote"><LockKeyhole size={15}/> Yetki verme/banlama için Admin Girişi ekranından tekrar giriş yap. Eski local admin oturumu adminToken içermez.</div>}
      ...
      <div className="authorityTable">
        ...
        {users.map((user) => (
          <div className="authorityRow" key={user.id || user.email}>
            ...
            <div><span className={`roleBadge ${user.role === 'admin' ? 'admin' : user.role === 'moderator' ? 'moderator' : 'user'}`}>{user.role || 'user'}</span></div>
            <div><span className={user.is_active ? 'statusGood' : 'statusBad'}>{user.is_active ? 'Aktif' : 'Banlı/Pasif'}</span>{user.ban_reason && <small>{user.ban_reason}</small>}</div>
            <div className="authorityActions">
              <button type="button" onClick={() => runUserAction('user-set-role', { userId: user.id, role: 'admin' }, `${user.email} admin yapıldı.`)} disabled={!adminToken || user.role === 'admin'}><ShieldCheck size={14}/> Admin yap</button>
              <button type="button" onClick={() => runUserAction('user-set-role', { userId: user.id, role: 'user' }, `${user.email} kullanıcı yapıldı.`)} disabled={!adminToken || user.role === 'user'}><UserRound size={14}/> Kullanıcı yap</button>
              {user.is_active ? <button type="button" className="dangerMini" onClick={() => runUserAction('user-ban', { userId: user.id, reason: 'Admin panelinden banlandı' }, `${user.email} banlandı.`)} disabled={!adminToken}><EyeOff size={14}/> Banla</button> : <button type="button" onClick={() => runUserAction('user-unban', { userId: user.id }, `${user.email} banı kaldırıldı.`)} disabled={!adminToken}><CheckCircle2 size={14}/> Banı kaldır</button>}
              <button type="button" className="dangerMini" onClick={() => window.confirm(`${user.email} silinsin mi?`) && runUserAction('user-delete', { userId: user.id }, `${user.email} silindi.`)} disabled={!adminToken}><X size={14}/> Sil</button>
            </div>
          </div>
        ))}
      </div>
    ...
    <section className="adminShell adminShellFix4">
      ...
      <div className="adminMain">
        <div className="adminTop">
          <div><h2>Yönetim Paneli</h2><p>v2.1.3 Fix 4 ile yönetim paneli buton/sekme kilidi düzeltildi; yetki, ban/silme ve global bakım modu korunur.</p></div>
  ...
  return (
    <section className="maintenancePage animatedMaintenance">
      ...
      <div className="maintenanceCard">
        ...
        <p>Arayüz, butonlar ve kullanıcı hesap sistemi düzenleniyor. Giriş yapmayan ziyaretçiler ve normal kullanıcılar bakım ekranını görür; sadece admin girişinden sonra yönetim paneli açılır.</p>
        ...
        <div className="maintenanceActions">
          {isAdmin ? <button type="button" className="primaryBtn" onClick={onAdmin}><ShieldCheck size={17}/> Yönetim Paneline Git</button> : <button type="button" className="primaryBtn" onClick={onAdminLogin}><ShieldCheck size={17}/> Admin Girişi</button>}
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section className="authLanding">
      ...
      <div className="authHeroCard">
        ...
        <p>v2.1.3 Fix 3 ile kayıtlar Supabase public.site_users tablosuna düşer. Bakım modu giriş yapmayan herkese global gösterilir; admin parolası ekranda görünmez.</p>
...
function App(){
  ...
  const goAdmin=()=>{ if (isAdmin) setActive('Yönetim Paneli'); else setAuthMode('admin'); };
  useEffect(()=>{
    refresh();
  ...
  const renderPageContent = () => {
    if (maintenanceMode && !isAdmin) return <MaintenancePage isAdmin={false} onAdmin={goAdmin} onAdminLogin={openAdminLogin}/>;
  ...
  return <>
    ...
    {authMode && <AuthModal mode={authMode} onClose={() => setAuthMode(null)} onLoginSuccess={(nextSession) => { setSession(nextSession); if (nextSession?.role === 'admin') { setActive('Yönetim Paneli'); setAdminActive('Genel Bakış'); } }} onAction={showAction}/>}    
