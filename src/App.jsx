...
import { Activity, AlertTriangle, Bell, BrainCircuit, CalendarCheck, CheckCircle2, Command, Database, Download, FileDown, Filter, Gamepad2, Gauge, ImagePlus, LockKeyhole, LogIn, LogOut, Menu, Palette, PlayCircle, Plus, RefreshCcw, Search, Settings, ShieldCheck, SlidersHorizontal, Sparkles, Star, Timer, TrendingUp, UploadCloud, UserPlus, UserRound, WandSparkles, X, Zap } from 'lucide-react';
import './styles.css';
...
const fixNotes = [
  ...
  'v2.1.2 Fix 1: Admin paneli sadece yönetici girişiyle açılır; üst menüde Yönetim butonu normal kullanıcıdan gizlendi.',
  ...
  'v2.1.2: Yönetim Paneli ayrı güvenli erişim butonuna alındı; kullanıcı kategorileri sade ve temiz kaldı.',
  'v2.1.2: Bakım Modu eklendi. Admin panelinden açılıp kapanır, kullanıcı tarafında bakım ekranı gösterir.',
  'v2.1.2: Bakım Modu açıkken admin paneli erişilebilir kalır; test ve düzeltme yaparken site kilitlenmez.',
  'v2.1.2: Tüm boş/göstermelik butonlara işlem eklendi; butona basınca panel içinde doğru aksiyon mesajı görünür.',
  'v2.1.2: Yönetim Paneli butonları panel içinde sekme değiştirir; ana sayfa kategorilerine yanlış geçiş yapmaz.',
  'v2.1.2: Mobilde kategori menüsü sadeleşti, buton taşmaları azaltıldı ve sağa kayma fixleri güçlendirildi.',
  'v2.1.2: Kullanıcı deneyimi için Kaldığın Yerden, AI Öneri ve Bildirim alanları ana sayfaya kart olarak taşındı; kategori kalabalığı azaltıldı.',
  'v2.1.2: Vercel Hobby uyumu korundu; api klasörü yok, serverless function limiti yok, statik Vite build çalışır.',
  'v2.1.2: Supabase zorunlu değil; site local JSON ile açılır, ENV keyleri Vercel içinde kalmaya devam eder.',
  'v2.1.2: Akşam test için kategori kontrol listesi, bakım durumu ve buton çalışma notları eklendi.',
  ...
  return (
    <section className="heroCard v211Hero">
      <div className="heroText">
        <span className="eyebrow"><Sparkles size={16}/> Kullanıcı menüsü + bakım modu: {VERSION}</span>
        <h1>Kullanıcı ana kategorileri sadeleşti; gizli admin/test sayfaları menüden kalktı ve bakım modu eklendi.</h1>
...
function AdminTabIntro({ active, children }) {
  return <div className="adminTabTitle"><div><span>Aktif admin sekmesi</span><h2>{active}</h2><p>{children}</p></div><b>v2.1.2</b></div>;
}
...
function AdminPanel({ health, analytics, games, updates, notifications, apiStatus, testCenter, errorReports, recommendations, watchProgress, schedule, collections, roadmap, themes, maintenanceMode, setMaintenanceMode, onAction }) {
  ...
  const adminTabs = [
    ['Genel Bakış', ShieldCheck], ['Oyunlar', Gamepad2], ['Kullanıcı Menüleri', Menu], ['Test Merkezi', ShieldCheck], ['Hata Raporları', AlertTriangle],
    ...
    <section className="adminShell adminShellFix3">
      ...
      <div className="adminMain">
        <div className="adminTop">
          <div><h2>Yönetim Paneli</h2><p>v2.1.2 ile gizli sayfalar ana menüden ayrıldı, bakım modu ve çalışan buton sistemi eklendi.</p></div>
...
function FeatureNotes(){
  return (
    <section className="sectionBlock">
      <div className="sectionTitle"><h2>v2.1.2 Özellikleri</h2><span><Sparkles size={14}/> son güncelleme notları</span></div>
      <div className="fixList">{fixNotes.map((item)=><div className="fixItem" key={item}><CheckCircle2 size={18}/><span>{item}</span></div>)}</div>
      <div className="warningBox"><AlertTriangle/><p>Bu sürüm kullanıcı menüsünü sadeleştirir, bakım modu ekler ve tüm görünür butonların doğru aksiyon vermesini sağlar.</p></div>
    </section>
  );
}
  ...
  return (
    <section className="authLanding">
      ...
      <div className="authHeroCard">
        ...
        <p>Fix 1 ile normal kullanıcılar artık yönetim, hata, API ve test alanlarını göremez. Admin paneli sadece yönetici girişiyle açılır.</p>
        ...
        <div className="authInfoGrid">
          ...
          <span><CheckCircle2 size={15}/> Bakım modu kullanıcıyı kapatır</span>
        </div>
      </div>
    </section>
  );
...
function AuthModal({ mode, onClose, onLoginSuccess, onAction }) {
  ...
  const submit = (event) => {
    ...
    if (tab === 'admin') {
      if ((cleanEmail === DEMO_ADMIN_USER || cleanEmail === 'admin@hayatimizoyun.local') && password === DEMO_ADMIN_PASSWORD) {
        const session = { name: 'Admin', email: 'admin@hayatimizoyun.local', role: 'admin' };
        saveSession(session); onLoginSuccess(session); onAction('Yönetici girişi başarılı. Yönetim Paneli açıldı.'); onClose();
      } else setError('Yönetici bilgisi hatalı. Test admin: admin / admin123');
      return;
    }

    const users = getSavedUsers();
    if (tab === 'register') {
      if (users.some((user) => user.email === cleanEmail)) { setError('Bu kullanıcı zaten kayıtlı. Giriş yap.'); return; }
      const user = { name: cleanName, email: cleanEmail, password, role: 'user' };
      saveUsers([...users, user]);
      const session = { name: user.name, email: user.email, role: 'user' };
      saveSession(session); onLoginSuccess(session); onAction('Kayıt başarılı. Kullanıcı hesabıyla giriş yapıldı.'); onClose();
      return;
    }

    const found = users.find((user) => user.email === cleanEmail && user.password === password);
    if (!found) { setError('Kullanıcı bulunamadı veya şifre hatalı. Kayıt olmayı dene.'); return; }
  ...
  return (
    <div className="authModalBackdrop" role="dialog" aria-modal="true">
      <form className="authModal" onSubmit={submit}>
        ...
        <p>{tab === 'admin' ? 'Yönetim paneli ve bakım modu sadece admin hesabıyla görünür.' : 'Normal kullanıcı hesabı sadece kullanıcı kategorilerini görür.'}</p>
        {tab === 'register' && <label>Ad Soyad<input value={name} onChange={(event) => setName(event.target.value)} placeholder="Örn: Mevlüt" /></label>}
        <label>{tab === 'admin' ? 'Admin kullanıcı adı' : 'E-posta'}<input value={email} onChange={(event) => setEmail(event.target.value)} placeholder={tab === 'admin' ? 'admin' : 'ornek@mail.com'} /></label>
        <label>Şifre<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder={tab === 'admin' ? 'admin123' : 'Şifre'} /></label>
        {error && <div className="authError"><AlertTriangle size={15}/> {error}</div>}
        <button type="submit" className="primaryBtn authSubmit">{tab === 'register' ? <UserPlus size={17}/> : tab === 'admin' ? <ShieldCheck size={17}/> : <LogIn size={17}/>} {tab === 'register' ? 'Kayıt Ol' : tab === 'admin' ? 'Admin Girişi' : 'Giriş Yap'}</button>
      </form>
    </div>
  );
}
