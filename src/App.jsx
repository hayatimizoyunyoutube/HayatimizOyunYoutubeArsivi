...
const VERSION = 'v2.1.3 Fix 6 Kullanıcı Ana Sayfa Temizliği';
...
const PUBLIC_HERO = {
  eyebrow: 'Oyun arşivi • yeni bölümler • koleksiyonlar',
  title: 'Hayatımız Oyun arşivine hoş geldin.',
  text: 'Popüler serileri, devam eden bölümleri, yayın takvimini ve koleksiyonları sade kullanıcı ekranından takip et.',
  stats: ['Popüler', 'Devam Eden', 'Oyun', 'Güncel']
};
...
const fixNotes = [
  'v2.1.3 Fix 6: Kullanıcı ana sayfasındaki Supabase, test skoru, kritik hata ve bakım teknik özetleri yönetim paneline taşındı.',
  'v2.1.3 Fix 6: Normal kullanıcı artık temiz oyun arşivi hero alanı, sade istatistikler, takvim ve koleksiyonları görür.',
...
function Hero({ config, analytics, health, syncStatus, testCenter, onNavigate }) {
  return (
    <section className="heroCard publicHeroCard">
      <div className="heroText">
        <span className="eyebrow"><Sparkles size={16}/> {PUBLIC_HERO.eyebrow}</span>
        <h1>{PUBLIC_HERO.title}</h1>
        <p>{PUBLIC_HERO.text}</p>
      ...
      <div className="heroStats publicHeroStats">
        <div><strong>{analytics.activeSeries || health.active || 0}</strong><span>Devam eden seri</span></div>
        <div><strong>{health.completed || 0}</strong><span>Tamamlanan</span></div>
        <div><strong>{health.total}</strong><span>Arşivde oyun</span></div>
        <div><strong>{syncStatus === 'online' ? 'Güncel' : 'Hazır'}</strong><span>Arşiv durumu</span></div>
      </div>
    </section>
  );
}
...
function MiniDashboard({ analytics, health, notifications }) {
  const cards = [
    ['Toplam Oyun', health.total, <Gamepad2/>],
    ['Devam Eden', health.active || analytics.activeSeries || 0, <Timer/>],
    ['Tamamlanan', health.completed || 0, <CheckCircle2/>],
    ['Yakında', health.planned || analytics.plannedSeries || 0, <CalendarCheck/>],
    ['Koleksiyon', analytics.topTags?.length || 0, <TrendingUp/>],
    ['Ortalama Puan', analytics.averageScore || '9.1', <Star/>]
  ];
  return <section className="statGrid publicStatGrid">{cards.map(([label, value, icon]) => <article className="statCard" key={label}>{icon}<strong>{value}</strong><span>{label}</span></article>)}</section>;
}
...
function AdminTechnicalDashboard({ health, analytics, syncStatus, testCenter, apiStatus, updates }) {
  return (
    <section className="sectionBlock adminTechnicalOnly">
      <div className="sectionTitle"><h2>Yönetim Teknik Özeti</h2><span><ShieldCheck size={14}/> sadece yetkili panelinde görünür</span></div>
      <div className="adminTechHero">
        <div>
          <span className="eyebrow"><Sparkles size={16}/> Supabase kayıt + yetki + bakım sistemi</span>
          <h2>Kayıt, yetki, ban, silme, global bakım modu ve test notları yönetim paneline taşındı.</h2>
          <p>Bu teknik özet kullanıcı ana sayfasından kaldırıldı. Normal kullanıcı artık sadece oyun arşivi, takvim, koleksiyon ve izleme alanlarını görür.</p>
        </div>
        <div className="heroStats adminTechStats">
      ...
      <div className="adminInternalGrid adminTechnicalCards">
        <article className="adminCard"><div className="cardHeader"><Database/><strong>Supabase</strong></div><p>Kullanıcı kayıtları, rol yönetimi, ban/silme ve bakım ayarı burada takip edilir.</p></article>
        <article className="adminCard"><div className="cardHeader"><Activity/><strong>Bakım Modu</strong></div><p>Bakım ekranı giriş yapmayan ve normal kullanıcıya gösterilir; yetkililer panelde kalır.</p></article>
        <article className="adminCard"><div className="cardHeader"><ClipboardList/><strong>Güncelleme Notları</strong></div><p>{updates?.[0]?.version || 'v2.1.3'} güncel; teknik notlar kullanıcı sayfasında gösterilmez.</p></article>
      </div>
    </section>
  );
}
...
function AdminPanel({ health, analytics, games, updates, notifications, apiStatus, testCenter, errorReports, recommendations, watchProgress, schedule, collections, roadmap, themes, maintenanceMode, setMaintenanceMode, onAction, session, adminActive, setAdminActive, syncStatus }) {
  const adminTabs = [
  ...
  const renderAdminContent = () => {
    ...
    return (
      <>
        <AdminTabIntro active="Genel Bakış">Panel içi sekme sistemi aktif. Kullanıcıya gereksiz teknik ekranlar artık sadece burada görünür.</AdminTabIntro>
        <AdminTechnicalDashboard health={health} analytics={analytics} syncStatus={syncStatus} testCenter={testCenter} apiStatus={apiStatus} updates={updates}/>
        <div className="adminMiniActions">
    ...
    <section className="adminShell adminShellFix4">
      ...
      <div className="adminMain">
        <div className="adminTop">
          <div><h2>Yönetim Paneli</h2><p>v2.1.3 Fix 6 ile kullanıcı ana sayfasındaki teknik admin ekranı yönetim paneline taşındı.</p></div>
...
function AdminFeatureNotes(){
  return (
    <section className="sectionBlock">
      <div className="sectionTitle"><h2>Yönetim Güncelleme Notları</h2><span><Sparkles size={14}/> sadece admin panelinde</span></div>
  ...
  return (
    <section className="authLanding">
      ...
      <div className="authHeroCard">
        ...
        <p>Hesap açtıktan sonra oyun arşivini, yayın takvimini ve koleksiyonları takip edebilirsin. Yetkili alanlar normal kullanıcılara kapalıdır.</p>
        ...
        <div className="authInfoGrid">
          ...
          <span><CheckCircle2 size={15}/> Kayıtların güvenli saklanır</span>
        </div>
      </div>
    </section>
  );
