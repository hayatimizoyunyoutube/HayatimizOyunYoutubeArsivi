...
const VERSION = 'v2.1.1 Oyun Ekle Meta + Koleksiyon Fix';
...
const DEFAULT_GAME_DRAFT = { title:'', genre:'', tags:'', releaseDate:'', status:'Devam Ediyor', eps:0, score:8.5, cover:'' };
...
function publicStats(){
  const collections = buildCollections();
  return `<section class="grid stats publicHighlights"><div class="card"><b>Devam Eden</b><h3>${state.games.filter(g=>g.status==='Devam Ediyor').length}</h3><span class="muted">aktif seri</span></div><div class="card"><b>Tamamlanan</b><h3>${state.games.filter(g=>g.status==='Tamamlandı').length}</h3><span class="muted">arşivde</span></div><div class="card"><b>Koleksiyon</b><h3>${collections.length}</h3><span class="muted">gerçek liste</span></div><div class="card"><b>Yakında</b><h3>${state.games.filter(g=>g.status==='Yakında').length}</h3><span class="muted">planlanan</span></div></section>`;
}
...
function gameGrid(){
  ...
  return `<section class="games">${games.map(g=>`<article class="game"><div class="coverWrap"><img src="${esc(coverFor(g))}" alt="${esc(g.title)}">${featureEnabled('missing_cover_warning') && !g.cover ? '<span class="coverWarn">Kapak eksik</span>' : ''}</div><div><span class="pill">${esc(g.status)}</span><h3>${esc(g.title)}</h3><p>${esc(g.genre)} • ${g.eps} bölüm${g.releaseDate ? ` • ${esc(g.releaseDate)}` : ''}</p>${g.tags ? `<p class="gameTags">${esc(g.tags)}</p>` : ''}<b>⭐ ${g.score}</b>${adminActions ? `<div class="gameAdminActions"><button class="miniBtn" data-game-edit="${esc(g.id)}">Düzenle</button><button class="miniBtn danger" data-game-delete="${esc(g.id)}">Sil</button></div>` : ''}</div></article>`).join('') || '<div class="card wide">Oyun bulunamadı.</div>'}</section>`;
}
...
function collectionsPage(){
  const collections = buildCollections();
  const cards = collections.map(c => `<div class="card soft collectionCard"><div class="sectionHead"><div><b>${esc(c.title)}</b><p class="muted">${esc(c.description)}</p></div><span class="pill green">${c.items.length} oyun</span></div><div class="collectionList">${c.items.slice(0,8).map(g=>`<button class="miniBtn" data-page="Popüler" data-toast="${esc(g.title)} koleksiyonda görünüyor.">${esc(g.title)}</button>`).join('')}</div></div>`).join('');
  return `<section class="card wide"><div class="sectionHead"><div><h2>Koleksiyonlar</h2><p class="muted">Bu sayı artık sabit 7 değil; Supabase games tablosundaki gerçek oyunların tür, durum ve etiketlerinden otomatik oluşur.</p></div><span class="pill green">${collections.length} koleksiyon</span></div><div class="grid collectionGrid">${cards || '<div class="card soft"><b>Henüz koleksiyon yok</b><span>Oyunlara tür, durum veya etiket ekleyince koleksiyonlar burada otomatik oluşur.</span></div>'}</div></section>`;
}
...
function overviewAdmin(){
  return `<section class="grid stats adminStats"><div class="card"><b>Oyun</b><h3>${state.games.length}</h3><span class="muted">arşiv verisi</span></div><div class="card"><b>Güncelleme</b><h3>${state.updates.length}</h3><span class="muted">not</span></div><div class="card"><b>Veri durumu</b><h3>${state.runtimeLoaded?'Bağlandı':'Local'}</h3><span class="muted">Supabase/API</span></div><div class="card"><b>Oyun ekleme</b><h3>Aktif</h3><span class="muted">meta/kapak/etiket</span></div></section><section class="card wide"><h2>v2.1.1 temiz oyun yönetimi</h2><p>AI Özellik bölümü kaldırıldı. Oyun ekleme meta çekme ve koleksiyon sayacı düzeltildi; koleksiyonlar artık gerçek oyun verisine göre oluşur.</p><button class="btn primary" data-admin="Oyunlar">Oyunlar Sekmesini Aç</button></section>`;
}
function gamesAdmin(){
  const buttons = `<button class="btn primary" data-action="toggle-game-form">+ Oyun Ekle</button><button class="btn" data-action="auto-cover-fetch">Kapaksızlara Kapak Öner</button><span class="pill green">Düzenle/Sil aktif</span>`;
  return `<section class="card wide"><div class="sectionHead"><div><h2>Oyun Yönetimi</h2><p class="muted">AI Özellik bölümü kaldırıldı. Oyun ekleme ve otomatik meta çekme doğrudan burada çalışır.</p></div><div class="heroActions">${buttons}</div></div><div class="note greenNote">Otomatik çekme sadece formu doldurur; oyun eklemez, silmez. Supabase kaydı sadece Kaydet butonuyla yapılır.</div>${state.showGameForm ? gameAddForm() : ''}${coverSuggestionPanel()}${gameGrid()}</section>`;
}
...
function gameAddForm(){
  ...
  const opts = ['Devam Ediyor','Tamamlandı','Popüler','Yakında'].map(o=>`<option ${d.status===o?'selected':''}>${o}</option>`).join('');
  return `<form class="card soft gameForm" id="gameAddForm" autocomplete="off"><h3>Yeni Oyun Ekle</h3><p class="muted">Oyun adını yaz, otomatik çekme kapak resmi, çıkış tarihi, tüm türler ve puanı doldursun. Etiketler butonlu ve manuel seçilir.</p><div class="formGrid"><label class="field">Oyun adı<input name="title" required placeholder="Örn: Assassin's Creed Origins" value="${esc(d.title)}" /></label><label class="field">Kategori / Tür<input name="genre" required placeholder="Aksiyon, Macera, RPG" value="${esc(d.genre)}" /></label><label class="field">Çıkış tarihi <small>gün.ay.yıl</small><input name="releaseDate" placeholder="27.10.2017" value="${esc(d.releaseDate || '')}" /></label><label class="field">Durum<select name="status">${opts}</select></label><label class="field">Bölüm<input name="eps" type="number" min="0" value="${esc(String(d.eps ?? 0))}" /></label><label class="field">Puan<input name="score" type="number" min="0" max="10" step="0.1" value="${esc(String(d.score ?? 8.5))}" /></label><label class="field wideField">Kapak URL<input name="cover" placeholder="https://..." value="${esc(d.cover)}" /></label><div class="field wideField"><span>Etiketler</span><small class="muted">Butonlardan seç: Türkçe Altyazılı, Türkçe Dublajlı, DLC, Coop, %100...</small>${tagButtonsHtml(d.tags)}</div></div><div class="coverPreview ${d.cover?'':'isEmpty'}">${d.cover?`<img src="${esc(d.cover)}" alt="Kapak önizleme">`:'Kapak çekilince burada önizleme görünür.'}</div><div class="note greenNote">Otomatik çekme oyun eklemez, oyun silmez, sadece form alanlarını doldurur.</div><div class="rowActions"><button class="btn" type="button" data-action="auto-game-meta">Oyun adından kapak/tarih/tür çek</button><button class="btn primary" type="submit">Supabase games tablosuna kaydet</button><button class="btn" type="button" data-action="toggle-game-form">Kapat</button></div></form>`;
}
...
async function autoGameMetaFill(){
  ...
  const title = state.gameDraft.title || '';
  ...
  state.gameDraft = {
    ...DEFAULT_GAME_DRAFT,
    ...state.gameDraft,
    genre: meta.genre || state.gameDraft.genre,
    releaseDate: normalizeReleaseDate(meta.released || meta.releaseDate || state.gameDraft.releaseDate),
    score: Number(meta.score || state.gameDraft.score || 8.5),
    cover: meta.cover || state.gameDraft.cover,
    tags: state.gameDraft.tags || ''
  };
  persistGameDraft();
  setFormValue(form, 'genre', state.gameDraft.genre);
  setFormValue(form, 'releaseDate', state.gameDraft.releaseDate);
  setFormValue(form, 'score', state.gameDraft.score);
  setFormValue(form, 'cover', state.gameDraft.cover);
  const preview = form.querySelector('.coverPreview');
  if(preview){
    preview.classList.toggle('isEmpty', !state.gameDraft.cover);
    preview.innerHTML = state.gameDraft.cover ? `<img src="${esc(state.gameDraft.cover)}" alt="Kapak önizleme">` : 'Kapak çekilince burada önizleme görünür.';
  }
  setToast('Form dolduruldu: kapak, çıkış tarihi ve tüm türler çekildi. Etiketleri butonlardan seç; kaydetmeden oyun eklenmez.');
}
