...
const VERSION = 'Akıllı Özellik + Siteye Uygula Fix';
...
function addFeatureToPlanner(feature, status='plan'){
  ...
  if(plannerItem){ plannerItem.status = status; plannerItem.featureKey = key; plannerItem.text = feature.title || plannerItem.text; plannerItem.group = feature.group || plannerItem.group; }
  else state.planner.unshift({ id:'local-'+Date.now(), group:feature.group || 'Adminin Önerileri', text:feature.title, status, featureKey:key });
}
