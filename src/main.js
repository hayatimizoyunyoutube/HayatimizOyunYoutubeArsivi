...
const VERSION = 'v2.1.4.3 Arayüz Fix + Güncelleme Notları';
...
const AUTH_SESSION_KEY = 'hayatimiz_session_v2143';
const MAINTENANCE_KEY = 'hayatimiz_maintenance_cache_v2143';
const ADMIN_TAB_KEY = 'hayatimiz_admin_tab_v2143';
const FEATURE_CACHE_KEY = 'hayatimiz_features_v2143';
...
let rotateIndex = Number(localStorage.getItem('hayatimiz_task_rotate_v2143') || '0');
...
function overviewAdmin(){
  ...
  return `<section class="grid stats adminStats"><div class="card"><b>Oyun</b><h3>${state.games.length}</h3><span class="muted">arşiv verisi</span></div><div class="card"><b>Aktif Özellik</b><h3>${activeFeatures}</h3><span class="muted">uygulandı</span></div><div class="card"><b>Güncelleme</b><h3>${state.updates.length}</h3><span class="muted">not</span></div><div class="card"><b>Veri durumu</b><h3>${state.runtimeLoaded?'Bağlandı':'Local'}</h3><span class="muted">Supabase/API</span></div></section><section class="card wide"><h2>Akıllı Uygulama Merkezi hazır</h2><p>Özellik Planı içine kendi istediğin özelliği yaz. Sistem eşleşen hazır modülü bulur, onay ister ve Siteye Uygula deyince ilgili butonu/paneli aktif eder.</p><button class="btn primary" data-admin="Özellik Planı">Özellik Planını Aç</button><button class="btn" data-admin="Uygulama Merkezi">Uygulama Merkezi</button><button class="btn" data-admin="Oyunlar">Oyunlar Sekmesi</button></section>`;
}
function gamesAdmin(){
  const addEnabled = featureEnabled('admin_games_add_button');
  const coverEnabled = featureEnabled('auto_cover_fetch');
  const buttons = `${addEnabled ? '<button class="btn primary" data-action="toggle-game-form">+ Oyun Ekle</button>' : '<button class="btn" data-admin="Özellik Planı">Oyun Ekle özelliğini uygula</button>'}${coverEnabled ? '<button class="btn" data-action="auto-cover-fetch">Otomatik Kapak Çek</button>' : '<button class="btn" data-feature-apply="auto_cover_fetch">Kapak Çekme Özelliğini Uygula</button>'}`;
  return `<section class="card wide"><div class="sectionHead"><div><h2>Oyun Yönetimi</h2><p class="muted">Oyun ekleme ve kapak modülleri Özellik Planı üzerinden aktif edilir.</p></div><div class="heroActions">${buttons}</div></div>${!addEnabled?'<div class="note">Oyun Ekle butonu henüz aktif değil. Özellik Planı içinde ilgili maddeye Siteye Uygula de.</div>':''}${coverEnabled?'<div class="note greenNote">Otomatik kapak modülü aktif. Kapaksız oyunlara güvenli kapak atanabilir.</div>':''}${addEnabled && state.showGameForm ? gameAddForm() : ''}${gameGrid()}</section>`;
}
...
function planItem(p){
  ...
  return `<div class="planItem ${enabled?'done':''}"><span>${enabled?'✅':'⬜'}</span><div><b>${esc(p.text)}</b><small>${esc(target)}</small><div class="planActions">${!enabled ? `<button class="miniBtn primary" data-feature-apply="${esc(key)}" data-feature-title="${esc(p.text)}">Siteye Uygula</button>` : '<span class="pill green">Sitede aktif</span>'}</div></div></div>`;
}
...
async function onUpdateNoteSubmit(e){
  ...
  const note = {
    ...
    image:String(fd.get('image') || '').trim() || 'previews/hayatimiz-oyun-v2143-update-notes-preview.png',
...
async function onAction(e){
  ...
  if(action === 'confirm-feature-apply'){ const pending = state.pendingFeature; state.pendingFeature = null; if(pending) await applyFeatureObject(pending); }
  ...
  if(action === 'clear-local'){ localStorage.removeItem(AUTH_SESSION_KEY); localStorage.removeItem(ADMIN_TAB_KEY); localStorage.removeItem(MAINTENANCE_KEY); localStorage.removeItem(FEATURE_CACHE_KEY); setToast('v2.1.4.3 local kayıtları temizlendi.'); }
}
...
async function onPlanComplete(e){
  ...
  const nextText = rotatingTasks[rotateIndex % rotatingTasks.length]; rotateIndex += 1; localStorage.setItem('hayatimiz_task_rotate_v2143', String(rotateIndex));
