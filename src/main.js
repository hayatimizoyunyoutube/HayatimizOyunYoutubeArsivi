...
const FIX_NAME = 'v2.1.2 FIX - Bakım, Kullanıcı ve Yönetim Menü Düzeltmesi';
...
function nav(){
  ...
  return `<aside class="side proSide"><div class="brand"><span class="logo">🎮</span><span><b>Hayatımız <em>Oyun</em></b><small>${VERSION} • FIX</small></span></div><nav class="nav publicNav"><div class="navLabel">🌐 Site</div>${publicItems.map(([href,label])=>`<a class="${active(href)?'active':''}" href="${href}">${label}</a>`).join('')}${canShowYönetimLinks()?`<div class="navLabel adminOnlyLabel">🛡️ Yönetim</div><a class="adminOpenSide ${active('/yonetim')?'active':''}" href="/yonetim">🛡️ Yönetim Paneli</a>`:''}</nav><button data-action="hard-refresh">🔄 Sayfayı Yenile</button><div class="sideFooter"><b class="okText">✅ v2.1.2 FIX</b><br>Yönetim alt sayfaları artık panelin içinde açılır.</div></aside>`;
}
function adminDock(){
...
function layout(content){ return `<main class="app">${nav()}<section class="main">${userTopbar()}${adminDock()}${content}</section></main>`; }
