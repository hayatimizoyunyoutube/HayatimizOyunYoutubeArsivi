...
function topbar(){
  ...
  const nav = ['Ana Sayfa','Oyunlar','Seriler','Yayın Takvimi','Bildirimler','Topluluk'];
  return html`<header class="topbar v220Topbar">
    <button class="brand cleanBtn" data-page="Ana Sayfa"><div class="mark">🎮</div><div><b>Hayatımız Oyun</b><span>${VERSION}</span></div></button>
    <nav class="topMainNav">${nav.map(n=>`<button class="topNavItem ${state.page===n || (n==='Oyunlar' && ['Popüler','Tamamlanan','Devam Eden','Yakında','Korku','Aksiyon','Hikaye Odaklı'].includes(state.page))?'active':''}" data-page="${esc(n==='Oyunlar'?'Popüler':n==='Yayın Takvimi'?'Takvim':n)}">${navIcon(n)} ${esc(n)}</button>`).join('')}</nav>
...
function categoryRail(){
  ...
  const cats = ['Ana Sayfa','Popüler','Tamamlanan','Devam Eden','Yakında','Korku','Aksiyon','Hikaye Odaklı','Takvim','Bildirimler','Profilim'];
  return `<nav class="category v220CategoryRail">${cats.map(c=>`<button class="tab ${state.page===c?'active':''}" data-page="${esc(c)}">${esc(c)}</button>`).join('')}</nav>`;
}
...
function publicPage(){
  ...
  if(state.page === 'Topluluk') return communityPage();
...
function gameArchiveDashboard(){
  ...
  return `<section class="v220Shell">
    <aside class="v220SideNav"><div class="sideLogo">🎮 <b>Arşiv Menüsü</b></div>${sideNavItems().map(item=>`<button class="sideNavItem ${state.page===item.page?'active':''}" data-page="${esc(item.page)}"><span>${item.icon}</span>${esc(item.label)}</button>`).join('')}<div class="supportBox">💜 <b>Destekle</b><small>Arşivi büyütmek için seri ve bölüm takibini güncel tut.</small></div></aside>
