...
const VERSION = 'v2.1.8 - Tam Otomatik Yayın Takvimi + Gelişmiş Medya Yönetimi';
...
const state = {
  ...
  gameViewMode: localStorage.getItem('hayatimiz_game_view_mode_v218') || localStorage.getItem('hayatimiz_game_view_mode_v217') || 'grid',
  showSeriesSortPanel:false,
  activeSeriesName: localStorage.getItem('hayatimiz_active_series_v217') || '',
  autoNextEpisode: localStorage.getItem('hayatimiz_auto_next_episode_v218') === '1' || localStorage.getItem('hayatimiz_auto_next_episode_v217') === '1',
  cinemaFullscreen: localStorage.getItem('hayatimiz_cinema_fullscreen_v218') === '1',
  episodeNotes: safeParse(localStorage.getItem('hayatimiz_episode_notes_v218'), {}),
...
function overviewAdmin(){
  return `<section class="grid stats adminStats"><div class="card"><b>Oyun</b><h3>${state.games.length}</h3><span class="muted">arşiv verisi</span></div><div class="card"><b>Güncelleme</b><h3>${state.updates.length}</h3><span class="muted">not</span></div><div class="card"><b>Veri durumu</b><h3>${state.runtimeLoaded?'Bağlandı':'Local'}</h3><span class="muted">Supabase/API</span></div><div class="card"><b>Oyun ekleme</b><h3>Aktif</h3><span class="muted">meta/kapak/etiket</span></div></section><section class="card wide"><h2>v2.1.8 paketi aktif</h2><p>Yayın takvimi, gelişmiş medya yönetimi, sinema modu, burada kaldım/geri al ve toplu senkronizasyon aktif.</p><button class="btn primary" data-admin="Oyunlar">Oyunlar Sekmesini Aç</button></section>`;
}
...
function gamesAdmin(){
  const buttons = `<button class="btn primary" data-action="toggle-game-form">+ Oyun Ekle</button><button class="btn" data-action="auto-cover-fetch">Kapaksızlara Kapak Öner</button><button class="btn" data-action="bulk-sync-playlists">Toplu Playlist Senkronize</button><button class="btn" data-action="open-series-sort-panel">Seri Sıralama</button><span class="pill green">v2.1.8 medya aktif</span>`;
...
function seriesWatchModal(){
  ...
  return `<div class="modalOverlay watchPageOverlay ${state.cinemaFullscreen?'cinemaFull':''}"><div class="modal seriesModalV216 professionalWatch v218Watch"><button class="close" type="button" data-action="close-series-watch">×</button><div class="watchHeader"><div><span class="eyebrow">Site İçi Seri İzleme</span><h2>${esc(g.title)}</h2><p class="muted">${esc(g.seriesName || 'Tek seri')} • ${watched}/${Number(g.eps||episodes.length||0)} bölüm • ${progressPercent(g)}%</p></div><div class="rowActions"><button class="miniBtn ${state.cinemaFullscreen?'primary':''}" data-action="toggle-cinema-fullscreen">Sinema: ${state.cinemaFullscreen?'Tam':'Normal'}</button><button class="miniBtn ${state.autoNextEpisode?'primary':''}" data-action="toggle-auto-next">Oto Sonraki: ${state.autoNextEpisode?'Açık':'Kapalı'}</button><button class="miniBtn" data-action="resume-episode">Burada Kaldım</button><button class="miniBtn primary" data-toggle-active-watched-index="${safeIndex}">${active?.watched?'İzlendi Geri Al':'Bu Bölümü İzledim'}</button></div></div><div class="watchLayout"><main>${player}<div class="activeEpisodeInfo"><span class="pill green">${active?`${active.number}. Bölüm`:'Bölüm yok'}</span><h3>${esc(active ? cleanEpisodeTitle(active.title, active.number) : g.title)}</h3><p class="muted">${esc(active?.description || g.description || 'Bölümü doğrudan site içinden izliyorsun.')}</p>${nextGame?`<button class="miniBtn" data-next-series-game="${esc(nextGame.id)}">Seride Sıradaki Oyun: ${esc(nextGame.title)}</button>`:''}</div><label class="field episodeNoteBox">Bölüm Notum<textarea data-episode-note="${esc(noteKey)}" rows="3" placeholder="Bu bölüm için kişisel not yaz...">${esc(userNote)}</textarea></label><div class="progressLine large"><span style="width:${progressPercent(g)}%"></span></div><div class="watchShortcuts">Kısayollar: ←/→ bölüm değiştir • Space izle/geri al • F sinema modu</div></main><aside><h3>Bölümler</h3><div class="episodeSidebar">${rows}</div></aside></div></div></div>`;
}
...
function bind(){
  ...
  document.querySelectorAll('[data-episode-note]').forEach(el=>el.addEventListener('input', e=>{ state.episodeNotes[el.dataset.episodeNote] = e.target.value || ''; localStorage.setItem('hayatimiz_episode_notes_v218', JSON.stringify(state.episodeNotes)); }));
  document.querySelectorAll('[data-view-mode]').forEach(el=>el.addEventListener('click', e=>{ e.preventDefault(); state.gameViewMode = el.dataset.viewMode || 'grid'; localStorage.setItem('hayatimiz_game_view_mode_v218', state.gameViewMode); render(); }));
...
async function onAction(e){
  ...
  if(action === 'toggle-auto-next'){ state.autoNextEpisode = !state.autoNextEpisode; localStorage.setItem('hayatimiz_auto_next_episode_v218', state.autoNextEpisode ? '1' : '0'); render(); }
  if(action === 'toggle-cinema-fullscreen'){ state.cinemaFullscreen = !state.cinemaFullscreen; localStorage.setItem('hayatimiz_cinema_fullscreen_v218', state.cinemaFullscreen ? '1' : '0'); render(); }
...
function bindV218Keyboard(){
  ...
  window.addEventListener('keydown', (e)=>{
    ...
    if(String(e.key).toLowerCase() === 'f'){ state.cinemaFullscreen = !state.cinemaFullscreen; localStorage.setItem('hayatimiz_cinema_fullscreen_v218', state.cinemaFullscreen ? '1' : '0'); render(); }
  });
}
bindV218Keyboard();
