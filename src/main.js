...
function watchButtonHtml(g){
  ...
  if(url) return `<a class="miniBtn primary watchBtn" href="${esc(url)}" target="_blank" rel="noopener">YouTube'da İzle</a>`;
  if(Number(g.eps || 0) > 0 || g.seriesName) return `<button class="miniBtn" data-toast="${esc(g.title)} serisi için playlist URL eklenmemiş.">Seriyi İzle</button>`;
  return `<button class="miniBtn" data-toast="${esc(g.title)} için izleme bağlantısı eklenmemiş.">YouTube'da İzle</button>`;
}
