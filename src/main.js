...
const VERSION = 'v2.1.5 Gerçek Storage + Gelişmiş İzleme';
...
function alphabetNav(games){
  const letters = Array.from(new Set(games.map(g=>firstLetter(g.title)))).sort((a,b)=>a.localeCompare(b,'tr'));
...
function gameGrid(){
  ...
  const sections = Array.from(groups.entries()).map(([letter,items])=>`<section class="letterSection" id="harf-${esc(routeSlug(letter))}"><div class="letterHead"><b>${esc(letter)} Harfinde Başlayan Seriler</b><span>${items.length} oyun</span></div><div class="games">${items.map(g=>gameCard(g, adminActions)).join('')}</div></section>`).join('');
...
function seriesWatchModal(){
  ...
  if(!g) return '';
  const url = watchTargetUrl(g);
  const episodes = seriesEpisodes(g);
  const watched = Number(g.watchedEps || 0);
  const rows = episodes.length ? episodes.map(n=>`<li class="${n<=watched?'done':''}"><span>${n}. Bölüm</span>${url ? `<a class="miniBtn" href="${esc(url)}" target="_blank" rel="noopener">YouTube'da İzle</a>` : '<button class="miniBtn disabled">Link yok</button>'}</li>`).join('') : '<li><span>Bölüm hedefi eklenmemiş.</span><button class="miniBtn disabled">Bekliyor</button></li>';
  return `<div class="modalOverlay"><div class="modal seriesModal"><button class="close" type="button" data-action="close-series-watch">×</button><span class="eyebrow">Seriyi İzle</span><h2>${esc(g.title)}</h2><p class="muted">${esc(g.seriesName || 'Tek seri')} • ${esc(g.genre)} • ${watched}/${Number(g.eps||0)} bölüm</p><div class="progressLine large"><span style="width:${progressPercent(g)}%"></span></div><ol class="episodeList">${rows}</ol></div></div>`;
}
