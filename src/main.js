...
const FIX_NAME = 'v2.2.2 - Supabase Kullanıcı ve Yetki Stabilite Merkezi';
...
const DEFAULT_NOTES = [
  ...
  {id:'plan-v223',version:'v2.2.3',title:'🛠️ Bakım Modu Premium Deneyim Geliştirme',summary:'Bakım ekranı mesajları, güncelleme notları ve ziyaretçi deneyimi daha sade ve profesyonel hale getirilecek.',status:'Planlandı'},
...
function maintenanceZiyaretçi(){
  ...
  const done=notes.filter(n=>!String(n.status||'').toLocaleLowerCase('tr').includes('plan')).slice(0,4);
  const planned=notes.filter(n=>String(n.status||'').toLocaleLowerCase('tr').includes('plan')).slice(0,4);
  const doneFallback=[
  ...
  const plannedFallback=[
    {version:'v2.2.3',title:'Arşiv görünümü geliştirme',summary:'Kapaklar, filtreler ve kart düzeni daha profesyonel hale getirilecek.'},
    {version:'v2.2.4',title:'Yayın takvimi geliştirme',summary:'Takvim oyun kapağı ve yayın bilgileri daha zengin gösterilecek.'},
    {version:'v2.2.5',title:'Playlist ve bölüm stabilitesi',summary:'Seri/bölüm listeleri daha güçlü senkron çalışacak.'}
  ];
...
function gameForm(){ return adminOnly(()=>{
  ...
  const quickTags=['Türkçe Altyazılı','Türkçe Dublajlı','DLC','%100','Co-op','Multiplayer','Singleplayer','Hikaye Odaklı'];
