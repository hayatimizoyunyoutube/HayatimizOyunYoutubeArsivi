...
function localUserRows(){
  const local=loadUsers().map((u,i)=>({id:u.id||`local-${i}`,email:u.email,displayName:u.displayName||u.full_name||u.email,role:roleValue(u.role||'user',u.email),is_active:u.is_active!==false,source:u.source||'Tarayıcı'}));
  const current=currentUser();
  if(current && !local.some(u=>String(u.email).toLowerCase()===String(current.email).toLowerCase())) local.unshift({id:'current-session',email:current.email,displayName:current.displayName||current.email,role:roleValue(current.role||'user',current.email),is_active:true,source:'Oturum'});
  return local;
}
