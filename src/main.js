...
const VERSION = 'v2.2.0 FIX 5 - Referans Tasarıma Yakın Arayüz';
...
function topbar(){
  ...
  return html`<header class="topbar v220Topbar v220TopbarClean fix4Topbar">
    <button class="brand cleanBtn compactBrand" data-page="Ana Sayfa"><div class="mark">🎮</div><div><b>Hayatımız Oyun</b><span>${VERSION}</span></div></button>
    <label class="search v220Search fix4Search">🔎 <input id="searchInput" value="${esc(state.query)}" placeholder="Oyun, seri veya etiket ara..." /><kbd>Ctrl K</kbd></label>
...
function archiveSideNav(){
  ...
  return `<aside class="v220SideNav fixedArchiveMenu fix5SideNav"><div class="sideLogo"><span class="logoMark">🎮</span><div><b>Hayatımız Oyun</b><small>Oyun Arşivi & Seriler</small></div></div><div class="sideNavLabel">MENÜ</div>${sideNavItems().map(item=>`<button class="sideNavItem ${isArchiveActive(item)?'active':''}" data-page="${esc(item.page)}"><span>${item.icon}</span>${esc(item.label)}</button>`).join('')}<div class="sideNavLabel">YÖNETİM</div><button class="sideNavItem manager ${state.page==='Yönetim Paneli'?'active':''}" data-admin="Genel Bakış"><span>👑</span><div><b>Yönetim</b><small>Yönetici Paneline Git</small></div><strong>›</strong></button><div class="supportBox">💜 <b>Koleksiyonunu Genişlet</b><small>Yeni oyunlar keşfet, favorilerini ayır ve seri takibini güncel tut.</small><button class="miniBtn primary" data-page="Oyun Arşivi">Keşfet</button></div><div class="sideFooter"><div class="socialRow"><button class="socialMiniBtn" data-toast="Discord bağlantısı sonraki pakette eklenecek.">💬</button><button class="socialMiniBtn" data-toast="X bağlantısı sonraki pakette eklenecek.">𝕏</button><button class="socialMiniBtn" data-toast="YouTube bağlantısı sonraki pakette eklenecek.">▶</button><button class="socialMiniBtn" data-toast="Instagram bağlantısı sonraki pakette eklenecek.">◎</button></div><small>© 2024 Hayatımız Oyun<br>Tüm hakları saklıdır.</small></div></aside>`;
}
...
function gamesAdmin(){
  const buttons = `<button class="btn primary" data-action="toggle-game-form">${state.showGameForm?'Formu Gizle':'Yeni Oyun Ekle'}</button><button class="btn" data-action="auto-cover-fetch">Kapaksızlara Kapak Öner</button><button class="btn" data-action="bulk-sync-playlists">Toplu Playlist Senkronize</button><button class="btn" data-action="open-series-sort-panel">Seri Sıralama</button><span class="pill green">v2.2.0 FIX 5</span>`;
  return `<section class="fix5AdminGamesPage"><div class="card wide fix5PanelHero"><div class="sectionHead"><div><span class="eyebrow">Yönetim Paneli</span><h2>Oyun Yönetimi</h2><p class="muted">Referans görsellerdeki yapıya daha yakın; form, önizleme, kapak kontrolü ve oyun listesi aynı ekranda daha düzenli gösterilir.</p></div><div class="heroActions">${buttons}</div></div></div><div class="fix5AdminSectionStack">${youtubeHealthPanel()}${missingCoverPanel()}${state.showGameForm ? gameAddForm() : '<section class="card wide fix5CollapsedHint"><div><h3>Oyun formu kapalı</h3><p class="muted">Yeni Oyun Ekle ile detaylı formu tekrar açabilirsin.</p></div><button class="btn primary" data-action="toggle-game-form">Formu Aç</button></section>'}${state.editingGameId ? gameEditForm() : ''}${rawgCandidatePanel()}${coverSuggestionPanel()}${advancedSearchPanel()}${gameGrid()}</div></section>`;
}
...
function gameFormFields(d, mode='add'){
  ...
  return `<div class="fix4AdminFormLayout"><div class="fix4FormMain"><div class="formGrid cleanGameFormGrid"><label class="field">Oyun adı<input name="title" required placeholder="Örn: Assassin's Creed Origins" value="${esc(d.title)}" /></label><label class="field">Seri adı<input name="seriesName" placeholder="Örn: Assassin's Creed" value="${esc(d.seriesName || '')}" /></label><label class="field">Kategori / Tür<input name="genre" required placeholder="Aksiyon, Macera, RPG" value="${esc(d.genre)}" /></label><label class="field">Çıkış tarihi<input name="releaseDate" placeholder="27.10.2017" value="${esc(d.releaseDate || '')}" /></label><label class="field">Durum<select name="status">${opts}</select></label><label class="field">Toplam Bölüm<input name="eps" type="number" min="0" value="${esc(String(d.eps ?? 0))}" /></label><label class="field">İzlenen Bölüm<input name="watchedEps" type="number" min="0" value="${esc(String(d.watchedEps ?? 0))}" /></label><label class="field">Puan<input name="score" type="number" min="0" max="10" step="0.1" value="${esc(String(d.score ?? 8.5))}" /></label><label class="field">Seri sıra no<input name="seriesOrder" type="number" min="0" value="${esc(String(d.seriesOrder ?? 0))}" /></label><label class="field wideField">YouTube oynatma listesi<input name="playlistUrl" placeholder="https://youtube.com/playlist?list=..." value="${esc(d.playlistUrl || '')}" /></label><label class="field wideField">Tek video URL<input name="videoUrl" placeholder="https://youtube.com/watch?v=..." value="${esc(d.videoUrl || '')}" /></label><label class="field wideField">Kapak URL<input name="cover" placeholder="https://..." value="${esc(d.cover)}" /></label><label class="field wideField storyField">Hikâye / Açıklama<textarea name="description" rows="7" placeholder="Hikaye Çek butonu RAWG notu eklemeden Türkçe hikaye oluşturur.">${esc(d.description || '')}</textarea></label><label class="field wideField episodeImportField">Bölüm Listesi <small>Playlistten gelen videolar profesyonel listeye dönüşür. Teknik veri gizlidir.</small>${episodeImportPreview(d.episodesText, d.episodes)}<textarea name="episodesText" rows="5" class="technicalEpisodes" placeholder="Teknik bölüm verisi otomatik oluşur">${esc(d.episodesText || episodesToText(d.episodes || []))}</textarea><button class="miniBtn" type="button" data-action="toggle-technical-episodes">Teknik veriyi göster/gizle</button></label><div class="field wideField"><span>Etiketler</span><small class="muted">Butonlardan seç: Türkçe Altyazılı, DLC, Coop, %100...</small>${tagButtonsHtml(d.tags)}</div></div></div><aside class="fix4FormPreview"><h3>Önizleme</h3><div class="coverPreview ${d.cover?'':'isEmpty'}">${d.cover?`<img src="${esc(d.cover)}" alt="Kapak önizleme">`:'Kapak çekilince burada önizleme görünür.'}</div><h2>${esc(d.title || 'Oyun adı')}</h2><p>${esc((d.description || 'Kısa açıklama burada görünecek.').slice(0,180))}</p><div class="rightStats"><div><b>${esc(String(d.score || '8.5'))}</b><span>Puan</span></div><div><b>${esc(String(d.eps || 0))}</b><span>Bölüm</span></div></div></aside></div>`;
}
