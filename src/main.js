...
const VERSION = 'v2.1.4.5 Özellik Uygula + Kapak Fix';
...
const FEATURE_CATALOG = [
  ...
  {
    key:'apply_refresh_flow',
    title:'Siteye Uygula + Siteyi Yenile akışını aktif et',
    group:'Eklenen Özellikler',
    target:'Yönetim Paneli > Özellik Planı',
    keywords:['uygula ve siteyi yenile','siteyi yenile','uygula yenile','otomatik yenile'],
    description:'Özellik uygulandıktan sonra panel verilerini otomatik yeniler; istersen sayfayı da güvenli şekilde yeniler.',
    next:'Uygulama sonrası mini kontrol raporu göster'
  },
  {
...
function findFeatureByText(text){
  ...
  return FEATURE_CATALOG.find(f => f.keywords?.some(k => q.includes(k))) ||
    FEATURE_CATALOG.find(f => q.includes(f.title.toLowerCase().slice(0,18)));
}
function proposeFeatureFromText(text){
  const clean = String(text || '').trim();
  const matched = findFeatureByText(clean);
  if(matched){ return { ...matched, matched:true, originalText:clean }; }
  return {
...
function settingsPanel(){ return `<section class="card wide"><h2>Ayarlar</h2><p>Kurulum notları ZIP içindeki KURULUM-KOMUTLARI.txt dosyasındadır. Mobil BAT kaldırıldı; sadece 01-temizle ve 02-GitHub'a gönder dosyalarını kullan.</p><button class="btn" data-action="clear-local">Eski sürüm local cache temizle</button><p class="note">Bu işlem oturumu silmez; çıkış sadece sen Çıkış butonuna basınca yapılır.</p></section>`; }
function usersPanel(){
...
function featurePlan(){
  ...
  const inputBox = `<div class="card wide smartFeatureBox"><h2>Akıllı Özellik Ekle</h2><p class="muted">Örn: <b>otomatik kapak resmi çekme</b>, <b>oyun adı çekme</b>, <b>etiket ekleme</b> yaz. Sistem hazır modülü bulur, “Bunu mu uygulayayım?” diye öneri verir.</p><form id="featureIdeaForm" class="noteForm"><input name="featureText" placeholder="Örn: Oyunlara otomatik kapak resmi çekme ekle" required /><button class="btn primary" type="submit">Özelliği Anla + Öner</button></form><p class="note">Öneride Düzenle, Sil/Vazgeç ve Uygula + Siteyi Yenile seçenekleri çıkar.</p></div>`;
...
function bind(){
  ...
  document.querySelectorAll('[data-plan-edit]').forEach(el=>el.addEventListener('click', onPlanEdit));
  document.querySelectorAll('[data-plan-delete]').forEach(el=>el.addEventListener('click', onPlanDelete));
...
async function onUpdateNoteSubmit(e){
  ...
  const note = {
    ...
    image:String(fd.get('image') || '').trim() || 'previews/hayatimiz-oyun-v2145-update-notes-preview.png',
...
async function onAction(e){
  ...
  if(action === 'auto-cover-fetch'){ await autoCoverFetch(); }
  if(action === 'auto-game-meta'){ await autoGameMetaFill(); }
  if(action === 'edit-pending-feature'){ if(state.pendingFeature){ const v = prompt('Özellik adını düzenle', state.pendingFeature.title); if(v){ state.pendingFeature.title = v; state.pendingFeature.originalText = v; } render(); } }
  if(action === 'cancel-feature-confirm'){ state.pendingFeature = null; render(); }
  if(action === 'confirm-feature-apply'){ const pending = state.pendingFeature; state.pendingFeature = null; if(pending) await applyFeatureObject(pending, false); }
  if(action === 'confirm-feature-apply-refresh'){ const pending = state.pendingFeature; state.pendingFeature = null; if(pending) await applyFeatureObject(pending, true); }
  if(action === 'refresh-site-data'){ await loadRuntime(); setToast('Site verileri yenilendi.'); }
...
function onFeatureEdit(e){
  ...
  const title = prompt('Özellik adını düzenle', feature?.title || key);
  if(!title) return;
  const item = state.planner.find(p=>p.featureKey===key);
  if(item) item.text = title;
  setToast('Özellik adı panelde güncellendi. Kalıcı kayıt için Siteye Uygula akışı kullanılabilir.');
  render();
}
async function onFeatureDelete(e){
  ...
  persistFeatures();
  const item = state.planner.find(p=>p.featureKey===key);
  if(item) item.status = 'plan';
  render();
  try{ await api('feature-disable', { adminToken: state.session?.adminToken, key }); setToast('Özellik pasife alındı.'); }
  catch(err){ setToast('Özellik local pasife alındı; Supabase sonra güncellenir.'); }
}

async function onPlanEdit(e){
  e.preventDefault();
  const id = e.currentTarget.dataset.planEdit;
  const key = e.currentTarget.dataset.planKey;
  const item = state.planner.find(p => String(p.id) === String(id) || String(p.featureKey) === String(key));
  const current = item?.text || FEATURE_CATALOG.find(f=>f.key===key)?.title || '';
  const title = prompt('Özellik adını düzenle', current);
  if(!title) return;
  if(item) item.text = title;
  render();
  try{ await api('planner-edit', { adminToken: state.session?.adminToken, id, key, title }); await loadPlanner(false); setToast('Özellik düzenlendi.'); }
  catch(err){ setToast('Özellik local düzenlendi; Supabase kaydı sonra denenir.'); }
}
async function onPlanDelete(e){
  e.preventDefault();
  const id = e.currentTarget.dataset.planDelete;
  const key = e.currentTarget.dataset.planKey;
  if(!confirm('Bu özelliği listeden silmek/pasife almak istiyor musun?')) return;
  state.planner = state.planner.filter(p => String(p.id) !== String(id) && String(p.featureKey) !== String(key));
  if(key) state.features[key] = false;
  persistFeatures();
  render();
  try{ await api('planner-delete', { adminToken: state.session?.adminToken, id, key }); await loadPlanner(false); setToast('Özellik silindi/pasife alındı.'); }
  catch(err){ setToast('Özellik local silindi; Supabase kaydı sonra denenir.'); }
}
...
async function autoGameMetaFill(){
  ...
  let genre, tags, cover;
  try{
    const data = await api('game-auto-meta', { adminToken: state.session?.adminToken, title });
    genre = data.genre; tags = data.tags; cover = data.cover;
  }catch(e){
    [genre,tags,cover] = guessGameMeta(title);
  }
  if(form.elements.genre) form.elements.genre.value = genre || 'Genel / Hikaye';
  if(form.elements.tags) form.elements.tags.value = tags || 'oyun, seri, bölüm';
  if(form.elements.cover) form.elements.cover.value = cover || form.elements.cover.value || '';
...
async function onFeatureIdeaSubmit(e){
  ...
  state.pendingFeature = proposeFeatureFromText(text);
  render();
}
...
async function applyFeatureObject(feature, refreshAfter = false){
  if(!isOwner()) return setToast('Siteye uygulama için kurucu veya yönetici yetkisi gerekir.');
  const key = feature.key;
  state.features[key] = true;
  persistFeatures();
  let plannerItem = state.planner.find(p => p.featureKey === key || p.text === feature.title || p.text === feature.originalText);
  ...
  try{
    ...
    if(refreshAfter){ await loadRuntime(); setToast(`${feature.title} siteye uygulandı ve veriler yenilendi.`); }
    else setToast(`${feature.title} siteye uygulandı.`);
  }catch(err){ setToast(`${feature.title} local aktif edildi; Supabase kaydı başarısız: ${err.message}`); }
}
async function autoCoverFetch(){
  if(!featureEnabled('auto_cover_fetch')) return setToast('Önce Otomatik kapak resmi çekme özelliğini Siteye Uygula.');
  try{
    const data = await api('games-auto-cover', { adminToken: state.session?.adminToken });
    if(Array.isArray(data.games)) state.games = data.games.map(mapGame);
    render();
    return setToast(data.changed ? `${data.changed} oyun için kapak Supabase'e kaydedildi.` : 'Kapaksız oyun bulunmadı veya kapaklar zaten dolu.');
  }catch(e){
    const covers = [
    ...
    render();
    setToast(changed ? `${changed} oyun için local kapak atandı. Supabase kaydı başarısız: ${e.message}` : 'Kapaksız oyun bulunmadı.');
  }
}
