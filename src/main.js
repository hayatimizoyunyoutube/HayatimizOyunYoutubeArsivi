...
function seriesWatchModal(){
  ...
  return `<div class="modalOverlay watchPageOverlay"><div class="modal seriesModalV216 professionalWatch"><button class="close" type="button" data-action="close-series-watch">×</button><div class="watchHeader"><div><span class="eyebrow">Site İçi Seri İzleme</span><h2>${esc(g.title)}</h2><p class="muted">${esc(g.seriesName || 'Tek seri')} • ${watched}/${Number(g.eps||episodes.length||0)} bölüm • ${progressPercent(g)}%</p></div><button class="miniBtn primary" data-mark-active-watched-index="${safeIndex}">Bu Bölümü İzledim</button></div><div class="watchLayout"><main>${player}<div class="activeEpisodeInfo"><span class="pill green">${active?`${active.number}. Bölüm`:'Bölüm yok'}</span><h3>${esc(active ? cleanEpisodeTitle(active.title, active.number) : g.title)}</h3><p class="muted">${esc(g.description || 'Bölümü doğrudan site içinden izliyorsun.')}</p></div><div class="progressLine large"><span style="width:${progressPercent(g)}%"></span></div></main><aside><h3>Bölümler</h3><div class="episodeSidebar">${rows}</div></aside></div></div></div>`;
}
...
async function estimatePlaylistEpisodes(formId='gameAddForm'){
  ...
  try{
    ...
    if(episodes.length > 0){
      ...
      setToast(`Playlistten ${episodes.length} video çekildi. Liste profesyonel görünüme aktarıldı; Seriyi İzle ekranında her bölüm ayrı açılır.`);
      return;
    }
    if(count > 0){
...
function markEpisodeWatchedIndex(index){
  const g = state.games.find(x=>String(x.id)===String(state.watchingGameId));
  if(!g) return;
  const eps = seriesEpisodes(g).map((ep,i)=>i===index ? { ...ep, watched:true } : ep);
  g.episodes = eps;
  g.watchedEps = eps.filter(ep=>ep.watched).length;
  setToast(`${index+1}. bölüm izlendi olarak işaretlendi. Kalıcı kayıt için admin düzenleme formundan oyunu güncelle.`);
  render();
}
