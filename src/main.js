...
const VERSION = 'v2.2.0 FIX 4 - Profesyonel Arayüz ve Kart Sistemi';
...
function archiveSideNav(){
  return `<aside class="v220SideNav fixedArchiveMenu fix4SideNav"><div class="sideLogo"><span class="logoMark">🎮</span><div><b>Hayatımız Oyun</b><small>Oyun Arşivi & Seriler</small></div></div>${sideNavItems().map(item=>`<button class="sideNavItem ${state.page===item.page || (item.page==='Oyun Arşivi' && ['Popüler','Tamamlanan','Devam Eden','Yakında','Korku','Aksiyon','Hikaye Odaklı'].includes(state.page))?'active':''}" data-page="${esc(item.page)}"><span>${item.icon}</span>${esc(item.label)}</button>`).join('')}<button class="sideNavItem manager" data-admin="Genel Bakış"><span>👑</span><div><b>Yönetim</b><small>Yönetici Paneline Git</small></div></button><div class="supportBox">💜 <b>Destekle</b><small>Arşivi büyütmek için seri ve bölüm takibini güncel tut.</small></div></aside>`;
}
...
function gamesAdmin(){
  const buttons = `<button class="btn primary" data-action="toggle-game-form">+ Yeni Oyun Ekle</button><button class="btn" data-action="auto-cover-fetch">Kapaksızlara Kapak Öner</button><button class="btn" data-action="bulk-sync-playlists">Toplu Playlist Senkronize</button><button class="btn" data-action="open-series-sort-panel">Seri Sıralama</button><span class="pill green">v2.2.0 FIX 4</span>`;
  return `<section class="card wide fix4AdminGames"><div class="sectionHead"><div><span class="eyebrow">Profesyonel Yönetim</span><h2>Oyun Yönetimi</h2><p class="muted">Oyun ekleme/düzenleme formu daha düzenli, önizlemeli ve okunabilir hale getirildi.</p></div><div class="heroActions">${buttons}</div></div>${youtubeHealthPanel()}${missingCoverPanel()}${state.showGameForm ? gameAddForm() : ''}${state.editingGameId ? gameEditForm() : ''}${rawgCandidatePanel()}${coverSuggestionPanel()}${advancedSearchPanel()}${gameGrid()}</section>`;
}
