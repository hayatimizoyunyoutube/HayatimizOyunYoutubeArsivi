...
const VERSION = 'v2.1.6 - Büyük Arşiv Otomasyonu + Bölüm Yönetimi';
...
async function estimatePlaylistEpisodes(formId='gameAddForm'){
  ...
  try{
    ...
    if(count > 0){
      setFormValue(form, 'eps', count);
      if(episodes.length) setFormValue(form, 'episodesText', episodesToText(episodes));
      if(form.id === 'gameAddForm') saveGameDraftFromForm(form);
      return setToast(episodes.length ? `Playlistten ${episodes.length} bölüm çekildi ve forma işlendi.` : `Playlist bölüm sayısı çekildi: ${count}. Bölüm başlıkları için YOUTUBE_API_KEY gerekli olabilir.`);
    }
    setToast('Playlist sayısı alınamadı. YouTube API key yoksa manuel sayı ve bölüm listesi girebilirsin.');
