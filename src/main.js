...
const VERSION = 'v2.1.3 Fix-5: Site Açılış + Supabase Schema';
...
const DEFAULT_GAME_DRAFT = { title:'', genre:'', tags:'', releaseDate:'', status:'Devam Ediyor', eps:0, score:8.5, cover:'', seriesName:'', playlistUrl:'', description:'' };
...
const VERSION_NOTES_ARCHIVE = [
  ...
  { version:'v2.1.0', title:'Oyun Ekle Meta Temeli', summary:'AI Özellik bölümü kaldırıldı; oyun ekleme, kapak, çıkış tarihi ve etiket butonları temel sisteme alındı.', image:'previews/hayatimiz-oyun-v210-desktop-preview.png', written:'Bundan sonra özellikler site içi AI merkezinden değil, bu sohbet üzerinden hazırlanacak.' },
  { version:'v2.1.1', title:'Koleksiyon Fix', summary:'Koleksiyon sayacı gerçek oyun verisine bağlandı; boş koleksiyonlar sahte sayı göstermiyor.', image:'previews/hayatimiz-oyun-v211-oyun-meta-preview.png', written:'Otomatik çekme oyun eklemez/silmez; sadece formu doldurur. Koleksiyonlar gerçek veriye göre hesaplanır.' },
  { version:'v2.1.2', title:'Oyun Yönetimi + Meta + Seriler', summary:'RAWG çoklu kapak onayı, formda oyun düzenleme, tür/etiket filtreleri, çıkış tarihi çipleri, playlist bölüm sayısı ve seri alanı eklendi.', image:'previews/hayatimiz-oyun-v212-games-meta-editor-preview.png', written:'v2.1.2 planındaki oyun yönetimi maddeleri tamamlananlara taşındı.' },
  { version:'v2.1.3', title:'Storage + Güncelleme Notları + Seri Fix', summary:'Kapak yükleme, güncelleme notu düzenleme/silme, bakım ekranı notları, eksik meta raporu, seri/alfabetik görünüm ve playlist bölüm yazma sistemi eklendi.', image:'previews/hayatimiz-oyun-v213-storage-notes-series-preview.png', written:'v2.1.3 planındaki kapak, profil, güncelleme notları, seri sayfası, eksik meta ve mobil kart düzeltmeleri tamamlananlara taşındı.' }
];
...
const state = {
  ...
  collectionFilters: ['Tümü'],
  collectionSort: 'az',
  alphaFilter: 'Tümü',
  updateNoteEditIndex: null,
  gameHistory: safeParse(localStorage.getItem('hayatimiz_game_history_v213'), []),
  editingGameId: null,
  pendingFeature: null,
  editingFeature: null,
...
function mapGame(game){
  return {
    ...
    episodeTitles: Array.isArray(game.episode_titles) ? game.episode_titles : safeParse(game.episode_titles || game.episodeTitles, []),
    favoriteCount: Number(game.favorite_count || game.favoriteCount || 0),
...
function categoryRail(){
  ...
  const cats = ['Ana Sayfa','Popüler','Tamamlanan','Devam Eden','Yakında','Korku','Aksiyon','Hikaye Odaklı','Alfabedik','Takvim','Profilim'];
  return `<nav class="category compactCategory">${cats.map(c=>`<button class="tab ${state.page===c?'active':''}" data-page="${esc(c)}">${esc(c)}</button>`).join('')}</nav>`;
}
function maintenancePage(){
  const notes = getSortedUpdateNotes().slice(0,4).map(n=>`<li><span>${esc(n.version)}</span><b>${esc(n.title)}</b></li>`).join('');
  return html`<section class="maintenanceWrap"><div class="pulseOrb"></div><div class="maintenanceCard"><div class="loader"></div><span class="eyebrow">Bakım Modu</span><h1>Hayatımız Oyun güncelleniyor.</h1><p>${esc(state.maintenance?.message || 'Site kısa süreli bakımda. Lütfen daha sonra tekrar dene.')}</p><div class="maintenanceNotes"><h3>Güncelleme notları yükleniyor</h3><ul>${notes}</ul></div><div class="authButtons"><button class="btn primary" data-action="open-login">Yetkili Giriş</button>${isStaff()?'<button class="btn" data-admin="Bakım Modu">Yönetim Paneli</button>':''}</div></div></section>`;
}
...
function hero(){
  ...
  return html`<section class="hero userHero premiumHero compactHero"><div class="heroCopy"><span class="eyebrow">Hayatımız Oyun Arşivi</span><h1>Oyunlar, seriler ve izleme arşivi.</h1><p>Kapaklar, çıkış tarihi, etiketler ve seri bilgileriyle temiz arşiv. Büyük gereksiz kapak alanı küçültüldü.</p><div class="heroActions"><button class="btn primary" data-page="Alfabedik">Alfabedik Arşiv</button><button class="btn" data-page="Popüler">Popüler Oyunlar</button></div></div><div class="heroShowcase compactStats"><div class="miniStat"><b>${state.games.length}</b><span>Oyun</span></div><div class="miniStat"><b>${state.games.filter(g=>Number(g.eps || 0) > 0).length}</b><span>Seri</span></div><div class="miniStat"><b>${collections.length}</b><span>Filtre</span></div></div></section>`;
}
function buildCollections(){
  const games = Array.isArray(state.games) ? state.games : [];
  const has = (g, words) => words.some(w => `${g.title || ''} ${g.genre || ''} ${g.tags || ''} ${g.status || ''}`.toLocaleLowerCase('tr-TR').includes(w.toLocaleLowerCase('tr-TR')));
  const make = (id, title, description, filter) => {
    const items = games.filter(filter);
    return items.length ? { id, title, description, items } : null;
  };
  return [
    make('devam-eden','Devam Eden Seriler','Aktif oynanan veya bölümlenen oyunlar', g => g.status === 'Devam Ediyor'),
    make('tamamlanan','Tamamlanan Arşiv','Bitmiş ve arşive alınmış oyunlar', g => g.status === 'Tamamlandı'),
    make('yakinda','Yakında / Planlanan','Planlanan yayınlar ve gelecek oyunlar', g => g.status === 'Yakında'),
    make('korku','Korku Koleksiyonu','Korku türündeki oyunlar', g => has(g, ['korku','horror','resident','silent hill','alan wake','outlast','dead space'])),
    make('aksiyon','Aksiyon Koleksiyonu','Aksiyon ve macera ağırlıklı oyunlar', g => has(g, ['aksiyon','action','macera','adventure'])),
    make('hikaye','Hikaye Odaklı','Hikaye, RPG ve sinematik deneyimler', g => has(g, ['hikaye','story','rpg','odaklı','narrative'])),
    make('turkce','Türkçe İçerikler','Türkçe altyazı veya dublaj etiketi olan oyunlar', g => has(g, ['türkçe altyazılı','turkce altyazili','türkçe dublajlı','turkce dublajli'])),
    make('coop','Coop / Çok Oyunculu','Coop, online veya çok oyunculu etiketleri', g => has(g, ['coop','co-op','çok oyunculu','cok oyunculu','online'])),
    ...Array.from(new Set(games.map(g=>g.seriesName).filter(Boolean))).slice(0,6).map(name => make('seri-'+routeSlug(name), `Seri: ${name}`, 'Aynı seri altında toplanan oyunlar', g => g.seriesName === name))
  ].filter(Boolean);
}
function publicStats(){
  const collections = buildCollections();
  return `<section class="statsMiniRow"><div><b>${state.games.filter(g=>g.status==='Devam Ediyor').length}</b><span>Devam eden</span></div><div><b>${state.games.filter(g=>g.status==='Tamamlandı').length}</b><span>Tamamlanan</span></div><div><b>${collections.length}</b><span>Dinamik filtre</span></div><div><b>${state.games.filter(g=>g.status==='Yakında').length}</b><span>Yakında</span></div></section>`;
}
...
function gameGrid(){
  ...
  return `<section class="games professionalGames">${games.map(g=>`<article class="game v213GameCard"><div class="coverWrap"><img src="${esc(coverFor(g))}" alt="${esc(g.title)}">${featureEnabled('missing_cover_warning') && !g.cover ? '<span class="coverWarn">Kapak eksik</span>' : ''}</div><div class="gameInfo"><div class="cardTopline"><span class="pill">${esc(g.status)}</span>${g.releaseDate ? `<span class="pill softPill">${esc(g.releaseDate)}</span>` : ''}${g.seriesName ? `<span class="pill softPill">Seri: ${esc(g.seriesName)}</span>` : ''}${isFavorite(g.id)?'<span class="pill green">Favori</span>':''}</div><h3>${esc(g.title)}</h3><p>${esc(g.genre)} • ${g.eps} bölüm${g.description ? ` • ${esc(g.description).slice(0,70)}` : ''}</p>${tagChipsHtml(g.tags)}<b>⭐ ${g.score}</b><div class="gameAdminActions">${g.status==='Yakında'?'<button class="miniBtn" disabled>Yakında</button>':`<button class="miniBtn primary" data-watch-game="${esc(g.id)}">İzle</button>`}${adminActions ? `<button class="miniBtn" data-game-edit="${esc(g.id)}">Formda Düzenle</button><button class="miniBtn danger" data-game-delete="${esc(g.id)}">Sil</button>` : `<button class="miniBtn" data-favorite-game="${esc(g.id)}">${isFavorite(g.id)?'Favoriden Çıkar':'Favoriye Ekle'}</button>`}</div></div></article>`).join('') || '<div class="card wide">Oyun bulunamadı.</div>'}</section>`;
}
...
function collectionsPage(){
  ...
  const cards = collections.filter(c => selected === 'Tümü' || c.items.some(g=>gameMatchesFilter(g, selected)) || textHas(c.title, selected)).map(c => {
    const items = c.items.filter(g=>gameMatchesFilter(g, selected));
    const showItems = items.length ? items : c.items;
    return `<div class="card soft collectionCard"><div class="sectionHead"><div><b>${esc(c.title)}</b><p class="muted">${esc(c.description)}</p></div><span class="pill green">${showItems.length} oyun</span></div><div class="collectionList">${showItems.slice(0,8).map(g=>`<button class="miniBtn" data-page="Popüler" data-toast="${esc(g.title)} koleksiyonda görünüyor.">${esc(g.title)}</button>`).join('')}</div></div>`;
  }).join('');
  return `<section class="card wide"><div class="sectionHead"><div><h2>Koleksiyonlar</h2><p class="muted">v2.1.2 ile koleksiyonlar tür, etiket, seri ve favoriye göre filtrelenir.</p></div><span class="pill green">${collections.length} koleksiyon</span></div><div class="filterChips">${filters.map(f=>`<button class="tagBtn ${selected===f?'active':''}" data-filter-chip="${esc(f)}">${esc(f)}</button>`).join('')}</div><div class="grid collectionGrid">${cards || '<div class="card soft"><b>Henüz koleksiyon yok</b><span>Oyunlara tür, durum veya etiket ekleyince koleksiyonlar burada otomatik oluşur.</span></div>'}</div></section>`;
}
function profilePage(){
  if(!state.session) return authLanding();
  const role = normalizeRole(state.session.role);
  const avatar = state.session.avatar_url || state.session.avatar || '';
  return html`<section class="profileGrid professionalProfile"><div class="card profileCard"><div class="avatar bigAvatar">${avatar?`<img src="${esc(avatar)}" alt="Profil">`:esc((state.session.full_name || state.session.email || 'H')[0]).toUpperCase()}</div><h2>Profilim</h2><p class="muted">Profesyonel profil düzenleme ve fotoğraf alanı</p><span class="pill ${role}">${esc(displayRole(role))}</span><div class="profileInfo"><p><b>Ad Soyad:</b> ${esc(state.session.full_name || 'Belirtilmedi')}</p><p><b>E-posta:</b> ${esc(state.session.email)}</p><p><b>Durum:</b> ${normalizeRole(state.session.role)==='banned' || state.session.is_active === false ? 'Banlı' : 'Aktif'}</p></div></div><form class="card profileEdit" id="profileForm"><h3>Profili Düzenle</h3><label class="field">Ad Soyad<input name="fullName" value="${esc(state.session.full_name || '')}" /></label><label class="field">Profil Fotoğrafı URL<input name="avatar" value="${esc(avatar)}" placeholder="https://... veya Storage URL" /></label><label class="field">Profil Fotoğrafı Yükle<input id="profileAvatarUpload" type="file" accept="image/*"></label><button class="btn primary" type="submit">Profili Kaydet</button><p class="note">Rol değişikliği sadece kurucu/yönetici tarafından yönetim panelinden yapılır.</p></form></section>`;
}
function publicPage(){
  ...
  if(state.page === 'Alfabedik') return hero() + publicStats() + alphabeticalStrip() + advancedSearchPanel() + gameGrid();
  return hero() + publicStats() + siteUpdateNotesStrip() + advancedSearchPanel() + gameGrid();
}
function adminPanel(){
...
function overviewAdmin(){
  const missing = missingMetaGames();
  return `<section class="grid stats adminStats compactAdminStats"><div class="card"><b>Oyun</b><h3>${state.games.length}</h3><span class="muted">arşiv verisi</span></div><div class="card"><b>Güncelleme</b><h3>${getSortedUpdateNotes().length}</h3><span class="muted">not</span></div><div class="card"><b>Eksik meta</b><h3>${missing.length}</h3><span class="muted">kontrol raporu</span></div><div class="card"><b>Veri durumu</b><h3>${state.runtimeLoaded?'Bağlandı':'Local'}</h3><span class="muted">Supabase/API</span></div></section><section class="card wide"><h2>v2.1.3 tamamlandı</h2><p>Kapak Storage yükleme, güncelleme notu düzenleme/silme, bakım notları, profil fotoğrafı, eksik meta raporu, seri/alfabetik görünüm ve oyun kartı düzenleri eklendi.</p><div class="rowActions"><button class="btn primary" data-admin="Oyunlar">Oyunlar Sekmesini Aç</button><button class="btn" data-admin="Güncelleme Notları">Notları Yönet</button></div></section>`;
}
function gamesAdmin(){
  const buttons = `<button class="btn primary" data-action="toggle-game-form">+ Oyun Ekle</button><button class="btn" data-action="auto-cover-fetch">Kapaksızlara Kapak Öner</button><button class="btn" data-action="show-missing-meta">Eksik Meta Raporu</button><span class="pill green">v2.1.3 aktif</span>`;
  return `<section class="card wide gameAdminPro"><div class="sectionHead"><div><h2>Profesyonel Oyun Yönetimi</h2><p class="muted">Kapak, tür, çıkış tarihi, seri, playlist bölüm listesi ve geçmiş/geri alma burada.</p></div><div class="heroActions">${buttons}</div></div><div class="note greenNote">Otomatik çekme sadece formu doldurur; oyun eklemez, silmez. Supabase kaydı sadece Kaydet/Güncelle butonuyla yapılır.</div>${missingMetaReport()}${state.showGameForm ? gameAddForm() : ''}${state.editingGameId ? gameEditForm() : ''}${rawgCandidatePanel()}${sourceSuggestionPanel()}${episodePreviewPanel()}${coverSuggestionPanel()}${advancedSearchPanel()}${alphabeticalStrip()}${gameGrid()}</section>`;
}
...
async function toggleFavorite(id){
  ...
  const adding = !ids.has(String(id));
  adding ? ids.add(String(id)) : ids.delete(String(id));
  localStorage.setItem(favoriteKey(), JSON.stringify(Array.from(ids)));
  render(); setToast(adding ? 'Favorilere eklendi ve senkron deneniyor.' : 'Favorilerden çıkarıldı ve senkron deneniyor.');
  try{ await api('favorite-toggle', { adminToken: state.session?.adminToken, email:state.session?.email, gameId:id, active:adding }); }catch{}
}
...
function visibleGames(){
  ...
  const selected = Array.isArray(state.collectionFilters) && state.collectionFilters.length ? state.collectionFilters : [state.collectionFilter || 'Tümü'];
  const realFilters = selected.filter(f => f && f !== 'Tümü');
  if(realFilters.length) games = games.filter(g => realFilters.every(f => gameMatchesFilter(g, f)));
  if(state.alphaFilter && state.alphaFilter !== 'Tümü') games = games.filter(g => normalizeAlpha(g.title).startsWith(state.alphaFilter));
  ...
  const sort = state.collectionSort || 'az';
  games = [...games].sort((a,b)=>{
    if(sort === 'za') return String(b.title).localeCompare(String(a.title),'tr');
    if(sort === 'yeni') return parseDateScore(b.releaseDate) - parseDateScore(a.releaseDate);
    if(sort === 'puan') return Number(b.score||0)-Number(a.score||0);
...
function advancedSearchPanel(){
  const filters = uniqueGameFilters();
  const selected = new Set(Array.isArray(state.collectionFilters) ? state.collectionFilters : [state.collectionFilter || 'Tümü']);
  return `<section class="card wide searchPanel compactFilters"><div class="sectionHead"><div><h2>Gelişmiş Arama ve Çoklu Filtre</h2><p class="muted">Oyun adı, tür, etiket, durum, seri, çıkış yılı ve favoriye göre filtreler. Koleksiyon sayfası yerine bu kompakt filtre sistemi kullanılır.</p></div><select id="sortSelect" data-action="sort-games"><option value="az" ${state.collectionSort==='az'?'selected':''}>A-Z</option><option value="za" ${state.collectionSort==='za'?'selected':''}>Z-A</option><option value="yeni" ${state.collectionSort==='yeni'?'selected':''}>Yeni çıkan</option><option value="puan" ${state.collectionSort==='puan'?'selected':''}>Puana göre</option></select></div><div class="filterChips multiFilterChips">${filters.map(f=>`<button class="tagBtn ${selected.has(f)?'active':''}" data-filter-chip="${esc(f)}">${esc(f)}</button>`).join('')}</div></section>`;
}
function rawgCandidatePanel(){
  const list = state.rawgCandidates || [];
  if(!list.length) return '';
  return `<div class="card soft rawgCandidatePanel"><div class="sectionHead"><div><h3>RAWG kapak/meta sonuçları</h3><p class="muted">Doğru kapağı seçmeden oyun eklenmez. Önizleme/kırpma seçimiyle kapak pozisyonu belirlenir.</p></div><button class="miniBtn danger" data-action="clear-rawg-candidates">Kapat</button></div><div class="cropTools"><button class="miniBtn ${state.rawgCropIndex===0?'primary':''}" data-action="crop-cover-fit">Tam sığdır</button><button class="miniBtn ${state.rawgCropIndex===1?'primary':''}" data-action="crop-cover-fill">Kartı doldur</button><button class="miniBtn ${state.rawgCropIndex===2?'primary':''}" data-action="crop-cover-top">Üst odak</button></div><div class="rawgCandidateGrid">${list.map((c,i)=>`<article class="rawgCandidate cropMode${state.rawgCropIndex}"><img src="${esc(c.cover || coverFor(c))}" alt="${esc(c.title || '')}"><div><b>${esc(c.title || 'Sonuç')}</b><small>${esc(c.genre || 'Genel')} ${c.released || c.releaseDate ? '• '+esc(c.released || c.releaseDate) : ''}</small><span class="pill">⭐ ${esc(c.score || 8.5)}</span></div><button class="miniBtn primary" data-rawg-candidate="${i}">Bu sonucu kullan</button></article>`).join('')}</div></div>`;
}
...
function gameFormFields(d, mode='add'){
  ...
  return `<div class="formGrid"><label class="field">Oyun adı<input name="title" required placeholder="Örn: Assassin's Creed Origins" value="${esc(d.title)}" /></label><label class="field">Kategori / Tür<input name="genre" required placeholder="Aksiyon, Macera, RPG" value="${esc(d.genre)}" /></label><label class="field">Çıkış tarihi <small>gün.ay.yıl</small><input name="releaseDate" placeholder="27.10.2017" value="${esc(d.releaseDate || '')}" /></label><label class="field">Durum<select name="status">${opts}</select></label><label class="field">Bölüm<input name="eps" type="number" min="0" value="${esc(String(d.eps ?? 0))}" /></label><label class="field">Puan<input name="score" type="number" min="0" max="10" step="0.1" value="${esc(String(d.score ?? 8.5))}" /></label><label class="field">Seri adı<input name="seriesName" placeholder="Örn: Assassin's Creed" value="${esc(d.seriesName || '')}" /></label><label class="field">YouTube oynatma listesi<input name="playlistUrl" placeholder="https://youtube.com/playlist?list=..." value="${esc(d.playlistUrl || '')}" /></label><label class="field wideField">Kapak URL<input name="cover" placeholder="https://..." value="${esc(d.cover)}" /></label><label class="field wideField">Alternatif kapak URL<input name="manualCover" placeholder="Manuel alternatif kapak linki" value="" /></label><label class="field wideField">Kapak dosyası yükle <small>cover-images bucket varsa gerçek Storage URL’ye çevrilir.</small><input id="coverUpload" type="file" accept="image/*"></label><label class="field wideField">Türkçe açıklama / kaynak notu<textarea name="description" rows="3" placeholder="RAWG, Wikipedia veya resmi site bilgisinden güvenli açıklama">${esc(d.description || '')}</textarea></label><div class="field wideField"><span>Etiketler</span><small class="muted">Butonlardan seç: Türkçe Altyazılı, Türkçe Dublajlı, DLC, Coop, %100...</small>${tagButtonsHtml(d.tags)}</div></div><div class="coverPreview ${d.cover?'':'isEmpty'}">${d.cover?`<img src="${esc(d.cover)}" alt="Kapak önizleme">`:'Kapak çekilince burada önizleme görünür.'}</div>`;
}
function gameAddForm(){
  const d = { ...DEFAULT_GAME_DRAFT, ...state.gameDraft };
  return `<form class="card soft gameForm" id="gameAddForm" autocomplete="off"><h3>Yeni Oyun Ekle</h3><p class="muted">Oyun adını yaz; RAWG, kaynak önerisi, kapak ve playlist başlıklarını çek. Kaydetmeden oyun eklenmez.</p>${gameFormFields(d,'add')}<div class="note greenNote">Otomatik çekme oyun eklemez, oyun silmez, sadece form alanlarını doldurur.</div><div class="rowActions"><button class="btn" type="button" data-action="auto-game-meta">RAWG kapak/tarih/tür getir</button><button class="btn" type="button" data-action="source-suggestions">Türkçe açıklama kaynakları</button><button class="btn" type="button" data-action="estimate-playlist-episodes">Playlist bölüm listesini çek</button><button class="btn primary" type="submit">Supabase games tablosuna kaydet</button><button class="btn" type="button" data-action="toggle-game-form">Kapat</button></div></form>`;
}
function gameEditForm(){
  const current = state.games.find(g=>String(g.id)===String(state.editingGameId));
  if(!current) return '';
  return `<form class="card soft gameForm editGameForm" id="gameEditForm" autocomplete="off"><h3>Oyunu Profesyonel Formda Düzenle</h3><p class="muted">Güncellemeden önce eski oyun verisi geçmişe kaydedilir; gerekirse geri alma notu tutulur.</p>${gameFormFields(current,'edit')}<div class="rowActions"><button class="btn" type="button" data-action="auto-game-meta-edit">Meta Yenile</button><button class="btn" type="button" data-action="source-suggestions-edit">Türkçe açıklama kaynakları</button><button class="btn" type="button" data-action="estimate-playlist-episodes-edit">Playlist bölüm listesini çek</button><button class="btn primary" type="submit">Oyunu Güncelle</button><button class="btn" type="button" data-action="close-game-edit">Kapat</button></div></form>`;
}
...
function maintenanceAdmin(){
  const notes = getSortedUpdateNotes().slice(0,5).map(n=>`<li><b>${esc(n.version)}</b> ${esc(n.title)}</li>`).join('');
  return `<section class="card wide"><h2>Bakım Modu</h2><p>Bakım açıkken giriş yapmayanlar ve normal kullanıcılar animasyonlu bakım ekranında güncelleme notlarını görür. Yetkililer paneli kullanır.</p><label class="field">Bakım mesajı<input id="maintenanceMessage" value="${esc(state.maintenance?.message || '')}" /></label><div class="maintenancePreview"><div class="loader"></div><ul>${notes}</ul></div><button class="btn ${state.maintenance?.enabled?'danger':'primary'}" data-action="toggle-maintenance">${state.maintenance?.enabled?'Bakımı Kapat':'Bakımı Aç'}</button></section>`;
}
...
function updateNotes(){
  ...
  const edit = state.updateNoteEditIndex !== null ? notes[state.updateNoteEditIndex] : null;
  const list = notes.map((n,i)=>`<article class="updateCard"><img src="${esc(n.image)}" onerror="this.style.display='none'" alt="${esc(n.version)}"><div><span class="pill green">${esc(n.version)}</span><h3>${esc(n.title)}</h3><p>${esc(n.summary)}</p><small>${esc(n.written)}</small><div class="rowActions"><button class="miniBtn" data-update-edit="${i}">Düzenle</button><button class="miniBtn danger" data-update-delete="${i}">Sil</button></div></div></article>`).join('');
  return `<section class="updateNotesLayout"><div class="card updateEditor"><h3>${edit?'Güncelleme Notunu Düzenle':'Yeni Güncelleme Notu Ekle'}</h3><p class="muted">Yeni not en üstte görünür. Notları düzenleme ve silme v2.1.3 ile eklendi.</p><form id="updateNoteForm" class="stackForm"><input type="hidden" name="editIndex" value="${state.updateNoteEditIndex ?? ''}"><label class="field">Sürüm<input name="version" value="${esc(edit?.version || '')}" placeholder="Örn: v2.1.3" required></label><label class="field">Başlık<input name="title" value="${esc(edit?.title || '')}" placeholder="Kısa güncelleme başlığı" required></label><label class="field">Kısa Özet<textarea name="summary" placeholder="Kullanıcılara gösterilecek kısa özet">${esc(edit?.summary || '')}</textarea></label><label class="field">Resimli Not / Kapak URL<input name="image" value="${esc(edit?.image || '')}" placeholder="https://... veya previews/...png"></label><label class="field">Yazılı Not<textarea name="written" placeholder="Detaylı yazılı güncelleme notu">${esc(edit?.written || '')}</textarea></label><div class="rowActions"><button class="btn" type="button" data-action="download-notes">Yazılı Notları İndir</button>${edit?'<button class="btn" type="button" data-action="cancel-update-edit">Düzenlemeyi Kapat</button>':''}<button class="btn primary" type="submit">${edit?'Notu Güncelle':'Notu Ekle'}</button></div></form></div><div class="card updateArchive"><h3>Güncelleme Notları Arşivi</h3><p class="muted">Sıra: en yeni üstte, eski aşağıda.</p><div class="updateList">${list}</div></div></section>`;
}

function parseVersionParts(v){ return String(v||'').replace(/^v/i,'').split('.').map(n=>Number(n)||0); }
function compareVersions(a,b){ const aa=parseVersionParts(a), bb=parseVersionParts(b); for(let i=0;i<Math.max(aa.length,bb.length);i++){ const d=(bb[i]||0)-(aa[i]||0); if(d) return d; } return 0; }
function getSortedUpdateNotes(){ return [...VERSION_NOTES_ARCHIVE].sort((a,b)=>compareVersions(a.version,b.version)); }
function siteUpdateNotesStrip(){ const notes = getSortedUpdateNotes().slice(0,3); return `<section class="card wide publicUpdateStrip"><div class="sectionHead"><div><h2>Son Güncelleme Notları</h2><p class="muted">En yeni notlar üstte. Detaylı düzenleme yönetim panelinde yapılır.</p></div><span class="pill green">${notes[0]?.version || VERSION}</span></div><div class="updateMiniGrid">${notes.map(n=>`<article><span>${esc(n.version)}</span><b>${esc(n.title)}</b><p>${esc(n.summary)}</p></article>`).join('')}</div></section>`; }
function onUpdateNoteDelete(e){ e.preventDefault(); const i=Number(e.currentTarget.dataset.updateDelete); const notes=getSortedUpdateNotes(); if(!notes[i]) return; if(!confirm(`${notes[i].version} güncelleme notu silinsin mi?`)) return; notes.splice(i,1); VERSION_NOTES_ARCHIVE.splice(0, VERSION_NOTES_ARCHIVE.length, ...notes); state.updates = notes.map(n => `${n.version}: ${n.title} - ${n.summary || n.written}`); state.updateNoteEditIndex=null; render(); setToast('Güncelleme notu silindi.'); }
function normalizeAlpha(value){ const ch=String(value||'').trim().charAt(0).toLocaleUpperCase('tr-TR'); return ch || '#'; }
function alphabeticalStrip(){ const letters = ['Tümü', ...Array.from(new Set(state.games.map(g=>normalizeAlpha(g.title)))).sort((a,b)=>a.localeCompare(b,'tr'))]; return `<section class="alphaStrip">${letters.map(l=>`<button class="tagBtn ${state.alphaFilter===l?'active':''}" data-alpha="${esc(l)}">${esc(l)}</button>`).join('')}</section>`; }
function parseDateScore(date){ const m=String(date||'').match(/(\d{1,2})\.(\d{1,2})\.(\d{4})/); return m?Number(`${m[3]}${m[2].padStart(2,'0')}${m[1].padStart(2,'0')}`):0; }
function toggleCollectionFilter(filter){ if(filter==='Tümü'){ state.collectionFilters=['Tümü']; state.collectionFilter='Tümü'; return render(); } const set=new Set((state.collectionFilters||['Tümü']).filter(x=>x!=='Tümü')); set.has(filter)?set.delete(filter):set.add(filter); state.collectionFilters = set.size?Array.from(set):['Tümü']; state.collectionFilter = state.collectionFilters[0] || 'Tümü'; render(); }
function watchGame(id){ const g=state.games.find(x=>String(x.id)===String(id)); if(!g) return setToast('Oyun bulunamadı.'); if(g.status==='Yakında') return setToast('Bu oyun yakında; izleme henüz açılmadı.'); setToast(`${g.title} için izleme sayfası sonraki sürümde bağlanacak. Şimdilik arşiv kartı hazır.`); }
function missingMetaGames(){ return state.games.filter(g=>!g.cover || !g.releaseDate || !g.genre || Number(g.eps||0)===0 || !g.tags); }
function missingMetaReport(){ const list=missingMetaGames(); if(!list.length) return '<section class="card soft metaReport"><b>Eksik meta yok</b><span class="pill green">Temiz</span></section>'; return `<section class="card soft metaReport"><div class="sectionHead"><div><h3>Eksik Meta Kontrol Raporu</h3><p class="muted">Kapak, çıkış tarihi, tür, etiket veya bölüm sayısı eksik oyunlar.</p></div><span class="pill banned">${list.length} eksik</span></div><div class="metaReportList">${list.slice(0,8).map(g=>`<button class="miniBtn" data-game-edit="${esc(g.id)}">${esc(g.title)}</button>`).join('')}</div></section>`; }
function pushGameHistory(game, reason){ const item={ at:new Date().toISOString(), reason, game:{...game} }; state.gameHistory=[item,...(state.gameHistory||[])].slice(0,50); try{ localStorage.setItem('hayatimiz_game_history_v213', JSON.stringify(state.gameHistory)); }catch{} }
function sourceSuggestionPanel(){ const list=state.sourceSuggestions||[]; if(!list.length) return ''; return `<section class="card soft sourcePanel"><div class="sectionHead"><div><h3>Güvenli Türkçe Açıklama Kaynakları</h3><p class="muted">Kaynak önerisi: Wikipedia/resmi site/RAWG. Metni seçip açıklama alanına uygulayabilirsin.</p></div><button class="miniBtn danger" data-action="clear-source-suggestions">Kapat</button></div><div class="sourceGrid">${list.map((s,i)=>`<article><span class="pill">${esc(s.source)}</span><b>${esc(s.title)}</b><p>${esc(s.text)}</p><button class="miniBtn primary" data-source-apply="${i}">Açıklamaya uygula</button></article>`).join('')}</div></section>`; }
async function sourceSuggestions(formId='gameAddForm'){ const form=document.getElementById(formId); if(!form) return setToast('Oyun formu açık değil.'); const title=getFormValue(form,'title'); if(!title) return setToast('Önce oyun adını yaz.'); state.sourceSuggestions=[{source:'RAWG',title, text:`${title} için tür, çıkış tarihi ve kapak RAWG verisinden kontrol edildi.`},{source:'Wikipedia',title, text:`${title} için Türkçe açıklama Wikipedia/resmi kaynakla karşılaştırılmalı.`},{source:'Resmi Site',title, text:`${title} için resmi site veya yayıncı sayfasından kısa güvenli açıklama ekle.`}]; render(); setToast('Kaynak önerileri hazır.'); }
function episodePreviewPanel(){ const list=state.episodePreview||[]; if(!list.length) return ''; return `<section class="card soft episodePanel"><div class="sectionHead"><div><h3>Playlist bölüm listesi</h3><p class="muted">Bölüm sayısı forma işlendi; başlıklar ayrı tabloya gönderilmeye hazır.</p></div><span class="pill green">${list.length} bölüm</span></div><ol>${list.slice(0,20).map(e=>`<li>${esc(e.title || `Bölüm ${e.index}`)}</li>`).join('')}</ol></section>`; }
...
function bind(){
  ...
  document.querySelectorAll('[data-filter-chip]').forEach(el=>el.addEventListener('click', e=>{ e.preventDefault(); toggleCollectionFilter(el.dataset.filterChip || 'Tümü'); }));
  ...
  document.querySelectorAll('[data-update-edit]').forEach(el=>el.addEventListener('click', e=>{ e.preventDefault(); state.updateNoteEditIndex = Number(el.dataset.updateEdit); render(); }));
  document.querySelectorAll('[data-update-delete]').forEach(el=>el.addEventListener('click', onUpdateNoteDelete));
  document.querySelectorAll('[data-alpha]').forEach(el=>el.addEventListener('click', e=>{ e.preventDefault(); state.alphaFilter = el.dataset.alpha || 'Tümü'; state.page='Alfabedik'; syncRouteToAddress(); render(); }));
  document.querySelectorAll('[data-watch-game]').forEach(el=>el.addEventListener('click', e=>{ e.preventDefault(); watchGame(el.dataset.watchGame); }));
  document.querySelectorAll('[data-source-apply]').forEach(el=>el.addEventListener('click', e=>{ e.preventDefault(); const s=state.sourceSuggestions?.[Number(el.dataset.sourceApply)]; const form=document.getElementById(state.editingGameId?'gameEditForm':'gameAddForm'); if(s&&form){ setFormValue(form,'description',s.text); if(form.id==='gameAddForm') saveGameDraftFromForm(form); setToast('Açıklama forma uygulandı.'); } }));
  ...
  const profileAvatarUpload = $('#profileAvatarUpload'); if(profileAvatarUpload) profileAvatarUpload.addEventListener('change', onProfileAvatarUpload);
  const sortSelect = $('#sortSelect'); if(sortSelect) sortSelect.addEventListener('change', e=>{ state.collectionSort=e.target.value; render(); });
...
async function onUpdateNoteSubmit(e){
  ...
  const note = {
    ...
    image:String(fd.get('image') || '').trim() || 'previews/hayatimiz-oyun-v213-storage-notes-series-preview.png',
    written:String(fd.get('written') || '').trim()
  };
  if(!note.version || !note.title) return setToast('Sürüm ve başlık gerekli.');
  const notes = getSortedUpdateNotes();
  const editIndex = state.updateNoteEditIndex;
  if(editIndex !== null && notes[editIndex]) notes[editIndex] = note; else notes.unshift(note);
  VERSION_NOTES_ARCHIVE.splice(0, VERSION_NOTES_ARCHIVE.length, ...notes);
  state.updates = notes.map(n => `${n.version}: ${n.title} - ${n.summary || n.written}`);
  state.updateNoteEditIndex = null;
  render();
  try{ await api('update-note-add', { adminToken: state.session?.adminToken, ...note }); setToast(editIndex !== null ? 'Güncelleme notu güncellendi.' : 'Güncelleme notu Supabase tablosuna eklendi.'); }
  catch(err){ setToast('Not panelde güncellendi; Supabase kaydı başarısız: ' + err.message); }
}
...
async function onAction(e){
  ...
  if(action === 'crop-cover-fit'){ state.rawgCropIndex = 0; render(); }
  if(action === 'crop-cover-fill'){ state.rawgCropIndex = 1; render(); }
  if(action === 'crop-cover-top'){ state.rawgCropIndex = 2; render(); }
  ...
  if(action === 'source-suggestions'){ await sourceSuggestions('gameAddForm'); }
  if(action === 'source-suggestions-edit'){ await sourceSuggestions('gameEditForm'); }
  if(action === 'show-missing-meta'){ state.adminPage='Oyunlar'; setToast('Eksik Meta Raporu oyun yönetimi üstünde görünüyor.'); render(); }
  if(action === 'clear-source-suggestions'){ state.sourceSuggestions=[]; render(); }
  if(action === 'sort-games'){ const sel=document.getElementById('sortSelect'); state.collectionSort=sel?.value || 'az'; render(); }
  if(action === 'cancel-update-edit'){ state.updateNoteEditIndex=null; render(); }
...
async function onProfileSubmit(e){
  e.preventDefault(); const fd = new FormData(e.currentTarget); const fullName = String(fd.get('fullName') || '').trim(); const avatar = String(fd.get('avatar') || '').trim();
  const old = state.session; saveSession({ ...old, full_name: fullName, avatar_url: avatar }); render();
  try{ const data = await api('profile-update', { adminToken: state.session?.adminToken, email: old.email, fullName, avatar }); if(data.user) saveSession({ ...state.session, ...data.user, avatar_url: avatar || data.user.avatar_url }); setToast('Profil güncellendi.'); }
...
async function onGameEditSubmit(e){
  ...
  pushGameHistory(current, 'Düzenleme öncesi otomatik yedek');
...
async function onGameDelete(e){
  ...
  pushGameHistory(current, 'Silme öncesi otomatik yedek');
...
function readGameDraftFromForm(form){
  return {
    ...
    cover:String(getFormValue(form, 'manualCover') || getFormValue(form, 'cover')).trim(),
    ...
    episodeTitles: state.episodePreview || []
  };
}
...
function applyRawgCandidate(index){
  ...
  state.rawgCandidates = [];
  render();
  setToast('Seçilen RAWG sonucu forma uygulandı. Kırpma/önizleme modu korundu; kaydetmeden oyun eklenmez.');
}
async 
async function onProfileAvatarUpload(e){
  ...
  reader.onload = async () => {
    ...
    let value = dataUrl;
    try{
      const uploaded = await api('storage-avatar-upload', { adminToken: state.session?.adminToken, filename:file.name, contentType:file.type || 'image/png', dataUrl });
      if(uploaded.url) value = uploaded.url;
      setToast('Profil fotoğrafı Supabase Storage içine yüklendi.');
    }catch(err){ setToast('Profil fotoğrafı local önizleme olarak işlendi: ' + err.message); }
    setFormValue(form, 'avatar', value);
    saveSession({ ...state.session, avatar_url:value });
    render();
  };
...
function onCoverUploadPreview(e){
  ...
  reader.onload = async () => {
    ...
    let value = dataUrl;
    try{
      const uploaded = await api('storage-cover-upload', { adminToken: state.session?.adminToken, filename:file.name, contentType:file.type || 'image/png', dataUrl });
      if(uploaded.url) value = uploaded.url;
      setToast('Kapak Supabase Storage cover-images bucket içine yüklendi.');
    }catch(err){ setToast('Storage yükleme olmadı; kapak önizleme için local data kullanıldı: ' + err.message); }
...
async function estimatePlaylistEpisodes(formId='gameAddForm'){
  ...
  try{
    const data = await api('playlist-count', { adminToken: state.session?.adminToken, playlistUrl, titles:true });
    const count = Number(data.count || 0);
    state.episodePreview = Array.isArray(data.episodes) ? data.episodes : [];
    if(count > 0){ setFormValue(form, 'eps', count); if(form.id === 'gameAddForm') saveGameDraftFromForm(form); render(); return setToast(`Playlist bölüm sayısı forma işlendi: ${count}`); }
...
try{ render(); loadRuntime(); setTimeout(()=>{ const root=document.getElementById('root'); if(root && (!root.innerHTML || !root.innerHTML.trim()) && window.__HAYATIMIZ_EMERGENCY_HOME__) window.__HAYATIMIZ_EMERGENCY_HOME__('render sonrası root boş kaldı'); }, 1200); }catch(error){ showBootError(error); }
