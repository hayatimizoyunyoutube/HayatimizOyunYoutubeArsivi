...
const VERSION = 'v2.1.4.7 Akıllı Özellik Ekle Fix';
...
function getRelatedFeatureSuggestions(text){
  ...
  return FEATURE_CATALOG
    .map(f => ({
      ...f,
      score: (f.keywords || []).reduce((sum,k)=>sum + (q.includes(String(k).toLowerCase()) ? 3 : 0),0) + (q && f.title.toLowerCase().includes(q) ? 2 : 0)
    }))
    .filter(f => f.score > 0 || !q)
