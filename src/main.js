...
const VERSION = 'v2.1.2 Oyun Yönetimi + Meta + Seriler';
...
const VERSION_NOTES_ARCHIVE = [
  ...
  { version:'v2.1.2', title:'Oyun Yönetimi + Meta + Seriler', summary:'RAWG çoklu kapak onayı, formda oyun düzenleme, tür/etiket filtreleri, çıkış tarihi çipleri, playlist bölüm sayısı ve seri alanı eklendi.', image:'previews/hayatimiz-oyun-v212-games-meta-editor-preview.png', written:'Yüklenen v2.1.2 planındaki oyun yönetimi maddeleri tamamlananlara taşındı. Otomatik çekme form doldurur; seçim/onay olmadan oyun eklemez veya silmez.' },
...
function gameGrid(){
  ...
  return `<section class="games">${games.map(g=>`<article class="game v212GameCard"><div class="coverWrap"><img src="${esc(coverFor(g))}" alt="${esc(g.title)}">${featureEnabled('missing_cover_warning') && !g.cover ? '<span class="coverWarn">Kapak eksik</span>' : ''}</div><div><div class="cardTopline"><span class="pill">${esc(g.status)}</span>${g.releaseDate ? `<span class="pill softPill">${esc(g.releaseDate)}</span>` : ''}${isFavorite(g.id)?'<span class="pill green">Favori</span>':''}</div><h3>${esc(g.title)}</h3><p>${esc(g.genre)} • ${g.eps} bölüm${g.seriesName ? ` • Seri: ${esc(g.seriesName)}` : ''}</p>${tagChipsHtml(g.tags)}<b>⭐ ${g.score}</b>${adminActions ? `<div class="gameAdminActions"><button class="miniBtn" data-game-edit="${esc(g.id)}">Formda Düzenle</button><button class="miniBtn danger" data-game-delete="${esc(g.id)}">Sil</button></div>` : `<div class="gameAdminActions"><button class="miniBtn" data-favorite-game="${esc(g.id)}">${isFavorite(g.id)?'Favoriden Çıkar':'Favoriye Ekle'}</button></div>`}</div></article>`).join('') || '<div class="card wide">Oyun bulunamadı.</div>'}</section>`;
}
...
function publicPage(){
  ...
  return hero() + publicStats() + advancedSearchPanel() + gameGrid();
}
function adminPanel(){
...
function overviewAdmin(){
  return `<section class="grid stats adminStats"><div class="card"><b>Oyun</b><h3>${state.games.length}</h3><span class="muted">arşiv verisi</span></div><div class="card"><b>Güncelleme</b><h3>${state.updates.length}</h3><span class="muted">not</span></div><div class="card"><b>Veri durumu</b><h3>${state.runtimeLoaded?'Bağlandı':'Local'}</h3><span class="muted">Supabase/API</span></div><div class="card"><b>Oyun ekleme</b><h3>Aktif</h3><span class="muted">meta/kapak/etiket</span></div></section><section class="card wide"><h2>v2.1.2 oyun yönetimi tamamlandı</h2><p>RAWG çoklu seçim, formda oyun düzenleme, tür/etiket filtreleri, favori koleksiyon, seri alanı, YouTube playlist bölüm tahmini ve manuel kapak yükleme alanı eklendi.</p><button class="btn primary" data-admin="Oyunlar">Oyunlar Sekmesini Aç</button></section>`;
}
function gamesAdmin(){
  ...
  return `<section class="card wide"><div class="sectionHead"><div><h2>Oyun Yönetimi</h2><p class="muted">v2.1.2: oyun ekleme, formda düzenleme, RAWG çoklu kapak seçimi, seri ve playlist alanları burada.</p></div><div class="heroActions">${buttons}</div></div><div class="note greenNote">Otomatik çekme sadece formu doldurur; oyun eklemez, silmez. Supabase kaydı sadece Kaydet butonuyla yapılır.</div>${state.showGameForm ? gameAddForm() : ''}${state.editingGameId ? gameEditForm() : ''}${rawgCandidatePanel()}${coverSuggestionPanel()}${advancedSearchPanel()}${gameGrid()}</section>`;
}
...
function advancedSearchPanel(){
  const filters = uniqueGameFilters();
  return `<section class="card wide searchPanel"><div class="sectionHead"><div><h2>Gelişmiş Arama</h2><p class="muted">Oyun adı, tür, etiket, durum, çıkış yılı ve seri adına göre arar.</p></div><span class="pill green">v2.1.2</span></div><div class="filterChips">${filters.map(f=>`<button class="tagBtn ${state.collectionFilter===f?'active':''}" data-filter-chip="${esc(f)}">${esc(f)}</button>`).join('')}</div></section>`;
}
function rawgCandidatePanel(){
  const list = state.rawgCandidates || [];
  if(!list.length) return '';
  return `<div class="card soft rawgCandidatePanel"><div class="sectionHead"><div><h3>RAWG kapak/meta sonuçları</h3><p class="muted">Birden fazla sonuç geldi. Doğru oyunu seç; seçmeden oyun eklenmez.</p></div><button class="miniBtn danger" data-action="clear-rawg-candidates">Kapat</button></div><div class="rawgCandidateGrid">${list.map((c,i)=>`<article class="rawgCandidate"><img src="${esc(c.cover || coverFor(c))}" alt="${esc(c.title || '')}"><div><b>${esc(c.title || 'Sonuç')}</b><small>${esc(c.genre || 'Genel')} ${c.released || c.releaseDate ? '• '+esc(c.released || c.releaseDate) : ''}</small><span class="pill">⭐ ${esc(c.score || 8.5)}</span></div><button class="miniBtn primary" data-rawg-candidate="${i}">Bu sonucu kullan</button></article>`).join('')}</div></div>`;
}
