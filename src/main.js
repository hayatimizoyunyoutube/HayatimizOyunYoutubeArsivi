...
const VERSION = 'Oyun Ekle Otomatik Çekme Stabil Fix';
...
const DEFAULT_GAME_DRAFT = { title:'', genre:'', tags:'', status:'Devam Ediyor', eps:0, score:8.5, cover:'' };
...
function gameAddForm(){
  ...
  return `<form class="card soft gameForm" id="gameAddForm" autocomplete="off"><h3>Yeni Oyun Ekle</h3><p class="muted">Oyun adını yazıp otomatik doldur dediğinde yalnızca bu form doldurulur; oyun listesi silinmez, kendiliğinden oyun eklenmez.</p><div class="formGrid"><label class="field">Oyun adı<input name="title" required placeholder="Örn: Red Dead Redemption 2" value="${esc(d.title)}" /></label><label class="field">Kategori / Tür<input name="genre" required placeholder="Aksiyon / Hikaye" value="${esc(d.genre)}" /></label><label class="field">Etiketler<input name="tags" placeholder="açık dünya, hikaye, aksiyon" value="${esc(d.tags)}" /></label><label class="field">Durum<select name="status">${opts}</select></label><label class="field">Bölüm<input name="eps" type="number" min="0" value="${esc(String(d.eps ?? 0))}" /></label><label class="field">Puan<input name="score" type="number" min="0" max="10" step="0.1" value="${esc(String(d.score ?? 8.5))}" /></label><label class="field wideField">Kapak URL<input name="cover" placeholder="https://..." value="${esc(d.cover)}" /></label></div><div class="note greenNote">Otomatik çekme artık sadece form alanlarını doldurur. Kaydet butonuna basmadan Supabase’e oyun eklenmez.</div><div class="rowActions"><button class="btn" type="button" data-action="auto-game-meta">Oyun adından tür/etiket/kapak çek</button><button class="btn primary" type="submit">Supabase games tablosuna kaydet</button><button class="btn" type="button" data-action="toggle-game-form">Kapat</button></div></form>`;
}
...
function featurePlan(){
  ...
  const ctx = getSmartFeatureContext();
  const inputBox = `<div class="card wide smartFeatureBox"><div class="sectionHead"><div><h2>AI Akıllı Özellik Merkezi</h2><p class="muted">Ne istediğini yaz; sistem 4-5 en yakın hazır modülü bulur. Aktif adres/kategori: <b>${esc(ctx.label)}</b>. Seçince aynı sayfada kalır, yeşile döner, Supabase tablosuna kaydedilir ve istersen F5 akışıyla entegre olur.</p></div><span class="pill green">Kategoriye göre şekillenir</span></div><form id="featureIdeaForm" class="noteForm smartInput"><input name="featureText" placeholder="${esc(ctx.label)} için ne düzeltilecek? Örn: oyunları düzenle sil butonu ekle" required /><button class="btn primary" type="submit">AI ile Önerileri Bul</button></form>${aiSuggestionPanel()}</div>`;
...
function autoGameMetaFill(){
  ...
  state.gameDraft = {
    ...DEFAULT_GAME_DRAFT,
    ...state.gameDraft,
    genre: state.gameDraft.genre?.trim() ? state.gameDraft.genre : genre,
    tags: state.gameDraft.tags?.trim() ? state.gameDraft.tags : tags,
    cover: state.gameDraft.cover?.trim() ? state.gameDraft.cover : cover
  };
  persistGameDraft();
  setFormValue(form, 'genre', state.gameDraft.genre);
  setFormValue(form, 'tags', state.gameDraft.tags);
  setFormValue(form, 'cover', state.gameDraft.cover);
  setToast('Form dolduruldu. Oyun kendiliğinden eklenmedi; eklemek için Kaydet butonuna bas.');
}
...
function autoCoverFetch(){
  ...
  state.coverSuggestions = missing.map(g => ({ id:g.id, title:g.title, genre:g.genre, cover:guessGameMeta(g.title)[2] }));
  render();
...
async function onGameAddSubmit(e){
  ...
  try{
    ...
    if(data.game) state.games.unshift(mapGame(data.game));
    else state.games.unshift({ id:'local-'+Date.now(), ...game });
    clearGameDraft();
