...
const VERSION = 'v2.1.9 - Yayın Otomasyonu + Bildirim Merkezi';
...
const state = {
  ...
  notificationsMuted: localStorage.getItem('hayatimiz_notifications_muted_v219') === '1',
  notificationSeen: safeParse(localStorage.getItem('hayatimiz_notification_seen_v219'), {}),
...
function topbar(){
  ...
  return html`<header class="topbar">
    ...
    <div class="topActions">
      ${state.maintenance?.enabled ? '<span class="pill banned">Bakım açık</span>' : '<span class="pill">Yayında</span>'}${state.session ? `<button class="notifyTopBtn" data-page="Bildirimler">🔔 <b>${userNotifications().filter(n=>!state.notificationSeen[n.id] && n.id!=='empty').length}</b></button>` : ''}
...
function categoryRail(){
  ...
  const cats = ['Ana Sayfa','Seriler','Bildirimler','Popüler','Tamamlanan','Devam Eden','Yakında','Korku','Aksiyon','Hikaye Odaklı','Takvim','Profilim'];
...
function publicPage(){
  ...
  if(state.page === 'Bildirimler') return notificationsPage();
...
function userNotifications(){
  ...
  state.games.forEach(game=>{
    normalizeEpisodes(game.episodes || []).forEach((ep,idx)=>{
      ...
      if(ep.videoUrl && !state.notificationSeen[`new-${game.id}-${idx}`]){
  ...
  if(state.maintenance?.enabled) items.unshift({ id:'maintenance-active', type:'bakım', tone:'warn', title:'Bakım modu aktif', text:state.maintenance.message || 'Site bakımda. Yönetici kullanıcılar kontrol edebilir.' });
...
function seriesWatchModal(){
  ...
  return `<div class="modalOverlay watchPageOverlay ${state.cinemaFullscreen?'cinemaFull':''}"><div class="modal seriesModalV216 professionalWatch v218Watch"><button class="close" type="button" data-action="close-series-watch">×</button><div class="watchHeader"><div><span class="eyebrow">Profesyonel Sitede İzle</span><h2>${esc(g.title)}</h2><p class="muted">${esc(g.seriesName || 'Tek seri')} • ${watched}/${Number(g.eps||episodes.length||0)} bölüm • ${progressPercent(g)}%</p></div><div class="rowActions"><button class="miniBtn ${state.cinemaFullscreen?'primary':''}" data-action="toggle-cinema-fullscreen">Sinema: ${state.cinemaFullscreen?'Tam':'Normal'}</button><button class="miniBtn ${state.autoNextEpisode?'primary':''}" data-action="toggle-auto-next">Oto Sonraki: ${state.autoNextEpisode?'Açık':'Kapalı'}</button><button class="miniBtn" data-action="resume-episode">Burada Kaldım</button><button class="miniBtn" data-action="watch-all-series">Seriyi Tümünü İzle</button><button class="miniBtn primary" data-toggle-active-watched-index="${safeIndex}">${active?.watched?'İzlendi Geri Al':'Bu Bölümü İzledim'}</button></div></div><div class="watchLayout"><main>${player}<div class="activeEpisodeInfo"><span class="pill green">${active?`${active.number}. Bölüm`:'Bölüm yok'}</span><h3>${esc(active ? cleanEpisodeTitle(active.title, active.number) : g.title)}</h3><p class="muted">${esc(active?.description || g.description || 'Bölümü doğrudan site içinden izliyorsun.')}</p>${nextGame?`<button class="miniBtn" data-next-series-game="${esc(nextGame.id)}">Seride Sıradaki Oyun: ${esc(nextGame.title)}</button>`:''}</div><label class="field episodeNoteBox">Bölüm Yorumum / Kişisel Notum<textarea data-episode-note="${esc(noteKey)}" rows="3" placeholder="Bu bölüm için yorum veya not yaz...">${esc(userNote)}</textarea></label><div class="watchHistoryBox"><h4>İzleme Geçmişi</h4>${state.watchHistory.slice(0,5).map(h=>`<p><b>${esc(h.title)}</b> ${h.episode}. bölüm • ${new Date(h.at).toLocaleString('tr-TR')}</p>`).join('') || '<p class="muted">Henüz izleme geçmişi yok.</p>'}</div><div class="progressLine large"><span style="width:${progressPercent(g)}%"></span></div><div class="watchShortcuts">Kısayollar: ←/→ bölüm değiştir • Space izle/geri al • F sinema modu</div></main><aside><h3>Bölümler</h3><div class="episodeSidebar">${rows}</div></aside></div></div></div>`;
}
...
function bind(){
  ...
  document.querySelectorAll('[data-notification-seen]').forEach(el=>el.addEventListener('click', e=>{ state.notificationSeen[el.dataset.notificationSeen]=true; localStorage.setItem('hayatimiz_notification_seen_v219', JSON.stringify(state.notificationSeen)); render(); }));
