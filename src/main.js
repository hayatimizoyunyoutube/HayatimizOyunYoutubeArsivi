...
const VERSION = 'Akıllı Özellik Düzenle Stabil Fix';
...
function getAiFeatureSuggestions(text){
  ...
  const scored = FEATURE_CATALOG
    .map(f => ({ ...f, score: featureScore(f, clean), matched:true, originalText:clean }))
  ...
  const custom = {
    ...
    title: clean || 'Yeni özel özellik isteği',
    group:'Adminin Önerileri',
    target:'Yönetim Paneli > Özellik Planı',
    keywords:[],
    description:'Hazır modül eşleşmezse bu istek tabloya plan/öneri olarak kaydedilir. Kod gerektiren gerçek özellikler sonraki paketle modüle dönüştürülür.',
    next:'Bu özel istek için gerçek modül kodu ekle',
    matched:false,
    originalText:clean,
    score:0
  };
...
function modal(){
  ...
  if(state.editingFeature){
    ...
    blocks.push(html`<div class="modalOverlay"><form class="modal stableEditModal" id="featureEditForm"><button class="close" type="button" data-action="close-feature-edit">×</button><span class="eyebrow">Sayfa içi düzenleme</span><h2>Özelliği düzenle</h2><p class="muted">Bu pencere tarayıcı uyarısı değildir; kaydedince aynı sayfada kalır ve kart anında güncellenir.</p><input type="hidden" name="key" value="${esc(f.key)}"><label class="field">Özellik adı<input name="title" value="${esc(f.title || '')}" required autofocus></label><label class="field">Hedef alan<input name="target" value="${esc(f.target || 'Yönetim Paneli > Özellik Planı')}"></label><label class="field">Açıklama<textarea name="description" rows="4">${esc(f.description || '')}</textarea></label><div class="editStatusBox"><span class="pill ${featureEnabled(f.key)?'green':'banned'}">${featureEnabled(f.key)?'Sitede aktif':'Henüz aktif değil'}</span><span>${esc(f.key)}</span></div><div class="rowActions"><button class="btn primary" type="submit">Kaydet</button>${!featureEnabled(f.key)?`<button class="btn" type="button" data-feature-apply="${esc(f.key)}">Kaydetmeden Siteye Uygula</button>`:''}<button class="btn" type="button" data-action="close-feature-edit">Vazgeç</button></div></form></div>`);
  }
  return blocks.join('');
}
...
async function onFeatureEditSubmit(e){
  ...
  if(item){
    ...
    state.planner.unshift({ id:'local-'+Date.now(), group:current.group || 'Adminin Önerileri', text:title, status:featureEnabled(key)?'tamam':'plan', featureKey:key, target, description });
  }
...
async function applyFeatureObject(feature, options={}){
  ...
  try{
    ...
    setToast(`${feature.title} siteye uygulandı. Aynı sayfada kaldın.`);
