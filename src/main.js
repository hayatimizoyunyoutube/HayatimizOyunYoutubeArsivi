...
const DEFAULT_NOTES = [
  {id:'v230-premium-seri-arsiv',version:'v2.3.0',title:'🎬 Premium Seri Sayfası ve Arşiv Deneyimi',summary:'Seriler artık daha sinematik hero, durum grupları, kompakt kartlar, sıradaki bölüm bilgisi ve gelişmiş arşiv görünümüyle listelenir.',status:'Tamamlandı'},
  {id:'v229-public-acilis-cilasi',version:'v2.2.9',title:'🏠 Public Açılış Cilası',summary:'Ana sayfa, arşiv, seriler, bakım notları ve kapak/bilgi çekme akışları v4.0.0 açılış hedefine göre cilalandı.',status:'Tamamlandı'},
  {id:'v228-bakim-planlari',version:'v2.2.8',title:'🛠️ Bakım Planları ve 4.0 Açılış Hedefi',summary:'Bakım ekranında güncel tamamlananlar, sıradaki planlar, otomatik açılış tarihi ve v4.0.0 ana açılış hedefi düzenlendi.',status:'Tamamlandı'},
  {id:'v225-veri-sagligi',version:'v2.2.5',title:'💾 Veri Sağlığı ve Otomatik Yedekleme',summary:'Veri sağlığı merkezi, eksik kapak/tarih kontrolü, takvim kontrolü ve güvenli JSON yedekleme araçları eklendi.',status:'Tamamlandı'},
...
function renderEpisodeList(episodes, watched=0){
  ...
  return `<div class="episodeList">${list.map(ep=>`<article class="episodeItem ${Number(ep.number)<=Number(watched)?'watched':''}"><img src="${esc(ep.thumbnail||game.cover||'/assets/hayatimiz-kapak.png')}" onerror="this.src='${esc(game.cover||'/assets/hayatimiz-kapak.png')}'" alt="${esc(ep.title)}"><div><span class="pill">${esc(ep.number)}. Bölüm</span><h4>${esc(ep.title)}</h4>${ep.videoUrl?`<a class="miniBtn" href="${esc(ep.videoUrl)}" target="_blank" rel="noreferrer">YouTube Aç</a>`:''}</div></article>`).join('')}</div>`;
}
...
function maintenanceZiyaretçi(){
  ...
  const plannedFallback=[
    {version:'v2.3.1',title:'Kullanıcı Profil Sistemi',summary:'Avatar, favori seriler, son izlenenler ve profil kartı geliştirilecek.'},
    {version:'v2.3.2',title:'Favoriler ve Takip Sistemi',summary:'Kullanıcılar seri ve oyunları favoriye alabilecek.'},
