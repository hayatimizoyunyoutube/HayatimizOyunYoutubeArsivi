...
const VERSION = 'v2.2.3 FIX 2 - Oyun Ekle ve AI Stabil';
...
/* v2.2.3 FIX 2 - oyun ekleme + AI özellik merkezi stabil fix */
const V223_FIX2_LABEL = 'v2.2.3 FIX 2 - Oyun Ekle ve AI Stabil';
const V223_FIX2_AI_APPLIED_KEY = 'hayatimiz_ai_feature_applied_fix2';
const V223_FIX2_AI_DRAFT_KEY = 'hayatimiz_ai_feature_draft_fix2';

function v223Fix2SetToast(message){ try{ setToast(message); }catch{ console.log(message); } }
function v223Fix2ActiveGameForm(){ return document.getElementById(state.editingGameId ? 'gameEditForm' : 'gameAddForm') || document.getElementById('gameAddForm') || document.getElementById('gameEditForm'); }
function v223Fix2ReadForm(form){ return form ? readGameDraftFromForm(form) : { ...DEFAULT_GAME_DRAFT }; }
function v223Fix2PatchForm(form, patch){
  if(!form) return;
  Object.entries(patch || {}).forEach(([k,v])=>setFormValue(form,k,v));
  updateOpenGameFormSnapshot(form);
  try{ fix12UpdateFormPreview(form); }catch{}
}
function v223Fix2Story(title='', genre=''){
  const name = String(title || 'Bu oyun').trim() || 'Bu oyun';
  if(typeof fix12DetailedStory === 'function') return fix12DetailedStory(name, genre || fix12GenreFromTitle(name));
  return `${name}, karakterlerin hedefleri, karşılaştıkları tehlikeler ve oyunun dünyası etrafında ilerleyen hikaye odaklı bir deneyim sunar. Oyuncu, bölüm bölüm gelişen olayları takip ederken keşif, mücadele ve karar anlarıyla oyunun atmosferini daha yakından yaşar.`;
}
function v223Fix2Genre(title=''){
  if(typeof fix12GenreFromTitle === 'function') return fix12GenreFromTitle(title);
  if(typeof fix10GenreFromTitle === 'function') return fix10GenreFromTitle(title);
  return 'Aksiyon-macera, hikaye odaklı, tek oyunculu';
}
async function v223Fix2MetaFill(form){
  if(!form) return v223Fix2SetToast('Oyun formu açık değil.');
  const title = getFormValue(form,'title').trim();
  if(!title) return v223Fix2SetToast('Önce oyun adını yaz.');
  const current = v223Fix2ReadForm(form);
  let meta = {};
  try{ meta = localGameMeta(title) || {}; }catch{ meta = {}; }
  try{
    const data = await api('game-meta', { adminToken: state.session?.adminToken, title });
    if(data?.meta) meta = { ...meta, ...data.meta };
    state.rawgCandidates = Array.isArray(data?.candidates) ? data.candidates : state.rawgCandidates;
  }catch{}
  const genre = meta.genre || current.genre || v223Fix2Genre(title);
  const releaseDate = (typeof fix14NormalizeDate === 'function') ? fix14NormalizeDate(meta.releaseDate || meta.released || current.releaseDate || '') : (meta.releaseDate || meta.released || current.releaseDate || '');
  v223Fix2PatchForm(form, {
    ...current,
    title: meta.title || current.title || title,
    genre,
    releaseDate,
    ...
    cover: meta.cover || current.cover || '',
    description: current.description || v223Fix2Story(title, genre)
  });
  v223Fix2SetToast('Meta, kapak, tarih ve türler formu silmeden güncellendi.');
}
function v223Fix2RefetchStory(form){
  if(!form) return v223Fix2SetToast('Oyun formu açık değil.');
  const title = getFormValue(form,'title').trim();
  if(!title) return v223Fix2SetToast('Önce oyun adını yaz.');
  const genre = getFormValue(form,'genre') || v223Fix2Genre(title);
  v223Fix2PatchForm(form,{ description:v223Fix2Story(title, genre), genre });
  v223Fix2SetToast('Hikaye formu silmeden tekrar çekildi.');
}
function v223Fix2RefetchGenres(form){
  if(!form) return v223Fix2SetToast('Oyun formu açık değil.');
  const title = getFormValue(form,'title').trim();
  if(!title) return v223Fix2SetToast('Önce oyun adını yaz.');
  v223Fix2PatchForm(form,{ genre:v223Fix2Genre(title) });
  v223Fix2SetToast('Türler Türkçe olarak tekrar forma işlendi.');
}
function v223Fix2GameForm(d={}, mode='add'){
  const draft = { ...DEFAULT_GAME_DRAFT, ...(d || {}) };
  const opts = ['Devam Ediyor','Tamamlandı','Popüler','Yakında'].map(st=>`<option ${draft.status===st?'selected':''}>${st}</option>`).join('');
  const dateValue = typeof formatDateTrFix6 === 'function' ? formatDateTrFix6(draft.releaseDate || '') : (draft.releaseDate || '');
  const desc = draft.description || '';
  return `<div class="fix2StableGameEditor"><section class="fix2StableFormMain"><div class="sectionHead"><div><span class="eyebrow">${mode==='edit'?'Mevcut Oyun Düzenleme':'Yeni Oyun Ekle'}</span><h2>${mode==='edit'?'Oyunu Düzenle':'Boş Oyun Formu'}</h2><p class="muted">Butonlar formu sıfırlamaz. Meta, kapak, hikaye ve türler aynı form üzerinde güvenli şekilde güncellenir.</p></div></div><div class="fix2StatusPills"><button type="button" class="tagBtn ${draft.status==='Tamamlandı'?'active':''}" data-status-pick="Tamamlandı">Tamamlanan</button><button type="button" class="tagBtn ${draft.status==='Devam Ediyor'?'active':''}" data-status-pick="Devam Ediyor">Devam Eden</button><button type="button" class="tagBtn ${draft.status==='Yakında'?'active':''}" data-status-pick="Yakında">Yakında</button></div><div class="fix2StableGrid"><label class="field">Oyun Adı *<input name="title" required placeholder="Örn: A Way Out" value="${esc(draft.title || '')}"></label><label class="field">Seri Adı<input name="seriesName" placeholder="Örn: A Way Out" value="${esc(draft.seriesName || '')}"></label><label class="field">Çıkış Tarihi <small>gün.ay.yıl</small><input name="releaseDate" placeholder="23.03.2018" value="${esc(dateValue)}"></label><label class="field">Durum<select name="status">${opts}</select></label><label class="field wideField">Türler<div class="inlineField"><input name="genre" placeholder="Aksiyon-macera, co-op, hikaye odaklı" value="${esc(draft.genre || '')}"><button type="button" class="miniBtn" data-action="fix2-refetch-genres">Türleri Tekrar Çek</button></div></label><div class="field wideField"><span>Etiketler</span>${tagButtonsHtml(draft.tags || '')}</div><label class="field">Toplam Bölüm<input name="eps" type="number" min="0" value="${esc(String(draft.eps ?? 0))}"></label><label class="field">İzlenen Bölüm<input name="watchedEps" type="number" min="0" value="${esc(String(draft.watchedEps ?? 0))}"></label><label class="field">Puan<input name="score" type="number" min="0" max="10" step="0.1" value="${esc(String(draft.score ?? 8.5))}"></label><label class="field">Seri Sıra No<input name="seriesOrder" type="number" min="0" value="${esc(String(draft.seriesOrder ?? 0))}"></label><label class="field wideField">Kapak URL<div class="inlineField"><input name="cover" placeholder="https://..." value="${esc(draft.cover || '')}"><button type="button" class="miniBtn" data-action="fix2-meta-fill">Meta + Kapak Çek</button></div></label><label class="field wideField">YouTube Playlist<input name="playlistUrl" placeholder="https://youtube.com/playlist?list=..." value="${esc(draft.playlistUrl || '')}"></label><label class="field wideField">Tek Video URL<input name="videoUrl" placeholder="https://youtube.com/watch?v=..." value="${esc(draft.videoUrl || '')}"></label><label class="field wideField fix2StoryBox"><span>Oyunun Hikayesi / Açıklama</span><textarea name="description" rows="7" placeholder="Hikayeyi Tekrar Çek butonu oyunla ilgili detaylı Türkçe bilgi yazar.">${esc(desc)}</textarea><button type="button" class="miniBtn" data-action="fix2-refetch-story">Hikayeyi Tekrar Çek</button></label><label class="field wideField episodeImportField fix2EpisodeBox"><span>Profesyonel Bölüm Listesi</span>${episodeImportPreview(draft.episodesText, draft.episodes)}<textarea name="episodesText" rows="5" class="technicalEpisodes" placeholder="Teknik bölüm verisi otomatik oluşur">${esc(draft.episodesText || episodesToText(draft.episodes || []))}</textarea><div class="rowActions"><button type="button" class="miniBtn" data-action="toggle-technical-episodes">Teknik Veriyi Göster/Gizle</button><button type="button" class="miniBtn" data-action="${mode==='edit'?'estimate-playlist-episodes-edit':'estimate-playlist-episodes'}">Playlist Bölümleri Çek</button></div></label></div><div class="fix2FormActions"><button type="button" class="btn" data-action="fix2-meta-fill">Meta + Kapak Çek</button><button type="button" class="btn" data-action="fix2-refetch-story">Hikayeyi Tekrar Çek</button><button type="button" class="btn" data-action="fix2-refetch-genres">Türleri Tekrar Çek</button><button type="submit" class="btn primary">${mode==='edit'?'Oyunu Güncelle':'Oyunu Kaydet'}</button></div></section><aside class="fix2PreviewSide"><div class="fix2PreviewSticky"><h3>Canlı Önizleme</h3><div class="fix2CoverPreview"><img src="${esc(draft.cover || (typeof FIX11_COVER !== 'undefined' ? FIX11_COVER : 'https://images.unsplash.com/photo-1511512578047-dfb367046420?q=80&w=1200&auto=format&fit=crop'))}" alt="${esc(draft.title || 'Oyun')}"></div><article class="fix2PreviewCard"><span class="scoreBadge">${esc(String(draft.score || '8.5'))}</span><h2>${esc(draft.title || 'Oyun adı')}</h2><p>${esc((desc || v223Fix2Story(draft.title, draft.genre)).slice(0,260))}</p><div class="fix6MetaGrid"><div><small>Çıkış</small><b>${esc(dateValue || '-')}</b></div><div><small>Tür</small><b>${esc(draft.genre || 'Genel')}</b></div><div><small>Durum</small><b>${esc(draft.status || 'Devam Ediyor')}</b></div><div><small>Bölüm</small><b>${esc(String(draft.watchedEps||0))}/${esc(String(draft.eps||0))}</b></div></div></article></div></aside></div>`;
}

gameAddForm = function(){ const d = { ...DEFAULT_GAME_DRAFT, ...state.gameDraft }; return `<form class="card soft gameForm fix2StableGameForm" id="gameAddForm" autocomplete="off">${v223Fix2GameForm(d,'add')}</form>`; };
gameEditForm = function(){ const current = state.games.find(g=>String(g.id)===String(state.editingGameId)); if(!current) return ''; return `<div class="fix11EditOverlay"><form class="card soft gameForm editGameForm fix2StableGameForm fix11EditModal" id="gameEditForm" autocomplete="off"><button class="close" type="button" data-action="close-game-edit">×</button>${v223Fix2GameForm(current,'edit')}</form></div>`; };

function v223Fix2AdminGames(){
  const tab = localStorage.getItem(typeof FIX8_GAME_TAB_KEY !== 'undefined' ? FIX8_GAME_TAB_KEY : 'hayatimiz_game_admin_tab_fix8') || 'add';
  const modal = state.editingGameId ? gameEditForm() : '';
  const list = (typeof adminGamesTableFix8 === 'function') ? adminGamesTableFix8() : gameGrid();
  return `<section class="fix2GamesAdmin"><div class="fix8AdminTabs fix10AdminTabs"><button class="btn ${tab==='add'?'primary':''}" data-action="fix2-game-tab" data-tab="add">Oyun Ekle</button><button class="btn ${tab==='list'?'primary':''}" data-action="fix2-game-tab" data-tab="list">Mevcut Oyunlar</button><button class="btn" data-action="auto-cover-fetch">Kapaksızlara Kapak Öner</button><button class="btn" data-admin="AI Özellik Merkezi">AI Özellik Merkezi</button></div>${tab==='add' ? `${gameAddForm()}${rawgCandidatePanel()}${coverSuggestionPanel()}` : list}${modal}</section>`;
}
const v223Fix2OldAdminBody = adminBody;
adminBody = function(){
  if(state.adminPage === 'Oyunlar') return v223Fix2AdminGames();
  if(state.adminPage === 'AI Özellik Merkezi') return v223Fix2FeatureCenter();
  return v223Fix2OldAdminBody();
};

function v223Fix2FeatureCenter(){
  const draft = localStorage.getItem(V223_FIX2_AI_DRAFT_KEY) || 'Yeni özellik fikrini yaz; sistem bunu versiyon planına ve güncelleme notlarına işler.';
  const applied = safeParse(localStorage.getItem(V223_FIX2_AI_APPLIED_KEY), []);
  const suggestions = v223FeatureSuggestions ? v223FeatureSuggestions(draft) : [
    { key:'fix2-custom-1', title:'Profesyonel özellik uygulama akışı', target:'Yönetim Paneli', version:'v2.2.4', description:'Özellik fikrini plana ve güncelleme notlarına işler.', next:'Özellik geçmişi detay ekranı' },
    { key:'fix2-custom-2', title:'Sürüm bazlı otomatik planlama', target:'Güncelleme Notları', version:'v2.2.4', description:'Uygulanan özellikten sıradaki planı üretir.', next:'Versiyon karşılaştırma ekranı' }
  ];
  return `<section class="fix2AiCenter"><div class="card wide v223Hero"><div><span class="eyebrow">AI Özellik Merkezi</span><h1>Site Üzerinden Özellik Ekle</h1><p class="muted">Yeni butonlar artık hata vermeden çalışır. Özellik fikrini yaz, önerileri üret, seçtiğini versiyon planına uygula.</p></div><span class="pill green">v2.2.3 FIX 2</span></div><div class="v223FeatureGrid"><form class="card v223AiForm" id="fix2AiFeatureForm"><h2>AI ile özellik öner</h2><label class="field">Hedef sürüm<input name="version" value="${esc(v223NextVersion ? v223NextVersion() : 'v2.2.4')}" placeholder="v2.2.4"></label><label class="field">Özellik isteği<textarea name="idea" rows="7">${esc(draft)}</textarea></label><button class="btn primary" type="submit">Önerileri Getir</button></form><section class="card"><h2>Önerilen Özellikler</h2><div class="v223SuggestList">${suggestions.map((f,i)=>`<article><span class="pill">${i+1}</span><div><b>${esc(f.title)}</b><small>${esc(f.target)} • ${esc(f.version || 'v2.2.4')}</small><p>${esc(f.description || '')}</p></div><button class="miniBtn primary" data-fix2-ai-apply="${esc(f.key)}" data-title="${esc(f.title)}" data-target="${esc(f.target)}" data-desc="${esc(f.description || '')}" data-next="${esc(f.next || '')}" data-version="${esc(f.version || 'v2.2.4')}">Versiyona Uygula</button></article>`).join('')}</div></section></div><section class="card wide"><div class="sectionHead"><div><h2>Uygulanan Özellik Geçmişi</h2><p class="muted">Uygulanan özellikler local + Supabase denemesiyle tutulur, güncelleme notlarına işlenir.</p></div><span class="pill green">${applied.length} kayıt</span></div><div class="v223AppliedList">${applied.map(f=>`<article><b>${esc(f.version)} • ${esc(f.title)}</b><small>${esc(f.target)} • ${esc(f.appliedAt)}</small><p>${esc(f.description)}</p></article>`).join('') || '<p class="muted">Henüz uygulanan özellik yok.</p>'}</div></section></section>`;
}

const v223Fix2OldAdminPanel = adminPanel;
adminPanel = function(){
  let html = v223Fix2OldAdminPanel();
  if(!String(html).includes('AI Özellik Merkezi')){
    html = html.replace('</div></div><div class="sideFooter">', '<button class="adminSubLink" data-admin="AI Özellik Merkezi"><span class="subDot"></span><div><b>AI Özellik Merkezi</b><small>Versiyon özellikleri</small></div></button></div></div><div class="sideFooter">');
    html = html.replace('<button class="adminSubLink" data-admin="Güncelleme Notları">', '<button class="adminSubLink" data-admin="AI Özellik Merkezi"><span class="subDot"></span><div><b>AI Özellik Merkezi</b><small>Versiyon özellikleri</small></div></button><button class="adminSubLink" data-admin="Güncelleme Notları">');
  }
  return html;
};

const v223Fix2OldOnGameAddSubmit = onGameAddSubmit;
onGameAddSubmit = async function(e){
  e.preventDefault();
  const form = e.currentTarget;
  const game = { ...DEFAULT_GAME_DRAFT, ...readGameDraftFromForm(form) };
  if(!game.title) return v223Fix2SetToast('Oyun adı gerekli.');
  try{
    const data = await api('games-add', { adminToken: state.session?.adminToken, game });
    state.games.unshift(mapGame(data.game || { ...game, id:'local-'+Date.now() }));
    clearGameDraft();
    localStorage.setItem(typeof FIX8_GAME_TAB_KEY !== 'undefined' ? FIX8_GAME_TAB_KEY : 'hayatimiz_game_admin_tab_fix8','list');
    state.showGameForm = true;
    state.rawgCandidates = [];
    render();
    v223Fix2SetToast('Oyun eklendi ve mevcut oyunlar listesine alındı.');
  }catch(err){
    // Formu silme: sadece ekranda bırak.
    saveGameDraftFromForm(form);
    v223Fix2SetToast('Oyun formu korundu. Supabase kayıt hatası: ' + err.message);
  }
};

const v223Fix2OldOnGameEditSubmit = onGameEditSubmit;
onGameEditSubmit = async function(e){
  e.preventDefault();
  const form = e.currentTarget;
  const id = state.editingGameId;
  const current = state.games.find(g=>String(g.id)===String(id));
  if(!current) return v223Fix2SetToast('Düzenlenecek oyun bulunamadı.');
  const patch = { ...current, ...readGameDraftFromForm(form) };
  if(!patch.title) return v223Fix2SetToast('Oyun adı gerekli.');
  state.games = state.games.map(g=>String(g.id)===String(id)?{...g,...patch}:g);
  try{
    const data = await api('games-update', { adminToken: state.session?.adminToken, gameId:id, game:patch });
    if(data.game) state.games = state.games.map(g=>String(g.id)===String(id)?mapGame(data.game):g);
    v223Fix2SetToast('Oyun güncellendi.');
  }catch(err){ v223Fix2SetToast('Oyun local güncellendi; Supabase hatası: '+err.message); }
  state.editingGameId = null;
  localStorage.setItem(typeof FIX8_GAME_TAB_KEY !== 'undefined' ? FIX8_GAME_TAB_KEY : 'hayatimiz_game_admin_tab_fix8','list');
  render();
};

const v223Fix2PreviousBind = bind;
bind = function(){
  v223Fix2PreviousBind();
  document.querySelectorAll('[data-action="fix2-game-tab"]').forEach(btn=>btn.addEventListener('click', e=>{ e.preventDefault(); e.stopImmediatePropagation(); const tab=btn.dataset.tab||'add'; localStorage.setItem(typeof FIX8_GAME_TAB_KEY !== 'undefined' ? FIX8_GAME_TAB_KEY : 'hayatimiz_game_admin_tab_fix8', tab); if(tab==='add'){ clearGameDraft(); state.editingGameId=null; state.rawgCandidates=[]; state.coverSuggestions=[]; } if(tab==='list') state.editingGameId=null; render(); }, true));
  document.querySelectorAll('[data-action="fix2-meta-fill"], [data-action="auto-game-meta"], [data-action="auto-game-meta-edit"]').forEach(btn=>btn.addEventListener('click', async e=>{ e.preventDefault(); e.stopImmediatePropagation(); await v223Fix2MetaFill(btn.closest('form') || v223Fix2ActiveGameForm()); }, true));
  document.querySelectorAll('[data-action="fix2-refetch-story"], [data-action="fix10-refetch-story"], [data-action="fix12-refetch-story"], [data-action="fetch-game-story"], [data-action="fetch-game-story-edit"]').forEach(btn=>btn.addEventListener('click', e=>{ e.preventDefault(); e.stopImmediatePropagation(); v223Fix2RefetchStory(btn.closest('form') || v223Fix2ActiveGameForm()); }, true));
  document.querySelectorAll('[data-action="fix2-refetch-genres"], [data-action="fix10-refetch-genres"], [data-action="fix12-refetch-genres"]').forEach(btn=>btn.addEventListener('click', e=>{ e.preventDefault(); e.stopImmediatePropagation(); v223Fix2RefetchGenres(btn.closest('form') || v223Fix2ActiveGameForm()); }, true));
  document.querySelectorAll('[data-status-pick]').forEach(btn=>btn.addEventListener('click', e=>{ e.preventDefault(); e.stopImmediatePropagation(); const form=btn.closest('form'); if(!form) return; setFormValue(form,'status',btn.dataset.statusPick); updateOpenGameFormSnapshot(form); form.querySelectorAll('[data-status-pick]').forEach(x=>x.classList.toggle('active', x===btn)); }, true));
  const addForm = document.getElementById('gameAddForm'); if(addForm){ addForm.onsubmit = null; addForm.addEventListener('submit', onGameAddSubmit, true); addForm.addEventListener('input', ()=>{ saveGameDraftFromForm(addForm); try{ fix12UpdateFormPreview(addForm); }catch{} }); addForm.addEventListener('change', ()=>saveGameDraftFromForm(addForm)); }
  const editForm = document.getElementById('gameEditForm'); if(editForm){ editForm.onsubmit = null; editForm.addEventListener('submit', onGameEditSubmit, true); editForm.addEventListener('input', ()=>{ try{ fix12UpdateFormPreview(editForm); }catch{} }); }
  const aiForm = document.getElementById('fix2AiFeatureForm'); if(aiForm){ aiForm.addEventListener('submit', e=>{ e.preventDefault(); e.stopImmediatePropagation(); const fd=new FormData(aiForm); localStorage.setItem(V223_FIX2_AI_DRAFT_KEY, String(fd.get('idea')||'')); try{ localStorage.setItem(V223_NEXT_VERSION_KEY, String(fd.get('version')||'v2.2.4')); }catch{} render(); v223Fix2SetToast('AI özellik önerileri yenilendi.'); }, true); }
  document.querySelectorAll('[data-fix2-ai-apply]').forEach(btn=>btn.addEventListener('click', async e=>{ e.preventDefault(); e.stopImmediatePropagation(); const item={ key:btn.dataset.fix2AiApply, title:btn.dataset.title, target:btn.dataset.target, description:btn.dataset.desc, next:btn.dataset.next, version:btn.dataset.version || 'v2.2.4', appliedAt:new Date().toLocaleString('tr-TR') }; const list=safeParse(localStorage.getItem(V223_FIX2_AI_APPLIED_KEY), []); list.unshift(item); localStorage.setItem(V223_FIX2_AI_APPLIED_KEY, JSON.stringify(list.slice(0,80))); if(!state.planner.some(p=>p.text===item.next) && item.next) state.planner.unshift({ id:'local-fix2-'+Date.now(), group:'Gelecek Versiyon Özellikleri', text:item.next, status:'plan', featureKey:item.key }); try{ await api('ai-feature-apply',{ adminToken:state.session?.adminToken, feature:item }); }catch{} render(); v223Fix2SetToast(`${item.version} için özellik uygulandı ve planlara işlendi.`); }, true));
};
