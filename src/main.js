...
const VERSION = 'v2.1.6 FIX - Form + Bölüm + Hikaye Düzeltmesi';
...
const DEFAULT_GAME_DRAFT = { title:'', genre:'', tags:'', releaseDate:'', status:'Devam Ediyor', eps:0, watchedEps:0, score:8.5, cover:'', seriesName:'', seriesOrder:0, playlistUrl:'', videoUrl:'', description:'', episodesText:'', episodes:[] };
...
const state = {
  ...
  gameViewMode: localStorage.getItem('hayatimiz_game_view_mode_v216') || 'grid',
...
function episodesToText(episodes){
  return normalizeEpisodes(episodes).map(ep => `${ep.number}|${ep.title}|${ep.videoUrl}|${ep.thumbnail}|${ep.description}`).join('\n');
}
function parseEpisodesText(text){
  return String(text || '').split(/\r?\n/).map((line,i)=>{
    const raw = line.trim(); if(!raw) return null;
    const parts = raw.split('|');
    if(parts.length === 1) return { id:`ep-${i+1}`, number:i+1, title:parts[0] || `${i+1}. Bölüm`, videoUrl:'', thumbnail:'', description:'', watched:false };
    return { id:`ep-${i+1}`, number:Number(parts[0] || i+1), title:parts[1] || `${i+1}. Bölüm`, videoUrl:parts[2] || '', thumbnail:parts[3] || '', description:parts.slice(4).join('|') || '', watched:false };
  }).filter(Boolean);
}
...
function seriesGroupPanel(games){
  const groups = new Map();
  games.forEach(g=>{ const key = String(g.seriesName || 'Tek Oyunlar').trim() || 'Tek Oyunlar'; if(!groups.has(key)) groups.set(key, []); groups.get(key).push(g); });
  const blocks = Array.from(groups.entries()).map(([name,items])=>`<details class="seriesGroup" open><summary><b>${esc(name)}</b><span>${items.length} kayıt</span></summary><div class="seriesOrderList">${items.sort((a,b)=>Number(a.seriesOrder||0)-Number(b.seriesOrder||0)||String(a.title).localeCompare(String(b.title),'tr')).map(g=>`<div><span>${esc(g.title)}</span><label>Sıra <input data-series-order-game="${esc(g.id)}" type="number" value="${esc(String(g.seriesOrder||0))}"></label></div>`).join('')}</div></details>`).join('');
  return `<section class="card wide seriesManager"><div class="sectionHead"><div><h3>Seri Gruplama ve Toplu Sıra</h3><p class="muted">Oyunlar seri adına göre gruplanır. Sıra numarasını değiştirince formda düzenleme kaydına hazır hale gelir.</p></div><span class="pill green">v2.1.6</span></div>${blocks}</section>`;
}
...
function gameFormFields(d, mode='add'){
  ...
  return `<div class="formGrid cleanGameFormGrid"><label class="field">Oyun adı<input name="title" required placeholder="Örn: Assassin's Creed Origins" value="${esc(d.title)}" /></label><label class="field">Kategori / Tür<input name="genre" required placeholder="Aksiyon, Macera, RPG" value="${esc(d.genre)}" /></label><label class="field">Çıkış tarihi <small>gün.ay.yıl</small><input name="releaseDate" placeholder="27.10.2017" value="${esc(d.releaseDate || '')}" /></label><label class="field">Durum<select name="status">${opts}</select></label><label class="field">Toplam Bölüm<input name="eps" type="number" min="0" value="${esc(String(d.eps ?? 0))}" /></label><label class="field">İzlenen Bölüm<input name="watchedEps" type="number" min="0" value="${esc(String(d.watchedEps ?? 0))}" /></label><label class="field">Puan<input name="score" type="number" min="0" max="10" step="0.1" value="${esc(String(d.score ?? 8.5))}" /></label><label class="field">Seri adı<input name="seriesName" placeholder="Örn: Assassin's Creed" value="${esc(d.seriesName || '')}" /></label><label class="field">Seri sıra no<input name="seriesOrder" type="number" min="0" value="${esc(String(d.seriesOrder ?? 0))}" /></label><label class="field wideField">YouTube oynatma listesi<input name="playlistUrl" placeholder="https://youtube.com/playlist?list=..." value="${esc(d.playlistUrl || '')}" /></label><label class="field wideField">Tek video URL<input name="videoUrl" placeholder="https://youtube.com/watch?v=..." value="${esc(d.videoUrl || '')}" /></label><label class="field wideField">Kapak URL<input name="cover" placeholder="https://..." value="${esc(d.cover)}" /></label><label class="field wideField">Manuel kapak yükle <small>Supabase Storage bağlantısı varsa URL’ye dönüştürülür; yoksa önizleme için local data kullanılır.</small><input id="coverUpload" type="file" accept="image/*"></label><label class="field wideField storyField">Hikayesi / Açıklama<textarea name="description" rows="5" placeholder="Hikaye Çek butonuna basınca Türkçe hikaye özeti burada oluşur.">${esc(d.description || '')}</textarea></label><label class="field wideField">Bölüm Listesi <small>Format: bölüm no|başlık|video linki|küçük kapak|açıklama</small><textarea name="episodesText" rows="7" placeholder="1|1. Bölüm|https://youtube.com/watch?v=...|https://kapak.jpg|Bölüm açıklaması">${esc(d.episodesText || episodesToText(d.episodes || []))}</textarea></label><div class="field wideField"><span>Etiketler</span><small class="muted">Butonlardan seç: Türkçe Altyazılı, Türkçe Dublajlı, DLC, Coop, %100...</small>${tagButtonsHtml(d.tags)}</div></div><div class="coverPreview ${d.cover?'':'isEmpty'}">${d.cover?`<img src="${esc(d.cover)}" alt="Kapak önizleme">`:'Kapak çekilince burada önizleme görünür.'}</div>`;
}

function gameAddForm(){
  const d = { ...DEFAULT_GAME_DRAFT, ...state.gameDraft };
  return `<form class="card soft gameForm" id="gameAddForm" autocomplete="off"><h3>Yeni Oyun Ekle</h3><p class="muted">Oyun adını yaz, RAWG sonuçlarını getir, doğru kapağı seç; kaydetmeden oyun eklenmez.</p>${gameFormFields(d,'add')}<div class="note greenNote">Otomatik çekme oyun eklemez, oyun silmez, sadece form alanlarını doldurur.</div><div class="formActionBar"><button class="btn" type="button" data-action="auto-game-meta">Meta / Kapak Çek</button><button class="btn storyBtn" type="button" data-action="fetch-game-story">Hikaye Çek</button><button class="btn" type="button" data-action="estimate-playlist-episodes">Playlistten Tüm Bölümleri Çek</button><button class="btn primary" type="submit">Oyunu Kaydet</button><button class="btn" type="button" data-action="toggle-game-form">Kapat</button></div></form>`;
}
function gameEditForm(){
  const current = state.games.find(g=>String(g.id)===String(state.editingGameId));
  if(!current) return '';
  return `<form class="card soft gameForm editGameForm" id="gameEditForm" autocomplete="off"><h3>Oyunu Formda Düzenle</h3><p class="muted">Düzenleme artık prompt değil; kapak, bölüm hedefi, izlenen bölüm, seri sıra no, video ve playlist alanları formdan güncellenir.</p>${gameFormFields(current,'edit')}<div class="formActionBar"><button class="btn" type="button" data-action="auto-game-meta-edit">Meta / Kapak Yenile</button><button class="btn storyBtn" type="button" data-action="fetch-game-story-edit">Hikaye Çek</button><button class="btn" type="button" data-action="estimate-playlist-episodes-edit">Playlistten Tüm Bölümleri Çek</button><button class="btn primary" type="submit">Oyunu Güncelle</button><button class="btn" type="button" data-action="close-game-edit">Kapat</button></div></form>`;
}
...
function youtubeHealthPanel(){
  const games = Array.isArray(state.games) ? state.games : [];
  const bad = games.filter(g=>{ const hasEpisodes = normalizeEpisodes(g.episodes).some(ep=>String(ep.videoUrl||'').trim()); return (Number(g.eps||0)>0 || g.seriesName) && !watchTargetUrl(g) && !hasEpisodes; });
  if(!bad.length) return `<div class="note greenNote">YouTube sağlık kontrolü temiz: boş seri linki görünmüyor.</div>`;
  return `<section class="card soft healthPanel"><div class="sectionHead"><div><h3>Hatalı / Boş YouTube Linkleri</h3><p class="muted">Seri görünüp linki olmayan oyunlar burada listelenir.</p></div><span class="pill banned">${bad.length} sorun</span></div><div class="missingCoverList">${bad.map(g=>`<div><b>${esc(g.title)}</b><small>${esc(g.seriesName || 'Seri adı yok')}</small><button class="miniBtn" data-game-edit="${esc(g.id)}">Link Ekle</button></div>`).join('')}</div></section>`;
}
...
function seriesWatchModal(){
  ...
  const active = episodes.find(ep=>String(ep.id)===String(state.selectedEpisodeId)) || episodes[0] || null;
  const embed = active ? episodeEmbedUrl(active, rawUrl) : youtubeEmbedUrl(rawUrl);
  const rows = episodes.length ? episodes.map(ep=>`<button class="episodeRow ${String(active?.id)===String(ep.id)?'active':''} ${ep.watched||Number(ep.number)<=watched?'done':''}" data-watch-episode="${esc(ep.id)}"><img src="${esc(ep.thumbnail || g.cover || coverFor(g))}" onerror="this.style.display='none'"><span><b>${esc(ep.number)}. ${esc(ep.title)}</b><small>${esc(ep.description || 'Siteden izle')}</small></span></button>`).join('') : '<div class="episodeEmpty">Bölüm hedefi eklenmemiş. Admin panelinden playlist import yap.</div>';
  const player = embed ? `<div class="sitePlayer cinemaPlayer"><iframe src="${esc(embed)}" title="${esc(active?.title || g.title)} site içi oynatıcı" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div>` : '<div class="sitePlayer empty cinemaPlayer"><b>Site içi oynatıcı için bağlantı yok.</b><span>Admin panelinde playlist import yap veya bölüm video linki ekle.</span></div>';
  return `<div class="modalOverlay watchPageOverlay"><div class="modal seriesModalV216"><button class="close" type="button" data-action="close-series-watch">×</button><div class="watchHeader"><div><span class="eyebrow">Sitede Seri İzleme</span><h2>${esc(g.title)}</h2><p class="muted">${esc(g.seriesName || 'Tek seri')} • ${watched}/${Number(g.eps||episodes.length||0)} bölüm • ${progressPercent(g)}%</p></div><button class="miniBtn primary" data-mark-active-watched="${esc(active?.id || '')}">Bu Bölümü İzledim</button></div><div class="watchLayout"><main>${player}<h3>${esc(active?.title || g.title)}</h3><p class="muted">${esc(active?.description || g.description || 'Bölümü doğrudan site içinden izliyorsun.')}</p><div class="progressLine large"><span style="width:${progressPercent(g)}%"></span></div></main><aside><h3>Bölümler</h3><div class="episodeSidebar">${rows}</div></aside></div></div></div>`;
}
...
function bind(){
  ...
  document.querySelectorAll('[data-watch-series]').forEach(el=>el.addEventListener('click', e=>{ e.preventDefault(); state.watchingGameId = el.dataset.watchSeries; state.selectedEpisodeId = null; render(); }));
  document.querySelectorAll('[data-watch-episode]').forEach(el=>el.addEventListener('click', e=>{ e.preventDefault(); state.selectedEpisodeId = el.dataset.watchEpisode; render(); }));
  document.querySelectorAll('[data-mark-active-watched]').forEach(el=>el.addEventListener('click', e=>{ e.preventDefault(); markEpisodeWatched(el.dataset.markActiveWatched); }));
  document.querySelectorAll('[data-view-mode]').forEach(el=>el.addEventListener('click', e=>{ e.preventDefault(); state.gameViewMode = el.dataset.viewMode || 'grid'; localStorage.setItem('hayatimiz_game_view_mode_v216', state.gameViewMode); render(); }));
  document.querySelectorAll('[data-series-order-game]').forEach(el=>el.addEventListener('change', e=>{ const game = state.games.find(g=>String(g.id)===String(el.dataset.seriesOrderGame)); if(game){ game.seriesOrder = Number(e.target.value||0); setToast('Sıra değeri ekranda güncellendi. Kalıcı kaydetmek için karttan Formda Düzenle > Oyunu Güncelle.'); } }));
...
async function onAction(e){
  ...
  if(action === 'close-series-watch'){ state.watchingGameId = null; state.selectedEpisodeId = null; render(); }
...
async function autoGameMetaFill(formId='gameAddForm'){
  ...
  try{
    ...
    const patch = {
      ...
      description: meta.description || draft.description || state.gameDraft.description || '',
...
async function fetchGameStoryToForm(formId='gameAddForm'){
  ...
  try{
    ...
    setToast('Türkçe hikaye/açıklama forma işlendi.');
  }catch(err){
...
function applyRawgCandidate(index){
  ...
  setFormValuesAndSnapshot(form, {
    ...
    description:item.description || getFormValue(form,'description')
  });
  state.rawgCandidates = [];
...
async function estimatePlaylistEpisodes(formId='gameAddForm'){
  ...
  try{
    ...
    if(count > 0){
      const text = episodes.length ? episodesToText(episodes) : getFormValue(form,'episodesText');
      setFormValuesAndSnapshot(form, { eps:count, episodesText:text, watchedEps:getFormValue(form,'watchedEps') || 0 });
      setToast(episodes.length ? `Playlistten ${episodes.length} bölüm çekildi; bölüm listesi ve toplam bölüm forma işlendi.` : `Playlist bölüm sayısı çekildi: ${count}. Başlık/kapak listesi için YOUTUBE_API_KEY gerekli.`);
      return;
    }
    setToast('Playlist sayısı alınamadı. YouTube API key yoksa manuel bölüm listesi girebilirsin.');
  }catch(err){ setToast('Playlist bölümleri alınamadı: ' + err.message); }
}
...
function markEpisodeWatched(epId){
  ...
  if(!g || !epId) return;
  const eps = seriesEpisodes(g).map(ep=>String(ep.id)===String(epId) ? { ...ep, watched:true } : ep);
  g.episodes = eps;
  g.watchedEps = eps.filter(ep=>ep.watched).length;
  setToast('Bölüm izlendi olarak işaretlendi. Kalıcı kayıt için admin düzenleme formundan oyunu güncelle.');
  render();
}
