...
const VERSION = 'v2.1.6 FIX 2 - Hikaye ve Playlist Düzeltmesi';
...
async function estimatePlaylistEpisodes(formId='gameAddForm'){
  ...
  try{
    ...
    if(episodes.length > 0){
      ...
      setFormValuesAndSnapshot(form, { eps:episodes.length, episodesText:text, watchedEps:getFormValue(form,'watchedEps') || 0 });
      setToast(`Playlistten ${episodes.length} video çekildi. Bölüm listesine sadece video başlığı, linki ve küçük kapak işlendi.`);
      return;
    }
    if(count > 0){
