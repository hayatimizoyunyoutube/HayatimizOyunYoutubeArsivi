...
const VERSION = 'v2.1.4 Gelişmiş Arşiv + Profesyonel UI';
...
function alphabetNav(games){
  const letters = Array.from(new Set(games.map(gameInitial))).sort((a,b)=>a.localeCompare(b,'tr'));
  if(!letters.length) return '';
  return `<div class="alphabetRail"><span>Harfe git</span>${letters.map(l=>`<a href="#harf-${esc(routeSlug(l))}">${esc(l)}</a>`).join('')}</div>`;
}
function watchButtonHtml(g){
  const url = String(g.playlistUrl || '').trim();
  if(String(g.status || '') === 'Yakında') return `<button class="miniBtn disabled" disabled>Yakında</button>`;
  if(url) return `<a class="miniBtn primary watchBtn" href="${esc(url)}" target="_blank" rel="noopener">Sitede İzle</a>`;
  if(Number(g.eps || 0) > 0 || g.seriesName) return `<button class="miniBtn" data-toast="${esc(g.title)} serisi için playlist URL eklenmemiş.">Serileri İzle</button>`;
  return `<button class="miniBtn" data-toast="${esc(g.title)} için izleme bağlantısı eklenmemiş.">Sitede İzle</button>`;
}
function gameCard(g, adminActions){
  const progress = getGameProgress(g);
  return `<article class="game v214GameCard"><div class="coverWrap"><img src="${esc(coverFor(g))}" alt="${esc(g.title)}">${featureEnabled('missing_cover_warning') && !g.cover ? '<span class="coverWarn">Kapak eksik</span>' : ''}<div class="progressBadge"><b>%${progress}</b><span>ilerleme</span></div></div><div class="gameBody"><div class="cardTopline"><span class="pill">${esc(g.status)}</span>${g.releaseDate ? `<span class="pill softPill">${esc(g.releaseDate)}</span>` : ''}${isFavorite(g.id)?'<span class="pill green">Favori</span>':''}</div><h3>${esc(g.title)}</h3><p>${esc(g.genre)} • ${Number(g.eps || 0)} bölüm${g.seriesName ? ` • Seri: ${esc(g.seriesName)}` : ''}</p>${tagChipsHtml(g.tags)}<div class="progressLine"><span style="width:${progress}%"></span></div><div class="gameMetaLine"><b>⭐ ${esc(g.score)}</b><small>${g.seriesName ? 'Seri sırası: '+esc(g.seriesName) : 'Tek oyun'}</small></div>${adminActions ? `<div class="gameAdminActions"><button class="miniBtn" data-game-edit="${esc(g.id)}">Formda Düzenle</button><button class="miniBtn danger" data-game-delete="${esc(g.id)}">Sil</button></div>` : `<div class="gameAdminActions">${watchButtonHtml(g)}<button class="miniBtn" data-favorite-game="${esc(g.id)}">${isFavorite(g.id)?'Favoriden Çıkar':'Favoriye Ekle'}</button></div>`}</div></article>`;
}
...
function gameGrid(){
  ...
  const sections = Array.from(groups.entries()).map(([letter,items])=>`<section class="letterSection" id="harf-${esc(routeSlug(letter))}"><div class="letterHead"><b>${esc(letter)}</b><span>${items.length} oyun</span></div><div class="games">${items.map(g=>gameCard(g, adminActions)).join('')}</div></section>`).join('');
...
function profilePage(){
  ...
  const role = normalizeRole(state.session.role);
  const avatarUrl = String(state.session.avatar_url || state.session.avatarUrl || '').trim();
  ...
  return html`<section class="profileGrid"><div class="card profileCard"><div class="avatar profileAvatar">${avatar}</div><h2>Profilim</h2><p class="muted">Hesap bilgileri ve yetki durumu</p><span class="pill ${role}">${esc(displayRole(role))}</span><div class="profileInfo"><p><b>Ad Soyad:</b> ${esc(state.session.full_name || 'Belirtilmedi')}</p><p><b>E-posta:</b> ${esc(state.session.email)}</p><p><b>Durum:</b> ${normalizeRole(state.session.role)==='banned' || state.session.is_active === false ? 'Banlı' : 'Aktif'}</p></div></div><form class="card profileEdit" id="profileForm"><h3>Profil Bilgilerini Güncelle</h3><label class="field">Ad Soyad<input name="fullName" value="${esc(state.session.full_name || '')}" /></label><label class="field">Profil Fotoğrafı URL<input name="avatarUrl" value="${esc(avatarUrl)}" placeholder="Supabase Storage public URL veya görsel linki" /></label><button class="btn primary" type="submit">Profili Kaydet</button><p class="note">v2.1.4: Profil fotoğrafı alanı eklendi. Supabase Storage bucket kurulumu dokümana yazıldı.</p></form></section>`;
}
function publicPage(){
...
function overviewAdmin(){
  return `<section class="grid stats adminStats"><div class="card"><b>Oyun</b><h3>${state.games.length}</h3><span class="muted">arşiv verisi</span></div><div class="card"><b>Güncelleme</b><h3>${state.updates.length}</h3><span class="muted">not</span></div><div class="card"><b>Veri durumu</b><h3>${state.runtimeLoaded?'Bağlandı':'Local'}</h3><span class="muted">Supabase/API</span></div><div class="card"><b>Oyun ekleme</b><h3>Aktif</h3><span class="muted">meta/kapak/etiket</span></div></section><section class="card wide"><h2>v2.1.4 profesyonel arşiv tamamlandı</h2><p>Koleksiyon sekmesi kaldırıldı; ana ekran istatistik odaklı hale getirildi. Oyun kartları, alfabetik arşiv, bakım modu ve güncelleme notları paneli geliştirildi.</p><button class="btn primary" data-admin="Oyunlar">Oyunlar Sekmesini Aç</button></section>`;
}
...
async function onProfileSubmit(e){
  ...
  const old = state.session; saveSession({ ...old, full_name: fullName, avatar_url: avatarUrl }); render();
