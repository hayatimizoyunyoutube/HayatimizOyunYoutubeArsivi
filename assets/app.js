...
function socialLinksFromSettings(){
  ...
  const rows=[];
  const push=(title,url)=>{url=normalizeUrl(url||''); if(url)rows.push({title,url,active:true});};
  push('YouTube',s.social_youtube);
  push('Kick',s.social_kick);
  push('Discord',s.social_discord);
  push('TikTok',s.social_tiktok);
  push('Instagram',s.social_instagram);
  push('Bağış',s.social_donate||s.social_bynogame||s.donate_url);
  for(const item of parseSocialLinks(s.social_links||'')){
    const u=normalizeUrl(item.url||'');
    if(u&&!rows.some(x=>x.url===u))rows.push({...item,url:u,active:item.active!==false});
  }
  return rows.filter(x=>x.url&&x.active!==false);
}
...
/* V2.0.0 Fix 1: loading ve güvenli açılış kesin düzeltme */
function v200fix1DefaultSettings(){
  return {
    site_title:'Hayatımız Oyun',
    share_title:'Hayatımız Oyun',
    share_description:'Oyun ve seri izleme arşivi.',
    maintenance:false,
    show_social_header:true,
    show_social_footer:true,
    show_streamer_card:false,
    background_intensity:75
  };
}
async function v200fix1LocalSample(){
  try{
    const r=await fetch('/data/sample-game.json',{cache:'no-store'});
    if(!r.ok)return [];
    const j=await r.json();
    const arr=Array.isArray(j)?j:[j];
    return arr.filter(x=>x&&x.title).map(g=>({
      status:'Devam Ediyor',
      genre:'Aksiyon, Macera',
      tags:['Türkçe Altyazılı'],
      episodes:[],
      ...g,
      slug:g.slug||slug(g.title),
      series:canonicalSeriesName(g.series||g.title,g.title)||g.series||g.title
    }));
  }catch{return []}
}
async function load(){
  const safe=async(label,pct,fn,fallback)=>{
    try{loaderSet(pct,label); return await fn();}
    catch(e){console.warn('V2.0.0 Fix 1:',label,e); return fallback;}
  };
  const s=await safe('Site ayarları hazırlanıyor...',12,()=>api('/api/settings'),{settings:v200fix1DefaultSettings()});
  state.settings={...v200fix1DefaultSettings(),...(s.settings||{})};
  const g=await safe('Oyunlar yükleniyor...',38,()=>api('/api/games'),{games:[]});
  state.games=Array.isArray(g.games)?g.games.filter(x=>x&&x.title):[];
  if(!state.games.length){
    const cached=JSON.parse(localStorage.ho_games_cache||'[]');
    state.games=Array.isArray(cached)?cached.filter(x=>x&&x.title):[];
  }
  if(!state.games.length){
    state.games=await v200fix1LocalSample();
  }
  if(state.games.length){
    try{localStorage.ho_games_cache=JSON.stringify(state.games.slice(0,500));}catch{}
  }
  const n=await safe('Güncelleme notları alınıyor...',65,()=>api('/api/notes'),{notes:[{id:'v200fix1',version:'V2.0.0 Fix 1',type:'Fix',title:'Açılış ve sosyal link hatası düzeltildi',body:'V2.0.0 geri dönüşünde siteyi güvenli moda düşüren eksik sosyal link fonksiyonu ve loading takılması düzeltildi.',created_at:new Date().toISOString(),public_visible:true}]});
  state.notes=Array.isArray(n.notes)?n.notes:[];
  const c=await safe('Takvim kontrol ediliyor...',82,()=>api('/api/calendar'),{events:[]});
  state.events=Array.isArray(c.events)?c.events:[];
  loaderSet(100,'Site açılıyor...');
}
function v200fix1HideLoader(){
  const ids=['siteLoader','loader','loading','splash','preloader'];
  ids.forEach(id=>{const el=document.getElementById(id); if(el){el.classList.add('hide'); el.style.pointerEvents='none'; setTimeout(()=>{try{el.remove()}catch{}},700);}});
  document.querySelectorAll('.loader,.loading,.splash,.preloader').forEach(el=>{el.style.display='none';el.style.pointerEvents='none';});
}
function v200fix1RenderFallback(){
  const app=document.getElementById('app');
  if(!app)return;
  if(/Yükleniyor|Güvenli mod/i.test(app.textContent||'') || !app.innerHTML.trim()){
    try{
      state.settings={...v200fix1DefaultSettings(),...(state.settings||{})};
      if(!Array.isArray(state.games))state.games=[];
      renderNav();
      home();
    }catch(e){
      console.error('V2.0.0 Fix 1 hard fallback:',e);
      app.innerHTML=`<section class="hero"><span class="version-pill">V${VERSION}</span><h1>Hayatımız Oyun</h1><p class="muted">Site açıldı. Veriler yüklenemediği için yenileme gerekebilir.</p><button onclick="location.reload()">Yenile</button></section>`;
    }
  }
  v200fix1HideLoader();
}
window.addEventListener('error',()=>setTimeout(v200fix1RenderFallback,120));
window.addEventListener('unhandledrejection',()=>setTimeout(v200fix1RenderFallback,120));
setTimeout(v200fix1RenderFallback,1600);
try{Object.assign(window,{socialLinksFromSettings,v200fix1RenderFallback});}catch{}
