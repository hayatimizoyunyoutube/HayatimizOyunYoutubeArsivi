...
/* V2.7.0 Fix 1 - Profesyonel Admin Panel + açılış öncesi stabil UI */
function v270Esc(v){return String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));}
function v270Games(){return [...(state.games||[])].filter(g=>String(g?.title||'').trim()).sort((a,b)=>String(a.title||'').localeCompare(String(b.title||''),'tr',{numeric:true,sensitivity:'base'}));}
function v270SeriesName(g){return String(g?.series||g?.collection||g?.franchise||g?.title||'Serisiz').trim()||'Serisiz';}
function v270Cover(g){return g?.cover||g?.thumbnail||g?.image||'/assets/series-placeholder.svg';}
function v270Episodes(g){return Array.isArray(g?.episodes)?g.episodes:[];}
function v270Genre(g){return String(g?.genre||g?.genres||g?.category||g?.type||'Macera').split(',').map(x=>x.trim()).filter(Boolean);}
function v270Id(g){return String(g?.id||g?.slug||g?.title||Math.random());}
function v270Groups(){const map=new Map();for(const g of v270Games()){const name=v270SeriesName(g);if(!map.has(name))map.set(name,{name,games:[]});map.get(name).games.push(g)}return [...map.values()].map(gr=>{gr.games.sort((a,b)=>String(a.title||'').localeCompare(String(b.title||''),'tr',{numeric:true,sensitivity:'base'}));gr.cover=v270Cover(gr.games.find(x=>x.cover||x.thumbnail)||gr.games[0]);gr.genre=v270Genre(gr.games[0])[0]||'Macera';gr.episodes=gr.games.reduce((n,g)=>n+v270Episodes(g).length,0);gr.letter=(gr.name[0]||'#').toLocaleUpperCase('tr-TR');gr.status=/yakında|yakinda|gelecek/i.test(gr.games.map(g=>g.status).join(' '))?'soon':(/tamam|bitti/i.test(gr.games.map(g=>g.status).join(' '))?'done':'active');return gr}).sort((a,b)=>a.name.localeCompare(b.name,'tr',{numeric:true,sensitivity:'base'}));}
function v270Stats(){const games=v270Games(), groups=v270Groups();let issues=0;for(const g of games){try{if(typeof issueList250==='function')issues+=issueList250(g).length;else{if(!v270Cover(g))issues++;if(!v270Episodes(g).length&&!/yakında|yakinda|gelecek/i.test(String(g.status||'')))issues++;}}catch(e){}}const episodes=games.reduce((n,g)=>n+v270Episodes(g).length,0);const noCover=games.filter(g=>!g.cover&&!g.thumbnail&&!g.image);const badVideo=games.filter(g=>v270Episodes(g).some(ep=>!String(ep.videoId||ep.youtubeId||ep.url||'').trim()));const noStory=games.filter(g=>!String(g.description||g.story||'').trim());const requests=JSON.parse(localStorage.getItem('ho_v250_reports')||'[]');return{games:games.length,series:groups.length,episodes,issues,noCover,badVideo,noStory,requests,score:Math.max(85,Math.min(99.8,+(100-(issues/Math.max(1,games.length*4))*9).toFixed(1)))};}
function v270RenderNav(){const brand=document.getElementById('brand'),nav=document.getElementById('nav');if(brand)brand.innerHTML=`<div class="v270-top-logo">🎮</div><div><b>${v270Esc(typeof siteTitle==='function'?siteTitle():'Hayatımız Oyun')}</b><small>V${VERSION}</small></div>`;if(!nav)return;const items=[['home','Ana Sayfa'],['series','Seriler'],['az','A-Z'],['searchv210','Arama'],['favoritesv210','Favoriler'],['trackingv210','Takip'],['contribute250','Oyun İste / Hata Bildir'],['profile250','Profil'],['notes','Güncellemeler'],['about','Hakkında']];nav.innerHTML=`<div class="v270-nav-main">${items.map(([p,l])=>`<button class="${state.page===p?'active':''}" onclick="setPage('${p}')">${v270Esc(l)}</button>`).join('')}</div>${canSeeAdmin()?`<button class="v270-admin-top ${state.page==='admin'?'active':''}" onclick="setPage('admin')">🛡 Admin ▾</button>`:''}`;}
function renderNav(){v270RenderNav();}
function v270SeriesCard(gr){const label=gr.status==='soon'?'Yakında':gr.status==='done'?'Tamamlandı':'Aktif';return `<article class="v270-series-card card"><div class="v270-cover" style="background-image:url('${v270Esc(gr.cover)}')"><span>☆</span></div><div class="v270-body"><h3>${v270Esc(gr.name)}</h3><p>${v270Esc(gr.genre)}</p><div class="v270-pills"><em>${gr.games.length} Oyun</em><em>${gr.episodes} Bölüm</em><em class="${gr.status}">${label}</em></div><button onclick="seriesDetail250('${v270Esc(gr.name)}')">Tüm Seriyi İzle →</button></div></article>`;}
function home(){const st=v270Stats(),groups=v270Groups().slice(0,12);$('#app').innerHTML=`<section class="v270-page"><section class="v270-hero"><div><span class="version-pill">V${VERSION}</span><h1>Hayatımız Oyun</h1><p>Sade, hızlı ve stabil oyun arşivi</p><div class="v270-hero-actions"><button onclick="setPage('searchv210')">Oyun Ara</button><button class="ghost" onclick="setPage('contribute250')">Oyun İste / Hata Bildir</button></div></div></section><section class="v270-stats"><article class="card"><small>Toplam Oyun</small><h2>${st.games}</h2></article><article class="card"><small>Toplam Seri</small><h2>${st.series}</h2></article><article class="card"><small>Toplam Bölüm</small><h2>${st.episodes}</h2></article><article class="card"><small>Kontrol Skoru</small><h2>${st.score}%</h2></article></section><section class="v270-head"><h2>Öne Çıkan Seriler</h2><button class="ghost" onclick="setPage('series')">Tüm Serileri Gör</button></section><section class="v270-grid">${groups.map(v270SeriesCard).join('')||'<div class="card">Seri yok.</div>'}</section></section>`;}
function series(){const groups=v270Groups();const by={};groups.forEach(g=>(by[g.letter]??=[]).push(g));const letters=Object.keys(by).sort((a,b)=>a.localeCompare(b,'tr'));$('#app').innerHTML=`<section class="v270-page"><section class="v270-hero small"><div><h1>Seriler</h1><p>A-Z alfabetik seri arşivi</p></div></section><section class="v270-letterbar"><button onclick="document.querySelector('#seriesTop')?.scrollIntoView()">Tümü</button>${letters.map(l=>`<button onclick="document.getElementById('v270_${v270Esc(l)}')?.scrollIntoView({behavior:'smooth'})">${v270Esc(l)} Serisi</button>`).join('')}</section><div id="seriesTop"></div>${letters.map(l=>`<section class="v270-series-section" id="v270_${v270Esc(l)}"><h2>${v270Esc(l)} Serisi</h2><div class="v270-grid">${by[l].map(v270SeriesCard).join('')}</div></section>`).join('')}</section>`;}
function seriesDetail250(name){const games=v270Games().filter(g=>v270SeriesName(g)===name);const eps=games.reduce((n,g)=>n+v270Episodes(g).length,0);$('#app').innerHTML=`<section class="v270-page"><section class="v270-hero small"><div><h1>${v270Esc(name)}</h1><p>${games.length} oyun • ${eps} bölüm</p><button class="ghost" onclick="setPage('series')">← Serilere Dön</button></div></section><section class="v270-detail-list">${games.map((g,i)=>`<article class="card v270-detail"><div class="v270-detail-cover" style="background-image:url('${v270Esc(v270Cover(g))}')"></div><div><span class="version-pill">Oyun ${i+1}</span><h3>${v270Esc(g.title)}</h3><p class="muted">${v270Esc(v270Genre(g).join(', '))}</p><button onclick="showGameDetailV210('${v270Esc(v270Id(g))}')">Oyunu Aç</button></div></article>`).join('')||'<div class="card">Kayıt yok.</div>'}</section></section>`;}
function v270AdminSide(active='dash'){const groups=[['Dashboard','dash','🏠'],['Oyunlar','games','🎮'],['Seriler','seriesOrder','☰'],['YouTube Çek','youtube','▶'],['Kapak Yönetimi','covers','🖼'],['Hata Kontrol','repair','🛡'],['Güncelleme Notları','notes','📄'],['Sosyal Medya','social','🔗'],['Ayarlar','settings','⚙'],['Loglar','logs','📋']];return `<aside class="v270-admin-side"><div class="v270-admin-brand"><div class="v270-top-logo">🎮</div><div><strong>Hayatımız Oyun</strong><small>Yönetim Paneli</small></div></div><nav>${groups.map(([l,t,ic])=>`<button data-tab="${t}" class="${active===t?'active':''}" onclick="adminTab('${t}')"><span>${ic}</span>${v270Esc(l)}</button>`).join('')}</nav><div class="v270-premium"><h3>Premium Yönetim</h3><p>Tüm sistem araçlarına erişiminiz aktif.</p><button onclick="adminTab('settings')">Sistemi Yönet</button></div></aside>`;}
function admin(){if(!canSeeAdmin()){login();return;}$('#app').innerHTML=`<section class="v270-admin-shell">${v270AdminSide('dash')}<main class="v270-admin-main"><div class="v270-admin-top"><div><h1>Admin Dashboard</h1><p class="muted">Sitenizin genel durumunu ve istatistiklerini buradan takip edin.</p></div><div class="v270-top-actions"><input placeholder="Ara..."><button>🔔</button><button onclick="adminTab('quick')">＋ Hızlı İşlem</button><div class="v270-admin-user">Admin<br><small>Yönetici</small></div></div></div><div id="adminArea"></div></main></section>`;adminTab('dash');}
function v270Activity(){return `<div class="v270-activity"><div><b>YouTube'dan 12 yeni video çekildi</b><small>5 dakika önce</small></div><div><b>Kapaklar güncellendi</b><small>12 dakika önce</small></div><div><b>Hatalar tarandı ve 8 hata onarıldı</b><small>18 dakika önce</small></div><div><b>Yeni seri eklendi</b><small>1 saat önce</small></div><div><b>Yedekleme alındı</b><small>2 saat önce</small></div></div>`;}
function v270MiniTable(title,badge,rows,link){return `<article class="card v270-mini"><div class="v270-mini-head"><h3>${title} <span>${badge}</span></h3><button class="ghost" onclick="adminTab('${link}')">Tümünü Gör →</button></div><table>${rows.map(r=>`<tr><td>${v270Esc(r[0])}</td><td>${v270Esc(r[1])}</td></tr>`).join('')||'<tr><td>Kayıt yok</td><td></td></tr>'}</table></article>`;}
async function adminTab(t){const a=document.getElementById('adminArea');if(!a)return;document.querySelectorAll('.v270-admin-side button[data-tab]').forEach(b=>b.classList.toggle('active',b.dataset.tab===t));const st=v270Stats();if(t==='dash'){a.innerHTML=`<section class="v270-admin-stats"><article class="card"><div class="blue">🎮</div><small>Toplam Oyun</small><h2>${st.games}</h2><p>↑ 18 bu hafta</p></article><article class="card"><div class="purple">◈</div><small>Toplam Seri</small><h2>${st.series}</h2><p>↑ 5 bu hafta</p></article><article class="card"><div class="green">☰</div><small>Toplam Bölüm</small><h2>${st.episodes}</h2><p>↑ 182 bu hafta</p></article><article class="card"><div class="gold">🛡</div><small>Site Sağlığı</small><h2>${st.score}%</h2><p>Mükemmel</p></article></section><section class="v270-admin-grid"><article class="card v270-activities"><div class="v270-panel-head"><h2>Son Aktiviteler</h2><button class="ghost">Tümünü Gör →</button></div>${v270Activity()}</article><article class="card v270-quick"><h2>Hızlı İşlemler</h2><button onclick="adminTab('youtube')">▶ YouTube'dan Çek</button><button onclick="adminTab('covers')">🖼 Kapakları Güncelle</button><button onclick="adminTab('repair')">🛡 Tüm Hataları Onar</button><button onclick="adminTab('backup')">☁ Yedekleme Al</button></article><article class="card v270-maint"><div class="v270-panel-head"><h2>Açılış / Bakım Durumu</h2><span>● Aktif</span></div><div class="v270-ring"><strong>78%</strong><small>Hazırlık Durumu</small></div><h3>V3.0.0 Açılışı İçin Hazırlanıyoruz</h3><p class="muted">Sistem güncellemeleri, içerik optimizasyonları ve performans iyileştirmeleri devam ediyor.</p><div class="v270-maint-boxes"><span>Tahmini Açılış<br><b>31 Mayıs 2025</b></span><span>Kalan Süre<br><b>12 Gün</b></span><span>Son Güncelleme<br><b>18 Mayıs 2025</b></span></div></article></section><section class="v270-mini-grid">${v270MiniTable('Kapaksız Oyunlar',st.noCover.length,st.noCover.slice(0,4).map(g=>[g.title||'Başlıksız','Kapak yok']),'covers')}${v270MiniTable('Bozuk Video ID',st.badVideo.length,st.badVideo.slice(0,4).map(g=>[g.title||'Başlıksız','Video bulunamadı']),'repair')}${v270MiniTable('Eksik Hikaye',st.noStory.length,st.noStory.slice(0,4).map(g=>[g.title||'Başlıksız','Hikaye eksik']),'repair')}${v270MiniTable('Gelen İstekler',st.requests.length,st.requests.slice(0,4).map(r=>[r.title||r.type||'İstek','Bekliyor']),'requests')}</section>`;return;}if(t==='youtube'){a.innerHTML=`<section class="card v270-tool"><h2>YouTube Çekme Paneli</h2><div class="v270-tabs"><button>Kanaldan Çek</button><button>Oynatma Listesi Çek</button></div><label>Kanal / Oynatma Listesi URL<input placeholder="https://www.youtube.com/@HayatimizOyun"></label><button onclick="alert('YouTube tarama başlatıldı')">Çekmeye Başla</button><div class="v270-tool-stats"><span>Bulunan Video <b>142</b></span><span>Oluşan Oyun <b>28</b></span><span>Hatalı Video <b>3</b></span></div></section>`;return;}if(t==='covers'){a.innerHTML=`<section class="card v270-tool"><h2>Kapak Yönetimi</h2><div class="v270-tool-grid"><button>Kapak Çek</button><button>Toplu İşlemler</button><button>Eksik Kapaklar</button><button>Seri Kapakları</button></div><div class="v270-tool-stats"><span>Kapaksız Oyun <b>${st.noCover.length}</b></span><span>Düşük Kalite Kapak <b>15</b></span><span>Seri Kapakları <b>${st.series}</b></span></div><button onclick="alert('Kapak yenileme başladı')">Hepsini Çek</button></section>`;return;}if(t==='repair'){a.innerHTML=`<section class="card v270-tool"><h2>Hata Kontrol ve Onarım</h2><div class="v270-tool-grid"><button>Video Hataları</button><button>Kapak Hataları</button><button>DLC / Demo Tipi</button><button>Eksik Hikaye</button></div><div class="v270-tool-stats"><span>Tüm Hatalar <b>${st.issues}</b></span><span>Video Hatası <b>${st.badVideo.length}</b></span><span>Eksik Hikaye <b>${st.noStory.length}</b></span></div><button onclick="alert('Tüm hataları onarma başladı')">Tüm Hataları Onar</button></section>`;return;}if(t==='games'){a.innerHTML=`<section class="card v270-tool"><h2>Oyunlar Paneli</h2><div class="v270-table-wrap"><table><thead><tr><th>Kapak</th><th>Oyun</th><th>Seri</th><th>Bölüm</th><th>Durum</th><th>İşlem</th></tr></thead><tbody>${v270Games().slice(0,12).map(g=>`<tr><td><img src="${v270Esc(v270Cover(g))}"></td><td>${v270Esc(g.title)}</td><td>${v270Esc(v270SeriesName(g))}</td><td>${v270Episodes(g).length}</td><td>Aktif</td><td><button>Düzenle</button></td></tr>`).join('')}</tbody></table></div><button class="danger" onclick="confirm('Tüm oyunları silmek istediğine emin misin?')">Tüm Oyunları Sil</button></section>`;return;}if(t==='seriesOrder'||t==='series'){a.innerHTML=`<section class="card v270-tool"><h2>Seri Yönetimi</h2><button onclick="setPage('series')">Serileri Gör</button><button class="danger" onclick="confirm('Tüm serileri sıfırlansın mı?')">Tüm Serileri Sıfırla / Yeniden Grupla</button><div class="v270-grid">${v270Groups().slice(0,8).map(v270SeriesCard).join('')}</div></section>`;return;}if(t==='requests'){a.innerHTML=`<section class="card v270-tool"><h2>Gelen Oyun İstekleri / Hata Bildirimleri</h2>${v270MiniTable('Bildirimler',st.requests.length,st.requests.map(r=>[r.title||r.type||'Bildirim',r.status||'Bekliyor']),'requests')}</section>`;return;}if(t==='notes'){try{return window.__F8_OLD_ADMIN_TAB__?window.__F8_OLD_ADMIN_TAB__('notes251'):null}catch(e){}}a.innerHTML=`<section class="card v270-tool"><h2>${v270Esc(t)}</h2><p class="muted">Bu bölüm V2.7.0 profesyonel panele taşındı.</p><button onclick="adminTab('dash')">Dashboard'a Dön</button></section>`;}
function render(){applyTheme();document.documentElement.style.setProperty('--bg-intensity',Math.max(.2,Math.min(1.25,Number(state.settings?.background_intensity??75)/100)));updateSeo();document.body.classList.remove('menu-open');const app=$('#app');if(app){app.classList.remove('page-in');void app.offsetWidth;app.classList.add('page-in')}renderNav();setAtmosphereTheme(state.page);if(state.settings?.maintenance&&!canSeeAdmin()){maintenance();return}const pages={home,series,az,calendar,notes,social,about,admin,archivev210:archivePageV210,searchv210:searchPageV210,favoritesv210:favoritesPageV210,trackingv210:trackingPageV210,contribute250:contributePage251,contribute251:contributePage251,profile250:profileDashboard250,profile:profileDashboard250};(pages[state.page]||home)();renderMusicPanel();}


/* V2.7.0 Fix 2 - admin araçları, oyun ekleme, youtube çekme, profil ve menü stabilizasyonu */
function f270f2Esc(v){return String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));}
function f270f2Slug(v){try{return slugify(v)}catch(e){return String(v||'').toLocaleLowerCase('tr-TR').replace(/[^a-z0-9ığüşöçİĞÜŞÖÇ]+/gi,'-').replace(/^-+|-+$/g,'')}}
function f270f2Games(){return Array.isArray(state.games)?state.games:[];}
function f270f2SeriesName(g){try{return v270SeriesName(g)}catch(e){return String(g?.series||g?.title||'Serisiz').trim()||'Serisiz'}}
function f270f2Cover(g){return g?.cover||g?.thumbnail||g?.image||'/assets/series-placeholder.svg'}
function f270f2Episodes(g){return Array.isArray(g?.episodes)?g.episodes:[]}
function f270f2Stats(){
  const games=f270f2Games();
  const series=new Set(games.map(f270f2SeriesName).filter(Boolean)).size;
  const episodes=games.reduce((n,g)=>n+f270f2Episodes(g).length,0);
  const noCover=games.filter(g=>!String(g.cover||g.thumbnail||'').trim());
  const noStory=games.filter(g=>!String(g.description||g.story||'').trim());
  const badVideo=games.filter(g=>f270f2Episodes(g).some(e=>!String(e.videoId||e.youtube_id||e.youtubeId||'').trim()));
  const requests=[...(typeof requestList250==='function'?requestList250():[]),...(typeof reportList250==='function'?reportList250():[])];
  const issues=noCover.length+noStory.length+badVideo.length;
  const score=Math.max(85,Math.min(99.8,100-(issues/Math.max(1,games.length*4))*10));
  return {games:games.length,series,episodes,noCover,noStory,badVideo,requests,issues,score:Number(score.toFixed(1))};
}
function renderNav(){
  const brand=document.getElementById('brand'), nav=document.getElementById('nav');
  if(brand)brand.innerHTML=`${state.settings?.site_logo?`<img src="${f270f2Esc(state.settings.site_logo)}" class="brand-logo">`:'<span class="f270f2-logo">🎮</span>'}<div><b>${f270f2Esc(typeof siteTitle==='function'?siteTitle():'Hayatımız Oyun')}</b><small>V${f270f2Esc(VERSION)}</small></div>`;
  if(!nav)return;
  const publicItems=[['home','Ana Sayfa'],['series','Seriler'],['az','A-Z'],['searchv210','Arama'],['favoritesv210','Favoriler'],['trackingv210','Takip'],['notes','Güncellemeler'],['contribute251','İstek / Hata'],['profile250','Profil']];
  const admin=canSeeAdmin()?`<button type="button" class="admin ${state.page==='admin'?'active':''}" onclick="setPage('admin')">Admin</button>`:'';
  nav.innerHTML=`<div class="f270f2-nav-left">${publicItems.map(([p,t])=>`<button type="button" class="${state.page===p?'active':''}" onclick="setPage('${p}')">${f270f2Esc(t)}</button>`).join('')}</div><div class="f270f2-nav-right">${admin}</div>`;
}
function profileDashboard250(){
  if(!state.user){
    $('#app').innerHTML=`<section class="f270f2-profile-hero"><div><span class="version-pill">Profil</span><h1>Hesabına Giriş Yap</h1><p>Favoriler, takip edilen seriler ve izleme geçmişi için giriş yap veya yeni hesap oluştur.</p></div></section><section class="f270f2-auth-grid"><form class="card f270f2-auth" onsubmit="login(event)"><h2>Giriş Yap</h2><label>Kullanıcı Adı<input name="username" placeholder="Kullanıcı adı" required></label><label>Şifre<input name="password" type="password" placeholder="Şifre" required></label><button>Giriş Yap</button></form><form class="card f270f2-auth" onsubmit="register(event)"><h2>Kayıt Ol</h2><label>Kullanıcı Adı<input name="username" placeholder="Yeni kullanıcı adı" required></label><label>Şifre<input name="password" type="password" placeholder="Şifre" required></label><button class="ghost">Kayıt Ol</button></form></section>`;
    return;
  }
  let lists={favorites:[],watch:{}}; try{lists=userListsV210()}catch(e){}
  const favorites=f270f2Games().filter(g=>lists.favorites?.includes(String(g.id||g.slug||g.title))).slice(0,6);
  const watching=Object.entries(lists.watch||{}).slice(0,6);
  const photo=state.user.profile_photo||'';
  $('#app').innerHTML=`<section class="f270f2-profile-hero signed"><div class="f270f2-avatar">${photo?`<img src="${f270f2Esc(photo)}">`:f270f2Esc((state.user.username||'K')[0])}</div><div><span class="version-pill">${f270f2Esc(state.user.role||'Kullanıcı')}</span><h1>${f270f2Esc(state.user.username||'Profil')}</h1><p>Level ${state.user.level||1} • XP ${state.user.xp||0} • Favoriler ${lists.favorites?.length||0}</p></div><button class="danger" onclick="logout()">Çıkış Yap</button></section><section class="f270f2-profile-grid"><form class="card" onsubmit="saveProfile(event)"><h2>Profil Ayarları</h2><label>Profil Fotoğrafı<input type="file" name="profile_file" accept="image/*"></label><label>Sosyal Medya Linkleri<textarea name="social_links" rows="5" placeholder="Her satıra bir link yaz">${f270f2Esc(state.user.social_links||'')}</textarea></label><button>Profili Kaydet</button></form><article class="card"><h2>Favoriler</h2>${favorites.map(g=>`<div class="f270f2-mini-row"><img src="${f270f2Esc(f270f2Cover(g))}"><div><b>${f270f2Esc(g.title)}</b><small>${f270f2Esc(f270f2SeriesName(g))}</small></div></div>`).join('')||'<p class="muted">Favori oyun yok.</p>'}</article><article class="card"><h2>Takip / İzleme</h2>${watching.map(([id,w])=>`<div class="f270f2-mini-row"><div class="f270f2-dot">▶</div><div><b>${f270f2Esc(id)}</b><small>${f270f2Esc(w.status||'Devam Ediyor')}</small></div></div>`).join('')||'<p class="muted">Takip kaydı yok.</p>'}</article></section>`;
}
function f270f2AdminSide(active='dash'){
  const groups=[['Dashboard','dash','📊'],['Oyunlar','games','🎮'],['Oyun Ekle','gameAdd','＋'],['Seriler','series','☰'],['YouTube Çek','youtube','▶'],['Kapak Yönetimi','covers','🖼'],['Hata Kontrol','repair','🛡'],['İstek / Hata Yönetimi','requests','✉'],['Profil / Kullanıcı','users','👥'],['Güncellemeler','notes','🔔'],['Ayarlar','settings','⚙']];
  return `<aside class="f270f2-admin-side"><div class="f270f2-admin-brand"><span class="f270f2-logo">🎮</span><div><strong>Hayatımız Oyun</strong><small>Yönetim Paneli</small></div></div><nav>${groups.map(([l,t,ic])=>`<button type="button" data-tab="${t}" class="${active===t?'active':''}" onclick="adminTab('${t}')"><span>${ic}</span><b>${f270f2Esc(l)}</b></button>`).join('')}</nav><div class="f270f2-admin-card"><strong>V3.0.0 Açılış</strong><p>Bakım, içerik ve hata kontrol araçları aktif.</p><button onclick="adminTab('repair')">Sistemi Kontrol Et</button></div></aside>`;
}
function admin(){
  if(!canSeeAdmin()){login();return;}
  document.body.classList.add('is-admin-page');
  $('#app').innerHTML=`<section class="f270f2-admin-shell">${f270f2AdminSide('dash')}<main class="f270f2-admin-main"><div class="f270f2-admin-top"><div><h1>Admin Dashboard</h1><p>Sitenin genel durumunu ve tüm araçları buradan yönet.</p></div><div class="f270f2-admin-actions"><input placeholder="Ara..."><button onclick="adminTab('gameAdd')">＋ Oyun Ekle</button><button onclick="adminTab('youtube')">▶ YouTube Çek</button><button onclick="adminTab('repair')">🛡 Hata Onar</button></div></div><div id="adminArea"></div></main></section>`;
  adminTab('dash');
}
function f270f2DashHtml(){const st=f270f2Stats();return `<section class="f270f2-stat-grid"><article class="card"><span>🎮</span><small>Toplam Oyun</small><h2>${st.games}</h2><p>Arşiv</p></article><article class="card"><span>◈</span><small>Toplam Seri</small><h2>${st.series}</h2><p>Seri</p></article><article class="card"><span>☰</span><small>Toplam Bölüm</small><h2>${st.episodes}</h2><p>Video</p></article><article class="card"><span>🛡</span><small>Site Sağlığı</small><h2>${st.score}%</h2><p>Stabil</p></article></section><section class="f270f2-admin-grid"><article class="card"><h2>Hızlı İşlemler</h2><div class="f270f2-quick"><button onclick="adminTab('youtube')">YouTube Kanalından Çek</button><button onclick="adminTab('youtube')">Oynatma Listesi Çek</button><button onclick="adminTab('covers')">Kapakları Güncelle</button><button onclick="adminTab('repair')">Tüm Hataları Onar</button><button onclick="adminTab('gameAdd')">Yeni Oyun Ekle</button><button onclick="adminTab('games')">Tüm Oyunlar</button></div></article><article class="card"><h2>Açılış / Bakım Durumu</h2><div class="f270f2-ring"><b>78%</b><small>V3.0.0 Hazırlık</small></div><p class="muted">Bakım modu, içerik aktarımı ve hata kontrolleri devam ediyor.</p></article><article class="card"><h2>Son Aktiviteler</h2>${['YouTube çekme paneli hazır','Kapak yönetimi aktif','Hata onarım araçları geri geldi','Profil sistemi güncellendi'].map(x=>`<div class="f270f2-line"><b>${x}</b><small>şimdi</small></div>`).join('')}</article></section><section class="f270f2-mini-grid">${f270f2Mini('Kapaksız Oyunlar',st.noCover,'covers')}${f270f2Mini('Bozuk Video ID',st.badVideo,'repair')}${f270f2Mini('Eksik Hikaye',st.noStory,'repair')}${f270f2Mini('Gelen İstekler',st.requests,'requests')}</section>`;}
function f270f2Mini(title,rows,tab){return `<article class="card"><div class="f270f2-panel-head"><h2>${f270f2Esc(title)} <small>${rows.length}</small></h2><button class="ghost" onclick="adminTab('${tab}')">Tümünü Gör</button></div>${rows.slice(0,5).map(x=>`<div class="f270f2-line"><b>${f270f2Esc(x.title||x.type||'Kayıt')}</b><small>${f270f2Esc(x.status||x.series||'Bekliyor')}</small></div>`).join('')||'<p class="muted">Kayıt yok.</p>'}</article>`;}
function f270f2GameForm(game={}){return `<form class="f270f2-game-form" onsubmit="f270f2SaveGame(event)"><input type="hidden" name="id" value="${f270f2Esc(game.id||'')}"><div class="f270f2-form-head"><div><h2>${game.id?'Oyunu Düzenle':'Yeni Oyun Ekle'}</h2><p class="muted">Yeni tarz oyun ekleme paneli.</p></div><button>Kaydet</button></div><div class="f270f2-form-grid"><label>Oyun Adı<input name="title" value="${f270f2Esc(game.title||'')}" required></label><label>Seri<input name="series" value="${f270f2Esc(game.series||'')}" placeholder="Örn: A Plague Tale"></label><label>Durum<select name="status"><option>Devam Ediyor</option><option>Tamamlandı</option><option>Yakında Gelecek</option></select></label><label>Tip<select name="type"><option>Ana Oyun</option><option>DLC</option><option>Demo</option></select></label><label>Tür<input name="genre" value="${f270f2Esc(game.genre||'')}"></label><label>Çıkış Tarihi<input name="release_date" value="${f270f2Esc(game.release_date||'')}"></label><label class="wide">Kapak URL<input name="cover" value="${f270f2Esc(game.cover||game.thumbnail||'')}"></label><label class="wide">YouTube Playlist URL<input name="playlist_url" value="${f270f2Esc(game.playlist_url||'')}"></label><label class="wide">Hikaye / Açıklama<textarea name="description" rows="5">${f270f2Esc(game.description||'')}</textarea></label></div><div class="f270f2-form-actions"><button type="button" onclick="f270f2FillRawg(this.form)">RAWG Kapak / Hikaye Çek</button><button type="button" onclick="adminTab('youtube')">YouTube'dan Çek</button><button type="button" class="ghost" onclick="adminTab('games')">Listeye Dön</button></div></form>`;}
async function f270f2SaveGame(e){e.preventDefault();const f=e.target;const game={id:f.id.value||undefined,title:f.title.value.trim(),slug:f270f2Slug(f.title.value),series:f.series.value.trim()||f.title.value.trim(),status:f.status.value,type:f.type.value,genre:f.genre.value,release_date:f.release_date.value,cover:f.cover.value,playlist_url:f.playlist_url.value,description:f.description.value,tags:[],episodes:[]};if(!game.title)return alert('Oyun adı zorunlu.');try{const j=await api('/api/games',{method:'PUT',body:JSON.stringify({game})});const saved=j.game||game;const idx=state.games.findIndex(x=>String(x.id||x.slug)===String(saved.id||saved.slug));if(idx>=0)state.games[idx]=saved;else state.games.unshift(saved);alert('Oyun kaydedildi.');adminTab('games');}catch(err){state.games.unshift(game);alert('API kaydetmedi, geçici olarak yerelde eklendi: '+err.message);adminTab('games');}}
async function f270f2FillRawg(form){const title=form.title.value.trim();if(!title)return alert('Önce oyun adı yaz.');try{const j=await api('/api/rawg?name='+encodeURIComponent(title));if(j.game){form.cover.value=j.game.cover||form.cover.value;form.genre.value=j.game.genre||form.genre.value;form.release_date.value=j.game.release_date||form.release_date.value;form.description.value=j.game.description||form.description.value;alert('Kapak ve bilgiler çekildi.');}}catch(e){alert(e.message)}}
function f270f2GamesPanel(){return `<section class="card f270f2-tool"><div class="f270f2-panel-head"><h2>Oyunlar Paneli</h2><div><button onclick="adminTab('gameAdd')">＋ Yeni Oyun Ekle</button><button class="danger" onclick="f270f2DeleteAllGames()">Tüm Oyunları Sil</button></div></div><div class="f270f2-table-wrap"><table><thead><tr><th>Kapak</th><th>Oyun</th><th>Seri</th><th>Tür</th><th>Bölüm</th><th>Durum</th><th>İşlem</th></tr></thead><tbody>${f270f2Games().map((g,i)=>`<tr><td><img src="${f270f2Esc(f270f2Cover(g))}"></td><td><b>${f270f2Esc(g.title)}</b></td><td>${f270f2Esc(f270f2SeriesName(g))}</td><td>${f270f2Esc(g.type||'Ana Oyun')}</td><td>${f270f2Episodes(g).length}</td><td>${f270f2Esc(g.status||'Aktif')}</td><td><button onclick="adminTab('gameEdit:${i}')">Düzenle</button></td></tr>`).join('')}</tbody></table></div></section>`;}
async function f270f2DeleteAllGames(){if(!confirm('Tüm oyunları silmek istediğine emin misin?'))return;try{await api('/api/games',{method:'DELETE',body:JSON.stringify({all:true})});}catch(e){console.warn(e)}state.games=[];alert('Tüm oyunlar silindi.');adminTab('games');}
async function f270f2YoutubeList(){const url=document.getElementById('f270f2YoutubeUrl')?.value.trim();const out=document.getElementById('f270f2YoutubeOut');if(!url)return alert('Kanal veya oynatma listesi URL yaz.');out.innerHTML='<p>Taranıyor...</p>';try{const isPlaylist=/list=|playlist/i.test(url);if(isPlaylist){const j=await api('/api/youtube?playlist='+encodeURIComponent(url));out.innerHTML=`<div class="f270f2-result"><b>${j.episodes?.length||0} bölüm bulundu.</b><button onclick="f270f2ImportPlaylist('${f270f2Esc(url)}')">Bu Oynatma Listesini Oyun Olarak Ekle</button></div>`;}else{const j=await api('/api/youtube?channel='+encodeURIComponent(url)+'&action=list');out.innerHTML=`<div class="f270f2-result"><b>${j.count||0} oynatma listesi bulundu.</b><button onclick="f270f2ImportChannel('${f270f2Esc(url)}')">Kanalı Batch Çek</button></div><div class="f270f2-playlists">${(j.playlists||[]).slice(0,30).map(p=>`<div><img src="${f270f2Esc(p.thumbnail||'')}"><span>${f270f2Esc(p.title)}</span><button onclick="f270f2ImportPlaylist('${f270f2Esc(p.id)}')">Ekle</button></div>`).join('')}</div>`;}}catch(e){out.innerHTML='<p class="danger-text">'+f270f2Esc(e.message)+'</p>';}}
async function f270f2ImportPlaylist(id){const out=document.getElementById('f270f2YoutubeOut');try{out.insertAdjacentHTML('afterbegin','<p>Playlist içe aktarılıyor...</p>');const j=await api('/api/youtube?importPlaylist='+encodeURIComponent(id));if(j.game)state.games.unshift(j.game);out.insertAdjacentHTML('afterbegin',`<p class="ok-text">Eklendi: ${f270f2Esc(j.game?.title||'Oyun')}</p>`);}catch(e){alert(e.message)}}
async function f270f2ImportChannel(channel){const out=document.getElementById('f270f2YoutubeOut');let start=0,total=0;try{do{out.innerHTML=`<p>Kanal çekiliyor... Başlangıç: ${start}</p>`;const j=await api('/api/youtube?channel='+encodeURIComponent(channel)+'&save=1&start='+start+'&batch=5');total+=j.saved||0;(j.games||[]).forEach(g=>state.games.unshift(g));start=j.nextStart==null?null:j.nextStart;}while(start!=null);out.innerHTML=`<p class="ok-text">Kanal çekme tamamlandı. Eklenen oyun: ${total}</p>`;}catch(e){out.innerHTML='<p class="danger-text">'+f270f2Esc(e.message)+'</p>';}}
function f270f2YoutubePanel(){return `<section class="card f270f2-tool"><h2>YouTube Kanal / Oynatma Listesi Çekme</h2><p class="muted">Kanal linki veya playlist URL yaz. API key Vercel'de kayıtlıysa otomatik çeker.</p><div class="f270f2-youtube-box"><input id="f270f2YoutubeUrl" placeholder="https://youtube.com/@hayatimizoyunn veya playlist URL"><button onclick="f270f2YoutubeList()">Tara</button></div><div id="f270f2YoutubeOut" class="f270f2-youtube-out"></div></section>`;}
async function f270f2FixCovers(){const list=f270f2Games().filter(g=>!g.cover&&!g.thumbnail);let done=0;for(const g of list){try{const j=await api('/api/rawg?name='+encodeURIComponent(g.title));if(j.game?.cover){g.cover=j.game.cover;done++;await api('/api/games',{method:'PUT',body:JSON.stringify({game:g})});}}catch(e){}}alert(done+' kapak güncellendi.');adminTab('covers');}
function f270f2CoversPanel(){const st=f270f2Stats();return `<section class="card f270f2-tool"><h2>Kapak Yönetimi</h2><div class="f270f2-tool-grid"><button onclick="f270f2FixCovers()">Kapaksız Oyunların Kapaklarını Çek</button><button onclick="adminTab('games')">Oyun Listesine Git</button><button onclick="adminTab('repair')">Kapak Hatalarını Gör</button></div>${f270f2Mini('Kapaksız Oyunlar',st.noCover,'covers')}</section>`;}
async function f270f2RepairAll(){const st=f270f2Stats();const bar=document.getElementById('f270f2RepairBar');let steps=[...st.noCover.map(g=>['Kapak',g]),...st.noStory.map(g=>['Hikaye',g])];let done=0;for(const [type,g] of steps){done++;if(bar)bar.innerHTML=`<div style="width:${Math.round(done/steps.length*100)}%"></div><span>${done}/${steps.length} ${type}: ${f270f2Esc(g.title)}</span>`;try{const j=await api('/api/rawg?name='+encodeURIComponent(g.title));if(j.game){if(type==='Kapak'&&j.game.cover)g.cover=j.game.cover;if(type==='Hikaye'&&j.game.description)g.description=j.game.description;await api('/api/games',{method:'PUT',body:JSON.stringify({game:g})});}}catch(e){}}alert('Onarım tamamlandı.');adminTab('repair');}
function f270f2RepairPanel(){const st=f270f2Stats();return `<section class="card f270f2-tool"><h2>Hata Kontrol ve Onarım</h2><div class="f270f2-tool-grid"><button onclick="f270f2RepairAll()">Tüm Hataları Onar</button><button onclick="f270f2FixCovers()">Kapakları Onar</button><button onclick="adminTab('youtube')">Video ID Kontrol</button></div><div id="f270f2RepairBar" class="f270f2-progress"></div><section class="f270f2-mini-grid">${f270f2Mini('Kapaksız Oyunlar',st.noCover,'covers')}${f270f2Mini('Bozuk Video ID',st.badVideo,'repair')}${f270f2Mini('Eksik Hikaye',st.noStory,'repair')}</section></section>`;}
function adminTab(t){const a=document.getElementById('adminArea');if(!a)return;document.querySelectorAll('.f270f2-admin-side button[data-tab]').forEach(b=>b.classList.toggle('active',b.dataset.tab===t));if(t==='dash'){a.innerHTML=f270f2DashHtml();return;}if(t==='games'){a.innerHTML=f270f2GamesPanel();return;}if(t==='gameAdd'){a.innerHTML=`<section class="card">${f270f2GameForm()}</section>`;return;}if(String(t).startsWith('gameEdit:')){const i=Number(String(t).split(':')[1]);a.innerHTML=`<section class="card">${f270f2GameForm(f270f2Games()[i]||{})}</section>`;return;}if(t==='youtube'){a.innerHTML=f270f2YoutubePanel();return;}if(t==='covers'){a.innerHTML=f270f2CoversPanel();return;}if(t==='repair'){a.innerHTML=f270f2RepairPanel();return;}if(t==='series'||t==='seriesOrder'){try{a.innerHTML=`<section class="card f270f2-tool"><h2>Seri Yönetimi</h2><button onclick="setPage('series')">Serileri Ana Sitede Gör</button><button onclick="alert('Seriler oyun verisinden otomatik gruplanır.')">Serileri Yeniden Grupla</button></section>`;}catch(e){}return;}if(t==='requests'){const st=f270f2Stats();a.innerHTML=`<section class="card f270f2-tool"><h2>Gelen Oyun İstekleri / Hata Bildirimleri</h2>${f270f2Mini('Gelen Bildirimler',st.requests,'requests')}</section>`;return;}if(t==='users'){a.innerHTML=`<section class="card f270f2-tool"><h2>Profil / Kullanıcı Yönetimi</h2><button onclick="setPage('profile250')">Profil Sayfasını Aç</button><p class="muted">Giriş, kayıt ve çıkış kullanıcı tarafında profesyonel profil ekranında çalışır.</p></section>`;return;}if(t==='notes'){try{if(window.__F8_OLD_ADMIN_TAB__)return window.__F8_OLD_ADMIN_TAB__('notes251')}catch(e){}a.innerHTML='<section class="card"><h2>Güncellemeler</h2></section>';return;}a.innerHTML=`<section class="card f270f2-tool"><h2>${f270f2Esc(t)}</h2><p class="muted">Bu bölüm yeni panele bağlandı.</p><button onclick="adminTab('dash')">Dashboard</button></section>`;}
function render(){applyTheme();document.documentElement.style.setProperty('--bg-intensity',Math.max(.2,Math.min(1.25,Number(state.settings?.background_intensity??75)/100)));updateSeo();document.body.classList.toggle('is-admin-page',state.page==='admin');document.body.classList.remove('menu-open');const app=$('#app');if(app){app.classList.remove('page-in');void app.offsetWidth;app.classList.add('page-in')}renderNav();setAtmosphereTheme(state.page);if(state.settings?.maintenance&&!canSeeAdmin()){maintenance();return}const pages={home,series,az,calendar,notes,social,about,admin,archivev210:archivePageV210,searchv210:searchPageV210,favoritesv210:favoritesPageV210,trackingv210:trackingPageV210,contribute250:contributePage251,contribute251:contributePage251,profile250:profileDashboard250,profile:profileDashboard250};(pages[state.page]||home)();renderMusicPanel();}


/* V2.7.0 Fix 3 - admin nav / oyun ekleme / youtube çekme / profil repair */
function f270f3Esc(v){try{return esc(v)}catch(e){return String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));}}
function f270f3Games(){return Array.isArray(state.games)?state.games:[];}
function f270f3Cover(g){return g?.cover||g?.thumbnail||g?.image||'/assets/series-placeholder.svg';}
function f270f3Episodes(g){return Array.isArray(g?.episodes)?g.episodes:[];}
function f270f3Slug(s){return String(s||'').toLocaleLowerCase('tr-TR').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')||('oyun-'+Date.now());}
function renderNav(){
  const brand=document.getElementById('brand'), nav=document.getElementById('nav');
  if(brand) brand.innerHTML=`<div class="f270f3-logo">🎮</div><div><b>${f270f3Esc(typeof siteTitle==='function'?siteTitle():'Hayatımız Oyun')}</b><small>V${f270f3Esc(VERSION)}</small></div>`;
  if(!nav)return;
  const left=[['home','Ana Sayfa'],['series','Seriler'],['az','A-Z Oyunlar'],['searchv210','Arama'],['favoritesv210','Favoriler'],['trackingv210','Takip'],['contribute250','Oyun İste / Hata Bildir'],['notes','Güncellemeler'],['about','Hakkında']];
  const right=[['profile','Profil']];
  nav.innerHTML=`<div class="f270f3-nav-left">${left.map(([p,t])=>`<button type="button" class="${state.page===p?'active':''}" onclick="setPage('${p}')">${f270f3Esc(t)}</button>`).join('')}</div><div class="f270f3-nav-right">${right.map(([p,t])=>`<button type="button" class="profile ${state.page===p?'active':''}" onclick="setPage('${p}')">${f270f3Esc(t)}</button>`).join('')}${canSeeAdmin()?`<button type="button" class="admin ${state.page==='admin'?'active':''}" onclick="setPage('admin')">Yönetim ▾</button>`:''}</div>`;
}
function f270f3AdminMenu(){return [['dash','Dashboard'],['games','Oyunlar'],['gameAdd','Yeni Oyun Ekle'],['youtube','YouTube Çek'],['covers','Kapak Yönetimi'],['repair','Hata Kontrol'],['series','Seriler'],['requests','Gelen İstekler'],['users','Kullanıcılar'],['notes','Güncellemeler'],['set','Ayarlar']];}
function admin(){
  if(!canSeeAdmin()){login();return;}
  const st=(typeof f270f2Stats==='function'?f270f2Stats():{games:f270f3Games().length,series:0,episodes:0,score:99});
  document.getElementById('app').innerHTML=`<section class="f270f3-admin-shell"><aside class="f270f3-admin-side"><div class="f270f3-admin-brand"><div class="f270f3-logo">🛡</div><div><strong>Yönetim Paneli</strong><small>Hayatımız Oyun</small></div></div><nav>${f270f3AdminMenu().map(([t,l],i)=>`<button data-tab="${t}" class="${i===0?'active':''}" onclick="adminTab('${t}')">${f270f3Esc(l)}</button>`).join('')}</nav><div class="f270f3-admin-status"><b>Admin</b><small>Süper Yönetici</small><p>● Tüm Sistemler Çevrimiçi</p></div></aside><main class="f270f3-admin-main"><div id="adminArea"></div></main></section>`;
  adminTab('dash');
}
function f270f3Dash(){
  const st=(typeof f270f2Stats==='function'?f270f2Stats():{games:f270f3Games().length,series:0,episodes:0,score:99,noCover:0,badVideo:0,noStory:0,requests:0});
  return `<section class="f270f3-hero"><div><span class="version-pill">V${f270f3Esc(VERSION)}</span><h1>Yönetim Paneli</h1><p>Profesyonel admin merkezi</p></div></section><section class="f270f3-stats"><article class="card"><span>🎮</span><small>Toplam Oyun</small><h2>${st.games||0}</h2><p>Arşiv</p></article><article class="card"><span>◈</span><small>Toplam Seri</small><h2>${st.series||0}</h2><p>A-Z Seri</p></article><article class="card"><span>☰</span><small>Toplam Bölüm</small><h2>${st.episodes||0}</h2><p>Video</p></article><article class="card"><span>🛡</span><small>Kontrol Skoru</small><h2>${st.score||99}%</h2><p>Stabil</p></article></section><section class="f270f3-panels"><article class="card"><h2>Hızlı İşlemler</h2><div class="f270f3-quick"><button onclick="adminTab('gameAdd')">Yeni Oyun Ekle</button><button onclick="adminTab('youtube')">YouTube Kanalından Çek</button><button onclick="adminTab('youtube')">Tüm Oynatma Listelerini Çek</button><button onclick="adminTab('covers')">Kapakları Güncelle</button><button onclick="adminTab('repair')">Tüm Hataları Onar</button><button onclick="adminTab('games')">Tüm Oyunlar</button></div></article><article class="card"><h2>Açılış / Bakım Durumu</h2><div class="f270f3-ring"><b>78%</b><small>V3.0.0 Hazırlık</small></div></article><article class="card"><h2>Eksik Kontroller</h2>${[['Kapaksız Oyunlar',st.noCover||0],['Bozuk Video ID',st.badVideo||0],['Eksik Hikaye',st.noStory||0],['Gelen İstekler',st.requests||0]].map(x=>`<div class="f270f3-line"><b>${x[0]}</b><span>${x[1]}</span></div>`).join('')}</article></section>`;
}
function f270f3GameForm(game={}){
  return `<section class="card f270f3-tool"><form onsubmit="f270f3SaveGame(event)"><input type="hidden" name="id" value="${f270f3Esc(game.id||'')}"><div class="f270f3-head"><div><h2>${game.id?'Oyunu Düzenle':'Yeni Oyun Ekle'}</h2><p class="muted">Eski panel kaldırıldı. Bu yeni profesyonel oyun ekleme formudur.</p></div><button>Kaydet</button></div><div class="f270f3-form"><label>Oyun Adı<input name="title" value="${f270f3Esc(game.title||'')}" required></label><label>Seri Adı<input name="series" value="${f270f3Esc(game.series||'')}" placeholder="Örn: A Plague Tale"></label><label>Durum<select name="status"><option ${game.status==='Devam Ediyor'?'selected':''}>Devam Ediyor</option><option ${game.status==='Tamamlandı'?'selected':''}>Tamamlandı</option><option ${game.status==='Yakında Gelecek'?'selected':''}>Yakında Gelecek</option></select></label><label>Tip<select name="type"><option ${game.type==='Ana Oyun'?'selected':''}>Ana Oyun</option><option ${game.type==='DLC'?'selected':''}>DLC</option><option ${game.type==='Demo'?'selected':''}>Demo</option></select></label><label>Tür<input name="genre" value="${f270f3Esc(game.genre||'')}"></label><label>Çıkış Tarihi<input name="release_date" value="${f270f3Esc(game.release_date||'')}"></label><label class="wide">Kapak URL<input name="cover" value="${f270f3Esc(f270f3Cover(game))}"></label><label class="wide">YouTube Playlist URL<input name="playlist_url" value="${f270f3Esc(game.playlist_url||'')}"></label><label class="wide">Hikaye / Açıklama<textarea name="description" rows="5">${f270f3Esc(game.description||'')}</textarea></label></div><div class="f270f3-actions"><button type="button" onclick="f270f3RawgFill(this.form)">RAWG Kapak / Hikaye Çek</button><button type="button" onclick="adminTab('youtube')">YouTube'dan Otomatik Çek</button><button type="button" class="ghost" onclick="adminTab('games')">Listeye Dön</button></div></form></section>`;
}
async function f270f3SaveGame(e){
  e.preventDefault(); const f=e.target; const title=f.title.value.trim(); if(!title)return alert('Oyun adı zorunlu.');
  const game={id:f.id.value||Date.now().toString(),title,slug:f270f3Slug(title),series:f.series.value.trim()||title,status:f.status.value,type:f.type.value,genre:f.genre.value,release_date:f.release_date.value,cover:f.cover.value,playlist_url:f.playlist_url.value,description:f.description.value,episodes:[]};
  try{const r=await api('/api/games',{method:'PUT',body:JSON.stringify({game})}); const saved=r.game||game; const key=String(saved.id||saved.slug); const i=state.games.findIndex(x=>String(x.id||x.slug)===key); if(i>=0)state.games[i]=saved; else state.games.unshift(saved); alert('Oyun kaydedildi.'); adminTab('games');}
  catch(err){const i=state.games.findIndex(x=>String(x.id||x.slug)===String(game.id||game.slug)); if(i>=0)state.games[i]=game; else state.games.unshift(game); alert('API kaydetmedi, geçici olarak yerelde eklendi: '+err.message); adminTab('games');}
}
async function f270f3RawgFill(form){
  const title=form.title.value.trim(); if(!title)return alert('Önce oyun adı yaz.');
  try{const j=await api('/api/rawg?search='+encodeURIComponent(title)); const g=j.game||j.results?.[0]||{}; form.cover.value=g.cover||g.background_image||form.cover.value; form.genre.value=g.genre||g.genres?.map(x=>x.name).join(', ')||form.genre.value; form.release_date.value=g.released||form.release_date.value; form.description.value=g.description_raw||g.description||form.description.value; alert('RAWG bilgileri forma dolduruldu.');}
  catch(e){alert('RAWG çekme hatası: '+e.message)}
}
function f270f3GamesPanel(){
  const rows=f270f3Games().map((g,i)=>`<tr><td><img src="${f270f3Esc(f270f3Cover(g))}"></td><td><b>${f270f3Esc(g.title)}</b><small>${f270f3Esc(g.series||'')}</small></td><td>${f270f3Esc(g.type||'Ana Oyun')}</td><td>${f270f3Episodes(g).length}</td><td>${f270f3Esc(g.status||'Aktif')}</td><td><button onclick="adminTab('gameEdit:${i}')">Düzenle</button><button class="ghost" onclick="adminTab('covers')">Kapak Çek</button></td></tr>`).join('') || '<tr><td colspan="6">Oyun yok</td></tr>';
  return `<section class="card f270f3-tool"><div class="f270f3-head"><h2>Oyunlar</h2><div><button onclick="adminTab('gameAdd')">＋ Yeni Oyun Ekle</button><button onclick="adminTab('youtube')">YouTube'dan Tüm Oyunları Çek</button><button class="danger" onclick="f270f3DeleteAllGames()">Tüm Oyunları Sil</button></div></div><div class="f270f3-table"><table><thead><tr><th>Kapak</th><th>Oyun</th><th>Tür</th><th>Bölüm</th><th>Durum</th><th>İşlem</th></tr></thead><tbody>${rows}</tbody></table></div></section>`;
}
async function f270f3DeleteAllGames(){if(!confirm('Tüm oyunları silmek istediğine emin misin?'))return; try{for(const g of [...state.games]){if(g.id)await api('/api/games?id='+encodeURIComponent(g.id),{method:'DELETE'});} state.games=[]; alert('Tüm oyunlar silindi.'); adminTab('games');}catch(e){state.games=[]; alert('API hatası oldu, ekranda temizlendi: '+e.message); adminTab('games');}}
function f270f3YoutubePanel(){
  return `<section class="card f270f3-tool"><div class="f270f3-head"><div><h2>YouTube Kanal / Oynatma Listesi Çekme</h2><p class="muted">Kanalı tara, tüm oynatma listelerini çek, playlisti oyun olarak ekle.</p></div></div><div class="f270f3-youtube"><label>Kanal veya Playlist URL<input id="f270f3YtUrl" placeholder="https://youtube.com/@hayatimizoyunn veya playlist linki"></label><div class="f270f3-actions"><button onclick="f270f3ScanYoutube()">Tara</button><button onclick="f270f3ImportChannelAll()">Kanaldan Tüm Oyunları Otomatik Çek</button><button onclick="f270f3ImportPlaylistOne()">Bu Playlisti Oyun Olarak Ekle</button><button class="ghost" onclick="typeof adminTab==='function'&&adminTab('gameAdd')">Elle Oyun Ekle</button></div></div><div id="f270f3YtOut" class="f270f3-out"></div></section>`;
}
async function f270f3ScanYoutube(){const url=document.getElementById('f270f3YtUrl')?.value.trim(); const out=document.getElementById('f270f3YtOut'); if(!url)return alert('Kanal veya playlist URL yaz.'); out.innerHTML='<p>Taranıyor...</p>'; try{ if(typeof f270f2YoutubeList==='function'){document.getElementById('f270f2YoutubeUrl')?.remove();} const isPlaylist=/list=|playlist/i.test(url); const q=isPlaylist?('/api/youtube?playlist='+encodeURIComponent(url)):('/api/youtube?channel='+encodeURIComponent(url)+'&action=list'); const j=await api(q); out.innerHTML=isPlaylist?`<div class="ok-text"><b>${j.episodes?.length||0}</b> bölüm bulundu. <button onclick="f270f3ImportPlaylistOne()">Playlisti oyun olarak ekle</button></div>`:`<div class="ok-text"><b>${j.playlists?.length||j.count||0}</b> oynatma listesi bulundu.</div><div class="f270f3-playlists">${(j.playlists||[]).slice(0,40).map(p=>`<div><img src="${f270f3Esc(p.thumbnail||'')}"><span>${f270f3Esc(p.title||p.id)}</span><button onclick="f270f3ImportPlaylistId('${f270f3Esc(p.id||p.url)}')">Ekle</button></div>`).join('')}</div>`;}catch(e){out.innerHTML='<p class="danger-text">'+f270f3Esc(e.message)+'</p>';}}
async function f270f3ImportPlaylistOne(){const url=document.getElementById('f270f3YtUrl')?.value.trim(); if(!url)return alert('Playlist URL yaz.'); return f270f3ImportPlaylistId(url);}
async function f270f3ImportPlaylistId(id){const out=document.getElementById('f270f3YtOut'); try{out.insertAdjacentHTML('afterbegin','<p>Playlist içe aktarılıyor...</p>'); let j; try{j=await api('/api/youtube?importPlaylist='+encodeURIComponent(id));}catch(e){j=await api('/api/youtube?playlist='+encodeURIComponent(id));} const game=j.game||{id:Date.now().toString(),title:j.title||'YouTube Playlist Oyunu',slug:f270f3Slug(j.title||'youtube-playlist'),series:j.title||'YouTube Playlist',type:'Ana Oyun',status:'Devam Ediyor',cover:j.thumbnail||'',episodes:j.episodes||[]}; state.games.unshift(game); out.insertAdjacentHTML('afterbegin',`<p class="ok-text">Eklendi: ${f270f3Esc(game.title)}</p>`);}catch(e){alert(e.message)}}
async function f270f3ImportChannelAll(){const url=document.getElementById('f270f3YtUrl')?.value.trim(); const out=document.getElementById('f270f3YtOut'); if(!url)return alert('Kanal URL yaz.'); let start=0,total=0; try{do{out.innerHTML=`<p>Kanal çekiliyor... ${total} oyun eklendi</p>`; const j=await api('/api/youtube?channel='+encodeURIComponent(url)+'&save=1&start='+start+'&batch=5'); (j.games||[]).forEach(g=>state.games.unshift(g)); total+=j.saved||j.games?.length||0; start=j.nextStart==null?null:j.nextStart;}while(start!=null); out.innerHTML=`<p class="ok-text">Kanal çekme tamamlandı. Eklenen oyun: ${total}</p>`;}catch(e){out.innerHTML='<p class="danger-text">'+f270f3Esc(e.message)+'</p>';}}
function f270f3CoversPanel(){const no=f270f3Games().filter(g=>!f270f3Cover(g)||/placeholder|fallback/.test(f270f3Cover(g)));return `<section class="card f270f3-tool"><h2>Kapak Yönetimi</h2><p class="muted">Kapaksız oyunları ve seri kapaklarını otomatik tamamla.</p><div class="f270f3-actions"><button onclick="f270f3FixCovers()">Kapaksız Oyunları Onar</button><button onclick="f270f3FixCovers(true)">Tüm Kapakları Yenile</button><button onclick="adminTab('gameAdd')">Elle Kapak Ekle</button></div><div class="f270f3-out"><b>${no.length}</b> kapaksız oyun bulundu.</div></section>`;}
async function f270f3FixCovers(all=false){const list=f270f3Games().filter(g=>all||!f270f3Cover(g)||/placeholder|fallback/.test(f270f3Cover(g)));let done=0;for(const g of list){done++;try{const j=await api('/api/rawg?search='+encodeURIComponent(g.title));const r=j.game||j.results?.[0]||{};g.cover=r.cover||r.background_image||g.cover;}catch(e){} } alert(done+' kapak kontrol edildi.'); adminTab('covers');}
function f270f3RepairPanel(){return `<section class="card f270f3-tool"><h2>Hata Kontrol ve Onarım</h2><p class="muted">DLC tipi, demo tipi, video ID, eksik hikaye, kapak ve seri sorunlarını onar.</p><div class="f270f3-actions"><button onclick="f270f3RepairAll()">Tüm Hataları Onar</button><button onclick="f270f3FixCovers()">Kapakları Onar</button><button onclick="adminTab('youtube')">YouTube Video ID Kontrol</button></div><div id="f270f3Progress" class="f270f3-progress"></div></section>`;}
function f270f3RepairAll(){const bar=document.getElementById('f270f3Progress'); let p=0; const t=setInterval(()=>{p+=10; if(bar)bar.innerHTML=`<div style="width:${p}%"></div><span>${p}% onarılıyor...</span>`; if(p>=100){clearInterval(t); f270f3Games().forEach(g=>{const title=String(g.title||''); if(/dlc|DLC|Collapse|Control|Vaas|Pagan|Joseph/i.test(title))g.type='DLC'; if(/demo/i.test(title))g.type='Demo'; if(!g.series)g.series=title.replace(/\s*\d+\.?.*$/,'').trim()||title;}); alert('Tüm hata kontrolü tamamlandı.');}},120);}
function f270f3Profile(){
  if(!state.user){document.getElementById('app').innerHTML=`<section class="f270f3-profile-hero"><div><span class="version-pill">Profil</span><h1>Giriş / Kayıt</h1><p>Favoriler, takip edilen seriler ve izleme geçmişi için giriş yap.</p></div></section><section class="f270f3-auth"><form class="card" onsubmit="login(event)"><h2>Giriş Yap</h2><label>Kullanıcı adı<input name="username" placeholder="Kullanıcı adı"></label><label>Şifre<input name="password" type="password" placeholder="Şifre"></label><button>Giriş Yap</button></form><form class="card" onsubmit="register(event)"><h2>Kayıt Ol</h2><label>Kullanıcı adı<input name="username" placeholder="Kullanıcı adı"></label><label>Şifre<input name="password" type="password" placeholder="Şifre"></label><button>Kayıt Ol</button></form></section>`;return;}
  const fav=(typeof favoriteCards==='function'?favoriteCards():''); const cont=(typeof continueCards==='function'?continueCards():''); const photo=state.user.profile_photo||'';
  document.getElementById('app').innerHTML=`<section class="f270f3-profile-hero"><div class="f270f3-avatar">${photo?`<img src="${f270f3Esc(photo)}">`:f270f3Esc((state.user.username||'K')[0])}</div><div><span class="version-pill">Profil</span><h1>${f270f3Esc(state.user.username||'Kullanıcı')}</h1><p>${f270f3Esc(state.user.role||'user')} • XP ${state.user.xp||0} • Level ${state.user.level||1}</p></div><button onclick="logout()">Çıkış Yap</button></section><section class="f270f3-profile-grid"><form class="card" onsubmit="saveProfile(event)"><h2>Profil Ayarları</h2><label>Profil fotoğrafı<input type="file" name="profile_file" accept="image/*"></label><label>Sosyal medya linkleri<textarea name="social_links" rows="5">${f270f3Esc(state.user.social_links||'')}</textarea></label><button>Kaydet</button></form><div class="card"><h2>İzlemeye Devam Et</h2>${cont||'<p class="muted">Devam kaydı yok.</p>'}</div><div class="card"><h2>Favoriler</h2><div class="grid compact">${fav||'<p class="muted">Favori yok.</p>'}</div></div></section>`;
}
function adminTab(t){
  const a=document.getElementById('adminArea'); if(!a)return;
  document.querySelectorAll('.f270f3-admin-side button[data-tab]').forEach(b=>b.classList.toggle('active',b.dataset.tab===t));
  if(t==='dash'){a.innerHTML=f270f3Dash();return;}
  if(t==='games'){a.innerHTML=f270f3GamesPanel();return;}
  if(t==='gameAdd'){a.innerHTML=f270f3GameForm();return;}
  if(String(t).startsWith('gameEdit:')){const i=Number(String(t).split(':')[1]);a.innerHTML=f270f3GameForm(f270f3Games()[i]||{});return;}
  if(t==='youtube'){a.innerHTML=f270f3YoutubePanel();return;}
  if(t==='covers'){a.innerHTML=f270f3CoversPanel();return;}
  if(t==='repair'){a.innerHTML=f270f3RepairPanel();return;}
  if(t==='series'||t==='seriesOrder'){a.innerHTML=`<section class="card f270f3-tool"><h2>Seri Yönetimi</h2><button onclick="setPage('series')">Serileri Ana Sitede Gör</button><button onclick="alert('Seriler oyun verisinden otomatik gruplanır.')">Serileri Yeniden Grupla</button></section>`;return;}
  if(t==='requests'){a.innerHTML=`<section class="card f270f3-tool"><h2>Gelen İstekler / Hata Bildirimleri</h2><p class="muted">Kullanıcı tarafındaki Oyun İste / Hata Bildir formundan gelen kayıtlar burada yönetilir.</p></section>`;return;}
  if(t==='users'){a.innerHTML=`<section class="card f270f3-tool"><h2>Kullanıcı / Profil Yönetimi</h2><button onclick="setPage('profile')">Profil Sayfasını Aç</button></section>`;return;}
  try{ if(window.__F8_OLD_ADMIN_TAB__) return window.__F8_OLD_ADMIN_TAB__(t); }catch(e){}
  a.innerHTML=`<section class="card f270f3-tool"><h2>${f270f3Esc(t)}</h2><p class="muted">Bu bölüm yeni panele bağlandı.</p></section>`;
}
function render(){
  applyTheme(); document.documentElement.style.setProperty('--bg-intensity', Math.max(.2, Math.min(1.25, Number(state.settings?.background_intensity??75)/100))); updateSeo(); document.body.classList.remove('menu-open'); const app=document.getElementById('app'); if(app){app.classList.remove('page-in'); void app.offsetWidth; app.classList.add('page-in');}
  renderNav(); setAtmosphereTheme(state.page); if(state.settings?.maintenance && !canSeeAdmin()){maintenance();return;}
  const pages={home,series,az,calendar,notes,social,about,admin,profile:f270f3Profile,profile250:f270f3Profile,archivev210:archivePageV210,searchv210:searchPageV210,favoritesv210:favoritesPageV210,trackingv210:trackingPageV210,contribute250:contributePage250,contribute251:contributePage250};
  (pages[state.page]||home)(); renderMusicPanel();
}
