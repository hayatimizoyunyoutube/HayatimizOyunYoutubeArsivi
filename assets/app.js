...
/* Hayatımız Oyun - V2.5.0 Mega Platform Update */
const VERSION='2.5.0';
...
function renderSocialSurfaces(){
  ...
  const foot=$('#siteFooter'); if(foot)foot.innerHTML=`<div class="footer-info"><b>${esc(siteTitle())}</b><small>V${VERSION} • V2.5.0</small><p>${esc(state.settings?.footer_text||state.settings?.share_description||'Oyun ve seri izleme arşivi.')}</p><div class="footer-links"><button class="ghost small" onclick="setPage('notes')">Güncelleme Notları</button><button class="ghost small" onclick="setPage('calendar')">Takvim</button><button class="ghost small" onclick="setPage('about')">Hakkında / Destek</button></div></div><div class="footer-social">${state.settings?.show_social_footer===false?'':(html||'<span class="muted">Sosyal medya linkleri ayarlardan eklenebilir.</span>')}</div>`;
...
function hideLoader(){
  const l=$('#siteLoader');
  if(l){
    loaderSet(100,'Hazır — V2.5.0 site açılıyor.');
    setTimeout(()=>{
...
async function load(){
  ...
  const steps=[
    {label:'V2.5.0 Mega sistem hazırlanıyor...', pct:6, delay:180},
    {label:'Ayarlar yükleniyor...', pct:16, delay:220},
    {label:'Oyun arşivi hazırlanıyor...', pct:34, delay:260},
    {label:'Video ID ve izleme sistemi kontrol ediliyor...', pct:52, delay:220},
    {label:'Katkı merkezi ve sağlık puanı hazırlanıyor...', pct:70, delay:220},
    {label:'Güncelleme notları sıralanıyor...', pct:84, delay:180},
    {label:'V2.5.0 platformu açılıyor...', pct:96, delay:220},
  ];
  const setStep=async(step)=>{loaderSet(step.pct,step.label);await wait(step.delay);};
  await setStep(steps[0]);
  await setStep(steps[1]);
  try{const r=await api('/api/settings');state.settings=r.settings||{};}catch(e){console.warn(e);state.settings=state.settings||{}}
  await setStep(steps[2]);
  try{const r=await api('/api/games');state.games=r.games||[];}catch(e){console.warn(e);state.games=state.games||[]}
  await setStep(steps[3]);
  await setStep(steps[4]);
  await setStep(steps[5]);
  try{const r=await api('/api/notes');state.notes=(typeof sortNotesV220==='function'?sortNotesV220(r.notes||[]):r.notes||[]);}catch(e){console.warn(e);state.notes=state.notes||[]}
  try{const r=await api('/api/calendar');state.events=r.events||[];}catch(e){console.warn(e);state.events=state.events||[]}
  await setStep(steps[6]);
  loaderSet(100,'V2.5.0 hazır.');
  await wait(420);
}
...
/* V2.5.0 - Büyük sistem güncellemesi */
...
/* V2.5.0 - YouTube video ID otomatik algılama */
...
function exportBackupV210(){
  ...
  a.download='hayatimiz-oyun-V2.5.0-yedek.json';
  a.click();
...
function v210AnnouncementBanner(){
  const list=storageGetV210('ho_v210_announcements',[{title:'V2.5.0 yayında',type:'Yeni Özellik',body:'Gelişmiş arşiv, arama, favoriler, izleme takip, sağlık kontrolü ve yedekleme sistemi eklendi.'}]);
...
function v210AdminPanel(){
  ...
  return `<section class="card v210-admin-panel"><div class="section-title"><div><span class="version-pill">V2.5.0</span><h2>Büyük Sistem Güncellemesi</h2><p class="muted">Arşiv, arama, favoriler, izleme takip, site sağlık kontrolü, duyuru ve yedekleme merkezi.</p></div><button onclick="exportBackupV210()">Yedek Al</button></div>
...
function archivePageV210(){
  ...
  return `<section class="hero"><span class="version-pill">V2.5.0</span><h1>Gelişmiş Arşiv</h1><p>Seriler, oyunlar, bölümler ve takip durumları tek merkezde.</p></section>
...
function searchPageV210(){
  ...
  return `<section class="hero"><span class="version-pill">V2.5.0</span><h1>Gelişmiş Arama</h1><p>Oyun, seri, durum, tip ve açıklama üzerinden arama yap.</p></section>
...
function favoritesPageV210(){
  ...
  return `<section class="hero"><span class="version-pill">V2.5.0</span><h1>Favoriler</h1><p>Favori oyunların ve serilerin hızlı erişim alanı.</p></section><section class="grid">${games.map(gameCardV210).join('')||'<div class="card"><p class="muted">Henüz favori eklenmedi.</p></div>'}</section>`;
}
function trackingPageV210(){
  const watch=userListsV210().watch||{};
  const games=(state.games||[]).filter(g=>watch[gameIdV210(g)]);
  return `<section class="hero"><span class="version-pill">V2.5.0</span><h1>İzleme Takibi</h1><p>İzledim, izliyorum, izleyeceğim ve yarım kaldı kayıtların.</p></section><section class="grid">${games.map(gameCardV210).join('')||'<div class="card"><p class="muted">Henüz takip kaydı yok.</p></div>'}</section>`;
}


/* V2.5.0 - Professional Stability Core */
...
function v220HomeBanner(){
  return `<section class="v220-banner card"><div><span class="version-pill">V2.5.0</span><h2>Site sürümü V2.5.0</h2><p class="muted">Site V2.5.0 sürümünde çalışıyor. Açılış/loading ekranı V2.5.0 açılış sistemiyle otomatik yüzde ilerlemesi kullanır.</p></div><button onclick="setPage('notes')">Güncelleme Notları</button></section>`;
}
...
function v250FeatureList(){
  return [
    ...
    'Oyun Öneri Motoru','Benzer Oyunlar Sistemi','Toplu Veri Temizleme','Tam Yedekleme Sistemi V3','V2.5.0 Final Kontrol Paneli'
  ];
}
...
function streamerCard(){
 ...
 return `<section class="card streamer-card streamer-v186"><div><span class="version-pill ${live?'live-pill':''}">${live?'● Canlı Yayında':'Yayıncı Alanı'}</span><h2>Hayatımız Oyun'u takip et ve destek ol</h2><p class="muted">${esc(desc)}</p><p class="muted"><b>V2.5.0:</b> ${esc(finalCountdown())}</p></div><div class="social-icons streamer-links">${buttons}</div></section>`;
}
...
/* V2.5.0 - A-Z/Seriler arama + sosyal medya kontrol */
...
function renderSocialCheckV300(){
  ...
  return `<section class="card social-check-v300"><div class="section-title"><div><span class="version-pill">V2.5.0</span><h2>Sosyal Medya Hata Kontrol</h2><p class="muted">${bad?bad+' sorun bulundu.':'Tüm sosyal medya bağlantıları iyi görünüyor.'}</p></div><button onclick="copySocialReportV300()">Raporu Kopyala</button></div>
...
function series(){
 const map=new Map();
 state.games.forEach(g=>{const name=getSeriesName(g)||'Serisiz'; if(!map.has(name))map.set(name,[]); map.get(name).push(g);});
 const entries=[...map.entries()].sort((a,b)=>trSort(a[0],b[0]));
 $('#app').innerHTML=`<section class="hero"><span class="version-pill">V${VERSION}</span><h1>Seriler</h1><p class="muted">Seri adına göre arama yapabilir, oyunları daha hızlı bulabilirsin.</p></section>
 ...
 <section class="grid">${entries.map(([name,items])=>{
  const cover=gameCoverV220?gameCoverV220(items[0]):(items[0].cover||'');
  const eps=items.reduce((a,g)=>a+(Array.isArray(g.episodes)?g.episodes.length:0),0);
  return `<article class="card series-card-v300" data-series-search-v300="${esc(name+' '+items.map(x=>x.title).join(' '))}"><div class="series-cover-v300" style="background-image:url('${esc(cover)}')"></div><div><h2>${esc(name)}</h2><p class="muted">${items.length} oyun • ${eps} bölüm</p><div class="row"><button onclick="setPage('searchv210');setTimeout(()=>{const q=document.getElementById('searchQv210');if(q){q.value='${esc(name)}';}},80)">Seriyi Aç</button></div></div></article>`;
 }).join('')}</section>`;
}
function az(){
 const games=[...(state.games||[])].sort((a,b)=>trSort(a.title,b.title));
 const groups={};
 games.forEach(g=>{
  const ch=(String(g.title||'#').trim()[0]||'#').toLocaleUpperCase('tr-TR');
  const key=/[A-ZÇĞİÖŞÜ0-9]/i.test(ch)?ch:'#';
  if(!groups[key])groups[key]=[];
  groups[key].push(g);
 });
 const keys=Object.keys(groups).sort((a,b)=>trSort(a,b));
 $('#app').innerHTML=`<section class="hero"><span class="version-pill">V${VERSION}</span><h1>A-Z Oyunlar</h1><p class="muted">Oyun adına, seri adına veya duruma göre hızlı arama.</p></section>
...
function notes(){
  const publicNotes=sortNotesV220(state.notes||[]).filter(n=>n.public_visible!==false&&n.type!=='Admin');
  $('#app').innerHTML=`<section class="hero notes-hero-v220"><span class="version-pill">V${VERSION}</span><h1>Güncelleme Notları</h1><p class="muted">Sürümler en yeniden eskiye doğru net ve karışmadan listelenir.</p></section>
  <section class="notes-v220">${publicNotes.map((n,idx)=>`<article class="card note-v220 ${idx===0?'latest':''}"><aside><b>${escV220(n.version||'Sürüm')}</b><span>${escV220(n.type||'Not')}</span>${idx===0?'<em>EN YENİ</em>':''}</aside><div><h2>${escV220(n.title||n.version||'Güncelleme')}</h2><p>${escV220(n.body||n.content||'')}</p><small class="muted">${escV220(typeof fmtDate==='function'?fmtDate(n.created_at):'')}</small></div></article>`).join('')||'<div class="card"><p>Güncelleme notu yok.</p></div>'}</section>`;
}
...
function adminV2Checklist(){
  ...
  return `<section class="card v2-checklist"><div class="section-title"><div><h2>V2.5.0 Hazırlık Kontrolü</h2><p class="muted">V1.9.0 final öncesi orta stabilizasyon paketidir. Admin, API, oyun, seri ve bakım sistemleri V2.5.0 açılışına hazırlanır.</p></div><span class="version-pill">Final hazırlığı</span></div><div class="grid compact"><div class="card"><h3>${missingCover}</h3><p>Kapaksız oyun</p></div><div class="card"><h3>${missingStory}</h3><p>Hikayesi eksik</p></div><div class="card"><h3>${noEpisodes}</h3><p>Bölümsüz oyun</p></div><div class="card"><h3>${upcoming}</h3><p>Yakında içerik</p></div></div><div class="roadmap-list"><span>Admin panel V3</span><span>YouTube senkronizasyon stabilizasyonu</span><span>RAWG kapak ve hikaye düzeltmeleri</span><span>Profil, sosyal ve bildirim düzeltmeleri</span><span>Tema, bakım ve loading düzeltmeleri</span><span>V2.5.0 profesyonel final açılış hazırlığı</span></div></section>`;
}
...
function cleanInstallCommands(){return `# Hayatımız Oyun V2.5.0 temiz kurulum
...
git add .
git commit -m "V2.5.0 stabil düzeltme"
...
function admin(){
  ...
  <section class="admin-shell-pro admin-shell-final">
    <div class="admin-tabs admin-toolbar-final" aria-label="Admin kategori menüsü">
      ...
      ${adminNavBtn('v250','V2.5.0 Mega','👑')}
      ${adminNavBtn('v220','V2.2.0 Merkez','🧩')}
      ${adminNavBtn('v250','V2.5.0 Mega','👑')}
      ${adminNavBtn('socialcheck','Sosyal Hata Kontrol','🧪')}
      ${adminNavBtn('v210','V2.5.0 Merkez','🚀')}
...
function aboutAdminPanelV203F1(){
  const st=state.settings||{};
  return `<section class="card about-admin-v203f1"><div class="section-title"><div><span class="version-pill">V2.5.0</span><h2>Hakkında Sayfası Yönetimi</h2><p class="muted">Hakkında sayfasındaki metinleri buradan düzenle. İstersen yapay zeka taslak metni oluşturabilirsin.</p></div><button onclick="generateAboutWithAIV203F1()">Yapay Zeka ile Oluştur</button></div>
...
function alpha3RepairPanel(){
  ...
  return `<section class="alpha3-panel card">
    <div class="section-title"><div><span class="version-pill">V2.5.0</span><h2>Kapak / Hikaye / Hatalı Oyun Onarım V2</h2><p class="muted">Bu aşamada sadece kapak, hikaye ve hatalı oyun kontrolü eklendi. Loading ve admin sistemi korunur.</p></div><button onclick="alpha3FixAllIssues()">Tüm Hataları Onar</button></div>
...
async function bootAppV300(){
  try{
    ...
    console.error('V2.5.0 açılış hatası:', e);
    state.settings=state.settings||{};
...
function enhanceAdminGamesV203F1(){
  ...
  toolbar.innerHTML=`<div><span class="version-pill">V2.5.0</span><h2>Toplu Oyun İşlemleri</h2><p class="muted">Tüm görünen oyunları seç, durum/seri/tip/etiket güncelle, onar veya sil. Tüm işlemlerde yüzde ilerleme görünür.</p></div>
...
function v220AdminCenter(){
  ...
  return `<section class="card v220-center"><div class="section-title"><div><span class="version-pill">V2.5.0</span><h2>Profesyonel Stabilite Merkezi</h2><p class="muted">Site ve admin paneli için geniş kontrol, kayıt ilerleme sistemi ve kalite özetleri.</p></div><button onclick="setPage('notes')">Notları Gör</button></div>
  <div class="grid compact">
...
function gameCardMega250(g){
  ...
  return `<article class="mega-game250 card"><div class="mega-cover250" style="background-image:url('${esc250(cover250(g))}')"><span>${esc250(g.status||'Durum')}</span><button onclick="favGame250('${esc250(id)}')" class="${fav?'active':''}">${fav?'★':'☆'}</button></div><div class="mega-body250"><h3>${esc250(g.title||'Başlıksız')}</h3><p class="muted">${esc250(seriesName250(g))} • ${episodes250(g).length} bölüm</p><div class="progress-mini250"><b style="width:${prog}%"></b></div><small>İlerleme: %${prog}</small><div class="row"><button onclick="showGameDetailV210('${esc250(id)}')">Detay</button><button class="ghost" onclick="favSeries250('${esc250(seriesName250(g))}')">Seri Favori</button></div></div></article>`;
}
function contributePage250(){
  $('#app').innerHTML=`<section class="hero"><span class="version-pill">V2.5.0</span><h1>Kullanıcı Katkı Merkezi</h1><p>Seri iste, hata bildir, öneri gönder ve arşivin gelişmesine katkı sağla.</p></section>
...
function profileDashboard250(){
  ...
  $('#app').innerHTML=`<section class="hero"><span class="version-pill">V2.5.0</span><h1>Profil Dashboard V2</h1><p>Favoriler, izleme geçmişi, rozetler ve kişisel istatistikler.</p></section>
...
function healthCenter250(){
  ...
  $('#app').innerHTML=`<section class="hero"><span class="version-pill">V2.5.0</span><h1>Site Sağlık Puanı: %${h.score}</h1><p>Kapaklar, hikayeler, video ID, sosyal linkler ve güncelleme notları kontrol edildi.</p></section>
...
function featuresPage250(){
  $('#app').innerHTML=`<section class="hero"><span class="version-pill">V2.5.0</span><h1>40 Mega Özellik</h1><p>Bu sürüm Hayatımız Oyun’u oyun arşivi platformuna dönüştüren mega güncellemedir.</p></section><section class="feature-grid250">${v250FeatureList().map((x,i)=>`<article class="card"><span>${i+1}</span><h2>${esc250(x)}</h2></article>`).join('')}</section>`;
}
...
function injectNav250(){
  try{
    ...
    const items=[['contribute250','Katkı'],['health250','Sağlık'],['features250','V2.5.0'],['profile250','Profil']];
...
function adminCenter250(){
  ...
  return `<section class="card admin250"><div class="section-title"><div><span class="version-pill">V2.5.0</span><h2>V2.5.0 Final Kontrol Paneli</h2><p class="muted">Seri istekleri, hata bildirimleri, sağlık puanı, admin işlem geçmişi, yedekleme ve mega sistem kontrolleri.</p></div><button onclick="exportMegaBackup250()">Tam Yedek Al</button></div>
  ...
  <section class="split"><div class="card"><h2>Hızlı İşlemler</h2><div class="row"><button onclick="setPage('health250')">Sağlık Paneli</button><button class="ghost" onclick="playlistImportMock250()">Playlist İçe Aktarma</button><button class="ghost" onclick="importMegaBackup250()">Yedek İçeri Al</button></div></div><div class="card"><h2>40 Özellik</h2><p class="muted">V2.5.0 mega özellik listesini kullanıcı tarafında göster.</p><button onclick="setPage('features250')">Özellikleri Gör</button></div></section>
