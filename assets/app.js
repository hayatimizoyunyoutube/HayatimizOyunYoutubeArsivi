...
/* Hayatımız Oyun - V2.5.1 Fix 2 2 Arayuz */
const VERSION='2.5.1 Fix 2';
...
function renderSocialSurfaces(){
  ...
  const foot=$('#siteFooter'); if(foot)foot.innerHTML=`<div class="footer-info"><b>${esc(siteTitle())}</b><small>V${VERSION} • V2.5.1 Fix 2</small><p>${esc(state.settings?.footer_text||state.settings?.share_description||'Oyun ve seri izleme arşivi.')}</p><div class="footer-links"><button class="ghost small" onclick="setPage('notes')">Güncelleme Notları</button><button class="ghost small" onclick="setPage('calendar')">Takvim</button><button class="ghost small" onclick="setPage('about')">Hakkında / Destek</button></div></div><div class="footer-social">${state.settings?.show_social_footer===false?'':(html||'<span class="muted">Sosyal medya linkleri ayarlardan eklenebilir.</span>')}</div>`;
...
function hideLoader(){
  const l=$('#siteLoader');
  if(l){
    loaderSet(100,'Hazır — V2.5.1 Fix 2 site açılıyor.');
    setTimeout(()=>{
...
/* V2.5.1 Fix 2 - açılış ayarı, not yönetimi, kart/video/canlı yayın stabilite */
function openingSettings251(){
  const st=state.settings||{};
  return {
    title: st.opening_title || 'Hayatımız Oyun',
    subtitle: st.opening_subtitle || 'V2.5.1 Fix 2 hazırlanıyor...',
...
function noteForm251(note={}){
  return `<section class="card note-editor251"><div class="section-title"><div><span class="version-pill">V2.5.1 Fix 2</span><h2>${note.id?'Güncelleme Notu Düzenle':'Güncelleme Notu Ekle'}</h2><p class="muted">Eski sürümleri düzenle, yeni not ekle veya local yedekten sil.</p></div></div>
    <input id="noteId251" type="hidden" value="${esc250(note.id||'')}">
    <label>Sürüm<input id="noteVersion251" value="${esc250(note.version||'V2.5.1 Fix 2')}" placeholder="V2.5.1 Fix 2"></label>
...
function renderNotesManager251(){
  ...
  return `<section class="card notes-manager251"><div class="section-title"><div><span class="version-pill">V2.5.1 Fix 2</span><h2>Güncelleme Notları Yönetimi</h2><p class="muted">Tüm güncellemeleri sürüm sırasına göre gör, düzenle, sil veya yeni not ekle.</p></div><button onclick="renderNoteEditor251()">Yeni Not Ekle</button></div>
...
async function saveNote251(){
  ...
  const note={
    id,
    version:document.getElementById('noteVersion251')?.value||'V2.5.1 Fix 2',
...
function openingAdmin251(){
  const st=openingSettings251();
  return `<section class="card opening-admin251"><div class="section-title"><div><span class="version-pill">V2.5.1 Fix 2</span><h2>Açılış / Loading Yönetimi</h2><p class="muted">Açılış metnini admin panelden düzenle. Yüzdeler adım sayısına göre otomatik hesaplanır.</p></div><button onclick="previewOpening251()">Önizle</button></div>
...
async function saveOpening251(){
  ...
  state.settings.opening_subtitle=document.getElementById('openingSubtitle251')?.value||'V2.5.1 Fix 2 hazırlanıyor...';
...
async function load(){
  ...
  loaderSet(100,'Hazır — V2.5.1 Fix 2 açılıyor.');
  await wait(350);
}
...
/* V2.5.1 Fix 2 - Büyük sistem güncellemesi */
...
/* V2.5.1 Fix 2 - YouTube video ID otomatik algılama */
...
function exportBackupV210(){
  ...
  a.download='hayatimiz-oyun-V2.5.1 Fix 2-yedek.json';
  a.click();
...
function v210AnnouncementBanner(){
  const list=storageGetV210('ho_v210_announcements',[{title:'V2.5.1 Fix 2 yayında',type:'Yeni Özellik',body:'Gelişmiş arşiv, arama, favoriler, izleme takip, sağlık kontrolü ve yedekleme sistemi eklendi.'}]);
...
function v210AdminPanel(){
  ...
  return `<section class="card v210-admin-panel"><div class="section-title"><div><span class="version-pill">V2.5.1 Fix 2</span><h2>Büyük Sistem Güncellemesi</h2><p class="muted">Arşiv, arama, favoriler, izleme takip, site sağlık kontrolü, duyuru ve yedekleme merkezi.</p></div><button onclick="exportBackupV210()">Yedek Al</button></div>
...
function archivePageV210(){
  ...
  return `<section class="hero"><span class="version-pill">V2.5.1 Fix 2</span><h1>Gelişmiş Arşiv</h1><p>Seriler, oyunlar, bölümler ve takip durumları tek merkezde. Alfabetik sıralama Türkçe karakter destekli düzeltildi.</p></section>
...
function searchPageV210(){
  ...
  return `<section class="hero"><span class="version-pill">V2.5.1 Fix 2</span><h1>Gelişmiş Arama</h1><p>Oyun, seri, durum, tip ve açıklama üzerinden arama yap. Oyun adları artık kesilmeden gösterilir.</p></section>
...
function favoritesPageV210(){
  ...
  return `<section class="hero"><span class="version-pill">V2.5.1 Fix 2</span><h1>Favoriler</h1><p>Favori oyunların ve serilerin hızlı erişim alanı. Oyun adları tam gösterilir.</p></section><section class="grid">${games.map(gameCardV210).join('')||'<div class="card"><p class="muted">Henüz favori eklenmedi.</p></div>'}</section>`;
}
function trackingPageV210(){
  const watch=userListsV210().watch||{};
  const games=sortGamesTitleOnly250F1((state.games||[]).filter(g=>watch[gameIdV210(g)]));
  return `<section class="hero"><span class="version-pill">V2.5.1 Fix 2</span><h1>İzleme Takibi</h1><p>İzledim, izliyorum, izleyeceğim ve yarım kaldı kayıtların. Oyun adları tam gösterilir.</p></section><section class="grid">${games.map(gameCardV210).join('')||'<div class="card"><p class="muted">Henüz takip kaydı yok.</p></div>'}</section>`;
}


/* V2.5.1 Fix 2 - Professional Stability Core */
...
function v220HomeBanner(){
  return `<section class="v220-banner card"><div><span class="version-pill">V2.5.1 Fix 2</span><h2>Site sürümü V2.5.1 Fix 2</h2><p class="muted">Site V2.5.1 Fix 2 sürümünde çalışıyor. Açılış/loading ekranı V2.5.1 Fix 2 açılış sistemiyle otomatik yüzde ilerlemesi kullanır.</p></div><button onclick="setPage('notes')">Güncelleme Notları</button></section>`;
}
...
function v250FeatureList(){
  return [
    ...
    'Oyun Öneri Motoru','Benzer Oyunlar Sistemi','Toplu Veri Temizleme','Tam Yedekleme Sistemi V3','V2.5.1 Fix 2 Final Kontrol Paneli'
  ];
}
...
function streamerCard(){
 ...
 return `<section class="card streamer-card streamer-v186"><div><span class="version-pill ${live?'live-pill':''}">${live?'● Canlı Yayında':'Yayıncı Alanı'}</span><h2>Hayatımız Oyun'u takip et ve destek ol</h2><p class="muted">${esc(desc)}</p><p class="muted"><b>V2.5.1 Fix 2:</b> ${esc(finalCountdown())}</p></div><div class="social-icons streamer-links">${buttons}</div></section>`;
}
...
/* V2.5.1 Fix 2 - A-Z/Seriler arama + sosyal medya kontrol */
...
function renderSocialCheckV300(){
  ...
  return `<section class="card social-check-v300"><div class="section-title"><div><span class="version-pill">V2.5.1 Fix 2</span><h2>Sosyal Medya Hata Kontrol</h2><p class="muted">${bad?bad+' sorun bulundu.':'Tüm sosyal medya bağlantıları iyi görünüyor.'}</p></div><button onclick="copySocialReportV300()">Raporu Kopyala</button></div>
...
function adminV2Checklist(){
  ...
  return `<section class="card v2-checklist"><div class="section-title"><div><h2>V2.5.1 Fix 2 Hazırlık Kontrolü</h2><p class="muted">V1.9.0 final öncesi orta stabilizasyon paketidir. Admin, API, oyun, seri ve bakım sistemleri V2.5.1 Fix 2 açılışına hazırlanır.</p></div><span class="version-pill">Final hazırlığı</span></div><div class="grid compact"><div class="card"><h3>${missingCover}</h3><p>Kapaksız oyun</p></div><div class="card"><h3>${missingStory}</h3><p>Hikayesi eksik</p></div><div class="card"><h3>${noEpisodes}</h3><p>Bölümsüz oyun</p></div><div class="card"><h3>${upcoming}</h3><p>Yakında içerik</p></div></div><div class="roadmap-list"><span>Admin panel V3</span><span>YouTube senkronizasyon stabilizasyonu</span><span>RAWG kapak ve hikaye düzeltmeleri</span><span>Profil, sosyal ve bildirim düzeltmeleri</span><span>Tema, bakım ve loading düzeltmeleri</span><span>V2.5.1 Fix 2 profesyonel final açılış hazırlığı</span></div></section>`;
}
...
function cleanInstallCommands(){return `# Hayatımız Oyun V2.5.1 Fix 2 Arayüz temiz kurulum
...
git add .
git commit -m "V2.5.1 Fix 2 stabil düzeltme"
...
function admin(){
  ...
  <section class="admin-shell-pro admin-shell-final">
    <div class="admin-tabs admin-toolbar-final" aria-label="Admin kategori menüsü">
      ...
      ${adminNavBtn('v250','V2.5.1 Fix 2','👑')}
      ${adminNavBtn('v220','V2.2.0 Merkez','🧩')}
      ${adminNavBtn('v250','V2.5.1 Fix 2','👑')}
      ${adminNavBtn('socialcheck','Sosyal Hata Kontrol','🧪')}
      ${adminNavBtn('v210','V2.5.1 Fix 2 Merkez','🚀')}
...
function aboutAdminPanelV203F1(){
  const st=state.settings||{};
  return `<section class="card about-admin-v203f1"><div class="section-title"><div><span class="version-pill">V2.5.1 Fix 2</span><h2>Hakkında Sayfası Yönetimi</h2><p class="muted">Hakkında sayfasındaki metinleri buradan düzenle. İstersen yapay zeka taslak metni oluşturabilirsin.</p></div><button onclick="generateAboutWithAIV203F1()">Yapay Zeka ile Oluştur</button></div>
...
function generateAboutText(){const el=document.getElementById('aboutTextField'); if(!el)return; el.value=`${siteTitle()}, oyun serilerini Türkçe içeriklerle düzenli şekilde takip edebileceğin bir oyun arşivi ve yayıncı platformudur. YouTube bölümleri, seri sıralamaları, izleme takibi, takvim, sosyal bağlantılar ve oyun istekleri tek yerde toplanır. V2.5.1 Fix 2 açılışına kadar profesyonel arşiv, topluluk ve yayıncı destek sistemi geliştirilmeye devam eder.`;}
...
function alpha3RepairPanel(){
  ...
  return `<section class="alpha3-panel card">
    <div class="section-title"><div><span class="version-pill">V2.5.1 Fix 2</span><h2>Kapak / Hikaye / Hatalı Oyun Onarım V2</h2><p class="muted">Bu aşamada sadece kapak, hikaye ve hatalı oyun kontrolü eklendi. Loading ve admin sistemi korunur.</p></div><button onclick="alpha3FixAllIssues()">Tüm Hataları Onar</button></div>
...
async function bootAppV300(){
  try{
    ...
    console.error('V2.5.1 Fix 2 açılış hatası:', e);
    state.settings=state.settings||{};
...
function enhanceAdminGamesV203F1(){
  ...
  toolbar.innerHTML=`<div><span class="version-pill">V2.5.1 Fix 2</span><h2>Toplu Oyun İşlemleri</h2><p class="muted">Tüm görünen oyunları seç, durum/seri/tip/etiket güncelle, onar veya sil. Tüm işlemlerde yüzde ilerleme görünür.</p></div>
...
function v220AdminCenter(){
  ...
  return `<section class="card v220-center"><div class="section-title"><div><span class="version-pill">V2.5.1 Fix 2</span><h2>Profesyonel Stabilite Merkezi</h2><p class="muted">Site ve admin paneli için geniş kontrol, kayıt ilerleme sistemi ve kalite özetleri.</p></div><button onclick="setPage('notes')">Notları Gör</button></div>
  <div class="grid compact">
...
/* V2.5.1 Fix 2 - Alfabetik sıralama ve tam oyun adı görünümü */
...
function contributePage250(){
  $('#app').innerHTML=`<section class="hero"><span class="version-pill">V2.5.1 Fix 2</span><h1>Kullanıcı Katkı Merkezi</h1><p>Seri iste, hata bildir, öneri gönder ve arşivin gelişmesine katkı sağla.</p></section>
...
function profileDashboard250(){
  ...
  $('#app').innerHTML=`<section class="hero"><span class="version-pill">V2.5.1 Fix 2</span><h1>Profil Dashboard V2</h1><p>Favoriler, izleme geçmişi, rozetler ve kişisel istatistikler.</p></section>
...
function healthCenter250(){
  ...
  $('#app').innerHTML=`<section class="hero"><span class="version-pill">V2.5.1 Fix 2</span><h1>Site Sağlık Puanı: %${h.score}</h1><p>Kapaklar, hikayeler, video ID, sosyal linkler ve güncelleme notları kontrol edildi.</p></section>
...
function featuresPage250(){
  $('#app').innerHTML=`<section class="hero"><span class="version-pill">V2.5.1 Fix 2</span><h1>40 Mega Özellik</h1><p>Bu sürüm Hayatımız Oyun’u oyun arşivi platformuna dönüştüren mega güncellemedir.</p></section><section class="feature-grid250">${v250FeatureList().map((x,i)=>`<article class="card"><span>${i+1}</span><h2>${esc250(x)}</h2></article>`).join('')}</section>`;
}
...
function injectNav250(){
  try{
    ...
    const items=[['contribute250','Katkı'],['health250','Sağlık'],['features250','V2.5.1 Fix 2'],['profile250','Profil']];
...
function adminCenter250(){
  ...
  return `<section class="card admin250"><div class="section-title"><div><span class="version-pill">V2.5.1 Fix 2</span><h2>V2.5.1 Fix 2 Final Kontrol Paneli</h2><p class="muted">Seri istekleri, hata bildirimleri, sağlık puanı, admin işlem geçmişi, yedekleme ve mega sistem kontrolleri.</p></div><button onclick="exportMegaBackup250()">Tam Yedek Al</button></div>
  ...
  <section class="split"><div class="card"><h2>Hızlı İşlemler</h2><div class="row"><button onclick="setPage('health250')">Sağlık Paneli</button><button class="ghost" onclick="playlistImportMock250()">Playlist İçe Aktarma</button><button class="ghost" onclick="importMegaBackup250()">Yedek İçeri Al</button></div></div><div class="card"><h2>40 Özellik</h2><p class="muted">V2.5.1 Fix 2 mega özellik listesini kullanıcı tarafında göster.</p><button onclick="setPage('features250')">Özellikleri Gör</button></div></section>
...
/* V2.5.1 Fix 2 - Sade Stabil Override */
function _esc251(x){
...
function contributePage251(){
  $('#app').innerHTML=`<section class="hero"><span class="version-pill">V2.5.1 Fix 2</span><h1>Seri İste / Hata Bildir</h1><p class="muted">Gereken katkı alanları sade şekilde bırakıldı.</p></section>
...
function adminCenter251(){
  const st=_stats251();
  return `<section class="card"><div class="section-title"><div><span class="version-pill">V2.5.1 Fix 2</span><h2>Sade Admin Merkezi</h2><p class="muted">Gereksiz mega bölümler temizlendi.</p></div></div><div class="grid compact"><div class="card"><h2>${st.games}</h2><p>Oyun</p></div><div class="card"><h2>${st.series}</h2><p>Seri</p></div><div class="card"><h2>${st.eps}</h2><p>Bölüm</p></div><div class="card"><h2>${st.issues}</h2><p>Uyarı</p></div></div><div class="row"><button onclick="adminTab('games')">Oyunları Yönet</button><button class="ghost" onclick="adminTab('opening251')">Açılış Yönetimi</button><button class="ghost" onclick="adminTab('notes251')">Not Yönetimi</button></div></section>`;
}
(function(){
...
/* V2.5.1 Fix 2 - Görseldeki profesyonel arayüz düzeni */
function uiEsc252(x){
...
(function(){
  ...
  if(oldAdminTab){
    window.adminTab=async function(t){
      if(t==='ui252'){
        const a=document.getElementById('adminArea');
        if(a)a.innerHTML=`<section class="card"><div class="section-title"><div><span class="version-pill">V2.5.1 Fix 2</span><h2>Arayüz Güncellemesi</h2><p class="muted">Görseldeki koyu profesyonel arayüz aktif. Ana sayfa, kartlar, hero ve istatistik alanları yenilendi.</p></div></div><div class="grid compact"><div class="card"><h2>Aktif</h2><p>Yeni UI</p></div><div class="card"><h2>Sade</h2><p>Menü</p></div><div class="card"><h2>Stabil</h2><p>Kartlar</p></div></div></section>`;
        return;
      }
      return await oldAdminTab.apply(this,arguments);
    };
