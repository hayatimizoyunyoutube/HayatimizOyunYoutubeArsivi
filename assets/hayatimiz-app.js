...
function userTopbar(){
  ...
  return `<header class="topBar proStickyTop topOnlyBar"><div class="topIdentity"><b>🎮 Hayatımız Oyun</b><small>${VERSION} • 👑 Tek Kurucu + Bakım/Ban Fix</small></div><nav class="topQuickNav fullTopNav">${publicLinks.map(([href,label])=>`<a class="${active(href)?'active':''}" href="${href}">${label}</a>`).join('')}</nav>${isYönetim()?'<a class="topAdminOpen" href="/yonetim">🛡️ Yönetim Paneli</a>':''}${auth}</header>`;
}
