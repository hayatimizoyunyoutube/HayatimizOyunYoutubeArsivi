...
const FIX_NAME = 'v2.2.5 - Veri Sağlığı ve Otomatik Yedekleme';
...
const DEFAULT_NOTES = [
  ...
  {id:'v222-supabase-kullanici-yetki-stabilite',version:'v2.2.2',title:'👥 Supabase Kullanıcı ve Yetki Stabilite Merkezi',summary:'Yeni açılan Supabase Auth hesapları güvenli şekilde çekilir; tek kurucu kilidi korunur, yanlış otomatik yetki verilmez, banlı kullanıcı erişimi kısıtlanır ve kullanıcı/yetki kayıtları Supabase ile senkron çalışır.',status:'Tamamlandı'},
  {id:'plan-v226',version:'v2.2.6',title:'📱 Mobil Menü ve Responsive Cila',summary:'Üst menü, yönetim menüsü, büyük kartlar ve public/admin sayfaları mobilde daha kompakt hale getirilecek.',status:'Planlandı'},
  {id:'v221-bakim-supabase-ban-guvenligi',version:'v2.2.1',title:'🛠️ Bakım Ekranı, Supabase Kalıcılık ve Ban Güvenliği',summary:'Bakım modu ziyaretçiler için sade ve oyun temalı güncelleme ekranına taşındı. Banlı kullanıcılar siteye ulaşamaz ve iletişim bağlantısı görür. Güncelleme notları ve sürüm etiketleri düzenlendi.',status:'Tamamlandı'},
  ...
  {id:'plan-v221',version:'v2.2.1',title:'🧩 Supabase Veri Sağlığı ve Yedek Planı',summary:'Sıradaki ana plan: Supabase tabloları, yedekleme, veri sağlık kontrolleri, admin logları ve güvenli aktarım sistemi güçlendirilecek.',status:'Planlandı'},
  ...
  {id:"plan-v221",version:"v2.2.1",title:"🗄️ Supabase Veri Sağlığı ve Yedek Planı",summary:"Canlı veri kaybı riskini azaltmak ve admin için veri kontrol araçları eklemek. Veri sağlık kontrol paneli eklenecek.",status:"Planlandı"},
  ...
  {id:"plan-v226-dashboard",version:"v2.2.6",title:"📊 Admin Dashboard İstatistikleri",summary:"Yönetim panelinde arşivin genel durumunu daha güçlü göstermek. Toplam oyun, seri, bölüm, kullanıcı ve güncelleme notu metrikleri güçlendirilecek.",status:"Planlandı"},
...
function maintenanceZiyaretçi(){
  ...
  const plannedFallback=[
    {version:'v2.2.6',title:'Mobil menü ve responsive cila',summary:'Üst menü, yönetim menüsü ve kartlar mobilde daha kompakt çalışacak.'},
    {version:'v2.2.7',title:'Playlist ve bölüm stabilitesi',summary:'Seri/bölüm listeleri daha güçlü senkron çalışacak.'},
...
function calendar(){ return adminOnly(()=>{ const rows=loadEvents(); const sync=syncState(); return layout(`<section class="adminRepairHero"><div><span class="badge green">🛡️ v2.2.4 • Yayın Takvimi ve Kapaklar</span><h1>📅 Yayın Takvimi Yönetimi</h1><p>Yönetim paneli kutuları toparlandı, takvim artık liste yerine gerçek takvim kutuları şeklinde görünür. Kayıt, silme ve Supabase yenileme akışı korunur.</p><small class="muted">Veri durumu: ${esc(sync.status||sync.mode)}</small></div><div class="actions"><button class="btn secondary" type="button" data-refresh-calendar>Supabase Takvimi Yenile</button><a class="btn secondary" href="/yayin-takvimi">Public Takvimi Aç</a></div></section><section class="adminCalendarLayout"><form class="panel formGrid adminCalendarForm" data-event-form><h2 class="full">➕ Yayın Planı Ekle</h2><label class="field">Tarih<input class="input" type="date" name="date" required></label><label class="field">Saat<input class="input" name="time" value="20:00"></label><label class="field">Tür<select name="type"><option>Ana Yayın</option><option>Video</option><option>Canlı Yayın</option><option>Plan</option></select></label><label class="field full">Oyun / Seri<input class="input" name="gameTitle" required list="calendarGameTitles" placeholder="Oyun adı veya seri adı"></label><datalist id="calendarGameTitles">${loadGames().map(g=>`<option value="${esc(g.title)}"></option>`).join("")}</datalist><input type="hidden" name="editId" value=""><label class="field">Bölüm<input class="input" name="episodeNumber" placeholder="2. Bölüm"></label><label class="field full">Video URL<input class="input" name="videoUrl" placeholder="https://youtube.com/..."></label><div class="full calendarFormActions"><button class="btn primary" type="submit" data-event-submit>Yayını Kalıcı Kaydet</button><button class="btn secondary" type="button" data-event-cancel-edit style="display:none">Düzenlemeyi İptal Et</button></div></form>${adminCalendarBoard(rows)}</section><section class="panel calendarListPanel"><div class="sectionHead compact"><div><h2>📌 Kayıtlı Yayınlar</h2><p>Takvimdeki tüm yayınlar burada hızlı silme için listelenir.</p></div></div><div class="miniList calendarEventList">${rows.length?rows.map((e,i)=>`<article class="event"><span class="pill">${esc(e.date||'Tarih yok')} • ${esc(e.time||'20:00')} • ${esc(e.type||'Yayın')}</span><h3>${esc(eventDisplayTitle(e))}</h3><p>${esc(e.type||'Yayın')} ${e.videoUrl?'• Video bağlantısı hazır':''}</p><div class="eventActions"><button class="btn secondary" data-edit-event="${i}">Düzenle</button><button class="btn danger" data-delete-event="${i}">Sil</button></div></article>`).join(''):'<div class="empty">Henüz yayın kaydı yok. Sol taraftan ilk yayını ekle.</div>'}</div></section>`); }); }
function updateNotes(){ return adminOnly(()=>{ const rows=loadNotes(); const sync=syncState(); return layout(`<div class="sectionHead"><div><h2>Güncelleme Notları</h2><p>Notlar Supabase <b>site_update_notes</b> tablosundan okunur/yazılır. Local güvenli mod korunur.</p><small class="muted">Veri durumu: ${esc(sync.status||sync.mode)}</small></div><div class="actions"><button class="btn secondary" type="button" data-refresh-notes>Supabase Notlarını Yenile</button></div></div><section class="panels"><form class="panel formGrid" data-note-form><label class="field">Sürüm<input class="input" name="version" value="v2.2.4"></label><label class="field">Durum<select name="status"><option>Tamamlandı</option><option>Planlandı</option></select></label><label class="field full">Başlık<input class="input" name="title" required placeholder="Güncelleme başlığı"></label><label class="field full">Özet<textarea name="summary" required placeholder="Bu sürümde yapılanları yaz..."></textarea></label><div class="full"><button class="btn primary" type="submit">Notu Kalıcı Kaydet</button></div></form><div class="panel"><h2>Kayıtlı Notlar</h2><div class="miniList">${rows.map((n,i)=>`<article class="note"><span class="pill ${String(n.status).includes('Plan')?'amber':'green'}">${esc(n.version||VERSION)} • ${esc(n.status||'Tamamlandı')}</span><h3>${esc(n.title)}</h3><p>${esc(n.summary||n.description||'')}</p><button class="btn danger" data-delete-note="${i}">Sil</button></article>`).join('')}</div></div></section>`); }); }
...
function bind(){
  document.body.addEventListener('click', e=>{
    ...
    const editEvent=e.target.closest('[data-edit-event]'); if(editEvent){ const rows=loadEvents(); const idx=Number(editEvent.dataset.editEvent); const event=rows[idx]; const form=document.querySelector('[data-event-form]'); if(!event || !form){ toast('Düzenlenecek yayın bulunamadı.'); return; } form.elements.editId.value=String(event.id||idx); form.elements.date.value=String(event.date||''); form.elements.time.value=String(event.time||'20:00'); form.elements.type.value=String(event.type||'Ana Yayın'); form.elements.gameTitle.value=String(event.gameTitle||event.title||''); form.elements.episodeNumber.value=String(event.episodeNumber||''); form.elements.videoUrl.value=String(event.videoUrl||''); const title=form.querySelector('h2'); if(title) title.textContent='✏️ Yayın Planı Düzenle'; const submit=form.querySelector('[data-event-submit]'); if(submit) submit.textContent='Değişiklikleri Kalıcı Kaydet'; const cancel=form.querySelector('[data-event-cancel-edit]'); if(cancel) cancel.style.display='inline-flex'; form.scrollIntoView({behavior:'smooth',block:'center'}); toast('Yayın düzenleme formuna alındı.'); return; }
    const cancelEventEdit=e.target.closest('[data-event-cancel-edit]'); if(cancelEventEdit){ const form=cancelEventEdit.closest('[data-event-form]'); if(form){ form.reset(); form.elements.editId.value=''; const title=form.querySelector('h2'); if(title) title.textContent='➕ Yayın Planı Ekle'; const submit=form.querySelector('[data-event-submit]'); if(submit) submit.textContent='Yayını Kalıcı Kaydet'; cancelEventEdit.style.display='none'; toast('Düzenleme iptal edildi.'); } return; }
    const delEvent=e.target.closest('[data-delete-event]'); if(delEvent){ const rows=loadEvents(); const idx=Number(delEvent.dataset.deleteEvent); const event=rows[idx]; if(!event){ toast('Silinecek yayın bulunamadı.'); return; } if(!confirm('Bu yayın takvimi kaydını silmek istiyor musun?')) return; rows.splice(idx,1); saveEvents(rows); deleteEventRemote(event).catch(err=>{ console.warn('Supabase takvim silme yerel modda kaldı:', err.message); }); toast('Yayın silindi.'); render(); return; }
  ...
  document.body.addEventListener('submit', e=>{
    ...
    const ef=e.target.closest('[data-event-form]'); if(ef){ e.preventDefault(); const fd=new FormData(ef); const rows=loadEvents(); const editId=String(fd.get('editId')||'').trim(); const gameTitle=String(fd.get('gameTitle')||'').trim(); const episodeNumber=String(fd.get('episodeNumber')||'').trim(); const date=String(fd.get('date')||'').trim(); if(!date){ toast('Yayın tarihi seçmelisin.'); return; } if(!gameTitle){ toast('Oyun / seri adı yazmalısın.'); return; } const foundGame=findGameForCalendarEvent({gameTitle,title:gameTitle}); const cover=(foundGame?.cover || foundGame?.banner || eventGameCover({gameTitle,title:gameTitle})); const existingIndex=editId ? rows.findIndex(r=>String(r.id)===editId) : -1; const oldEvent=existingIndex>=0 ? rows[existingIndex] : null; const event={...(oldEvent||{}),id:oldEvent?.id || editId || 'event-'+Date.now(),title:gameTitle+(episodeNumber?' • '+episodeNumber:''),date,time:String(fd.get('time')||'20:00'),type:String(fd.get('type')||'Ana Yayın'),gameTitle,gameId:foundGame?.id||oldEvent?.gameId||'',seriesName:foundGame?.seriesName||oldEvent?.seriesName||'',episodeNumber,videoUrl:String(fd.get('videoUrl')||''),note:'',cover,source:foundGame?'game-match':'local'}; if(existingIndex>=0){ rows[existingIndex]=event; }else{ rows.unshift(event); } saveEvents(rows); persistEventToSupabase(event).then(()=>render()).catch(err=>{ console.warn('Supabase takvim kayıt yerel modda kaldı:', err.message); saveSyncState({mode:'local', status:'Takvim yerel kayıt aktif', message:err.message}); }); toast(existingIndex>=0 ? 'Yayın düzenlendi ve kaydedildi.' : (foundGame?'Yayın kaydedildi, oyun kapağı otomatik eşleşti.':'Yayın kaydedildi. Oyun kapağı bulunamazsa varsayılan kapak kullanılır.')); render(); return; }
