...
function publicCalendarPage(){
  const rows=(loadEvents()||[]).slice().sort((a,b)=>String(a.date||'9999').localeCompare(String(b.date||'9999')) || String(a.time||'99:99').localeCompare(String(b.time||'99:99')));
  const months=['Ocak','Şubat','Mart','Nisan','Mayıs','Haziran','Temmuz','Ağustos','Eylül','Ekim','Kasım','Aralık'];
  const grouped=new Map();
  for(const e of rows){
    const d=String(e.date||'');
    const key=d.length>=7 ? `${months[Math.max(0,Math.min(11,Number(d.slice(5,7))-1))]} ${d.slice(0,4)}` : 'Tarihsiz Planlar';
    if(!grouped.has(key)) grouped.set(key, []);
    grouped.get(key).push(e);
  }
  return layout(`<section class="archiveHero calendarPublicHero"><div><span class="badge green">📅 ${VERSION} FIX • Public Yayın Takvimi</span><h1>📅 Yayın Takvimi</h1><p>Bu takvim sadece yönetim panelinde kalmaz; normal kullanıcılar ve kayıt yapmayan ziyaretçiler de yaklaşan yayınları profesyonel takvim görünümünde takip eder.</p></div><div class="actions"><a class="btn primary" href="/ana-sayfa">🏠 Ana Sayfa</a><a class="btn secondary" href="/oyun-arsivi?sort=az">🎮 Arşiv</a>${isYönetim()?'<a class="btn secondary" href="/yonetim/yayin-takvimi">🛠️ Takvimi Yönet</a>':''}</div></section><section class="archiveStats"><article><b>${rows.length}</b><span>📅 Toplam Plan</span></article><article><b>${rows.filter(e=>String(e.type||'').includes('Canlı')).length}</b><span>🔴 Canlı Yayın</span></article><article><b>${rows.filter(e=>String(e.type||'').includes('Video')).length}</b><span>▶️ Video</span></article><article><b>${grouped.size}</b><span>🗓️ Ay Grubu</span></article></section>${rows.length?Array.from(grouped.entries()).map(([month,items])=>`<section class="publicCalendarMonth"><div class="sectionHead"><div><h2>🗓️ ${esc(month)}</h2><p>${items.length} yayın planı listeleniyor.</p></div></div><div class="publicCalendarGrid">${items.map(e=>{ const day=String(e.date||'--').slice(-2)||'--'; const mon=String(e.date||'').slice(5,7)||'--'; return `<article class="publicCalendarEvent"><div class="calendarDateBadge"><b>${esc(day)}</b><span>${esc(mon)}</span></div><div><span class="pill green">${esc(e.type||'Yayın')}</span><h3>${esc(e.title||'Yayın Planı')}</h3><p>${esc(e.gameTitle||'Genel yayın')} ${e.episodeNumber?`• ${esc(e.episodeNumber)}`:''}</p><small>🕘 ${esc(e.time||'20:00')} ${e.note?`• ${esc(e.note)}`:''}</small>${e.videoUrl?`<div class="actions"><a class="miniBtn primary" href="${esc(e.videoUrl)}" target="_blank" rel="noreferrer">▶️ Video / Yayını Aç</a></div>`:''}</div></article>`; }).join('')}</div></section>`).join(''):`<section class="panel emptyCalendarPublic"><h2>📅 Henüz yayın planı yok</h2><p class="muted">Takvim kaydı eklenince burada tüm kullanıcılara açık profesyonel takvim olarak görünecek.</p>${isYönetim()?'<a class="btn primary" href="/yonetim/yayin-takvimi">📅 İlk Yayını Ekle</a>':''}</section>`}`);
}
