...
const DEFAULT_NOTES = [
  ...
  {id:'v401-veri-guvenligi-final',version:'v4.0.1 FIX',title:'🛡️ Veri Güvenliği ve Yönetim',summary:'Oyunlar, bölümler, kullanıcılar, favoriler, takip, başarımlar ve takvim kayıtları için mevcut veri koruma mantığı korunur. Bakım modu açılışta kapalıdır.',status:'Tamamlandı'},
  {id:'plan-v401-acilis-sonrasi-stabilite',version:'v4.0.1',title:'🛠️ Açılış Sonrası Stabilite',summary:'Canlı kullanımdan sonra küçük görünüm, veri ve mobil düzeltmeleri yapılacak.',status:'Planlandı'},
  {id:'plan-v410-topluluk-bildirim',version:'v4.1.0',title:'🔔 Topluluk ve Bildirim Merkezi',summary:'Favori serilere yeni bölüm bildirimi, duyuru alanları ve kullanıcı etkileşimi geliştirilecek.',status:'Planlandı'}
];
...
function localGameMetaCandidate(title){
  ...
  return {title:q,seriesName:q.split(':')[0],genre:'Aksiyon, Macera, Hikaye',releaseDate:'',platforms:'PC, PlayStation 5, Xbox Series S/X',tags:'',score:0,rawgSlug:slug,cover:'',banner:'',source:'Yerel güvenli varsayılan'};
}
...
async function refreshGamesFromSupabase({force=false}={}){
  ...
  try{
    ...
    // v4.0.1: Supabase 0 döndü diye yerel veya ekrandaki oyunları asla silme.
...
function mapMeta(raw, title){
  ...
  return {
    ...
    tags: '', // v4.0.1: Etiketler otomatik çekilmez, sadece kullanıcı seçerse kaydedilir.
...
function fillMetaToForm(form, meta, mode='safe'){
  ...
  // v4.0.1: Etiket alanı otomatik doldurulmaz. Kullanıcı hazır etiketlerden manuel seçer.
  ...
  setField(form,'metaSource',meta.source || 'v4.0.1 final meta paneli', false);
  setField(form,'metaCheckedAt',new Date().toISOString(), false);
  const box=form.querySelector('[data-meta-status]');
  if(box) box.innerHTML=`<b>Bilgi hazır:</b> ${esc(meta.source || 'v4.0.1')} • ${esc(meta.title || '')}`;
...
async function resolveMetaForForm(form, source){
  ...
  let raw=null;
  try{
    raw = await apiJson(source==='steam'?'steam-meta-lite':'game-meta-lite', {title, steamAppId:form?.elements?.steamAppId?.value || '', releaseDate:form?.elements?.releaseDate?.value || '', cover:form?.elements?.cover?.value || ''});
  }catch(err){
    raw = {ok:true, meta:localGameMetaCandidate(title), source:'Servis yok / yerel güvenli bilgi'};
  }
  const meta=mapMeta(raw, title);
  // İlk kutu oyun hikayesi, ikinci kutu karakter/mekan/atmosfer için ayrılır.
  meta.description = professionalStoryText({title:meta.title||title, genre:meta.genre, seriesName:meta.seriesName});
  meta.storyText = characterAtmosphereText({title:meta.title||title, genre:meta.genre, seriesName:meta.seriesName});
...
function loadMaintenance(){
  ...
  return sanitizeMaintenance(found || {enabled:false,message:'Hayatımız Oyun kısa süreli bakımda.',eta:'',percent:0,adminBypass:true,managedBy:'v4.0.1-runtime'});
}
function saveMaintenance(v){
  const cfg = sanitizeMaintenance({...v, enabled: parseMaintenanceEnabled(v?.enabled), percent: Math.max(0, Math.min(100, Number(v?.percent||0))), managedBy:'v4.0.1-runtime', updatedAt:new Date().toISOString()});
...
function characterAtmosphereText(gameOrTitle, genre='', seriesName=''){
  const g=typeof gameOrTitle==='object' ? gameOrTitle : {title:gameOrTitle, genre, seriesName};
  const title=String(g.title||'Bu oyun').trim();
  const series=cleanSeriesName(g.seriesName, title) || title;
  const lower=`${title} ${g.genre||''} ${series}`.toLocaleLowerCase('tr');
  if(/007|first\s*light|james\s*bond/.test(lower)) return 'Ana karakter: genç James Bond. Merkez: MI6 operasyon ekibi, gizli düşman yapılanması ve uluslararası görev ağı. Mekanlar: Londra istihbarat merkezi, Avrupa şehirleri, kıyı bölgeleri ve yüksek güvenlikli tesisler. Atmosfer: sinematik ajanlık, gizlilik, takip, gerilim ve aksiyon. Bölüm yapısı: her bölüm yeni bir operasyon adımı, yeni bir ipucu ve Bond’un ajan kimliğini güçlendiren bir sınav gibi ilerler.';
  if(/plague.*innocence|innocence/.test(lower)) return 'Ana karakterler: Amicia ve Hugo. Mekanlar: Orta Çağ Fransa’sının köyleri, kaleleri, salgınla çökmüş yolları ve karanlık geçitleri. Atmosfer: çaresizlik, kardeşlik, gizlilik, korku ve hayatta kalma. Bölüm yapısı: kaçış, takip, bulmaca ve hikaye ağırlıklı sahnelerle ilerler.';
  if(/alan\s*wake/.test(lower)) return 'Ana karakter: Alan Wake. Mekanlar: sisli kasaba yolları, ormanlık alanlar, karanlık yapılar ve psikolojik gerilimi yükselten kapalı mekanlar. Atmosfer: psikolojik korku, gizem, karanlık ve anlatı odaklı gerilim. Bölüm yapısı: her bölüm yeni bir ipucu, yeni bir tehdit ve gerçeklikle kurgu arasındaki sınırı zorlayan olaylarla ilerler.';
  return `Ana karakterler ve yan karakterler ${series} evreninin çatışmasına göre şekillenir. Mekanlar; oyunun ana görev alanları, keşif bölgeleri ve atmosferi güçlendiren ara sahnelerden oluşur. Atmosfer; ${String(g.genre||'hikaye ve aksiyon').toLocaleLowerCase('tr')} temasını öne çıkarır. Bölüm yapısı; başlangıç, gelişme, önemli kırılma noktaları ve final hazırlığı şeklinde takip edilebilir.`;
}
...
function loadEpisodes(gameId){
  ...
  // v4.0.1: Eski kayıt playlist çekmiş ama bölüm JSON'u oyuna yazılmamışsa
...
async function fetchPlaylistEpisodes(playlistUrl, title){
  ...
  const data=await apiJson('playlist-items', {playlistUrl, playlistId, title});
  const episodes=Array.isArray(data.episodes) ? data.episodes.map(normalizeEpisode).filter(ep=>ep.videoId || ep.videoUrl) : [];
  if(episodes.length) return {episodes, count:data.count || episodes.length, source:data.source || 'YouTube Data API v3'};
  throw new Error(data.error || 'YouTube API gerçek bölüm döndürmedi. Playlist URL, gizlilik ve YOUTUBE_API_KEY kontrol edilmeli.');
}
function renderEpisodeList(episodes, watched=0, fallbackCover='/assets/hayatimiz-kapak.png'){
  ...
  return `<div class="episodeList">${list.map(ep=>{ const img=ep.thumbnail && !isSiteFallbackThumb(ep.thumbnail) ? ep.thumbnail : (fallbackCover && !isSiteFallbackThumb(fallbackCover) ? fallbackCover : '/assets/episode-placeholder.svg'); return `<article class="episodeItem ${Number(ep.number)<=Number(watched)?'watched':''}"><img src="${esc(img||'/assets/hayatimiz-kapak.png')}" onerror="this.src='${esc(fallbackCover||'/assets/hayatimiz-kapak.png')}'" alt="${esc(ep.title)}"><div><span class="pill">${esc(ep.number)}. Bölüm</span><h4>${esc(ep.title)}</h4>${ep.videoUrl?`<a class="miniBtn" href="${esc(ep.videoUrl)}" target="_blank" rel="noreferrer">YouTube Aç</a>`:''}</div></article>`; }).join('')}</div>`;
}
...
function maintenanceZiyaretçi(){
  ...
  const plannedFallback=[
    {version:'v4.0.1',title:'Açılış Sonrası Stabilite',summary:'Canlı yayından sonra mobil, veri ve görünüm kontrolleri yapılacak.'},
...
function admin(){ return adminOnly(()=>{
  try{
    ...
    const episodeTotal=games.reduce((sum,g)=>sum+Number(g.episodeCount||0),0);
    const cards=[
      ['/yonetim/oyun-ekle','➕','Oyun Ekle','Profesyonel kartlı form, RAWG/Steam ve YouTube oynatma listesi alanları.','v4.0.1 aktif'],
      ...
      ['/koleksiyonlar','🗂️','Koleksiyonlar','Seri, tür, durum ve koleksiyon sayaçlarını ziyaretçi tarafta kontrol et.','v4.0.1'],
      ['/yonetim/yayin-takvimi','📅','Yayın Takvimi','Manuel yayın tarihleri ve video linkleri.','Korundu'],
      ['/yonetim/guncelleme-notlari','📝','Güncelleme Notları','Tamamlanan ve planlanan notları düzenle.','v4.0.1 işlendi'],
...
function gameForm(){ return adminOnly(()=>{
  ...
  const game=existing || {title:'',status:'Devam Eden',genre:'',tags:'',seriesName:'',cover:'',banner:'',releaseDate:'',platforms:'',description:'',storyText:'',youtubePlaylistUrl:'',youtubePlaylistId:'',rawgId:'',steamAppId:'',episodeCount:0,watchedEpisodeCount:0};
...
function bind(){
  document.body.addEventListener('click', e=>{
    ...
    const storyBtn=e.target.closest('[data-story-action]'); if(storyBtn){ const form=storyBtn.closest('form'); if(form){ const game={title:form.elements?.title?.value, genre:form.elements?.genre?.value, seriesName:form.elements?.seriesName?.value}; setField(form,'description',professionalStoryText(game),false); setField(form,'storyText',characterAtmosphereText(game),false); toast('Oyun hikayesi ve karakter/mekan/atmosfer alanı oluşturuldu.'); } return; }
  });
  ...
  document.body.addEventListener('submit', e=>{
    ...
    const gf=e.target.closest('[data-game-form]'); if(gf){
      ...
      const payload=normalizeGame({
        ...
        seriesName:fd.get('seriesName'), cover:isSiteFallbackThumb(fd.get('cover'))?'':fd.get('cover'), banner:isSiteFallbackThumb(fd.get('banner'))?'':fd.get('banner'),
...
window.addEventListener('error', ev=>{ console.error('v4.0.1 hata:', ev.message); const root=document.getElementById('root'); if(root && (root.textContent||'').trim().length<40) root.innerHTML=emergencySafeHtml(ev.message || 'Beklenmeyen site hatası yakalandı.'); });
...
window.addEventListener('unhandledrejection', ev=>{ console.error('v4.0.1 promise hata:', ev.reason); const root=document.getElementById('root'); if(root && (root.textContent||'').trim().length<40) root.innerHTML=emergencySafeHtml('Beklenmeyen işlem hatası yakalandı.'); });
window.HAYATIMIZ_OYUN_VERSION = VERSION;
window.HAYATIMIZ_OYUN_FIX = FIX_NAME;


// v4.0.1 FIX: 007 First Light form meta kesin Steam App ID/kapak
try{
  const __v401PrevLocalMetaCandidate = localGameMetaCandidate;
  localGameMetaCandidate = function(title){
    const q=String(title||'').toLowerCase();
    if(/007|first\s*light|james\s*bond/.test(q)){
      return {title:'007 First Light', seriesName:'James Bond', genre:'Aksiyon, Macera, Gizlilik', releaseDate:'26.05.2026', released:'26.05.2026', score:8.6, steamAppId:'3768760', rawgSlug:'007-first-light', rawgId:'', cover:'https://cdn.akamai.steamstatic.com/steam/apps/3768760/header.jpg', banner:'https://cdn.akamai.steamstatic.com/steam/apps/3768760/capsule_616x353.jpg', platforms:'PC', tags:'', source:'SteamDB/Steam App ID 3768760'};
    }
    return __v401PrevLocalMetaCandidate(title);
  };
}catch(error){ console.warn('v4.0.1 frontend 007 meta override kurulamadı:', error); }
