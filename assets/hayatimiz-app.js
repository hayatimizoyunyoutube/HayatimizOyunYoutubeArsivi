...
const STORAGE = {
  ...
  favorites: 'hayatimiz_favorites_v232'
};
...
const DEFAULT_NOTES = [
  ...
  {id:'plan-v400-ana-acilis',version:'v4.0.0',title:'🎉 Ana Açılış Sürümü',summary:'Public arşiv, yayın takvimi, seri merkezi, bakım/ban güvenliği ve yönetim paneli yayın için son hale getirilecek.',status:'Planlandı'},
  {id:'v225-public-acilis-cilasi',version:'v2.2.9',title:'💾 Public Açılış Cilası',summary:'Supabase veri sağlığı merkezi eklendi. Oyun, kullanıcı, takvim, not ve bakım kayıtları kontrol edilir; eksik kapak/tarih uyarıları gösterilir, manuel JSON yedek alma ve veri onarma araçları hazırlandı.',status:'Tamamlandı'},
  {id:'v224-yayin-takvimi-oyun-kapaklari',version:'v2.2.9',title:'📅 Yayın Takvimi ve Oyun Kapakları',summary:'Public takvim gerçek ay görünümüyle güçlendirildi. Yayın planlarında oyun kapakları daha net gösterilir, bugün/saat göstergesi geliştirildi, yayın ekleme formu sade tutuldu ve takvim kayıtları Supabase üzerinde kalıcı korunur.',status:'Tamamlandı'},
  ...
  {id:'plan-v226',version:'v2.2.9',title:'📱 Mobil Menü ve Responsive Cila',summary:'Üst menü, yönetim menüsü, büyük kartlar ve public/admin sayfaları mobilde daha kompakt hale getirilecek.',status:'Planlandı'},
  ...
  {id:"plan-v226-dashboard",version:"v2.2.9",title:"📊 Admin Dashboard İstatistikleri",summary:"Yönetim panelinde arşivin genel durumunu daha güçlü göstermek. Toplam oyun, seri, bölüm, kullanıcı ve güncelleme notu metrikleri güçlendirilecek.",status:"Planlandı"},
...
/* v2.2.9 FIX - Bilgi çekme kesin onarım
   Önceki paketlerde eksik kalan normalizeText yardımcı fonksiyonu eklendi.
   Bu eksik fonksiyon yüzünden RAWG/Steam/yerel meta eşleme aşamasında form hata veriyordu. */
...
function userTopbar(){
  ...
  const publicLinks = [
    ['/ana-sayfa','🏠 Ana Sayfa'],['/oyun-arsivi','🎮 Arşiv'],['/koleksiyonlar','🗂️ Koleksiyonlar'],['/favoriler','⭐ Favoriler'],['/seriler','🎬 Seriler'],['/yayin-takvimi','📅 Yayın Takvimi'],['/status','📡 Site Durumu'],['/site-rehberi','📘 Site Rehberi']
  ];
  const auth = user
...
function isGenericEpisodeTitle(title=''){
  const t=String(title||'').trim();
  return !t || /^\d+\.\s*Bölüm$/i.test(t) || /^[^|]{1,80}\s+\d+\.\s*Bölüm$/i.test(t) || /^Bölüm\s*\d+$/i.test(t);
}
function isBadEpisodeThumbnail(url=''){
  const u=String(url||'');
  return !u || /hayatimiz-kapak|hayatimiz-logo|site-logo|logo\.png/i.test(u);
}
function hasRealYoutubeEpisodeData(ep={}){
...
function oldHomeTile(g){
  ...
  return `<article class="oldHomeTile"><div class="oldHomePoster"><img src="${esc(g.cover)}" alt="${esc(g.title)}" onerror="this.src='/assets/hayatimiz-kapak.png'"><span class="pill ${cls}">${esc(g.status || 'Arşiv')}</span></div><div class="oldHomeTileBody"><h3>${esc(g.title)}</h3><p>${esc(g.description || 'Açıklama eklenmedi.')}</p><div class="oldHomeMeta"><span>🎮 ${esc(g.genre || 'Kategori yok')}</span><span>🎬 ${esc(g.seriesName || 'Serisiz')}</span><span>📅 ${esc(g.releaseDate || 'Tarih yok')}</span></div><div class="progressMini"><i><span style="width:${pct}%"></span></i><small>%${pct} takip / ${Number(g.episodeCount||0)} bölüm</small></div><div class="cardActions"><a class="miniBtn primary" href="/izle?id=${encodeURIComponent(g.id)}">Siteden İzle</a><a class="miniBtn" href="/oyun-detay?id=${encodeURIComponent(g.id)}">Detay</a><button class="miniBtn" data-favorite-game="${esc(g.id)}">${isFavoriteGame(g.id)?'⭐ Favoriden Çıkar':'☆ Favori'}</button></div></div></article>`;
}
function oldContinueCard(g){
  const episodes = loadEpisodes(g.id);
  const total = episodes.length || Number(g.episodeCount || 0);
  const watched = Number(g.watchedEpisodeCount || 0);
  const pct = total ? Math.max(0, Math.min(100, Math.round((watched/total)*100))) : progressPercent(g);
  const nextNo = Math.max(1, watched + 1);
  const nextEp = episodes.find(ep=>Number(ep.number)===nextNo) || episodes.find(ep=>Number(ep.number)>watched);
  const nextTitle = nextEp ? (nextEp.title || `${nextNo}. Bölüm`) : (total && watched < total ? `${nextNo}. Bölüm` : 'Yeni bölüm bekleniyor');
  return `<article class="oldContinueCard continueSeriesCard"><img src="${esc(g.cover)}" alt="${esc(g.title)}" onerror="this.src='/assets/hayatimiz-kapak.png'"><div><span class="pill ${statusClass(g.status)}">${esc(g.status || 'Devam Eden')}</span><h3>${esc(g.seriesName || g.title)}</h3><p>${esc(g.title)} • ${watched}/${total || 0} bölüm</p><div class="nextEpisodeBadge"><b>⏭️ Sıradaki Bölüm:</b><span>${esc(nextTitle)} yakında</span></div><div class="progressMini"><i><span style="width:${pct}%"></span></i></div><a class="miniBtn primary" href="/oyun-arsivi?q=${encodeURIComponent(g.title)}">Seriyi Aç</a></div></article>`;
}
...
function bind(){
  document.body.addEventListener('click', e=>{
    ...
    const favGame=e.target.closest('[data-favorite-game]'); if(favGame){ e.preventDefault(); const added=toggleFavoriteGame(favGame.dataset.favoriteGame); toast(added?'Oyun favorilere eklendi.':'Oyun favorilerden çıkarıldı.'); render(); return; }
    const favSeries=e.target.closest('[data-favorite-series]'); if(favSeries){ e.preventDefault(); const added=toggleFavoriteSeries(favSeries.dataset.favoriteSeries); toast(added?'Seri takip listesine eklendi.':'Seri takipten çıkarıldı.'); render(); return; }
  ...
  document.body.addEventListener('submit', e=>{
    ...
    persistGameToSupabase(payload, editId).then(()=>{ render(); }).catch(err=>{ console.warn('Supabase kayıt yerel modda kaldı:', err.message); saveSyncState({mode:'local', status:'Yerel kayıt aktif', message:err.message}); }); toast(editId?'Oyun güncellendi; bölüm thumbnailleri korundu.':'Oyun kaydedildi.'); setRoute('/yonetim/mevcut-oyunlar'); return; }
