...
function localUserRows(){ const local=loadUsers().map((u,i)=>({id:u.id||`local-${i}`,email:u.email,displayName:u.displayName||u.full_name||u.email,role:u.role||'user',is_active:u.is_active!==false,source:'Yerel Yedek'})); const current=currentUser(); if(current && !local.some(u=>String(u.email).toLowerCase()===String(current.email).toLowerCase())) local.unshift({id:'current-session',email:current.email,displayName:current.displayName||current.email,role:current.role||'user',is_active:true,source:'Oturum / Supabase Yenile'}); return local; }
...
function renderUserRows(rows){
  ...
  return list.map(u=>{ const email=String(u.email||''); const role=roleValue(u.role,email); const active=u.is_active!==false && role!=='banned'; const created=u.createdAt || u.created_at || u.updated_at || ''; return `<article class="userRow proUserRow v218UserRow" data-user-id="${esc(u.id||'')}" data-user-email="${esc(email)}" data-user-source="${esc(u.source||'Yerel')}"><div class="userAvatar">${esc((u.displayName||u.full_name||email||'?').slice(0,1).toLocaleUpperCase('tr'))}</div><div class="userCell"><b>${esc(u.displayName||u.full_name||email)}</b><small>${esc(email)} • ${esc(u.source||'Supabase')} ${created?'• '+esc(String(created).slice(0,10)):''}</small></div><span class="roleChip roleChipTable role-${esc(role)}">${esc(roleLabel(role,email))}</span><select class="input roleSelect" data-role-select aria-label="Yetki seç">${roleOptionsHtml(role,email)}</select><span class="userState ${active?'ok':'blocked'}">${active?'✅ Aktif':'🚫 Kısıtlı'}</span><div class="userRowActions"><button class="miniBtn primary" data-user-role-save>💾 Yetkiyi Kaydet</button><button class="miniBtn danger" data-user-delete>🗑️ Sil</button></div></article>`; }).join('');
}
...
function bind(){
  document.body.addEventListener('click', e=>{
    ...
    const userAdd=e.target.closest('[data-user-add-form]'); if(userAdd){ e.preventDefault(); const fd=new FormData(userAdd); const email=String(fd.get('email')||'').trim().toLowerCase(); const displayName=String(fd.get('displayName')||'').trim() || email; const role=roleValue(fd.get('role')||'user', email); if(!email){ toast('E-posta yazmalısın.'); return; } userAdd.querySelector('button[type="submit"]')?.setAttribute('disabled','disabled'); apiJson('user-role-set',{adminToken:sessionToken(),email,displayName,role}).then(()=>{ userAdd.reset(); return refreshUsersPanel(); }).then(()=>toast('✅ Kullanıcı/yetki Supabase tablosuna kaydedildi.')).catch(err=>{ console.warn('Supabase yetki kaydı olmadı, yerel yedek:', err.message); let users=loadUsers().filter(u=>String(u.email||'').toLowerCase()!==email); users.unshift({id:'local-'+Date.now(),email,displayName,role,is_active:role!=='banned',source:'Yerel Yedek',createdAt:new Date().toISOString()}); saveUsers(users); toast('⚠️ Supabase kaydı yapılamadı; geçici yerel yedeğe yazıldı: '+err.message); render(); }).finally(()=>userAdd.querySelector('button[type="submit"]')?.removeAttribute('disabled')); return; }
    if(e.target.closest('[data-refresh-users]')){ e.preventDefault(); refreshUsersPanel(); return; }
    const roleSave=e.target.closest('[data-user-role-save]'); if(roleSave){ const row=roleSave.closest('[data-user-id]'); const id=row?.dataset?.userId||''; const email=String(row?.dataset?.userEmail||row?.querySelector('small')?.textContent?.split('•')?.[0]?.trim()||'').toLowerCase(); const role=roleValue(row?.querySelector('[data-role-select]')?.value||'user', email); const displayName=row?.querySelector('b')?.textContent||email; roleSave.setAttribute('disabled','disabled'); apiJson('user-role-set',{adminToken:sessionToken(),userId:id,email,displayName,role}).then(()=>{ const session=currentUser(); if(session && String(session.email).toLowerCase()===email){ localStorage.setItem(STORAGE.session, JSON.stringify({...session,role,adminToken:session.adminToken})); } return refreshUsersPanel(); }).then(()=>toast(`✅ Supabase yetkisi ${roleLabel(role,email)} olarak kaydedildi.`)).catch(err=>{ let users=loadUsers(); if(!users.some(u=>String(u.email).toLowerCase()===email)) users.push({id:id||('local-'+Date.now()),email,displayName,role,source:'Yerel Yedek'}); users=users.map(u=>String(u.id)===id||String(u.email).toLowerCase()===email?{...u,role,is_active:role!=='banned'}:u); saveUsers(users); toast('⚠️ Supabase yetkisi güncellenemedi; yerel yedek yazıldı: '+err.message); render(); }).finally(()=>roleSave.removeAttribute('disabled')); return; }
  ...
  document.body.addEventListener('submit', e=>{
    ...
    const mf=e.target.closest('[data-maint-form]'); if(mf){ e.preventDefault(); const fd=new FormData(mf); const enabled=String(fd.get('enabled')||'false')==='true'; const stamp=new Date().toISOString(); const maintenance={enabled,percent:Number(fd.get('percent')||0),message:fd.get('message'),eta:fd.get('eta'),adminBypass:true,source:'admin-form',updatedAt:stamp,updated_at:stamp}; saveMaintenance(maintenance); const btn=mf.querySelector('button[type="submit"]'); btn?.setAttribute('disabled','disabled'); persistMaintenanceToSupabase(maintenance).then(()=>{ saveMaintenance({...maintenance,source:'supabase'}); toast(enabled?'✅ Bakım modu Supabase üzerinde açık kaydedildi.':'✅ Bakım modu Supabase üzerinde kapalı kaydedildi.'); render(); }).catch(err=>{ console.warn('Supabase bakım kayıt yerel modda kaldı:', err.message); saveSyncState({mode:'local', status:'Bakım yerel kayıt aktif', message:err.message}); toast('⚠️ Bakım yerel yedekte kaldı: '+err.message); render(); }).finally(()=>btn?.removeAttribute('disabled')); return; }
  });
}
function render(){
