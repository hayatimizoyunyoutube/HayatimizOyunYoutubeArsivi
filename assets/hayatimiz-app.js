...
const FIX_NAME = 'v2.2.0 - Bakım Ekranı, Supabase Kalıcılık ve Ban Güvenliği';
...
const DEFAULT_NOTES = [
  {id:'v220-bakim-supabase-ban-guvenligi',version:'v2.2.0',title:'🛠️ Bakım Ekranı, Supabase Kalıcılık ve Ban Güvenliği',summary:'Bakım modu animasyonlu ziyaretçi ekranına taşındı. Oyun, takvim, not, kullanıcı ve bakım kayıtlarında Supabase kalıcılığı güçlendirildi. Yeni fix ve güncellemelerde veriler sıfırlanmasın diye schema mantığı korumalı hale getirildi. Banlı kullanıcılar siteye ulaşamaz ve Discord iletişim bağlantısı görür.',status:'Tamamlandı'},
...
async function apiJson(action, payload){
  ...
  const res = await fetch(`/api?action=${encodeURIComponent(action)}&t=${Date.now()}`, {method:'POST',headers:{'Content-Type':'application/json','Cache-Control':'no-store'},body:JSON.stringify(merged)});
  ...
  try{
    ...
    if(data && data.maintenance){
      ...
      if(publicRoute || force || remoteKapali){
        ...
        saveMaintenance({...remote, enabled:parseMaintenanceEnabled(remote.enabled), source:'supabase'});
      }
      if(remote.enabled === true && publicRoute && !isAuthRoute(route())) setTimeout(()=>{ try{ render(); }catch{} }, 0);
    }
...
async function refreshCurrentSessionFromSupabase(){
  ...
  try{
    const data=await apiJson('session-refresh',{email, adminToken:sessionToken()});
    if(data?.user){
      const next={...session,...data.user,displayName:data.user.displayName||data.user.full_name||session.displayName||email,role:roleValue(data.user.role,email),is_active:data.user.is_active!==false};
      if(data.adminToken) next.adminToken=data.adminToken;
      localStorage.setItem(STORAGE.session, JSON.stringify(next));
      if(next.is_active===false || roleValue(next.role,email)==='banned'){
        saveSyncState({mode:'supabase',status:'Hesap banlı',message:'Supabase kaydı banlı olduğu için site erişimi engellendi.'});
      }
      return next;
    }
  }catch(err){
    console.warn('Oturum Supabase üzerinden yenilenemedi:', err.message);
  }
  return session;
}
function mapMeta(raw, title){
  ...
  const finalTitle = m.title || local.title || title;
...
function localUserRows(){
  // v2.2.0 FIX: kullanıcı listesine tarayıcı oturumu artık sahte satır olarak eklenmez.
  // Liste Supabase Auth + site_users + yetki kayıtlarından gelir; böylece panel kafasına göre kullanıcı üretmez.
  return loadUsers()
    .filter(u=>String(u.email||'').trim())
    .map((u,i)=>({id:u.id||`local-${i}`,email:String(u.email||'').trim().toLowerCase(),displayName:u.displayName||u.full_name||u.email,role:roleValue(u.role||'user',u.email),is_active:u.is_active!==false && roleValue(u.role||'user',u.email)!=='banned',source:u.source||'Supabase Kayıt'}));
}
...
function maintenanceZiyaretçi(){
  ...
  return `<main class="maintenanceZiyaretçi proMaintenance v220Maintenance"><section class="maintenanceCard proMaintenanceCard v220MaintenanceCard"><div class="maintenanceOrbit"><span></span><span></span><span></span></div><div class="maintenanceGlow"></div><span class="badge amber">🛠️ Bakım Modu Aktif</span><h1>Hayatımız Oyun Bakımda</h1><p>${esc(m.message || 'Siteyi daha stabil, daha hızlı ve daha güvenli hale getirmek için kısa süreli bakım yapılıyor.')}</p><div class="maintenanceProgress"><i><span style="width:${pct}%"></span></i><b>%${pct}</b></div><div class="maintenanceInfoGrid"><article><b>🎮 Arşiv</b><span>Veriler korunuyor.</span></article><article><b>💾 Supabase</b><span>Kayıtlar sıfırlanmaz.</span></article><article><b>⏰ Tahmini Açılış</b><span>${esc(m.eta || 'Yakında güncellenecek.')}</span></article></div><div class="maintenancePublicActions"><button class="btn ghost" type="button" data-maintenance-retry>🔄 Tekrar Kontrol Et</button></div><small class="maintenanceNote">Bakım kapanınca site otomatik normal ekrana döner. Yetkili hesaplar yönetim için giriş yapabilir.</small></section></main>`;
}
function bannedZiyaretçi(){
  const discord='https://discord.gg/QXc74Q6UUE';
  const u=currentUser() || {};
  return `<main class="maintenanceZiyaretçi bannedPage v220Banned"><section class="maintenanceCard proMaintenanceCard bannedCard"><div class="maintenanceGlow"></div><span class="badge red">🚫 Hesap Banlandı</span><h1>Siteye erişimin kısıtlandı</h1><p>Bu hesap Hayatımız Oyun sitesine erişemez. Hata olduğunu düşünüyorsan kurucu veya yetkililerle iletişime geç.</p><div class="banInfo"><b>👤 Hesap:</b> <span>${esc(u.email || 'Bilinmiyor')}</span></div><div class="maintenancePublicActions"><a class="btn primary" href="${discord}" target="_blank" rel="noreferrer">💬 Discord ile iletişime geç</a><button class="btn secondary" type="button" data-action="logout">Çıkış Yap</button></div><small class="maintenanceNote">Banlı kullanıcılar ana sayfa, arşiv, izleme, takvim, profil ve yönetim alanlarına ulaşamaz.</small></section></main>`;
}
function gameCard(g){
...
function renderUserRows(rows){
  ...
  return list.map(u=>{ const email=String(u.email||''); const role=roleValue(u.role,email); const active=u.is_active!==false && role!=='banned'; const created=u.createdAt || u.created_at || u.updated_at || ''; const src=String(u.source||'Supabase').replace(/Yerel Tablo|Yerel/g,'Tarayıcı'); return `<article class="userRow proUserRow v218UserRow" data-user-id="${esc(u.id||'')}" data-user-email="${esc(email)}" data-user-source="${esc(src)}"><div class="userAvatar">${esc((u.displayName||u.full_name||email||'?').slice(0,1).toLocaleUpperCase('tr'))}</div><div class="userCell"><b>${esc(u.displayName||u.full_name||email)}</b><small>${esc(email)} • ${esc(src)} ${created?'• '+esc(String(created).slice(0,10)):''}</small></div><span class="roleChip roleChipTable role-${esc(role)}">${esc(roleLabel(role,email))}</span><select class="input roleSelect" data-role-select aria-label="Yetki seç">${roleOptionsHtml(role,email)}</select><span class="userState ${active?'ok':'blocked'}">${active?'✅ Aktif':'🚫 Kısıtlı'}</span><div class="userRowActions"><button class="miniBtn primary" data-user-role-save>💾 Supabase’ye Kaydet</button><button class="miniBtn danger" data-user-delete>🗑️ Sil</button></div></article>`; }).join('');
}
...
function bind(){
  document.body.addEventListener('click', e=>{
    ...
    const userAdd=e.target.closest('[data-user-add-form]'); if(userAdd){ e.preventDefault(); const fd=new FormData(userAdd); const email=String(fd.get('email')||'').trim().toLowerCase(); const displayName=String(fd.get('displayName')||'').trim() || email; const role=roleValue(fd.get('role')||'user', email); if(!email){ toast('E-posta yazmalısın.'); return; } let users=loadUsers().filter(u=>String(u.email||'').toLowerCase()!==email); const user={id:'local-'+Date.now(),email,displayName,role,is_active:role!=='banned',source:'Tarayıcı',createdAt:new Date().toISOString()}; users.unshift(user); saveUsers(users); userAdd.reset(); toast(`${displayName} tabloya ${roleLabel(role,email)} olarak eklendi.`); apiJson('user-role-set',{adminToken:sessionToken(),email,displayName,role}).then(()=>refreshUsersPanel()).then(()=>toast('Yetki Supabase tablosuna da kaydedildi.')).catch(err=>{ console.warn('Supabase yetki kaydı yerel modda kaldı:', err.message); render(); }); return; }
    ...
    const userDel=e.target.closest('[data-user-delete]'); if(userDel){ const row=userDel.closest('[data-user-id]'); const id=row?.dataset?.userId||''; const email=String(row?.dataset?.userEmail||row?.querySelector('small')?.textContent?.split('•')?.[0]?.trim()||'').toLowerCase(); if(!confirm('Kullanıcıyı Supabase site_users/yetki kayıtlarından silmek istiyor musun?')) return; saveUsers(loadUsers().filter(u=>String(u.id)!==id && String(u.email).toLowerCase()!==email)); apiJson('user-delete',{adminToken:sessionToken(),userId:id,email}).then(()=>refreshUsersPanel()).then(()=>toast('Kullanıcı Supabase kayıtlarından silindi.')).catch(err=>{ toast('Supabase silme hatası: '+err.message); render(); }); return; }
...
window.addEventListener('error', ev=>{ console.error('v2.2.0 FIX hata:', ev.message); const root=document.getElementById('root'); if(root && (root.textContent||'').trim().length<40) root.innerHTML=emergencySafeHtml(ev.message || 'Beklenmeyen site hatası yakalandı.'); });
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  Promise.allSettled([refreshCurrentSessionFromSupabase(), refreshSiteDataFromSupabase({force:false})]).then(()=>render()).catch(()=>{});
  setTimeout(()=>{ const r=document.getElementById('root'); if(!r || (r.textContent||'').trim().length<40) render(); }, 300);
});
window.addEventListener('unhandledrejection', ev=>{ console.error('v2.2.0 FIX promise hata:', ev.reason); const root=document.getElementById('root'); if(root && (root.textContent||'').trim().length<40) root.innerHTML=emergencySafeHtml('Beklenmeyen işlem hatası yakalandı.'); });
