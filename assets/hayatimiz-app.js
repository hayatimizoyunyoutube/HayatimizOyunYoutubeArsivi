...
const FIX_NAME = 'v2.2.8 - 3.0 Açılış Planı ve Bakım İlerleme Sistemi';
...
const DEFAULT_NOTES = [
  ...
  {id:'plan-v226',version:'v2.2.8',title:'📱 Mobil Menü ve Responsive Cila',summary:'Üst menü, yönetim menüsü, büyük kartlar ve public/admin sayfaları mobilde daha kompakt hale getirilecek.',status:'Planlandı'},
  ...
  {id:"plan-v226-dashboard",version:"v2.2.8",title:"📊 Admin Dashboard İstatistikleri",summary:"Yönetim panelinde arşivin genel durumunu daha güçlü göstermek. Toplam oyun, seri, bölüm, kullanıcı ve güncelleme notu metrikleri güçlendirilecek.",status:"Planlandı"},
...
function pickEpisodeThumb(ep={}, videoId=''){
  const direct=ep.thumbnail || ep.thumbnailUrl || ep.thumbnail_url || ep.cover || ep.coverUrl || ep.image || ep.imageUrl;
  ...
  if(Array.isArray(thumbs) && thumbs.length) return String(thumbs[thumbs.length-1]?.url || thumbs[0]?.url || '');
  if(thumbs && typeof thumbs==='object'){
    const order=['maxres','standard','high','medium','default'];
    for(const k of order){ if(thumbs[k]?.url) return String(thumbs[k].url); }
  }
  return videoId ? `https://i.ytimg.com/vi/${videoId}/mqdefault.jpg` : '/assets/hayatimiz-kapak.png';
}
function cleanEpisodeTitle(raw='', n=1){
  const text=String(raw||'').replace(/\s+/g,' ').trim();
  if(!text || /^page[-_ ]?\d+\.(png|jpg|jpeg|webp)$/i.test(text)) return `${n}. Bölüm`;
  return text;
}
function normalizeEpisode(ep, i){
  ep = ep || {};
  const rawNumber=Number(ep.number || ep.episodeNumber || ep.episode_number || ep.position || ep.index || i+1);
  ...
  const url=ep.videoUrl || ep.video_url || ep.url || ep.watchUrl || (videoId?`https://www.youtube.com/watch?v=${videoId}`:'');
  const rawTitle=ep.title || ep.episodeTitle || ep.episode_title || ep.name || ep.snippet?.title || ep.videoTitle || '';
  const title=cleanEpisodeTitle(rawTitle, n);
  const thumb=pickEpisodeThumb(ep, videoId);
  const description=ep.description || ep.episodeDescription || ep.episode_description || ep.snippet?.description || '';
  return {id:ep.id || (videoId?`yt-${videoId}`:`ep-${n}`), number:n, title, description, thumbnail:thumb, videoId, videoUrl:url, watched:ep.watched === true || ep.is_watched === true};
}
...
async function syncPlaylistForForm(form){
  ...
  window.__HAYATIMIZ_PENDING_PLAYLIST_EPISODES__ = result.episodes;
  try{ localStorage.setItem('hayatimiz_pending_playlist_episodes', JSON.stringify(result.episodes)); }catch{}
...
function bind(){
  ...
  document.body.addEventListener('submit', e=>{
    ...
    const gf=e.target.closest('[data-game-form]'); if(gf){ e.preventDefault(); const fd=new FormData(gf); const rows=loadGames(); const editId=String(fd.get('gameId')||'').trim(); const title=String(fd.get('title')||'').trim(); if(!title){ toast('Oyun adı boş olamaz.'); return; } const episodeCount=Math.max(0, Number(fd.get('episodeCount')||0)); const watchedEpisodeCount=Math.max(0, Math.min(episodeCount || 9999, Number(fd.get('watchedEpisodeCount')||0))); const payload=normalizeGame({id:editId || slugify(title)+'-'+Date.now(),title,status:fd.get('status'),genre:Array.from(new Set([...(splitText(fd.get('genre'))), ...fd.getAll('genreCheck').map(x=>String(x))])).join(', '),seriesName:fd.get('seriesName'),cover:fd.get('cover')||'/assets/hayatimiz-kapak.png',banner:fd.get('banner'),releaseDate:bestReleaseDate(fd.get('releaseDate')),platforms:fd.get('platforms'),description:fd.get('description'),storyText:String(fd.get('storyText')||'').trim() || professionalStoryText({title, genre:fd.get('genre'), seriesName:fd.get('seriesName')}),youtubePlaylistUrl:fd.get('youtubePlaylistUrl'),youtubePlaylistId:fd.get('youtubePlaylistId') || extractYoutubePlaylistId(fd.get('youtubePlaylistUrl')),tags:Array.from(new Set([...(splitText(fd.get('tags'))), ...fd.getAll('tagsCheck').map(x=>String(x))])).join(', '),rawgId:fd.get('rawgId'),rawgSlug:fd.get('rawgSlug'),steamAppId:fd.get('steamAppId'),score:fd.get('score'),metaSource:fd.get('metaSource'),metaCheckedAt:fd.get('metaCheckedAt'),coverSource:fd.get('metaSource'),episodeCount,watchedEpisodeCount},0); let embeddedEpisodes=[]; try{ let eps=JSON.parse(String(fd.get('episodesJson')||'[]')); if((!Array.isArray(eps)||!eps.length) && Array.isArray(window.__HAYATIMIZ_PENDING_PLAYLIST_EPISODES__)) eps=window.__HAYATIMIZ_PENDING_PLAYLIST_EPISODES__; if((!Array.isArray(eps)||!eps.length)){ try{ eps=JSON.parse(localStorage.getItem('hayatimiz_pending_playlist_episodes')||'[]'); }catch{} } if(Array.isArray(eps) && eps.length) embeddedEpisodes=eps.map(ep=>normalizeEpisode({...ep, watched:Number(ep.number)<=Number(payload.watchedEpisodeCount||0)})); }catch(err){ console.warn('Bölüm JSON okunamadı; eski liste korunacak:', err); } const oldGame=editId ? rows.find(g=>String(g.id)===editId) : null; const oldEpisodes=oldGame ? (loadEpisodes(oldGame.id).length ? loadEpisodes(oldGame.id) : (Array.isArray(oldGame.episodes)?oldGame.episodes:[])) : []; if(embeddedEpisodes.length){ payload.episodes=embeddedEpisodes; payload.episodeCount=embeddedEpisodes.length; } else if(oldEpisodes.length){ payload.episodes=oldEpisodes; payload.episodeCount=oldEpisodes.length; } const next=editId ? rows.map(g=>String(g.id)===editId ? {...g,...payload,id:g.id,episodes:embeddedEpisodes.length?embeddedEpisodes:(g.episodes||[])} : g) : [payload, ...rows]; saveGames(next); if(embeddedEpisodes.length) saveEpisodes(payload.id, embeddedEpisodes); else if(typeof oldEpisodes!=='undefined' && oldEpisodes.length) saveEpisodes(payload.id, oldEpisodes, {skipGamePatch:true}); persistGameToSupabase(payload, editId).then(()=>{ render(); }).catch(err=>{ console.warn('Supabase kayıt yerel modda kaldı:', err.message); saveSyncState({mode:'local', status:'Yerel kayıt aktif', message:err.message}); }); toast(editId?'Oyun güncellendi.':'Oyun kaydedildi.'); setRoute('/yonetim/mevcut-oyunlar'); return; }
