...
const FIX_NAME = 'v4.0.0 - Temiz Final Tam Paket';
const RELEASE_TAG = 'v4.0.0-ANA-ACILIS-FINAL';
...
const DEFAULT_NOTES = [
  {id:'v400-ana-acilis-profesyonel-final',version:'v4.0.0',title:'🎉 Ana Açılış Sürümü',summary:'Hayatımız Oyun public arşiv sitesi profesyonel ana sayfa, arşiv, seriler, yayın takvimi, profil, favoriler, başarımlar, veri sağlığı, bakım/ban güvenliği ve yönetim paneliyle yayına hazır hale getirildi.',status:'Tamamlandı'},
  {id:'v400-profesyonel-tasarim-final',version:'v4.0.0 FIX',title:'🎨 Profesyonel Tasarım Finali',summary:'Ana sayfa, arşiv kartları, seri merkezi, kategori/koleksiyon sayfaları, profil merkezi ve yönetim paneli daha modern, kompakt ve mobil uyumlu hale getirildi.',status:'Tamamlandı'},
  {id:'v400-versiyon-senkron-final',version:'v4.0.0 FIX',title:'🏷️ Sürüm Senkronizasyonu',summary:'Site sürümü, Vercel etiketi, schema.sql Results çıktısı, status, health, README ve güncelleme notları v4.0.0 olarak eşitlendi. Eski aktif sürüm etiketleri pasifleştirildi.',status:'Tamamlandı'},
  {id:'v400-veri-guvenligi-final',version:'v4.0.0 FIX',title:'🛡️ Veri Güvenliği ve Yönetim',summary:'Oyunlar, bölümler, kullanıcılar, favoriler, takip, başarımlar ve takvim kayıtları için mevcut veri koruma mantığı korunur. Bakım modu açılışta kapalıdır.',status:'Tamamlandı'},
...
async function refreshGamesFromSupabase({force=false}={}){
  ...
  try{
    ...
    // v4.0.0: Supabase 0 döndü diye yerel veya ekrandaki oyunları asla silme.
...
function fillMetaToForm(form, meta, mode='safe'){
  ...
  setField(form,'metaSource',meta.source || 'v4.0.0 final meta paneli', false);
  setField(form,'metaCheckedAt',new Date().toISOString(), false);
  const box=form.querySelector('[data-meta-status]');
  if(box) box.innerHTML=`<b>Bilgi hazır:</b> ${esc(meta.source || 'v4.0.0')} • ${esc(meta.title || '')}`;
...
function saveGames(rows, options={}){
  ...
  // v4.0.0: Ben silmeden mevcut oyunlar silinmesin.
...
function sanitizeMaintenance(raw){
  const base = {enabled:false,message:'Hayatımız Oyun kısa süreli bakımda.',eta:'',percent:0,adminBypass:true,managedBy:'v4.0.0-runtime'};
...
function loadMaintenance(){
  ...
  return sanitizeMaintenance(found || {enabled:false,message:'Hayatımız Oyun kısa süreli bakımda.',eta:'',percent:0,adminBypass:true,managedBy:'v4.0.0-runtime'});
}
function saveMaintenance(v){
  const cfg = sanitizeMaintenance({...v, enabled: parseMaintenanceEnabled(v?.enabled), percent: Math.max(0, Math.min(100, Number(v?.percent||0))), managedBy:'v4.0.0-runtime', updatedAt:new Date().toISOString()});
...
function loadEpisodes(gameId){
  ...
  // v4.0.0: Eski kayıt playlist çekmiş ama bölüm JSON'u oyuna yazılmamışsa
...
function admin(){ return adminOnly(()=>{
  try{
    ...
    const episodeTotal=games.reduce((sum,g)=>sum+Number(g.episodeCount||0),0);
    const cards=[
      ['/yonetim/oyun-ekle','➕','Oyun Ekle','Profesyonel kartlı form, RAWG/Steam ve YouTube oynatma listesi alanları.','v4.0.0 aktif'],
      ...
      ['/koleksiyonlar','🗂️','Koleksiyonlar','Seri, tür, durum ve koleksiyon sayaçlarını ziyaretçi tarafta kontrol et.','v4.0.0'],
      ['/yonetim/yayin-takvimi','📅','Yayın Takvimi','Manuel yayın tarihleri ve video linkleri.','Korundu'],
      ['/yonetim/guncelleme-notlari','📝','Güncelleme Notları','Tamamlanan ve planlanan notları düzenle.','v4.0.0 işlendi'],
...
window.addEventListener('error', ev=>{ console.error('v4.0.0 hata:', ev.message); const root=document.getElementById('root'); if(root && (root.textContent||'').trim().length<40) root.innerHTML=emergencySafeHtml(ev.message || 'Beklenmeyen site hatası yakalandı.'); });
...
window.addEventListener('unhandledrejection', ev=>{ console.error('v4.0.0 promise hata:', ev.reason); const root=document.getElementById('root'); if(root && (root.textContent||'').trim().length<40) root.innerHTML=emergencySafeHtml('Beklenmeyen işlem hatası yakalandı.'); });
