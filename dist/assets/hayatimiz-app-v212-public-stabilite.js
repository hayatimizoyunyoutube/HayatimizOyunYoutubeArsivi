...
const FIX_NAME = 'Public Yayın Öncesi Stabilite';
...
const DEFAULT_NOTES = [
  {id:'fix-v212-profesyonel-menu-bakim-ui',version:'v2.1.2 FIX',title:'✨ Profesyonel Menü, Yönetim Ayrımı ve Bakım Modu Tasarım Fix',summary:'Yeni sürüm yapılmadan sol menü toparlandı, yönetim kısayolları ayrı yönetim alanına taşındı, ana sayfadaki arama yanındaki Profil/Yönetim karışıklığı kaldırıldı, üst bar sabitlendi, bakım modu kapatma/açma davranışı güçlendirildi ve ziyaretçiler için profesyonel bakım ekranı Giriş Yap/Kayıt Ol akışıyla güncellendi. Schema gerekli değildir.',status:'Tamamlandı'},
  {id:'v212-public-yayin-oncesi-stabilite',version:'v2.1.2',title:'🚀 Public Yayın Öncesi Stabilite',summary:'Ana sayfa, oyun arşivi, seriler, koleksiyonlar, siteden izleme, rehberler, yönetim bağlantıları, bakım modu ve boş ekran koruması yayın öncesi kontrol edildi. Mobil/tablet/masaüstü görünüm güçlendirildi; /status durum sayfası ve güvenli açılış raporu eklendi.',status:'Tamamlandı'},
  {id:'v213-plan-supabase-admin-fix',version:'v2.1.3',title:'🛠️ Supabase Veri Fix ve Admin Güçlendirme',summary:'Sıradaki adımda Supabase kayıt/yenileme, admin kullanıcı yönetimi, veri eşitleme ve yetki kontrolleri daha detaylı güçlendirilecek.',status:'Planlandı'},
  {id:'fix-v211-rehber-yetkili-alfabetik-entegre',version:'v2.1.1 FIX',title:'📘 Site Rehberi, 👑 Yetkili Rehberi ve 🔤 Entegre Alfabetik Sıralama Fix',summary:'Yeni sürüm yapılmadan site rehberi ve yetkili rehberi eklendi. Yetki rolleri emojili ve Türkçe anlatıldı. Alfabetik sıralama ayrı bölüm olmaktan çıkarılıp oyun arşivi ve seriler içine taşındı. Schema gerekli değildir.',status:'Tamamlandı'},
  {id:'fix-v211-seri-izleme-siralama-supabase',version:'v2.1.1 FIX',title:'Seri İzleme, Seri Yönetimi ve Supabase Kayıt Fix',summary:'Yeni sürüm yapılmadan seriler için Tüm Seriyi İzle, seri içindeki oyunları seçme, seri adını düzenleme, sürükle-bırak ve sayı ile sıralama, Supabase kalıcı kayıt akışı eklendi. Schema sıfırlanmadı; mevcut games alanları kullanıldı.',status:'Tamamlandı'},
  {id:'fix-v211-izleme-kalite-alfabetik-yetki-bakim',version:'v2.1.1 FIX',title:'Site İçi İzleme, Kalite, Alfabetik Sıralama, Yetki ve Bakım Fix',summary:'Yeni sürüm yapılmadan site içinden izleme sayfası, kalite tercihi paneli, A/B/0-9 alfabetik sıralama, profesyonel hikaye üretimi, kayıtlı kullanıcıları görme ve yetki verme ekranı eklendi. Bakım modunda Supabase yenileme kapalı değeri zorla basmayacak şekilde düzeltildi.',status:'Tamamlandı'},
  {id:'fix-v211-profesyonel-arayuz-turkce',version:'v2.1.1 FIX',title:'Profesyonel Arayüz ve Türkçe Menü Fix',summary:'Yönetim butonları solda kalacak şekilde düzenlendi; profil, çıkış yap, giriş yap ve kayıt ol işlemleri sağ üst alana taşındı. Kurucu, Moderatör, İçerik Editörü ve Üye rol adları Türkçeleştirildi. Yeni sürüm yapılmadı.',status:'Tamamlandı'},
  ...
  {id:'v209-koleksiyon-seri-durum',version:'v2.0.9',title:'Koleksiyon, Seri, Durum ve Sayaçlar',summary:'Koleksiyonlar, seri grupları, durum sayaçları ve arşiv istatistikleri aynı veri kaynağından beslenen stabil yapıya alındı. Koleksiyonlar boş görünmez, durum ayrımı ve seri sayıları doğru hesaplanır.',status:'Tamamlandı'},
  {id:'v208-youtube-playlist-bolum',version:'v2.0.8',title:'YouTube Oynatma Listesi ve Bölüm Takibi',summary:'YouTube oynatma listesi otomasyonu, bölüm listesi, bölüm sayısı güncelleme ve kaldığımız bölüm takibi kontrollü şekilde geri eklendi. Servis çalışmazsa site boş kalmaz; yerel güvenli bölüm listesi oluşturulur.',status:'Tamamlandı'},
  {id:'v207-rawg-steam-kapak-meta',version:'v2.0.7',title:'RAWG / Steam / Kapak / Bilgi Geri Dönüş',summary:'RAWG / Steam bilgi paneli oyun ekle ve düzenle formuna kontrollü şekilde geri eklendi. Oyun adından tür, çıkış tarihi, kapak, banner, puan, RAWG slug ve Steam App ID önerileri alınabiliyor; eski kırık otomasyon aynen geri basılmadı.',status:'Tamamlandı'},
  {id:'v206-oyun-ekle-duzenle-formu',version:'v2.0.6',title:'Oyun Ekle / Düzenle Formu',summary:'Oyun ekleme ve düzenleme formu profesyonel kartlı yapıya alındı. Tür ve etiket ayrıldı; Türkçe Altyazılı gibi etiketler status alanına karışmayacak şekilde kaydediliyor. Kapak, banner, çıkış tarihi, platform, seri, hikaye, RAWG/Steam ve YouTube alanları okunabilir bölümlere ayrıldı.',status:'Tamamlandı'},
  {id:'fix-v205-eski-ana-sayfa-tam-geri-donus',version:'v2.0.5 FIX',title:'Eski Ana Sayfa Tam Geri Dönüş',summary:'Profesyonel eski ana sayfa; büyük arşiv vitrini, üst arama, istatistik panelleri, öne çıkan oyunlar, devam eden seriler, hızlı işlem ve etkinlik alanlarıyla tekrar ana sayfaya alındı. Sürüm artırılmadı.',status:'Tamamlandı'},
  {id:'v205-admin-panel-geri-donus',version:'v2.0.5',title:'Yönetim Paneli Geri Dönüş',summary:'Yönetim paneli eski dolu yapıya kontrollü şekilde yaklaştırıldı. Buton taşmaları, görünmez beyaz yazı sorunları ve güvenli hata ekranı düzeltildi.',status:'Tamamlandı'},
...
async function refreshSiteRuntimeFromSupabase({force=false}={}){
  try{
    ...
    if(data && data.maintenance){
      ...
      if(localUpdated && remoteUpdated && localUpdated > remoteUpdated + 1000){
...
async function persistMaintenanceToSupabase(maintenance){
  ...
  const data=await apiJson('maintenance-save',{adminToken:token,key:'maintenance_mode',maintenance:{...maintenance, updatedAt:new Date().toISOString()}});
  if(data?.maintenance) saveMaintenance({...data.maintenance, source:'supabase'});
  saveSyncState({mode:'supabase', status:'Supabase bakım modu aktif', message:'Bakım modu Supabase runtime config içine kaydedildi.'});
  return data;
}
...
function userTopbar(){
  ...
  const publicLinks = `<nav class="topQuickNav"><a href="/ana-sayfa">🏠 Ana Sayfa</a><a href="/oyun-arsivi">🎮 Arşiv</a><a href="/seriler">🎬 Seriler</a><a href="/site-rehberi">📘 Rehber</a></nav>`;
  const auth = user
  ...
  return `<header class="topBar proStickyTop"><div class="topIdentity"><b>Hayatımız Oyun</b><small>${VERSION} • ✨ Profesyonel FIX</small></div>${publicLinks}${isYönetim()?'<a class="topAdminOpen" href="/yonetim" target="_blank" rel="noopener" data-new-tab="1">🛡️ Yönetim Paneli ↗</a>':''}${auth}</header>`;
}
...
function nav(){
  const publicItems=[['/ana-sayfa','🏠 Ana Sayfa'],['/oyun-arsivi','🎮 Oyun Arşivi'],['/koleksiyonlar','🗂️ Koleksiyonlar'],['/seriler','🎬 Seriler'],['/status','📡 Site Durumu'],['/site-rehberi','📘 Site Rehberi'],['/yetkili-rehberi','👑 Yetkili Rehberi']];
  const adminItems=[['/yonetim','🛡️ Yönetim Paneli'],['/yonetim/oyun-ekle','➕ Oyun Ekle'],['/yonetim/mevcut-oyunlar','🎮 Mevcut Oyunlar'],['/yonetim/seriler','🎬 Serileri Yönet'],['/yonetim/bolum-takibi','▶️ Bölüm Takibi'],['/yonetim/yayin-takvimi','📅 Yayın Takvimi'],['/yonetim/guncelleme-notlari','📝 Güncelleme Notları'],['/yonetim/bakim-modu','🛠️ Bakım Modu'],['/yonetim/kullanicilar','👥 Kullanıcılar ve Yetkiler']];
  return `<aside class="side"><div class="brand"><span class="logo">🎮</span><span><b>Hayatımız <em>Oyun</em></b><small>${VERSION} • 🚀 Yayın Öncesi Stabilite</small></span></div><nav class="nav"><div class="navLabel">🌐 Site</div>${publicItems.map(([href,label])=>`<a class="${active(href)?'active':''}" href="${href}">${label}</a>`).join('')}${canShowYönetimLinks()?`<div class="navLabel">🛡️ Yönetim</div>${adminItems.map(([href,label])=>`<a class="${active(href)?'active':''}" href="${href}">${label}</a>`).join('')}`:''}</nav><button data-action="hard-refresh">🔄 Sayfayı Yenile</button><div class="sideFooter"><b class="okText">✅ v2.1.2</b><br>🚀 Yayın öncesi kontroller aktif.</div></aside>`;
}
function layout(content){ return `<main class="app">${nav()}<section class="main">${userTopbar()}${content}</section></main>`; }
...
function maintenanceZiyaretçi(){
  ...
  return `<main class="maintenanceZiyaretçi proMaintenance"><section class="maintenanceCard proMaintenanceCard"><div class="maintenanceGlow"></div><span class="badge amber">🛠️ Bakım Modu Aktif</span><h1>Hayatımız Oyun kısa süreli bakımda</h1><p>${esc(m.message || 'Siteyi daha profesyonel ve stabil hale getirmek için kısa süreli bakım yapılıyor.')}</p><div class="maintenanceProgress"><i><span style="width:${pct}%"></span></i><b>%${pct}</b></div><p class="muted">${esc(m.eta || 'Tahmini açılış bilgisi güncellendiğinde burada görünecek.')}</p><div class="maintenancePublicActions"><a class="btn primary" href="/giris-yap">🔐 Giriş Yap</a><a class="btn secondary" href="/kayit-ol">📝 Kayıt Ol</a><a class="btn ghost" href="/ana-sayfa">🔄 Yeniden Dene</a></div><small class="maintenanceNote">👑 Yetkili girişi yazısı ziyaretçiye gösterilmez. Kurucu veya yetkili hesabın varsa normal giriş ekranından oturum açabilirsin.</small></section></main>`;
}
function gameCard(g){
...
async function refreshUsersPanel(){
  ...
  try{ const data=await apiJson('users-list',{adminToken:sessionToken()}); target.innerHTML=renderUserRows((data.users||[]).map(u=>({id:u.id,email:u.email,displayName:u.full_name||u.displayName||u.email,role:u.role,is_active:u.is_active,source:'Supabase'}))); toast('Supabase kullanıcıları yenilendi.'); }
...
function safeErrorPanel(message='Yönetim paneli güvenli moda geçti.'){
...
function bind(){
  document.body.addEventListener('click', e=>{
    ...
    if(e.target.closest('[data-action="maintenance-off"]')){ const m={...loadMaintenance(), enabled:false}; saveMaintenance(m); persistMaintenanceToSupabase(m).catch(err=>{ console.warn('Supabase bakım kapatma yerel modda kaldı:', err.message); }); toast('Bakım modu kapatıldı.'); render(); return; }
  ...
  document.body.addEventListener('submit', e=>{
    ...
    const register=e.target.closest('[data-register-form]'); if(register){ e.preventDefault(); (async()=>{ const fd=new FormData(register); const email=String(fd.get('email')||'').trim().toLowerCase(); const displayName=String(fd.get('displayName')||'').trim(); const password=String(fd.get('password')||''); if(password.length<3){ toast('Şifre en az 3 karakter olsun.'); return; } let adminToken=''; try{ const data=await apiJson('register',{email,password,fullName:displayName}); adminToken=data.adminToken||''; toast('Supabase kayıt oluşturuldu.'); }catch(err){ console.warn('Supabase kayıt yok, yerel kayıt:', err.message); toast('Yerel kayıt oluşturuldu. Supabase daha sonra bağlanabilir.'); } let users=loadUsers().filter(u=>String(u.email).toLowerCase()!==email); const role=isYönetimEmail(email)?'owner':'user'; const user={id:'user-'+Date.now(),email,displayName,password,role,adminToken,createdAt:new Date().toISOString()}; users.push(user); saveUsers(users); localStorage.setItem(STORAGE.session, JSON.stringify({email:user.email,displayName:user.displayName,role:user.role,adminToken})); setRoute(role==='owner'?'/yonetim':'/ana-sayfa'); })(); return; }
