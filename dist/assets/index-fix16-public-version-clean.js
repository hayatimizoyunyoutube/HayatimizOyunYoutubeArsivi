...
var ho240Applied;
const VERSION = 'v2.4.0 FIX 23 - AI ve Yayına Alma Panelleri Kaldırıldı';
...
/* v2.4.0 FIX 16 - kullanıcı arayüzünde FIX etiketini tamamen gizleme */
const HO240F16_INTERNAL_VERSION = 'v2.4.0 FIX 16';
const HO240F16_PUBLIC_VERSION_KEYS = [
  'hayatimiz_last_public_version',
  'ho240f14_current_site_version',
  'ho240f11_next_version',
  'ho240f10_ai_version'
];
function ho240f16ExtractPublicVersion(raw){
  const text = String(raw || '').trim();
  if(!text) return '';
  const matches = text.match(/v?\d+\.\d+\.\d+/ig) || [];
  for(const candidate of matches){
    const index = text.toLocaleLowerCase('tr-TR').indexOf(candidate.toLocaleLowerCase('tr-TR'));
    const after = text.slice(index + candidate.length, index + candidate.length + 12);
    if(!/^\s*fix\b/i.test(after)) return candidate.startsWith('v') ? candidate : `v${candidate}`;
  }
  return '';
}
function ho240f16PublicVersion(){
  try{
    for(const key of HO240F16_PUBLIC_VERSION_KEYS){
      const found = ho240f16ExtractPublicVersion(localStorage.getItem(key));
      if(found) return found;
    }
  }catch{}
  return 'v2.4.1';
}
function ho240f16SetPublicVersion(value){
  const clean = ho240f16ExtractPublicVersion(value) || ho240f16PublicVersion() || 'v2.4.1';
  try{
    HO240F16_PUBLIC_VERSION_KEYS.forEach(key => localStorage.setItem(key, clean));
    localStorage.setItem('hayatimiz_last_fix_version', clean);
    document.title = `Hayatımız Oyun - ${clean}`;
  }catch{}
  return clean;
}
function ho240f16ReadVersionInput(){
  const full = String(document.getElementById('ho240f13VersionInput')?.value || document.getElementById('ho240f11VersionInput')?.value || '').trim();
  const num = String(document.getElementById('ho240f15VersionNumberInput')?.value || '').trim();
  const base = ho240f16ExtractPublicVersion(full) || ho240f16PublicVersion();
  if(num && /^\d+$/.test(num)){
    const m = base.match(/^(v?\d+\.\d+\.)\d+$/i);
    const prefix = m ? (m[1].startsWith('v') ? m[1] : `v${m[1]}`) : 'v2.4.';
    return `${prefix}${num}`;
  }
  return base;
}
function ho240f16StripFixLabels(html){
  const v = ho240f16PublicVersion();
  return String(html || '')
    // v2.5.0 - v2.4.0 FIX 15 / v2.5.0 • v2.4.0 FIX 14 gibi birleşik etiketleri temizle.
    .replace(/v?\d+\.\d+\.\d+\s*(?:[-–—•|])\s*v?\d+\.\d+\.\d+\s*FIX\s*\d+[^<]*/gi, v)
    // Herhangi bir span içinde FIX geçiyorsa sadece aktif public sürümü bırak.
    .replace(/<span>(?=[^<]*\bFIX\b)[^<]*<\/span>/gi, `<span>${esc(v)}</span>`)
    // Tek başına görünen v2.4.0 FIX 15 / v2.4.0 FIX 14 yazılarını temizle.
    .replace(/v?\d+\.\d+\.\d+\s*FIX\s*\d+[^<]*/gi, v)
    // Sol menüdeki "Yönetim • FIX" kalıntılarını temizle.
    .replace(/(Hayatımız\s+Oyun\s*)v?\d+\.\d+\.\d+\s*Yönetim\s*•\s*[^<]*/gi, `$1${v} Yönetim`)
    .replace(/v?\d+\.\d+\.\d+\s+Yönetim\s*•\s*[^<]*/gi, `${v} Yönetim`)
    .replace(/\s+•\s*FIX\s*\d+/gi, '')
    .replace(/\s+-\s*FIX\s*\d+/gi, '');
}
try{
  ho240f14CurrentVersion = ho240f16PublicVersion;
  ho240f14VersionLine = ho240f16PublicVersion;
  ho240f14SetVersion = ho240f16SetPublicVersion;
  ho240f13SetVersion = ho240f16SetPublicVersion;
  ho240f15CleanUiVersion = ho240f16PublicVersion;
  ho240f15SetVersion = ho240f16SetPublicVersion;
  ho240f15ReadVersionInput = ho240f16ReadVersionInput;
}catch{}
try{
  const ho240f16PrevTopbar = topbar;
  topbar = function(){ return ho240f16StripFixLabels(ho240f16PrevTopbar()); };
}catch{}
try{
  const ho240f16PrevAdminPanel = adminPanel;
  adminPanel = function(){ return ho240f16StripFixLabels(ho240f16PrevAdminPanel()); };
}catch{}
try{
  const ho240f16PrevDeployCenter = ho240f15DeployCenter || ho240f13DeployCenter || ho240DeployCenter;
  ho240f15DeployCenter = function(){ return ho240f16StripFixLabels(ho240f16PrevDeployCenter()); };
  ho240f13DeployCenter = ho240f15DeployCenter;
  ho240f11DeployCenter = ho240f15DeployCenter;
  ho240DeployCenter = ho240f15DeployCenter;
  hoFix8DeployPanel = ho240f15DeployCenter;
...
try{
  const ho240f16PrevAiCenter = ho240AiCenter;
  ho240AiCenter = function(){ return ho240f16StripFixLabels(ho240f16PrevAiCenter()); };
}catch{}
const ho240f16PrevOnAction = onAction;
onAction = async function(e){
  const action = e.currentTarget?.dataset?.action || '';
  if(action === 'ho240f13-save-version' || action === 'ho240f11-save-version'){
    e.preventDefault(); e.stopImmediatePropagation();
    const v = ho240f16SetPublicVersion(ho240f16ReadVersionInput());
    try{ if(typeof ho240f13MakePlannedUpdates === 'function') ho240f13MakePlannedUpdates(); }catch{}
    try{ if(typeof ho240f13SaveScanState === 'function') ho240f13SaveScanState({updates:true,suggestions:true,last:`Versiyon ${v} olarak kaydedildi. Üst marka alanında artık paket FIX etiketi gösterilmez.`}); }catch{}
    render(); setToast(`Sürüm ${v} olarak güncellendi. Üstte sadece bu sürüm görünür.`); return;
  }
  if(action === 'ho240f13-scan-updates'){
    e.preventDefault(); e.stopImmediatePropagation();
    const v = ho240f16SetPublicVersion(ho240f16ReadVersionInput());
    try{ localStorage.setItem(HO240F13_SEED_KEY, String(Number(localStorage.getItem(HO240F13_SEED_KEY)||0)+29)); }catch{}
    try{ if(typeof ho240f12WriteHidden === 'function') ho240f12WriteHidden([]); }catch{}
    try{ localStorage.setItem('ho240_ai_view','new'); }catch{}
    const plans = typeof ho240f13MakePlannedUpdates === 'function' ? ho240f13MakePlannedUpdates() : [];
    try{ if(typeof ho240f13SaveScanState === 'function') ho240f13SaveScanState({updates:true,suggestions:true,last:`Yeni güncellemeler ${new Date().toLocaleString('tr-TR')} tarihinde tarandı. ${v} için öneriler yenilendi; üst marka alanında sadece public sürüm gösterildi.`,count:(ho240f13ScanState?.().count || 0)+1}); }catch{}
    try{ api('ai-feature-registry-save',{version:v,features:(plans||[]).map(p=>({key:p.id||p.key,title:p.title,target:p.target,table:p.cat,status:'fix16_public_version_clean'}))}).catch(()=>{}); }catch{}
    render(); setToast('Tepedeki güncellemeler yenilendi; marka sürümündeki FIX kalıntısı temizlendi.'); return;
  }
  return ho240f16PrevOnAction(e);
};
try{
  ho240f16SetPublicVersion(ho240f16PublicVersion());
  if(document.getElementById('root')?.dataset?.mounted === '1') render();
}catch(error){ console.warn('FIX16 sürüm temizlik katmanı atlandı:', error); }

/* v2.4.0 FIX 17 - Özellik Yaz içinden v2.x.x Planı sürümünü otomatik algılama */
const HO240F17_INTERNAL_VERSION = 'v2.4.0 FIX 17';
const HO240F17_LAST_DETECTED_PLAN_KEY = 'ho240f17_last_detected_plan_version';
const HO240F17_VERSION_KEYS = [
  'hayatimiz_last_public_version',
  'ho240f14_current_site_version',
  'ho240f11_next_version',
  'ho240f10_ai_version',
  'ho240f17_last_detected_plan_version'
];
function ho240f17ExtractPlanVersion(raw){
  const text = String(raw || '').trim();
  if(!text) return '';
  const planMatch = text.match(/\bv?((?:\d+\.){2}\d+)\s*(?:plan[ıi]?|sürüm|surum|güncelleme|guncelleme)?\b/i);
  if(planMatch) return `v${planMatch[1]}`;
  const any = text.match(/\bv?((?:\d+\.){2}\d+)\b/i);
  return any ? `v${any[1]}` : '';
}
function ho240f17SetEverywhere(version, source='Özellik Yaz'){
  const clean = ho240f17ExtractPlanVersion(version) || (typeof ho240f16ExtractPublicVersion === 'function' ? ho240f16ExtractPublicVersion(version) : '') || String(version || '').trim();
  if(!clean) return (typeof ho240f16PublicVersion === 'function' ? ho240f16PublicVersion() : 'v2.4.1');
  try{
    if(typeof ho240f16SetPublicVersion === 'function') ho240f16SetPublicVersion(clean);
    else if(typeof ho240f15SetVersion === 'function') ho240f15SetVersion(clean);
  }catch{}
  try{
    HO240F17_VERSION_KEYS.forEach(key => localStorage.setItem(key, clean));
    localStorage.setItem('hayatimiz_last_fix_version', clean);
    localStorage.setItem('hayatimiz_current_visible_version', clean);
    localStorage.setItem('ho240f13_selected_scan_version', clean);
    document.title = `Hayatımız Oyun - ${clean}`;
    const now = new Date().toLocaleString('tr-TR');
    localStorage.setItem('ho240f17_version_source_note', `${source}: ${clean} ${now} tarihinde otomatik algılandı.`);
  }catch{}
  try{
    const input = document.getElementById('ho240f13VersionInput') || document.getElementById('ho240f11VersionInput');
    if(input) input.value = clean;
    const small = document.getElementById('ho240f15VersionNumberInput');
    const n = clean.match(/(\d+)$/)?.[1] || '';
    if(small && n) small.value = n;
  }catch{}
  return clean;
}
function ho240f17IdeaWithoutPlanVersion(raw){
  return String(raw || '').replace(/\bv?\d+\.\d+\.\d+\s*(?:plan[ıi]?|sürüm|surum|güncelleme|guncelleme)\b\s*[:\-–—]?\s*/i, '').trim();
}
try{
  const ho240f17PrevAnalyzeIdea = ho240f15AnalyzeIdea;
  ho240f15AnalyzeIdea = function(raw){
    const detected = ho240f17ExtractPlanVersion(raw);
    if(detected) ho240f17SetEverywhere(detected, 'Özellik Yaz plan başlığı');
    const suggestion = ho240f17PrevAnalyzeIdea(raw);
    if(detected){
      suggestion.version = detected;
      suggestion.detectedPlanVersion = detected;
      suggestion.desc = `Plan sürümü ${detected} olarak algılandı. ` + String(suggestion.desc || '');
      suggestion.steps = [
        `Yazıdaki ${detected} Planı otomatik algılandı ve site sürümü her alanda ${detected} yapıldı.`,
        ...(Array.isArray(suggestion.steps) ? suggestion.steps : [])
      ];
    }
    return suggestion;
  };
}catch(error){ console.warn('FIX17 analiz katmanı atlandı:', error); }
try{
  const ho240f17PrevApplyCustom = ho240f15ApplyCustomFeature;
  ho240f15ApplyCustomFeature = function(){
    const idea = localStorage.getItem(HO240F15_CUSTOM_IDEA_KEY) || '';
    const detected = ho240f17ExtractPlanVersion(idea);
    if(detected){
      const clean = ho240f17SetEverywhere(detected, 'Özellik uygula');
      try{
        const suggestion = ho240f15Read(HO240F15_CUSTOM_SUGGESTION_KEY, null);
        if(suggestion){
          suggestion.version = clean;
          suggestion.detectedPlanVersion = clean;
          ho240f15Write(HO240F15_CUSTOM_SUGGESTION_KEY, suggestion);
        }
      }catch{}
    }
    return ho240f17PrevApplyCustom();
  };
}catch(error){ console.warn('FIX17 uygulama katmanı atlandı:', error); }
try{
  const ho240f17PrevIdeaForm = ho240f15IdeaForm;
  ho240f15IdeaForm = function(){
    const html = String(ho240f17PrevIdeaForm());
    const v = ho240f17ExtractPlanVersion(localStorage.getItem(HO240F15_CUSTOM_IDEA_KEY) || '') || (typeof ho240f16PublicVersion === 'function' ? ho240f16PublicVersion() : 'v2.4.1');
    return html.replace(
      '<span class="pill green">Yaz → Öner → Uygula → F5</span>',
      `<span class="pill green">Yaz → Öner → Uygula → F5</span><span class="pill">Algılanan sürüm: ${esc(v)}</span>`
    ).replace(
      'Ne istediğini normal cümleyle yaz. Sistem hedef sayfayı, nasıl çalışması gerektiğini ve riskini otomatik önerir.',
      'Ne istediğini normal cümleyle yaz. Başına v2.4.3 Planı gibi sürüm yazarsan site sürümü otomatik o sürüme çevrilir; sistem hedef sayfayı, nasıl çalışması gerektiğini ve riskini otomatik önerir.'
    );
  };
  ho240AiCenter = function(){ return ho240f15AiCenter(); };
}catch(error){ console.warn('FIX17 form katmanı atlandı:', error); }
try{
  const ho240f17PrevBind = bind;
  bind = function(){
    try{ ho240f17PrevBind(); }catch(err){ console.warn('FIX17 önceki bind atlandı:', err); }
    const box = document.getElementById('ho240f15IdeaText');
    if(box && !box.dataset.ho240f17Bound){
      box.dataset.ho240f17Bound = '1';
      box.addEventListener('input', e=>{
        const detected = ho240f17ExtractPlanVersion(e.currentTarget.value);
        if(detected){
          const last = localStorage.getItem(HO240F17_LAST_DETECTED_PLAN_KEY) || '';
          const clean = ho240f17SetEverywhere(detected, 'Özellik Yaz canlı algılama');
          if(last !== clean){
            try{ setToast(`Plan sürümü algılandı: ${clean}. Site adı, üst logo, deploy ve öneri sürümü buna çevrildi.`); }catch{}
          }
        }
      }, true);
    }
  };
}catch(error){ console.warn('FIX17 bind katmanı atlandı:', error); }
try{
  const ho240f17PrevDeployCenter = ho240DeployCenter;
  ho240DeployCenter = function(){
    const html = String(ho240f17PrevDeployCenter());
    const note = localStorage.getItem('ho240f17_version_source_note') || '';
    return note ? html.replace('<section class="card wide"><h2>Son Durum</h2>', `<section class="card wide"><h2>Son Durum</h2><p class="muted">${esc(note)}</p>`) : html;
  };
  ho240f15DeployCenter = ho240DeployCenter;
  ho240f13DeployCenter = ho240DeployCenter;
  ho240f11DeployCenter = ho240DeployCenter;
  hoFix8DeployPanel = ho240DeployCenter;
}catch(error){ console.warn('FIX17 deploy not katmanı atlandı:', error); }
try{
  const existingIdea = localStorage.getItem(HO240F15_CUSTOM_IDEA_KEY) || '';
  const detected = ho240f17ExtractPlanVersion(existingIdea);
  if(detected) ho240f17SetEverywhere(detected, 'Kayıtlı özellik planı');
  if(Array.isArray(VERSION_NOTES_ARCHIVE) && !VERSION_NOTES_ARCHIVE.some(n=>n.version===HO240F17_INTERNAL_VERSION)){
    VERSION_NOTES_ARCHIVE.unshift({version:HO240F17_INTERNAL_VERSION,title:'Özellik Yaz Sürüm Planı Otomatik Algılama',summary:'Özellik Yaz alanına v2.4.3 Planı gibi sürüm yazıldığında site adı, üst logo, Deploy Merkezi, öneri kartı ve uygulanan özellik kayıtları otomatik aynı public sürüme çevrilir.',image:'previews/hayatimiz-oyun-v240-fix17-plan-surum-preview.png',written:'FIX17 ile AI özellik isteği içindeki plan sürümü algılanır; FIX etiketi gösterilmez ve F5 sonrası da seçilen sürüm korunur.'});
  }
  if(document.getElementById('root')?.dataset?.mounted === '1') render();
}catch(error){ console.warn('FIX17 başlangıç katmanı atlandı:', error); }
...
/* v2.4.0 FIX 21 - Profesyonel AI özellik hedef router + yanlış yere ekleme koruması */
const HO240F21_INTERNAL_VERSION = 'v2.4.0 FIX 21';
const HO240F21_ROUTER_KEY = 'ho240f21_professional_router_enabled';
function ho240f21Norm(value){
  return String(value || '').toLocaleLowerCase('tr-TR')
    .replace(/[ıİ]/g,'i').replace(/[öÖ]/g,'o').replace(/[üÜ]/g,'u').replace(/[ğĞ]/g,'g').replace(/[şŞ]/g,'s').replace(/[çÇ]/g,'c')
    .replace(/[^a-z0-9]+/g,' ').replace(/\s+/g,' ').trim();
}
function ho240f21VisibleText(value){ return String(value || '').replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim(); }
function ho240f21StripPlanHeader(value){
  return String(value || '')
    .replace(/^\s*v?\d+\.\d+\.\d+\s*(?:plan[ıi]?|plani|plan)?\s*[-:–—]?\s*/i,'')
    .replace(/^\s*plan\s*[-:–—]?\s*/i,'')
    .trim();
}
function ho240f21Score(text, words, weight){
  const t = ` ${ho240f21Norm(text)} `;
  return words.reduce((sum, word)=> sum + (t.includes(` ${ho240f21Norm(word)} `) || t.includes(ho240f21Norm(word)) ? weight : 0), 0);
}
function ho240f21TargetFromText(raw){
  const original = String(raw || '');
  const stripped = ho240f21StripPlanHeader(original);
  const t = ho240f21Norm(stripped || original);
  const whole = ho240f21Norm(original);
  const explicitTakvim = /\b(yayin takvimi|yayın takvimi|takvim|yayin tarihi|yayın tarihi|program|calendar)\b/i.test(stripped);
  const explicitAi = /\b(ai ozellik|ai özellik|ozellik yaz|özellik yaz|ozellik merkezi|özellik merkezi|ozellik ekle|özellik ekle|yeni oneriler|yeni öneriler|oneriyi siteye uygula|öneriyi siteye uygula|siteye uygula|uygulanan ozellik|uygulanan özellik|nereye eklendi|nereye eklenecek|hedef sayfa|akilli ozellik|akıllı özellik)\b/i.test(stripped);
  const scores = {
    ai: ho240f21Score(stripped, ['ai özellik','ai ozellik','özellik yaz','ozellik yaz','özellik merkezi','ozellik merkezi','özellik ekle','ozellik ekle','yeni öneriler','yeni oneriler','öneri','oneri','siteye uygula','uygulanan özellik','uygulanan ozellik','nereye eklendi','hedef sayfa','akıllı özellik','akilli ozellik'], 18),
    oyun: ho240f21Score(stripped, ['oyun','oyunlar','kapak','meta','rawg','steam','tür','tur','etiket','playlist','bölüm','bolum','oyun ekle','oyunu güncelle','oyun düzenle','oyun duzenle'], 14),
    seri: ho240f21Score(stripped, ['seri','seriler','sezon','seri izleme','seri geçmişi','seri gecmisi','sıralama','siralama'], 14),
    takvim: explicitTakvim ? ho240f21Score(stripped, ['yayın takvimi','yayin takvimi','takvim','yayın tarihi','yayin tarihi','program','calendar'], 20) : 0,
    bildirim: ho240f21Score(stripped, ['bildirim','mail','e posta','eposta','uyarı','uyari','kuyruk'], 14),
    supabase: ho240f21Score(stripped, ['supabase','schema','sql','tablo','kolon','veritabanı','veritabani'], 16),
    deploy: ho240f21Score(stripped, ['deploy','vercel','github','build','redeploy','cache'], 16),
    guncelleme: ho240f21Score(stripped, ['güncelleme notu','guncelleme notu','sürüm notu','surum notu','changelog','tamamlananlar','planlananlar'], 16),
    rapor: ho240f21Score(stripped, ['hata','bug','rapor','log','çökme','cokme','sorun'], 12),
    profil: ho240f21Score(stripped, ['profil','avatar','fotoğraf','fotograf'], 12),
    bakim: ho240f21Score(stripped, ['bakım','bakim','kapalı','kapali'], 12),
    arayuz: ho240f21Score(stripped, ['ana sayfa','anasayfa','menü','menu','tasarım','tasarim','arayüz','arayuz'], 12)
  };
  // "v2.4.3 Planı" içindeki plan kelimesi artık takvim sayılmaz.
  if(/v?\d+\.\d+\.\d+\s*(plan[ıi]?|plani)/i.test(original) && !explicitTakvim) scores.takvim = 0;
  if(explicitAi) scores.ai += 80;
  if(/kod|kodlari|kodları|yanlis yere|yanlış yere|stabil|profesyonel/.test(t) && /ai|ozellik|özellik/.test(whole)) scores.ai += 55;
  if(/kapak|rawg|steam|meta|oyun ekle|oyunu guncelle|oyunu güncelle/.test(t)) scores.oyun += 35;
  if(/seri/.test(t) && !/oyun kapak|kapak/.test(t)) scores.seri += 18;
  if(/deploy|vercel|github|building|build/.test(t)) scores.deploy += 40;
  if(/schema|sql|supabase|tablo|kolon/.test(t)) scores.supabase += 35;
  if(/guncelleme|güncelleme|surum notu|sürüm notu/.test(stripped)) scores.guncelleme += 30;
  const ordered = Object.entries(scores).sort((a,b)=>b[1]-a[1]);
  const [bestKey, bestScore] = ordered[0] || ['genel',0];
  const confidence = bestScore >= 80 ? 'Çok yüksek' : bestScore >= 45 ? 'Yüksek' : bestScore >= 18 ? 'Orta' : 'Düşük';
  const map = {
    ai:{cat:'AI Özellik', target:'Yönetim Paneli > AI Özellik Ekle', admin:'AI Özellik Ekle', publicTarget:'Yönetim Paneli', reason:'AI/Özellik kelimeleri ve uygulama akışı algılandı.'},
    oyun:{cat:'Oyunlar', target:'Yönetim Paneli > Oyunlar', admin:'Oyunlar', publicTarget:'Oyun Arşivi', reason:'Oyun/kapak/meta/playlist kelimeleri algılandı.'},
    seri:{cat:'Seriler', target:'Yönetim Paneli > Seri İzleme', admin:'Seri İzleme', publicTarget:'Seriler', reason:'Seri/sezon/sıralama kelimeleri algılandı.'},
    takvim:{cat:'Takvim', target:'Yönetim Paneli > Yayın Takvimi', admin:'Yayın Takvimi', publicTarget:'Takvim', reason:'Açık yayın takvimi/tarih/program isteği algılandı.'},
    bildirim:{cat:'Bildirim', target:'Yönetim Paneli > Bildirim Kuyruğu', admin:'Bildirim Kuyruğu', publicTarget:'Bildirimler', reason:'Bildirim/e-posta/kuyruk isteği algılandı.'},
    supabase:{cat:'Supabase', target:'Yönetim Paneli > Schema Geçmişi', admin:'Schema Geçmişi', publicTarget:'Yönetim Paneli', reason:'Supabase/schema/sql isteği algılandı.'},
    deploy:{cat:'Deploy', target:'Yönetim Paneli > Deploy Merkezi', admin:'Deploy Merkezi', publicTarget:'Yönetim Paneli', reason:'Vercel/GitHub/deploy isteği algılandı.'},
    guncelleme:{cat:'Güncelleme', target:'Yönetim Paneli > Güncelleme Notları', admin:'Güncelleme Notları', publicTarget:'Ana Sayfa', reason:'Güncelleme/sürüm notu isteği algılandı.'},
    rapor:{cat:'Raporlar', target:'Yönetim Paneli > Raporlar', admin:'Raporlar', publicTarget:'Ana Sayfa', reason:'Hata/rapor/log isteği algılandı.'},
    profil:{cat:'Profil', target:'Yönetim Paneli > Profil', admin:'Profil', publicTarget:'Profilim', reason:'Profil/avatar/fotoğraf isteği algılandı.'},
    bakim:{cat:'Bakım', target:'Yönetim Paneli > Bakım Modu', admin:'Bakım Modu', publicTarget:'Ana Sayfa', reason:'Bakım modu isteği algılandı.'},
    arayuz:{cat:'Arayüz', target:'Ana Sayfa', admin:'Genel Bakış', publicTarget:'Ana Sayfa', reason:'Arayüz/menü/tasarım isteği algılandı.'}
  };
  const result = map[bestKey] || {cat:'Genel', target:'Yönetim Paneli > Genel Bakış', admin:'Genel Bakış', publicTarget:'Ana Sayfa', reason:'Belirgin hedef yok; güvenli genel panele alındı.'};
  return { ...result, confidence, score:bestScore, routerVersion:HO240F21_INTERNAL_VERSION, raw:original, cleanText:stripped || original, scores };
}
function ho240f21NormalizeSuggestion(item, raw){
  const base = { ...(item || {}) };
  const source = raw || base.originalText || base.title || base.description || base.desc || '';
  const route = ho240f21TargetFromText(source);
  const version = (typeof ho240f17ExtractPlanVersion === 'function' && ho240f17ExtractPlanVersion(source)) || base.version || (typeof ho240f15CleanUiVersion === 'function' ? ho240f15CleanUiVersion() : 'v2.4.1');
  const title = ho240f21VisibleText(base.title || route.cleanText || 'Yeni özel özellik').slice(0,110);
  const desc = `FIX21 profesyonel hedef router bu isteği ${route.target} alanına kilitledi. Kod/özellik yanlış sayfaya yazılmaz; önce hedef doğrulanır, sonra uygulananlar ve ilgili panel kaydı aynı hedefe işlenir.`;
  return {
    ...base,
    key: base.key || `custom_${typeof ho240f15Slug === 'function' ? ho240f15Slug(title) : Date.now()}`,
    title,
    cat: route.cat,
    category: route.cat,
    target: route.target,
    adminPage: route.admin,
    publicTarget: route.publicTarget,
    version,
    routeConfidence: route.confidence,
    routeScore: route.score,
    routeReason: route.reason,
    routerVersion: HO240F21_INTERNAL_VERSION,
    desc: base.desc && String(base.desc).includes('FIX21') ? base.desc : desc,
    description: `${base.description || base.desc || title}\n\nProfesyonel hedef doğrulama: ${route.reason} Hedef: ${route.target}. Güven: ${route.confidence}.`,
    steps: [
      'FIX21 hedef router ile istek analiz edildi; v2.x Planı ifadesindeki Planı kelimesi artık Takvim olarak algılanmaz.',
      `Hedef sayfa kilitlendi: ${route.target}.`,
      `Güven seviyesi: ${route.confidence}. Düşük güven olursa özellik Genel Bakış yerine güvenli önizleme kartı olarak tutulur.`,
      'Siteye Uygula sonrası hem Siteye Uygulandı listesi hem hedef panel aynı adminPage değerini kullanır.',
      'Nereye Eklendiyse Git butonu aynı router ile çalışır ve yanlış sayfaya göndermez.'
    ],
    schemaNeed: base.schemaNeed || (/schema|sql|supabase|tablo|kolon/i.test(source) ? 'schema.sql kontrollü uygulanmalı' : 'Ek schema zorunlu görünmüyor'),
    next: 'Kalıcı backend/API kodu gerekiyorsa sonraki ZIP içinde gerçek dosya değişikliği olarak uygulanır; bu akış yanlış hedefe kart/kayıt yazmaz.'
  };
}
function ho240f21RetargetStoredItems(){
  const fixList = list => (Array.isArray(list) ? list.map(item => {
    const source = `${item?.title || ''} ${item?.description || ''} ${item?.desc || ''}`;
    const route = ho240f21TargetFromText(source);
    const targetNorm = ho240f21Norm(item?.target || '');
    const wrongTakvim = targetNorm.includes('yayin takvimi') && route.cat === 'AI Özellik';
    const aiNeedsRoute = route.cat === 'AI Özellik' && (!item?.adminPage || wrongTakvim || /AI Özellik|ozellik|özellik/i.test(source));
    if(aiNeedsRoute || wrongTakvim){ return ho240f21NormalizeSuggestion(item, source); }
    return item;
  }) : []);
  try{ ho240f15Write(HO240F15_TARGET_PANELS_KEY, fixList(ho240f15AppliedPanels())); }catch{}
  try{
    const applied = typeof ho240Applied === 'function' ? ho240Applied() : ho240f15Read('ho240_ai_applied_features', []);
    const fixed = fixList(applied);
    if(typeof ho240Write === 'function' && typeof HO240_KEYS !== 'undefined') ho240Write(HO240_KEYS.aiApplied, fixed); else ho240f15Write('ho240_ai_applied_features', fixed);
  }catch{}
  try{ localStorage.setItem(HO240F21_ROUTER_KEY, '1'); }catch{}
}
try{
  ho240f15TargetFromText = function(text){ return ho240f21TargetFromText(text); };
}catch(error){ console.warn('FIX21 hedef fonksiyonu bağlanamadı:', error); }
try{
  ho240f14TargetPage = function(target=''){
    const t = ho240f21Norm(target);
    if(t.includes('ai ozellik') || t.includes('ozellik yaz') || t.includes('ozellik ekle') || t.includes('akilli ozellik')) return {type:'admin', page:'AI Özellik Ekle'};
    if(t.includes('deploy') || t.includes('vercel') || t.includes('github') || t.includes('build')) return {type:'admin', page:'Deploy Merkezi'};
    if(t.includes('schema') || t.includes('sql') || t.includes('supabase')) return {type:'admin', page:'Schema Geçmişi'};
    if(t.includes('saglik') || t.includes('health')) return {type:'admin', page:'Sistem Sağlık'};
    if(t.includes('rapor') || t.includes('hata') || t.includes('bug')) return {type:'admin', page:'Raporlar'};
    if(t.includes('oyun iste')) return {type:'admin', page:'Oyun İstekleri'};
    if(t.includes('oyun')) return {type:'admin', page:'Oyunlar'};
    if(t.includes('seri')) return {type:'admin', page:'Seri İzleme'};
    if(t.includes('takvim') || t.includes('yayin')) return {type:'admin', page:'Yayın Takvimi'};
    if(t.includes('bakim')) return {type:'admin', page:'Bakım Modu'};
    if(t.includes('guncelleme')) return {type:'admin', page:'Güncelleme Notları'};
    if(t.includes('profil')) return {type:'admin', page:'Profil'};
    return {type:'admin', page:'Genel Bakış'};
  };
  hoFix10TargetPage = ho240f14TargetPage;
  ho240f10Target = function(target){ return ho240f14TargetPage(target).page; };
}catch(error){ console.warn('FIX21 hedef sayfa fonksiyonu bağlanamadı:', error); }
try{
  const ho240f21PrevAnalyze = ho240f15AnalyzeIdea;
  ho240f15AnalyzeIdea = function(raw){
    let base = {};
    try{ base = ho240f21PrevAnalyze(raw) || {}; }catch{ base = {}; }
    return ho240f21NormalizeSuggestion(base, raw);
  };
}catch(error){ console.warn('FIX21 analiz fonksiyonu bağlanamadı:', error); }
try{
  const ho240f21PrevIdeaForm = ho240f15IdeaForm;
  ho240f15IdeaForm = function(){
    try{
      const idea = localStorage.getItem(HO240F15_CUSTOM_IDEA_KEY) || '';
      const suggestion = ho240f15Read(HO240F15_CUSTOM_SUGGESTION_KEY, null);
      if(suggestion){ ho240f15Write(HO240F15_CUSTOM_SUGGESTION_KEY, ho240f21NormalizeSuggestion(suggestion, idea || suggestion.title)); }
    }catch{}
    const html = String(ho240f21PrevIdeaForm());
    const routerInfo = `<div class="ho240f21Guard"><b>FIX21 Profesyonel Hedef Koruması Aktif</b><span>Planı kelimesi Takvim sayılmaz.</span><span>AI/Özellik istekleri AI Özellik Ekle alanına kilitlenir.</span><span>Nereye Eklendiyse Git aynı hedef router ile çalışır.</span></div>`;
    return html
      .replace('<span class="pill green">Yaz → Öner → Uygula → F5</span>', '<span class="pill green">Yaz → Öner → Uygula → F5</span><span class="pill">Profesyonel Router</span>')
      .replace('</form>', `</form>${routerInfo}`)
      .replace('Sistem hedef sayfayı, nasıl çalışması gerektiğini ve riskini otomatik önerir.', 'Sistem hedef sayfayı profesyonel router ile doğrular; yanlış sayfaya ekleme yapmaz, nasıl çalışması gerektiğini ve riskini otomatik önerir.');
  };
}catch(error){ console.warn('FIX21 form katmanı bağlanamadı:', error); }
try{
  const ho240f21PrevApplyCustom = ho240f15ApplyCustomFeature;
  ho240f15ApplyCustomFeature = function(){
    try{
      const idea = localStorage.getItem(HO240F15_CUSTOM_IDEA_KEY) || '';
      const suggestion = ho240f15Read(HO240F15_CUSTOM_SUGGESTION_KEY, null);
    ...
    return ho240f21PrevApplyCustom();
  };
}catch(error){ console.warn('FIX21 uygulama katmanı bağlanamadı:', error); }
try{
  const ho240f21PrevPanelForAdmin = ho240f15PanelForAdmin;
  ho240f15PanelForAdmin = function(adminPage){
    try{ ho240f21RetargetStoredItems(); }catch{}
    return ho240f21PrevPanelForAdmin(adminPage);
  };
}catch(error){ console.warn('FIX21 hedef panel katmanı bağlanamadı:', error); }
try{
  const ho240f21PrevBind = bind;
  bind = function(){
    try{ ho240f21PrevBind(); }catch(err){ console.warn('FIX21 önceki bind atlandı:', err); }
    document.querySelectorAll('[data-ho-ai-go],[data-ho240f10-ai-go]').forEach(btn=>{
      if(btn.dataset.ho240f21Bound) return;
      btn.dataset.ho240f21Bound = '1';
      btn.addEventListener('click', e=>{
        e.preventDefault(); e.stopImmediatePropagation();
        const target = btn.dataset.hoAiGo || btn.dataset.ho240f10AiGo || 'Genel Bakış';
        const dest = ho240f14TargetPage(target);
        if(dest.type === 'admin') adminNavigate(dest.page); else navigate(dest.page);
        setToast(`${target} alanı profesyonel router ile açıldı.`);
      }, true);
    });
  };
}catch(error){ console.warn('FIX21 bind katmanı bağlanamadı:', error); }
try{
  const fix21Css = document.createElement('style');
  fix21Css.textContent = `.ho240f21Guard{display:flex;flex-wrap:wrap;gap:10px;margin:14px 0;padding:12px;border:1px solid rgba(34,197,94,.35);border-radius:16px;background:rgba(16,185,129,.08);color:#d8ffe9}.ho240f21Guard b{width:100%;font-size:14px}.ho240f21Guard span{font-size:12px;padding:6px 10px;border-radius:999px;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.09)}`;
  document.head.appendChild(fix21Css);
}catch{}
try{
  ho240f21RetargetStoredItems();
  if(Array.isArray(VERSION_NOTES_ARCHIVE) && !VERSION_NOTES_ARCHIVE.some(n=>n.version===HO240F21_INTERNAL_VERSION)){
    VERSION_NOTES_ARCHIVE.unshift({
      version: HO240F21_INTERNAL_VERSION,
      title: 'AI Özellik Profesyonel Hedef Router Fix',
      summary: 'Özellik Yaz alanında AI özellikleri yanlış sayfaya ekleyen hedef algılama düzeltildi. v2.x Planı ifadesindeki Planı kelimesi artık Yayın Takvimi sayılmaz; AI/Özellik istekleri AI Özellik Ekle alanına kilitlenir.',
      image: 'previews/hayatimiz-oyun-v240-fix21-ai-router-preview.png',
      written: 'FIX21 ile Siteye Uygula, Nereye Eklendiyse Git, Siteye Uygulandı listesi ve hedef panel kayıtları aynı profesyonel router üzerinden çalışır. Yanlış hedefe kart basan eski kayıtlar açılışta düzeltilir.'
    });
  }
  if(document.getElementById('root')?.dataset?.mounted === '1') render();
}catch(error){ console.warn('FIX21 başlangıç katmanı atlandı:', error); }


/* v2.4.0 FIX 22 - AI ile özellik yazma/önerme tamamen kaldırıldı */
const HO240F22_INTERNAL_VERSION = 'v2.4.0 FIX 22';
const HO240F22_STATUS_KEY = 'ho240f22_deploy_state';
const HO240F22_UPDATES_KEY = 'ho240f22_top_updates';
const HO240F22_REMOVED_ADMIN_PAGES = new Set(['AI Özellik Ekle','AI Özellik Merkezi','Özellik Yaz / AI Uygula','Özellik Planı','Uygulama Merkezi']);
function ho240f22Version(){
  try{ return (typeof ho240f15CleanUiVersion === 'function' ? ho240f15CleanUiVersion() : (localStorage.getItem('ho240f14_current_site_version') || localStorage.getItem('ho240f11_next_version') || 'v2.4.1')); }
  catch{ return 'v2.4.1'; }
}
function ho240f22Read(key, fallback){ try{ const raw = localStorage.getItem(key); return raw ? JSON.parse(raw) : fallback; }catch{ return fallback; } }
function ho240f22Write(key, value){ try{ localStorage.setItem(key, JSON.stringify(value)); }catch{} return value; }
function ho240f22IsRemovedPage(page){ return HO240F22_REMOVED_ADMIN_PAGES.has(String(page || '').trim()); }
function ho240f22CleanText(value){ return String(value || '').toLocaleLowerCase('tr-TR').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/ı/g,'i'); }
function ho240f22LooksAi(value){
  const t = ho240f22CleanText(value);
  return /\b(ai|yapay zeka)\b/.test(t) || t.includes('ai ozellik') || t.includes('ozellik yaz') || t.includes('ozellik oner') || t.includes('yeni oneriler oner') || t.includes('siteye uygulandi') || t.includes('akilli ozellik') || t.includes('ozellik plani');
}
function ho240f22CleanLocalAiData(){
  try{
    const removeExact = new Set([
      'hayatimiz_smart_feature_draft','hayatimiz_feature_overrides_stable','ho240_ai_applied_features','ho240_ai_view','ho240_ai_cat','ho240_ai_search',
      'ho240f15_custom_feature_idea','ho240f15_custom_feature_suggestion','ho240f15_target_feature_panels','ho240f15_after_apply_target',
      'ho240f21_professional_router_enabled','v223_fix3_ai_applied','v223_ai_applied','v224_ai_applied','v224_sql_drafts'
    ]);
    const removePrefixes = ['ho240f12_','ho240f13_','ho240f15_custom_','ho240f15_target_','ho240f15_after_','ho240f21_','v223_ai_','v223_fix3_ai_','v224_ai_'];
    const keys = [];
    for(let i=0;i<localStorage.length;i++){ const k = localStorage.key(i); if(k) keys.push(k); }
    keys.forEach(k=>{
      if(removeExact.has(k) || removePrefixes.some(p=>k.startsWith(p))){ try{ localStorage.removeItem(k); }catch{} }
    });
    try{
      const featureMap = safeParse(localStorage.getItem(FEATURE_CACHE_KEY) || '{}', {});
      ['feature_edit_delete','active_features_bulk_clear','apply_and_refresh_flow','ai_feature_replace_flow','ai_new_suggestions_request_panel','ai_suggestion_category_focus'].forEach(k=>{ delete featureMap[k]; });
      if(state?.features){ ['feature_edit_delete','active_features_bulk_clear','apply_and_refresh_flow','ai_feature_replace_flow','ai_new_suggestions_request_panel','ai_suggestion_category_focus'].forEach(k=>{ delete state.features[k]; }); }
      localStorage.setItem(FEATURE_CACHE_KEY, JSON.stringify(featureMap));
    }catch{}
    try{
      if(Array.isArray(state?.planner)) state.planner = state.planner.filter(p=>!ho240f22LooksAi(`${p.group || ''} ${p.group_name || ''} ${p.title || ''} ${p.text || ''} ${p.featureKey || ''}`));
    }catch{}
    try{
      if(Array.isArray(FEATURE_CATALOG)){
        const keep = FEATURE_CATALOG.filter(f=>!ho240f22LooksAi(`${f.key || ''} ${f.group || ''} ${f.target || ''} ${f.title || ''}`));
        FEATURE_CATALOG.splice(0, FEATURE_CATALOG.length, ...keep);
      }
    }catch{}
    try{ state.aiSuggestions = null; }catch{}
    try{
      if(Array.isArray(VERSION_NOTES_ARCHIVE)){
        const keepNotes = VERSION_NOTES_ARCHIVE.filter(n => n?.version === HO240F22_INTERNAL_VERSION || !ho240f22LooksAi(`${n?.title || ''} ${n?.summary || ''} ${n?.written || ''}`));
        VERSION_NOTES_ARCHIVE.splice(0, VERSION_NOTES_ARCHIVE.length, ...keepNotes);
      }
    }catch{}
  }catch(error){ console.warn('FIX22 AI local temizlik atlandı:', error); }
}
function ho240f22SetVersion(v){
  const clean = String(v || ho240f22Version() || 'v2.4.1').trim().replace(/\s*-?\s*v?2\.4\.0\s*FIX\s*\d+.*$/i,'') || 'v2.4.1';
  try{ if(typeof ho240f15SetVersion === 'function') ho240f15SetVersion(clean); else if(typeof ho240f14SetVersion === 'function') ho240f14SetVersion(clean); }catch{}
  try{
    localStorage.setItem('ho240f14_current_site_version', clean);
    localStorage.setItem('ho240f11_next_version', clean);
    localStorage.setItem('hayatimiz_last_public_version', clean);
    localStorage.setItem('hayatimiz_last_fix_version', HO240F22_INTERNAL_VERSION);
    document.title = `Hayatımız Oyun - ${clean}`;
  }catch{}
  return clean;
}
function ho240f22ReadVersionInput(){
  try{
    const full = String(document.getElementById('ho240f13VersionInput')?.value || document.getElementById('ho240f11VersionInput')?.value || ho240f22Version()).trim();
    const num = String(document.getElementById('ho240f15VersionNumberInput')?.value || '').trim();
    if(num && /^\d+$/.test(num) && typeof ho240f15VersionPrefix === 'function') return `${ho240f15VersionPrefix(full)}${num}`;
    return full.startsWith('v') ? full : `v${full}`;
  }catch{ return ho240f22Version(); }
}
function ho240f22Status(){ return ho240f22Read(HO240F22_STATUS_KEY, {updates:false,github:false,supabase:false,deploy:false,last:'AI özellik sistemi kaldırıldı; sadece manuel Deploy/Supabase kontrolü aktif.'}); }
function ho240f22SaveStatus(patch){ return ho240f22Write(HO240F22_STATUS_KEY, {...ho240f22Status(), ...patch}); }
function ho240f22MakeUpdates(){
  const version = ho240f22Version();
  const now = new Date().toLocaleString('tr-TR');
  const list = [
    {cat:'Oyunlar', title:'Oyun ekle/düzenle formları stabil kontrol edildi', target:'Yönetim Paneli > Oyunlar', createdAt:now},
    {cat:'Kapak', title:'Kapak ve meta çekme alanları oyun formunda tutuldu', target:'Yönetim Paneli > Oyunlar', createdAt:now},
    {cat:'Seriler', title:'Seri sıralama ve durum butonları korunuyor', target:'Yönetim Paneli > Seri İzleme', createdAt:now},
    {cat:'Takvim', title:'Yayın takvimi kayıtları AI olmadan manuel yönetilir', target:'Yönetim Paneli > Yayın Takvimi', createdAt:now},
    {cat:'Deploy', title:'Deploy merkezi AI tanı olmadan sadeleştirildi', target:'Yönetim Paneli > Deploy Merkezi', createdAt:now},
    {cat:'Supabase', title:'Schema geri bildirimi manuel kayıt modunda çalışır', target:'Yönetim Paneli > Schema Geçmişi', createdAt:now},
    {cat:'Raporlar', title:'Oyun istekleri ve hata bildirimleri panelde tutulur', target:'Yönetim Paneli > Raporlar', createdAt:now},
    {cat:'Sürüm', title:'Site sürümü yalnızca seçilen public sürümü gösterir', target:'Üst bar ve yönetim paneli', createdAt:now},
    {cat:'Temizlik', title:'AI özellik yazma/önerme kayıtları temizlendi', target:'Yönetim Paneli', createdAt:now},
    {cat:'Güncelleme', title:'Güncelleme notları normal editör üzerinden yönetilir', target:'Yönetim Paneli > Güncelleme Notları', createdAt:now}
  ].map(x=>({...x, version}));
  return ho240f22Write(HO240F22_UPDATES_KEY, list);
}
function ho240f22DeployCenter(){
  const version = ho240f22Version();
  const s = ho240f22Status();
  const plans = ho240f22Read(HO240F22_UPDATES_KEY, []);
  const schemaText = localStorage.getItem('ho240f11_schema_feedback') || '';
  const rows = [
    ['updates','Güncellemeler arandı',s.updates,'Seçilen sürüm için AI içermeyen güncelleme kontrol listesi yenilenir.'],
    ['github','GitHub yükleme hazır',s.github,'02-githuba-otomatik-gonder.bat ile temiz force push sonrası Vercel otomatik deploy beklenir.'],
    ['supabase','Supabase schema kontrolü',s.supabase,'Yeni tablo/kolon gerekiyorsa schema.sql veya schema geri bildirimi manuel takip edilir.'],
    ['deploy','Vercel deploy kontrolü',s.deploy,'Hazır dist klasörü yayınlanır; build bekleme sorunu için statik çıktı kontrol edilir.']
  ];
  return `<section class="ho240f13DeployPanel ho240f15DeployPanel ho240f22DeployPanel"><div class="card wide ho240Hero"><div><span class="eyebrow">${esc(version)}</span><h1>Deploy Merkezi</h1><p class="muted">AI ile özellik yazma, önerme, uygulama ve AI tanı akışı kaldırıldı. Bu ekran sadece sürüm, GitHub, Vercel ve Supabase durumunu yönetir.</p></div><div class="rowActions"><button class="btn primary" data-action="ho240f22-scan-updates">Güncellemeleri Kontrol Et</button><button class="btn" data-action="ho240f13-github-upload">GitHub’a Yükle</button></div></div><section class="card wide"><div class="ho240f11DeployGrid ho240f15VersionGrid"><label class="field">Yeni güncelleme versiyonu<input id="ho240f13VersionInput" value="${esc(version)}" placeholder="Örn: v2.4.1"></label><label class="field ho240f15SmallVersion">Sadece son sayı<input id="ho240f15VersionNumberInput" type="number" min="0" step="1" value="${esc(typeof ho240f15LastNumber === 'function' ? ho240f15LastNumber(version) : '1')}" placeholder="1"></label><label class="field">Schema / yeni tablo geri bildirimi<textarea id="ho240f13SchemaFeedback" rows="5" placeholder="Yeni tablo veya kolon gerekiyorsa yaz. Manuel schema notu olarak kaydedilir.">${esc(schemaText)}</textarea></label></div><div class="rowActions"><button class="btn primary" data-action="ho240f13-save-version">Versiyonu Güncelle</button><button class="btn" data-action="ho240f13-mark-supabase">Supabase Kontrol Edildi</button><button class="btn" data-action="ho240f22-mark-deploy">Deploy Kontrol Edildi</button></div></section><div class="hoFix7StatusGrid">${rows.map(([key,label,done,note])=>`<article class="${done?'done':''}"><b>${esc(label)}</b><span>${done?'Başarılı':'Bekliyor'}</span><small>${esc(note)}</small></article>`).join('')}</div><section class="card wide ho240f13UpdateList"><div class="sectionHead"><div><h2>Tepedeki Güncellemeler</h2><p class="muted">AI önerileri kaldırıldı. Burada sadece manuel kontrol maddeleri görünür.</p></div><span class="pill green">${plans.length} kayıt</span></div><div class="ho240FeatureGrid">${plans.slice(0,10).map(p=>`<article class="ho240Feature"><span class="pill">${esc(p.cat)}</span><h3>${esc(p.title)}</h3><p>${esc(p.target)} alanına kontrol maddesi olarak eklendi.</p><small>${esc(version)} • ${esc(p.createdAt)}</small></article>`).join('') || '<p class="muted">Henüz kontrol yapılmadı. Güncellemeleri Kontrol Et butonuna bas.</p>'}</div></section><section class="card wide"><h2>Son Durum</h2><p class="muted">${esc(s.last || 'Henüz işlem yok.')}</p></section></section>`;
}
function ho240f22CleanOverview(){
  const v = ho240f22Version();
  return `<section class="grid stats adminStats"><div class="card"><b>Oyun</b><h3>${state.games.length}</h3><span class="muted">arşiv verisi</span></div><div class="card"><b>Sürüm</b><h3>${esc(v)}</h3><span class="muted">public sürüm</span></div><div class="card"><b>Veri durumu</b><h3>${state.runtimeLoaded?'Bağlandı':'Local'}</h3><span class="muted">Supabase/API</span></div><div class="card"><b>AI Özellik</b><h3>Kapalı</h3><span class="muted">tamamen kaldırıldı</span></div></section><section class="card wide"><h2>AI özellik sistemi kaldırıldı</h2><p class="muted">Özellik Yaz, Yeni Öneriler, Siteye Uygulandı, AI Özellik Ekle, AI Özellik Merkezi, Özellik Planı ve Uygulama Merkezi menüleri artık görünmez. Yeni istekler gerçek ZIP güncellemesi olarak hazırlanacak.</p><div class="rowActions"><button class="btn primary" data-admin="Oyunlar">Oyunlar Sekmesini Aç</button><button class="btn" data-admin="Deploy Merkezi">Deploy Merkezini Aç</button></div></section>`;
}
function ho240f22RemovedPageNotice(){
  return `<section class="card wide"><h2>Bu AI paneli kaldırıldı</h2><p class="muted">AI ile özellik yazma/önerme/uygulama sistemi iptal edildi. Yeni özellikler artık site içinden otomatik eklenmez; ZIP paketi içinde gerçek kod değişikliği olarak hazırlanır.</p><button class="btn primary" data-admin="Genel Bakış">Genel Bakışa Dön</button></section>`;
}
function ho240f22AdminBody(){
  if(ho240f22IsRemovedPage(state.adminPage)) return ho240f22RemovedPageNotice();
  if(state.adminPage === 'Genel Bakış') return ho240f22CleanOverview();
  if(state.adminPage === 'Kullanıcı Yetkileri') return usersPanel();
  if(state.adminPage === 'Bakım Modu') return maintenanceAdmin();
  if(state.adminPage === 'Güncelleme Notları') return updateNotes();
  if(state.adminPage === 'Profil') return profilePage();
  ...
  if(state.adminPage === 'Deploy Merkezi') return ho240f22DeployCenter();
  ...
  return ho240f22CleanOverview();
}
function ho240f22AdminPanel(){
  if(!isStaff()) return `<section class="card"><h2>Yetki gerekiyor</h2><p>Yönetim paneli sadece yetkili hesaplara görünür.</p></section>`;
  const links = [
    ['Genel Bakış','Özet'],['Oyunlar','Oyun ekle/düzenle'],['Seri İzleme','Serileri sırala'],['Seri Geçmişi','Karşılaştır / geri al'],['Yayın Takvimi','Takvim görünümü'],['Bildirim Kuyruğu','Tarayıcı/e-posta'],['Oyun İstekleri','Gelen istekler'],['Hata Bildir','Gelen hatalar'],['Raporlar','Filtre/dışa aktar'],['Deploy Merkezi','GitHub/Vercel'],['Schema Geçmişi','SQL zaman çizelgesi'],['Bakım Modu','Yol haritası'],['Yönetim Kısayolları','Role göre'],['Sistem Sağlık','Tek tık kontrol'],['Güncelleme Notları','Sürüm notları'],['API/ENV Durumu','Bağlantı'],['Ayarlar','Tema/tercihler']
  ];
  if(!links.some(([p])=>p===state.adminPage) || ho240f22IsRemovedPage(state.adminPage)){
    state.adminPage='Genel Bakış';
    try{ localStorage.setItem(ADMIN_TAB_KEY, state.adminPage); }catch{}
  }
  const version = ho240f22Version();
  const headerActions = state.adminPage === 'Oyunlar'
    ? `<div class="rowActions"><button class="btn" data-action="toggle-game-form">${state.showGameForm?'Formu Gizle':'Yeni Oyun Ekle'}</button><button class="btn primary" data-action="bulk-sync-playlists">Toplu İşlemler</button></div>`
    : state.adminPage === 'Deploy Merkezi'
      ? `<span class="pill green">AI kapalı</span>`
      : `<span class="pill ${state.maintenance?.enabled?'banned':'green'}">${state.maintenance?.enabled?'Bakım açık':'Bakım kapalı'}</span>`;
  return `<section class="fix5AdminShell ho240AdminShell ho240f22AdminShell"><aside class="fix5AdminSidebar"><div class="sideLogo"><span class="logoMark">🎮</span><div><b>Hayatımız Oyun</b><small>${esc(version)} Yönetim</small></div></div><div class="sideNavLabel">MENÜ</div><button class="sideNavItem" data-page="Ana Sayfa"><span>⌂</span>Ana Sayfa</button><button class="sideNavItem" data-page="Oyun Arşivi"><span>🎮</span>Oyun Arşivi</button><button class="sideNavItem" data-page="Seriler"><span>◈</span>Seriler</button><button class="sideNavItem" data-page="Takvim"><span>▣</span>Yayın Takvimi</button><div class="sideNavLabel">YÖNETİM</div><div class="adminAccordion open"><button class="adminAccordionHead active"><span>👑</span><div><b>Yönetim Paneli</b><small>${esc(state.session?.full_name||'Yetkili')}</small></div><strong>⌄</strong></button><div class="adminAccordionBody">${links.map(([page,cap])=>`<button class="adminSubLink ${state.adminPage===page?'active':''}" data-admin="${esc(page)}"><span class="subDot"></span><div><b>${esc(page)}</b><small>${esc(cap)}</small></div></button>`).join('')}</div></div></aside><div class="adminContent fix5AdminContent"><div class="fix5AdminHeader"><div><div class="adminBreadcrumb">Yönetim Paneli <span>›</span> ${esc(state.adminPage)}</div><h1>${esc(state.adminPage)}</h1><p>${state.adminPage==='Genel Bakış'?'AI özellik sistemi kaldırıldı; yönetim paneli sade moda alındı.':adminSubtitle(state.adminPage)}</p></div>${headerActions}</div>${ho240f22AdminBody()}</div></section>`;
}
try{
  ho240f22CleanLocalAiData();
  adminBody = ho240f22AdminBody;
  adminPanel = ho240f22AdminPanel;
  ho240DeployCenter = ho240f22DeployCenter;
  ho240f13DeployCenter = ho240f22DeployCenter;
  ho240f15DeployCenter = ho240f22DeployCenter;
  hoFix8DeployPanel = ho240f22DeployCenter;
}catch(error){ console.warn('FIX22 ana override uygulanamadı:', error); }
try{
  const ho240f22PrevAdminNavigate = adminNavigate;
  adminNavigate = function(page){
    const cleanPage = ho240f22IsRemovedPage(page) ? 'Genel Bakış' : page;
    state.page = 'Yönetim Paneli';
    state.adminPage = cleanPage || 'Genel Bakış';
    if(state.adminPage === 'Oyunlar' && !state.editingGameId) state.showGameForm = true;
    ...
    try{ if(state.adminPage === 'Kullanıcı Yetkileri') loadUsers(); }catch{}
    if(ho240f22IsRemovedPage(page)) setToast('AI özellik yazma/önerme paneli kaldırıldı.');
    render();
  };
}catch(error){ console.warn('FIX22 adminNavigate override atlandı:', error); }
try{
  const ho240f22PrevAdminNameFromSlug = adminNameFromSlug;
  adminNameFromSlug = function(slug){
    const name = ho240f22PrevAdminNameFromSlug(slug);
    return ho240f22IsRemovedPage(name) ? 'Genel Bakış' : name;
  };
}catch{}
try{
  const ho240f22PrevTopbar = topbar;
  topbar = function(){
    return String(ho240f22PrevTopbar())
      .replace(/v2\.4\.\d+\s*[-•]\s*v2\.4\.0\s*FIX\s*\d+[^<]*/gi, ho240f22Version())
      .replace(/v2\.4\.0\s*FIX\s*\d+[^<]*/gi, ho240f22Version())
      .replace(/AI\s*Özellik[^<]*/gi, 'Yönetim')
      .replace(/Özellik Yaz\s*\/\s*AI Uygula/gi, 'Yönetim');
  };
}catch{}
try{
  const ho240f22PrevOnAction = onAction;
  onAction = async function(e){
    const action = e.currentTarget?.dataset?.action || '';
    if(['ho240f13-ai-suggest-open','ho240f10-ai-new','ho240f10-ai-applied','ho240f10-ai-refresh','v224-refresh-ai-suggestions','ho240f15-reset-custom','ho240f15-apply-custom'].includes(action)){
      e.preventDefault(); e.stopImmediatePropagation();
      ho240f22CleanLocalAiData();
      adminNavigate('Genel Bakış');
      setToast('AI ile özellik yazma/önerme sistemi kaldırıldı.');
      return;
    }
    if(action === 'ho240f13-save-version' || action === 'ho240f11-save-version'){
      e.preventDefault(); e.stopImmediatePropagation();
      const v = ho240f22SetVersion(ho240f22ReadVersionInput());
      ho240f22SaveStatus({last:`Versiyon ${v} olarak kaydedildi. AI özellik sistemi kapalı kalır.`});
      render(); setToast(`Sürüm ${v} olarak güncellendi.`); return;
    }
    if(action === 'ho240f13-scan-updates' || action === 'ho240f22-scan-updates'){
      e.preventDefault(); e.stopImmediatePropagation();
      const v = ho240f22SetVersion(ho240f22ReadVersionInput());
      const list = ho240f22MakeUpdates();
      ho240f22SaveStatus({updates:true,last:`${v} için AI içermeyen ${list.length} kontrol maddesi ${new Date().toLocaleString('tr-TR')} tarihinde yenilendi.`});
      render(); setToast('AI içermeyen güncelleme kontrol listesi yenilendi.'); return;
    }
    if(action === 'ho240f13-mark-supabase' || action === 'ho240f11-mark-supabase'){
      e.preventDefault(); e.stopImmediatePropagation();
      const feedback = String(document.getElementById('ho240f13SchemaFeedback')?.value || document.getElementById('ho240f11SchemaFeedback')?.value || '').trim();
      if(feedback) localStorage.setItem('ho240f11_schema_feedback', feedback);
      ho240f22SaveStatus({supabase:true,last:`Supabase kontrolü ${new Date().toLocaleString('tr-TR')} tarihinde manuel olarak işaretlendi.`});
      try{ if(feedback) await api('schema-feedback-add',{feedback:{version:ho240f22Version(),source:'deploy_center_manual',message:feedback}}); }catch{}
      render(); setToast('Supabase kontrolü kaydedildi.'); return;
    }
    if(action === 'ho240f22-mark-deploy' || action === 'ho240f13-auto-flow' || action === 'ho240f11-auto-flow'){
      e.preventDefault(); e.stopImmediatePropagation();
      ho240f22SaveStatus({deploy:true,last:`Deploy kontrolü ${new Date().toLocaleString('tr-TR')} tarihinde manuel olarak işaretlendi.`});
      render(); setToast('Deploy kontrolü kaydedildi.'); return;
    }
    if(action === 'ho240f13-github-upload' || action === 'ho240f11-github-upload'){
      e.preventDefault(); e.stopImmediatePropagation();
      ho240f22SaveStatus({github:true,last:`GitHub yükleme ${new Date().toLocaleString('tr-TR')} tarihinde sıraya alındı. 02-githuba-otomatik-gonder.bat ile temiz yükleme yap.`});
      render(); setToast('GitHub yükleme durumu kaydedildi.'); return;
    }
    return ho240f22PrevOnAction(e);
  };
}catch(error){ console.warn('FIX22 onAction override atlandı:', error); }
try{
  const ho240f22PrevBind = bind;
  bind = function(){
    try{ ho240f22PrevBind(); }catch(err){ console.warn('FIX22 önceki bind atlandı:', err); }
    document.querySelectorAll('[data-ho240f13-ai-apply],[data-ho240-ai-apply],[data-v223-ai-apply],[data-v224-ai-apply],[data-ho240f10-ai-apply],[data-ho-ai-go],[data-ho240f10-ai-go]').forEach(btn=>{
      if(btn.dataset.ho240f22Blocked) return;
      btn.dataset.ho240f22Blocked = '1';
      btn.addEventListener('click', e=>{ e.preventDefault(); e.stopImmediatePropagation(); ho240f22CleanLocalAiData(); adminNavigate('Genel Bakış'); setToast('AI özellik uygulama/önerme sistemi kaldırıldı.'); }, true);
    });
  };
}catch(error){ console.warn('FIX22 bind override atlandı:', error); }
try{
  const style = document.createElement('style');
  style.textContent = `.ho240f22AdminShell .adminSubLink b,.ho240f22DeployPanel h1{letter-spacing:-.02em}.ho240f22DeployPanel .ho240Feature{border-color:rgba(56,189,248,.25)}.ho240f22DeployPanel .ho240Hero{background:linear-gradient(135deg,rgba(15,23,42,.96),rgba(8,47,73,.55))}.ho240f22AdminShell [data-admin="AI Özellik Ekle"],.ho240f22AdminShell [data-admin="AI Özellik Merkezi"],.ho240f22AdminShell [data-admin="Özellik Planı"],.ho240f22AdminShell [data-admin="Uygulama Merkezi"]{display:none!important}`;
  document.head.appendChild(style);
}catch{}
try{
  ho240f22SetVersion(ho240f22Version());
  ho240f22CleanLocalAiData();
  if(Array.isArray(VERSION_NOTES_ARCHIVE) && !VERSION_NOTES_ARCHIVE.some(n=>n.version===HO240F22_INTERNAL_VERSION)){
    VERSION_NOTES_ARCHIVE.unshift({
      version:HO240F22_INTERNAL_VERSION,
      title:'AI Özellik Yazma / Önerme Sistemi Kaldırıldı',
      summary:'AI ile özellik yazma, önerme, siteye uygulama, AI Özellik Ekle, AI Özellik Merkezi, Özellik Planı ve Uygulama Merkezi menüleri kaldırıldı. Deploy Merkezi sade GitHub/Vercel/Supabase kontrol ekranına çevrildi.',
      image:'previews/hayatimiz-oyun-v240-fix22-ai-temizleme-preview.png',
      written:'FIX22 ile site içinden otomatik özellik ekleyen tüm AI akışları kapatıldı. Yeni özellikler artık bu ZIP paketlerinde gerçek kod değişikliği olarak uygulanacak.'
    });
  }
  if(document.getElementById('root')?.dataset?.mounted === '1') render();
}catch(error){ console.warn('FIX22 başlangıç katmanı atlandı:', error); }


/* v2.4.0 FIX 23 - Deploy/Redeploy/GitHub/Vercel paneli tamamen kaldırıldı */
const HO240F23_INTERNAL_VERSION = 'v2.4.0 FIX 23';
const HO240F23_REMOVED_ADMIN_PAGES = new Set([
  'AI Özellik Ekle','AI Özellik Merkezi','Özellik Yaz / AI Uygula','Özellik Planı','Uygulama Merkezi',
  'Deploy Merkezi','Redeploy / AI Tanı Merkezi','Redeploy Merkezi','Deploy Kontrol Listesi','Deploy Tanı','Vercel Deploy','GitHub / Vercel Deploy Kontrol Listesi'
]);
function ho240f23IsRemovedPage(page){
  const raw = String(page || '').trim();
  const t = raw.toLocaleLowerCase('tr-TR');
  return HO240F23_REMOVED_ADMIN_PAGES.has(raw) || t.includes('deploy') || t.includes('redeploy') || t.includes('vercel') || t.includes('github');
}
function ho240f23Version(){
  try{ return (typeof ho240f22Version === 'function' ? ho240f22Version() : (localStorage.getItem('ho240f14_current_site_version') || localStorage.getItem('ho240f11_next_version') || 'v2.4.1')); }
  catch{ return 'v2.4.1'; }
}
function ho240f23CleanLocalData(){
  try{
    const removeExact = new Set([
      'ho240f22_deploy_state','ho240f22_top_updates','hayatimiz_v224_deploy_checklist','hayatimiz_deploy_checklist','hayatimiz_redeploy_requests','hayatimiz_deploy_events',
      'v224_deploy_checklist','ho240_deploy_state','ho240_redeploy_state'
    ]);
    const removePrefixes = ['deploy_','redeploy_','vercel_','github_deploy_','ho240f13_deploy_','ho240f22_deploy_'];
    const keys = [];
    for(let i=0;i<localStorage.length;i++){ const k = localStorage.key(i); if(k) keys.push(k); }
    keys.forEach(k=>{ if(removeExact.has(k) || removePrefixes.some(p=>k.toLocaleLowerCase('tr-TR').startsWith(p))){ try{ localStorage.removeItem(k); }catch{} } });
    if(ho240f23IsRemovedPage(localStorage.getItem(ADMIN_TAB_KEY))){ localStorage.setItem(ADMIN_TAB_KEY, 'Genel Bakış'); }
    if(ho240f23IsRemovedPage(state?.adminPage)) state.adminPage = 'Genel Bakış';
  }catch(error){ console.warn('FIX23 local temizlik atlandı:', error); }
}
function ho240f23Overview(){
  const v = ho240f23Version();
  return `<section class="grid stats adminStats"><div class="card"><b>Oyun</b><h3>${state.games.length}</h3><span class="muted">arşiv verisi</span></div><div class="card"><b>Sürüm</b><h3>${esc(v)}</h3><span class="muted">public sürüm</span></div><div class="card"><b>Veri durumu</b><h3>${state.runtimeLoaded?'Bağlandı':'Local'}</h3><span class="muted">Supabase/API</span></div><div class="card"><b>Ek özellik panelleri</b><h3>Kapalı</h3><span class="muted">sade yönetim</span></div></section><section class="card wide"><h2>Yönetim paneli sadeleştirildi</h2><p class="muted">AI ile özellik yazma/önerme akışları ve site içinden yayına alma kontrol panelleri kaldırıldı. Bundan sonraki yeni istekler bu ZIP paketlerinde gerçek kod değişikliği olarak hazırlanacak.</p><div class="rowActions"><button class="btn primary" data-admin="Oyunlar">Oyunlar Sekmesini Aç</button><button class="btn" data-admin="Güncelleme Notları">Güncelleme Notlarını Aç</button></div></section>`;
}
function ho240f23RemovedPageNotice(){
  return `<section class="card wide"><h2>Bu panel kaldırıldı</h2><p class="muted">Bu alan artık sitede kullanılmıyor. Yeni güncelleme ve düzeltmeler site içinden otomatik çalıştırılmayacak; ZIP paketiyle hazırlanacak.</p><button class="btn primary" data-admin="Genel Bakış">Genel Bakışa Dön</button></section>`;
}
function ho240f23AdminBody(){
  if(ho240f23IsRemovedPage(state.adminPage)) return ho240f23RemovedPageNotice();
  if(state.adminPage === 'Genel Bakış') return ho240f23Overview();
  if(state.adminPage === 'Kullanıcı Yetkileri') return usersPanel();
  if(state.adminPage === 'Bakım Modu') return maintenanceAdmin();
  if(state.adminPage === 'Güncelleme Notları') return updateNotes();
  if(state.adminPage === 'Profil') return profilePage();
  ...
  if(state.adminPage === 'API/ENV Durumu') return apiStatus();
  if(state.adminPage === 'Ayarlar') return (typeof ho240SettingsPanel === 'function' ? ho240SettingsPanel() : settingsPanel());
  return ho240f23Overview();
}
function ho240f23AdminPanel(){
  if(!isStaff()) return `<section class="card"><h2>Yetki gerekiyor</h2><p>Yönetim paneli sadece yetkili hesaplara görünür.</p></section>`;
  const links = [
    ['Genel Bakış','Özet'],['Oyunlar','Oyun ekle/düzenle'],['Seri İzleme','Serileri sırala'],['Seri Geçmişi','Karşılaştır / geri al'],['Yayın Takvimi','Takvim görünümü'],['Bildirim Kuyruğu','Tarayıcı/e-posta'],['Oyun İstekleri','Gelen istekler'],['Hata Bildir','Gelen hatalar'],['Raporlar','Filtre/dışa aktar'],['Schema Geçmişi','SQL zaman çizelgesi'],['Bakım Modu','Yol haritası'],['Yönetim Kısayolları','Role göre'],['Sistem Sağlık','Tek tık kontrol'],['Güncelleme Notları','Sürüm notları'],['API/ENV Durumu','Bağlantı'],['Ayarlar','Tema/tercihler']
  ];
  if(!links.some(([p])=>p===state.adminPage) || ho240f23IsRemovedPage(state.adminPage)){
    state.adminPage='Genel Bakış';
    try{ localStorage.setItem(ADMIN_TAB_KEY, state.adminPage); }catch{}
  }
  const version = ho240f23Version();
  const headerActions = state.adminPage === 'Oyunlar'
    ? `<div class="rowActions"><button class="btn" data-action="toggle-game-form">${state.showGameForm?'Formu Gizle':'Yeni Oyun Ekle'}</button><button class="btn primary" data-action="bulk-sync-playlists">Toplu İşlemler</button></div>`
    : `<span class="pill ${state.maintenance?.enabled?'banned':'green'}">${state.maintenance?.enabled?'Bakım açık':'Bakım kapalı'}</span>`;
  const subtitle = state.adminPage==='Genel Bakış' ? 'Yönetim paneli sade modda. Gereksiz otomasyon panelleri kaldırıldı.' : adminSubtitle(state.adminPage);
  return `<section class="fix5AdminShell ho240AdminShell ho240f23AdminShell"><aside class="fix5AdminSidebar"><div class="sideLogo"><span class="logoMark">🎮</span><div><b>Hayatımız Oyun</b><small>${esc(version)} Yönetim</small></div></div><div class="sideNavLabel">MENÜ</div><button class="sideNavItem" data-page="Ana Sayfa"><span>⌂</span>Ana Sayfa</button><button class="sideNavItem" data-page="Oyun Arşivi"><span>🎮</span>Oyun Arşivi</button><button class="sideNavItem" data-page="Seriler"><span>◈</span>Seriler</button><button class="sideNavItem" data-page="Takvim"><span>▣</span>Yayın Takvimi</button><div class="sideNavLabel">YÖNETİM</div><div class="adminAccordion open"><button class="adminAccordionHead active"><span>👑</span><div><b>Yönetim Paneli</b><small>${esc(state.session?.full_name||'Yetkili')}</small></div><strong>⌄</strong></button><div class="adminAccordionBody">${links.map(([page,cap])=>`<button class="adminSubLink ${state.adminPage===page?'active':''}" data-admin="${esc(page)}"><span class="subDot"></span><div><b>${esc(page)}</b><small>${esc(cap)}</small></div></button>`).join('')}</div></div></aside><div class="adminContent fix5AdminContent"><div class="fix5AdminHeader"><div><div class="adminBreadcrumb">Yönetim Paneli <span>›</span> ${esc(state.adminPage)}</div><h1>${esc(state.adminPage)}</h1><p>${esc(subtitle)}</p></div>${headerActions}</div>${ho240f23AdminBody()}</div></section>`;
}
try{
  ho240f23CleanLocalData();
  adminBody = ho240f23AdminBody;
  adminPanel = ho240f23AdminPanel;
  ho240DeployCenter = ho240f23RemovedPageNotice;
  ho240f13DeployCenter = ho240f23RemovedPageNotice;
  ho240f15DeployCenter = ho240f23RemovedPageNotice;
  hoFix8DeployPanel = ho240f23RemovedPageNotice;
}catch(error){ console.warn('FIX23 ana override uygulanamadı:', error); }
try{
  const ho240f23PrevAdminNavigate = adminNavigate;
  adminNavigate = function(page){
    if(ho240f23IsRemovedPage(page)){
      state.page = 'Yönetim Paneli';
      state.adminPage = 'Genel Bakış';
      try{ localStorage.setItem(PAGE_KEY, 'Yönetim Paneli'); localStorage.setItem(ADMIN_TAB_KEY, state.adminPage); syncRouteToAddress(); }catch{}
      setToast('Bu panel kaldırıldı. Bundan sonra güncellemeler ZIP paketiyle yapılacak.');
      render(); return;
    }
    return ho240f23PrevAdminNavigate(page);
  };
}catch(error){ console.warn('FIX23 adminNavigate override atlandı:', error); }
try{
  const ho240f23PrevAdminNameFromSlug = adminNameFromSlug;
  adminNameFromSlug = function(slug){
    const name = ho240f23PrevAdminNameFromSlug(slug);
    return ho240f23IsRemovedPage(name) ? 'Genel Bakış' : name;
  };
}catch{}
try{
  const ho240f23PrevOnAction = onAction;
  onAction = async function(e){
    const action = e.currentTarget?.dataset?.action || '';
    if(/deploy|redeploy|github-upload/i.test(action) || ['ho240f13-github-upload','ho240f11-github-upload','ho240f13-auto-flow','ho240f11-auto-flow','ho240f22-mark-deploy'].includes(action)){
      e.preventDefault(); e.stopImmediatePropagation();
      ho240f23CleanLocalData();
      adminNavigate('Genel Bakış');
      setToast('Site içi yayına alma kontrol paneli kaldırıldı. Güncelleme ZIP paketiyle yapılacak.');
      return;
    }
    return ho240f23PrevOnAction(e);
  };
}catch(error){ console.warn('FIX23 onAction override atlandı:', error); }
try{
  const ho240f23PrevBind = bind;
  bind = function(){
    try{ ho240f23PrevBind(); }catch(err){ console.warn('FIX23 önceki bind atlandı:', err); }
    document.querySelectorAll('[data-admin="Deploy Merkezi"],[data-admin="Redeploy / AI Tanı Merkezi"],[data-action*="deploy"],[data-action*="redeploy"],[data-action*="github-upload"],[data-v224-deploy-toggle]').forEach(el=>{
      try{ el.remove(); }catch{}
    });
  };
}catch(error){ console.warn('FIX23 bind override atlandı:', error); }
try{
  const style = document.createElement('style');
  style.textContent = `.ho240f23AdminShell [data-admin="Deploy Merkezi"],.ho240f23AdminShell [data-admin*="Redeploy"],.ho240f23AdminShell [data-action*="deploy"],.ho240f23AdminShell [data-action*="redeploy"],.ho240f23AdminShell [data-action*="github-upload"],.ho240f23AdminShell .deployBadges{display:none!important}`;
  document.head.appendChild(style);
}catch{}
try{
  ho240f23CleanLocalData();
  if(Array.isArray(VERSION_NOTES_ARCHIVE) && !VERSION_NOTES_ARCHIVE.some(n=>n.version===HO240F23_INTERNAL_VERSION)){
    VERSION_NOTES_ARCHIVE.unshift({
      version:HO240F23_INTERNAL_VERSION,
      title:'Sade Yönetim Paneli',
      summary:'Site içindeki otomatik yayına alma ve tekrar yayına alma kontrol panelleri kaldırıldı. Yönetim paneli oyun, seri, takvim, rapor, bakım ve ayar odaklı sade hale getirildi.',
      image:'previews/hayatimiz-oyun-v240-fix23-sade-panel-preview.png',
      written:'FIX23 ile gereksiz otomasyon panelleri menüden, yönlendirmelerden, local kayıtlarından ve API pasif akışlarından temizlendi. Yeni geliştirmeler ZIP paketi olarak hazırlanacak.'
    });
  }
  if(document.getElementById('root')?.dataset?.mounted === '1') render();
}catch(error){ console.warn('FIX23 başlangıç katmanı atlandı:', error); }
