...
var ho240Applied;
const VERSION = 'v2.4.0 FIX 25 - Site Acilis Modul Import Fix';
...
/* FIX25: Eski AI/Deploy bloğu kaynak dosyadan silindi. */

/* FIX25: Eski gereksiz AI/Deploy kaynak bloğu silindi. */
...
/* FIX25: eski AI/Deploy satırı silindi. */
/* FIX25: eski AI/Deploy satırı silindi. */
};
...
/* FIX25: ho240f11 deploy merkezi ve aksiyonları kaynak dosyadan silindi. */
...
const HO240F24_INTERNAL_VERSION = 'v2.4.0 FIX 24';
...
function ho240f24CleanStorage(){
  ...
  try{
    ...
    localStorage.setItem('hayatimiz_last_fix_version', HO240F24_INTERNAL_VERSION);
  }catch{}
}
...
try{
  ...
  ho240Applied = () => [];
}catch(error){ console.warn('FIX25 hard-delete override uygulanamadı:', error); }
try{
...
try{
  ...
  bind = function(){ try{ ho240f24PrevBind(); }catch(err){ console.warn('FIX25 önceki bind atlandı:', err); } ho240f24RemoveOrphans(); };
}catch{}
try{
...
try{
  if(Array.isArray(VERSION_NOTES_ARCHIVE) && !VERSION_NOTES_ARCHIVE.some(n=>n.version===HO240F24_INTERNAL_VERSION)){
    VERSION_NOTES_ARCHIVE.unshift({
      version:HO240F24_INTERNAL_VERSION,
      ...
      image:'previews/hayatimiz-oyun-v240-fix25-temiz-panel-preview.png',
      written:'FIX25 ile sade yönetim paneli kalıcı hale getirildi. Yeni özellikler artık site içinden AI/deploy butonlarıyla değil, ZIP güncellemesi olarak hazırlanacak.'
    });
  }
  document.title = `Hayatımız Oyun - ${ho240f24Version()}`;
  if(document.getElementById('root')?.dataset?.mounted === '1') render();
}catch(error){ console.warn('FIX25 başlangıç katmanı atlandı:', error); }
