...
function home(){
  ...
  const featured=games.find(g=>g.isFeatured) || games[0] || DEFAULT_GAMES[0];
  const featuredEpisodes=loadEpisodes(featured.id);
  const featuredTotal=featuredEpisodes.length || Number(featured.episodeCount||0);
  const featuredWatched=Number(featured.watchedEpisodeCount||0);
  const featuredPct=featuredTotal?Math.round(Math.min(100,(featuredWatched/featuredTotal)*100)):progressPercent(featured);
