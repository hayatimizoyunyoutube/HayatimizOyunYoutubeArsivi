...
/* FIX49 early boot guard: uygulama dosyası yüklenir yüklenmez açılış bekleme ekranını durdurur. */
try{
  const __ho240f49Root = document.getElementById('root');
  if(__ho240f49Root) __ho240f49Root.dataset.mounted = '1';
  if(window.__HAYATIMIZ_BOOT_TIMER__) window.clearTimeout(window.__HAYATIMIZ_BOOT_TIMER__);
}catch{}
...
function ho240f10Versions(){ return ['v2.4.1','v2.4.1','v2.4.3','v2.4.4','v2.4.5','v2.4.6','v2.4.7','v2.4.8','v2.4.9','v2.5.0','v2.5.1','v2.5.2','v2.5.3','v2.5.4','v2.5.5']; }
...
/* FIX48: v2.4.1 görünür sürüm, üst bar scroll fix, Supabase sürüm notu ve eksik alan modal açılış stabilizasyonu */
const HO240F48_INTERNAL_VERSION = 'v2.4.1 FIX 48';
const HO240F48_PUBLIC_VERSION = 'v2.4.1';
try{
  localStorage.setItem('hayatimiz_last_public_version', HO240F48_PUBLIC_VERSION);
  localStorage.setItem('ho240f14_current_site_version', HO240F48_PUBLIC_VERSION);
  if(typeof HO240F24_PUBLIC_VERSION_KEY !== 'undefined') localStorage.setItem(HO240F24_PUBLIC_VERSION_KEY, HO240F48_PUBLIC_VERSION);
}catch{}
try{ ho240f24Version = function(){ return HO240F48_PUBLIC_VERSION; }; }catch{}
function ho240f48PublicItems(){
  return [
    {label:'Ana Sayfa', page:'Ana Sayfa', icon:'⌂'},
    {label:'Oyunlar', page:'Oyun Arşivi', icon:'🎮'},
    {label:'Seriler', page:'Seriler', icon:'◈'},
    ...
    {label:'Oyun İste', page:'Oyun İstekleri', icon:'💡'},
    {label:'Hata Bildir', page:'Hata Bildir', icon:'🐞'},
    {label:'Güncelleme Notları', page:'Güncellemeler', icon:'📝'},
    {label:'Bildirimler', page:'Bildirimler', icon:'🔔'}
  ];
}
try{
  topbar = function(){
    const role = normalizeRole(state.session?.role);
    const unread = userNotifications().filter(n=>!state.notificationSeen[n.id] && n.id!=='empty').length;
    return html`<header class="topbar v220Topbar v220TopbarClean fix6Topbar fix43Topbar fix45Topbar fix48Topbar">
      <button class="brand cleanBtn compactBrand topbarBrandChip fix43Brand fix45Brand fix48Brand" data-page="Ana Sayfa"><div class="mark">🎮</div><div><b>Hayatımız Oyun</b><span>${esc(HO240F48_PUBLIC_VERSION)}</span></div></button>
      <label class="search v220Search fix4Search fix6Search fix43Search fix45Search fix48Search">🔎 <input id="searchInput" value="${esc(state.query)}" placeholder="Oyun, seri veya etiket ara..." /><kbd>Ctrl K</kbd></label>
      <div class="topActions v220UserActions fix6TopActions fix43Actions fix45Actions fix48Actions">
        <button class="miniBtn fix45PublicCta" data-page="Oyun İstekleri">Oyun İste</button>
        <button class="miniBtn fix45PublicCta" data-page="Hata Bildir">Hata Bildir</button>
        ${state.session ? `<button class="notifyTopBtn fix43Notify" data-page="Bildirimler" title="Bildirimler">🔔 <b>${unread}</b></button>` : ''}
        ${state.session ? `<button class="avatarChip fix6AvatarChip fix43Avatar" data-page="Profilim"><span>${esc((state.session.full_name || state.session.email || 'H')[0]).toUpperCase()}</span><div><b>${esc(state.session.full_name || 'Hayatımız Oyun')}</b><small>${esc(displayRole(role))}</small></div></button>` : `<button class="btn" data-action="open-login">Giriş</button><button class="btn primary" data-action="open-register">Kayıt</button>`}
  ...
  archiveSideNav = function(){
    const activeCheck = item => state.page===item.page || (item.page==='Oyun Arşivi' && ['Popüler','Tamamlanan','Devam Eden','Yakında','Korku','Aksiyon','Hikaye Odaklı'].includes(state.page));
    return `<aside class="v220SideNav fixedArchiveMenu fix5SideNav fix6SideNav fix43Sidebar fix45Sidebar fix48Sidebar"><div class="sideLogo fix6SideLogo fix43SideLogo"><span class="logoMark">🎮</span><div><b>Hayatımız Oyun</b><small>${esc(HO240F48_PUBLIC_VERSION)} • Arşiv</small></div></div><div class="sideNavLabel">MENÜ</div>${ho240f48PublicItems().map(item=>`<button class="sideNavItem fix43SideItem ${activeCheck(item)?'active':''}" data-page="${esc(item.page)}"><span>${item.icon}</span><em>${esc(item.label)}</em></button>`).join('')}${isStaff() ? `<div class="sideNavLabel">YÖNETİM</div><button class="sideNavItem manager fix43SideItem ${state.page==='Yönetim Paneli'?'active':''}" data-admin="Genel Bakış"><span>👑</span><div><b>Yönetim Paneli</b><small>Profesyonel kontrol</small></div><strong>›</strong></button>` : ''}<div class="fix43SidebarCard fix45SidebarCard"><b>İstek / Hata Merkezi</b><small>Kullanıcılar oyun isteği ve hata bildirimini buradan gönderebilir.</small><div class="rowActions"><button class="miniBtn primary" data-page="Oyun İstekleri">Oyun İste</button><button class="miniBtn" data-page="Hata Bildir">Hata Bildir</button></div></div></aside>`;
  };
}catch(error){ console.warn('FIX48 public UI kurulamadı:', error); }
function ho240f48OpenGameEditModal(id){
  const current = (Array.isArray(state.games) ? state.games : []).find(g=>String(g.id)===String(id));
  if(!current){ setToast('Düzenlenecek oyun bulunamadı.'); return; }
  state.page = 'Yönetim Paneli';
  state.adminPage = state.adminPage || 'İçerik Kontrol';
  if(state.adminPage === 'Oyunlar') state.adminPage = 'Mevcut Oyunlar';
  state.ho240f47EditModalId = current.id;
  state.editingGameId = current.id;
  state.showGameForm = false;
  ...
  state.ho240f11CoverCandidates = [];
  state.ho240f10CoverCandidates = [];
  try{ localStorage.setItem(PAGE_KEY,'Yönetim Paneli'); localStorage.setItem(ADMIN_TAB_KEY,state.adminPage); syncRouteToAddress(); }catch{}
  render();
  setToast(`${current.title} ayrı düzenleme penceresinde açıldı.`);
}
function ho240f48GameButtonHandler(event){
  const edit = event.target?.closest?.('[data-game-edit]');
  const del = event.target?.closest?.('[data-game-delete]');
  if(!edit && !del) return;
  ...
  if(edit) return ho240f48OpenGameEditModal(edit.dataset.gameEdit);
  if(del && typeof ho240f30DeleteGameById === 'function') return ho240f30DeleteGameById(del.dataset.gameDelete);
}
try{
  if(typeof ho240f30GameButtonHandler === 'function') document.removeEventListener('click', ho240f30GameButtonHandler, true);
  if(!window.__HO240F48_GAME_HANDLER__){
    window.__HO240F48_GAME_HANDLER__ = true;
    document.addEventListener('click', ho240f48GameButtonHandler, true);
  }
  const ho240f48PrevBind = typeof bind === 'function' ? bind : null;
  bind = function(){
    if(ho240f48PrevBind) ho240f48PrevBind();
    try{ if(typeof ho240f30GameButtonHandler === 'function') document.removeEventListener('click', ho240f30GameButtonHandler, true); }catch{}
    document.querySelectorAll('[data-game-edit],[data-game-delete]').forEach(el=>{
      try{ if(typeof ho240f30GameButtonHandler === 'function') el.removeEventListener('click', ho240f30GameButtonHandler, true); }catch{}
    });
  };
}catch(error){ console.warn('FIX48 oyun modal handler kurulamadı:', error); }
try{
  if(Array.isArray(VERSION_NOTES_ARCHIVE) && !VERSION_NOTES_ARCHIVE.some(n=>n.version===HO240F48_INTERNAL_VERSION)){
    VERSION_NOTES_ARCHIVE.unshift({
      version:HO240F48_INTERNAL_VERSION,
      title:'v2.4.1 Sürüm Sabitleme + Eksik Alan Modal Fix',
      summary:'Üst ve sol menüde görünen sürüm v2.4.1 olarak sabitlendi; üst bar sayfa kaydırırken içerikle birlikte aşağı taşmayacak şekilde düzeltildi; Eksik Alanlar bölümündeki Eksiği Gider butonu gerçekten ayrı düzenleme penceresini açar.',
      image:'previews/hayatimiz-oyun-v241-fix48-modal-version.png',
      written:'FIX48 ile eski FIX30 yakalama katmanının modal açılışını engellemesi düzeltildi. data-game-edit tıklamaları doğrudan ayrı düzenleme penceresine bağlanır. Supabase schema bloğu public sürümü v2.4.1 olarak günceller.'
    });
  }
  document.title = `Hayatımız Oyun - ${HO240F48_PUBLIC_VERSION}`;
  try{ localStorage.setItem('hayatimiz_last_fix_version', HO240F48_INTERNAL_VERSION); }catch{}
  if(document.getElementById('root')?.dataset?.mounted === '1') render();
}catch(error){ console.warn('FIX48 final render atlandı:', error); }


/* FIX49: Açılış bekleme, veri çekme butonları, modal düzenleme ve v2.4.1 stabilizasyonu */
const HO240F49_INTERNAL_VERSION = 'v2.4.1 FIX 49';
const HO240F49_PUBLIC_VERSION = 'v2.4.1';
try{
  localStorage.setItem('hayatimiz_last_public_version', HO240F49_PUBLIC_VERSION);
  localStorage.setItem('ho240f14_current_site_version', HO240F49_PUBLIC_VERSION);
  if(typeof HO240F24_PUBLIC_VERSION_KEY !== 'undefined') localStorage.setItem(HO240F24_PUBLIC_VERSION_KEY, HO240F49_PUBLIC_VERSION);
}catch{}
try{ ho240f24Version = function(){ return HO240F49_PUBLIC_VERSION; }; }catch{}
try{ document.title = `Hayatımız Oyun - ${HO240F49_PUBLIC_VERSION}`; }catch{}
try{ categoryRail = function(){ return ''; }; }catch{}
function ho240f49Norm(value){ return String(value||'').toLocaleLowerCase('tr-TR').replace(/[’'`]/g,'').replace(/[^a-z0-9ğüşöçıİĞÜŞÖÇ]+/gi,' ').replace(/\s+/g,' ').trim(); }
function ho240f49Form(source){ return source?.closest?.('form') || document.getElementById('gameEditForm') || document.getElementById('gameAddForm') || document.querySelector('form.gameForm'); }
function ho240f49SetField(form, name, value){ if(form?.elements?.[name] && value !== undefined && value !== null) form.elements[name].value = String(value); }
function ho240f49Capture(form){ try{ if(typeof ho240f41Capture === 'function') ho240f41Capture(form); else if(typeof ho240f10CaptureForm === 'function') ho240f10CaptureForm(form); else if(typeof saveGameDraftFromForm === 'function') saveGameDraftFromForm(form); }catch{} }
function ho240f49PatchNoRender(form, patch={}){
  if(!form || !patch) return;
  const title = String(form.elements?.title?.value || '').trim();
  Object.entries(patch).forEach(([key,value])=>{ if(key !== 'title' && key !== 'name') ho240f49SetField(form,key,value); });
  if(title && form.elements?.title) form.elements.title.value = title;
  ho240f49Capture(form);
}
try{
  const __ho240f49PrevPatch = typeof ho240f42Patch === 'function' ? ho240f42Patch : null;
  ho240f42Patch = function(form, patch={}, shouldRender=false){
    ho240f49PatchNoRender(form, patch);
    if(shouldRender === true && !window.__HO240F49_SUPPRESS_RENDER__){ try{ render(); }catch{} }
  };
  window.__HO240F49_PREV_PATCH__ = __ho240f49PrevPatch;
}catch(error){ console.warn('FIX49 patch override kurulamadı:', error); }
function ho240f49ExactLocal(title){
  const n = ho240f49Norm(title);
  let rows = [];
  try{ if(typeof ho240f37AlanWakeRows === 'function') rows = rows.concat(ho240f37AlanWakeRows()); }catch{}
  try{ if(typeof HO240F36_ALAN_WAKE_ROWS !== 'undefined') rows = rows.concat(HO240F36_ALAN_WAKE_ROWS); }catch{}
  try{ if(typeof ho240f42LocalCandidates === 'function') rows = rows.concat(ho240f42LocalCandidates(title)); }catch{}
  const exact = rows.find(r=>ho240f49Norm(r.title) === n || ho240f49Norm(r.name) === n);
  if(exact) return exact;
  return rows.find(r=>{
    const rn = ho240f49Norm(r.title || r.name || '');
    return rn && (rn.includes(n) || n.includes(rn)) && Math.abs(rn.length - n.length) < 12;
  }) || null;
}
async function ho240f49GetRelease(title){
  const local = ho240f49ExactLocal(title);
  if(local?.releaseDate || local?.released) return { release: (typeof ho240f10Date==='function' ? ho240f10Date(local.releaseDate || local.released) : (local.releaseDate || local.released)), source:'yerel kesin katalog' };
  try{
    const data = await api('game-release-date', { adminToken:state.session?.adminToken, title, fix:'49', strict:true });
    const rel = (typeof ho240f10Date==='function' ? ho240f10Date(data?.releaseDate || data?.released || '') : (data?.releaseDate || data?.released || ''));
    if(rel) return { release:rel, source:data?.source || 'API' };
  }catch(error){ console.warn('FIX49 release API alınamadı:', error); }
  try{
    const meta = typeof ho240f42Meta === 'function' ? await ho240f42Meta(title) : null;
    const rel = (typeof ho240f10Date==='function' ? ho240f10Date(meta?.meta?.releaseDate || meta?.meta?.released || '') : (meta?.meta?.releaseDate || meta?.meta?.released || ''));
    if(rel) return { release:rel, source:meta?.source || 'meta' };
  }catch{}
  return { release:'', source:'' };
}
async function ho240f49StableRelease(form){
  if(!form) return setToast('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast('Önce oyun adını yaz.');
  const { release, source } = await ho240f49GetRelease(title);
  if(!release) return setToast('Çıkış tarihi bulunamadı. Manuel yazabilirsin.');
  ho240f49PatchNoRender(form, { releaseDate:release });
  setToast(`Çıkış tarihi forma işlendi: ${release}${source ? ' ('+source+')' : ''}.`);
}
async function ho240f49StableGenres(form){
  if(!form) return setToast('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast('Önce oyun adını yaz.');
  let genre = ho240f49ExactLocal(title)?.genre || '';
  if(!genre){ try{ const data = await api('game-genres', { adminToken:state.session?.adminToken, title, fix:'49' }); genre = data?.genre || ''; }catch{} }
  if(!genre && typeof ho240f34GenreFromTitle === 'function') genre = ho240f34GenreFromTitle(title);
  genre = genre || 'Genel, Hikaye Odaklı';
  ho240f49PatchNoRender(form, { genre, tags:String(form.elements?.tags?.value || '') || genre });
  setToast(`Türler forma işlendi: ${genre}.`);
}
async function ho240f49StableDescription(form){
  if(!form) return setToast('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast('Önce oyun adını yaz.');
  let description = '';
  try{ const data = await api('game-description', { adminToken:state.session?.adminToken, title, fix:'49' }); description = data?.description || ''; }catch{}
  if(!description){ const genre = String(form.elements?.genre?.value || ho240f49ExactLocal(title)?.genre || ''); description = typeof fix12DetailedStory === 'function' ? fix12DetailedStory(title, genre) : `${title}, Hayatımız Oyun arşivinde bölüm, seri ve kapak bilgileriyle takip edilen bir oyun kaydıdır.`; }
  ho240f49PatchNoRender(form, { description });
  setToast('Açıklama forma işlendi.');
}
function ho240f49RenderCoverPanel(form){
  if(!form || typeof ho240f11CoverPanel !== 'function') return;
  const old = form.querySelector('.ho240f49DynamicCoverPanel');
  if(old) old.remove();
  const wrap = document.createElement('div');
  wrap.className = 'ho240f49DynamicCoverPanel';
  wrap.innerHTML = ho240f11CoverPanel();
  form.appendChild(wrap);
  wrap.querySelectorAll('[data-ho240f11-cover-pick]').forEach(btn=>{
    btn.addEventListener('click', ev=>{ ev.preventDefault(); ev.stopPropagation(); if(ev.stopImmediatePropagation) ev.stopImmediatePropagation(); try{ ho240f11PickCover(btn.dataset.ho240f11CoverPick); }catch(error){ console.warn(error); setToast('Kapak seçilemedi.'); } });
  });
}
async function ho240f49StableCovers(form){
  if(!form) return setToast('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast('Önce oyun adını yaz.');
  let list = [];
  try{ list = typeof ho240f42BuildCovers === 'function' ? await ho240f42BuildCovers(title) : []; }catch(error){ console.warn('FIX49 kapak listesi alınamadı:', error); }
  state.ho240f11CoverCandidates = list;
  state.ho240f10CoverCandidates = list;
  const first = list[0] || ho240f49ExactLocal(title) || {};
  const patch = {
    seriesName:String(form.elements?.seriesName?.value || '') || first.seriesName || '',
    genre:String(form.elements?.genre?.value || '') || first.genre || '',
    tags:String(form.elements?.tags?.value || '') || first.tags || first.genre || '',
    releaseDate:String(form.elements?.releaseDate?.value || '') || first.releaseDate || first.released || '',
    score:String(form.elements?.score?.value || '') || first.score || 8.5,
    description:String(form.elements?.description?.value || '') || first.description || ''
  };
  if(!String(form.elements?.cover?.value || '').trim() && first.cover) patch.cover = first.cover;
  ho240f49PatchNoRender(form, patch);
  ho240f49RenderCoverPanel(form);
  setToast(list.length ? `${list.length} kapak adayı getirildi. Sayfa yenilenmedi; doğru kapağı seçebilirsin.` : 'Kapak bulunamadı. Manuel URL alanı açık.');
}
async function ho240f49StableFetchAll(form){
  if(!form) return setToast('Önce oyun formunu aç.');
  await ho240f49StableCovers(form);
  await ho240f49StableRelease(form);
  await ho240f49StableGenres(form);
  await ho240f49StableDescription(form);
  setToast('Tüm bilgiler forma işlendi. Kayıt sadece Kaydet/Güncelle ile yapılır.');
}
function ho240f49ModalHtml(game){
  state.editingGameId = game.id;
  return `<div class="ho240f47ModalBackdrop ho240f49ModalBackdrop" data-action="ho240f47-close-edit-modal"><section class="ho240f47EditModal ho240f49EditModal" role="dialog" aria-modal="true" onclick="event.stopPropagation()"><div class="ho240f47ModalHead"><div><span class="eyebrow">Ayrı Düzenleme Penceresi • ${HO240F49_PUBLIC_VERSION}</span><h2>${esc(game.title || 'Oyun Düzenle')}</h2><p class="muted">Eksiği gider, tarih/kapak/tür çek ve yalnızca Güncelle butonuyla kaydet.</p></div><button class="modalCloseBtn" type="button" data-action="ho240f47-close-edit-modal">✕</button></div><form class="card soft gameForm editGameForm fix6GameForm ho240f46GameForm ho240f47ModalForm" id="gameEditForm" autocomplete="off">${gameFormFields(game,'edit')}</form></section></div>`;
}
function ho240f49EnsureModal(game){
  setTimeout(()=>{
    try{
      if(document.querySelector('.ho240f47ModalBackdrop,.ho240f49ModalBackdrop')) return;
      document.body.insertAdjacentHTML('beforeend', ho240f49ModalHtml(game));
      bind();
    }catch(error){ console.warn('FIX49 modal inject atlandı:', error); }
  }, 0);
}
function ho240f49OpenGameEditModal(id){
  const game = (Array.isArray(state.games)?state.games:[]).find(g=>String(g.id)===String(id));
  if(!game) return setToast('Düzenlenecek oyun bulunamadı.');
  state.page = 'Yönetim Paneli';
  state.adminPage = 'İçerik Kontrol';
  state.ho240f47EditModalId = game.id;
  state.editingGameId = game.id;
  state.showGameForm = false;
  state.rawgCandidates = [];
  state.coverSuggestions = [];
  ...
  try{ localStorage.setItem(PAGE_KEY,'Yönetim Paneli'); localStorage.setItem(ADMIN_TAB_KEY,state.adminPage); syncRouteToAddress(); }catch{}
  render();
  ho240f49EnsureModal(game);
  setToast(`${game.title} düzenleme penceresi açıldı.`);
}
try{
  document.addEventListener('click', async function(e){
    const actionEl = e.target?.closest?.('[data-action]');
    const gameEditEl = e.target?.closest?.('[data-game-edit]');
    if(gameEditEl){
      e.preventDefault(); e.stopPropagation(); if(e.stopImmediatePropagation) e.stopImmediatePropagation();
      return ho240f49OpenGameEditModal(gameEditEl.dataset.gameEdit);
    }
    if(!actionEl) return;
    const action = actionEl.dataset.action || '';
    const formActions = ['ho240f42-refetch-release','ho240f41-refetch-release','ho240f42-find-covers','ho240f41-find-covers','ho240f11-find-covers','ho240f42-refetch-genres','ho240f41-refetch-genres','ho240f42-refetch-description','ho240f42-fetch-all','auto-game-meta','auto-game-meta-edit','fetch-game-story','fetch-game-story-edit'];
    if(!formActions.includes(action)) return;
    e.preventDefault(); e.stopPropagation(); if(e.stopImmediatePropagation) e.stopImmediatePropagation();
    const form = ho240f49Form(actionEl);
    if(action.includes('find-covers')) return await ho240f49StableCovers(form);
    if(action.includes('refetch-release')) return await ho240f49StableRelease(form);
    if(action.includes('refetch-genres')) return await ho240f49StableGenres(form);
    if(action === 'ho240f42-refetch-description' || action.includes('fetch-game-story')) return await ho240f49StableDescription(form);
    if(action === 'ho240f42-fetch-all' || action.includes('auto-game-meta')) return await ho240f49StableFetchAll(form);
  }, true);
}catch(error){ console.warn('FIX49 click guard kurulamadı:', error); }
try{
  const __ho240f49PrevBind = typeof bind === 'function' ? bind : null;
  bind = function(){
    if(__ho240f49PrevBind) __ho240f49PrevBind();
    document.querySelectorAll('form button:not([type])').forEach(btn=>btn.setAttribute('type','button'));
    document.querySelectorAll('.ho240f47ModalBackdrop,[data-action="ho240f47-close-edit-modal"]').forEach(el=>{
      el.addEventListener('click', ev=>{
        if(ev.target.closest && ev.target.closest('.ho240f47EditModal') && !ev.target.matches('[data-action="ho240f47-close-edit-modal"]')) return;
        ev.preventDefault(); ev.stopPropagation(); if(ev.stopImmediatePropagation) ev.stopImmediatePropagation();
        state.ho240f47EditModalId = null; state.editingGameId = null; state.rawgCandidates = []; state.ho240f11CoverCandidates = []; state.ho240f10CoverCandidates = [];
        render();
      });
    });
  };
}catch(error){ console.warn('FIX49 bind stabil kurulamadı:', error); }
try{
  topbar = function(){
    const role = normalizeRole(state.session?.role);
    const unread = userNotifications().filter(n=>!state.notificationSeen[n.id] && n.id!=='empty').length;
    return html`<header class="topbar v220Topbar v220TopbarClean fix6Topbar fix43Topbar fix45Topbar fix48Topbar fix49Topbar">
      <button class="brand cleanBtn compactBrand topbarBrandChip fix43Brand fix45Brand fix48Brand" data-page="Ana Sayfa"><div class="mark">🎮</div><div><b>Hayatımız Oyun</b><span>${esc(HO240F49_PUBLIC_VERSION)}</span></div></button>
      <label class="search v220Search fix4Search fix6Search fix43Search fix45Search fix48Search">🔎 <input id="searchInput" value="${esc(state.query)}" placeholder="Oyun, seri veya etiket ara..." /><kbd>Ctrl K</kbd></label>
      <div class="topActions v220UserActions fix6TopActions fix43Actions fix45Actions fix48Actions">
        <button class="miniBtn fix45PublicCta" data-page="Oyun İstekleri">Oyun İste</button>
        <button class="miniBtn fix45PublicCta" data-page="Hata Bildir">Hata Bildir</button>
  ...
  archiveSideNav = function(){
    const activeCheck = item => state.page===item.page || (item.page==='Oyun Arşivi' && ['Popüler','Tamamlanan','Devam Eden','Yakında','Korku','Aksiyon','Hikaye Odaklı'].includes(state.page));
    return `<aside class="v220SideNav fixedArchiveMenu fix5SideNav fix6SideNav fix43Sidebar fix45Sidebar fix48Sidebar"><div class="sideLogo fix6SideLogo fix43SideLogo"><span class="logoMark">🎮</span><div><b>Hayatımız Oyun</b><small>${esc(HO240F49_PUBLIC_VERSION)} • Arşiv</small></div></div><div class="sideNavLabel">MENÜ</div>${ho240f48PublicItems().map(item=>`<button class="sideNavItem fix43SideItem ${activeCheck(item)?'active':''}" data-page="${esc(item.page)}"><span>${item.icon}</span><em>${esc(item.label)}</em></button>`).join('')}${isStaff() ? `<div class="sideNavLabel">YÖNETİM</div><button class="sideNavItem manager fix43SideItem ${state.page==='Yönetim Paneli'?'active':''}" data-admin="Genel Bakış"><span>👑</span><div><b>Yönetim Paneli</b><small>Profesyonel kontrol</small></div><strong>›</strong></button>` : ''}</aside>`;
  };
}catch(error){ console.warn('FIX49 UI sürüm override kurulamadı:', error); }
try{
  if(Array.isArray(VERSION_NOTES_ARCHIVE) && !VERSION_NOTES_ARCHIVE.some(n=>n.version===HO240F49_INTERNAL_VERSION)){
    VERSION_NOTES_ARCHIVE.unshift({
      version:HO240F49_INTERNAL_VERSION,
      title:'v2.4.1 Açılış, Modal ve Form Buton Stabilizasyonu',
      summary:'İlk açılışta yükleniyor ekranına düşme süresi uzatıldı ve uygulama yüklenir yüklenmez boot ekranı kapatılır. Eksiği Gider artık modalı garanti açar. Kapak, tarih, tür ve açıklama çekme butonları sayfayı yenilemeden forma işler.',
      image:'previews/hayatimiz-oyun-v241-fix49-form-modal-stabil.png',
      written:'FIX49 ile form içi veri çekme işlemleri ayrı capture katmanına alındı. Veri çekme butonları submit/reload yapmaz; sadece form alanlarını doldurur ve kayıt yine Kaydet/Güncelle ile yapılır.'
    });
  }
  localStorage.setItem('hayatimiz_last_fix_version', HO240F49_INTERNAL_VERSION);
  const r=document.getElementById('root'); if(r) r.dataset.mounted='1';
  if(window.__HAYATIMIZ_BOOT_TIMER__) window.clearTimeout(window.__HAYATIMIZ_BOOT_TIMER__);
  if(document.getElementById('root')?.innerHTML === '') render();
}catch(error){ console.warn('FIX49 final render atlandı:', error); }

/* FIX50: Schema JSON düzeltmesi + form veri çekme butonlarının sayfa yenilemesini kesin engelleme */
const HO240F50_INTERNAL_VERSION = 'v2.4.1 FIX 50';
const HO240F50_PUBLIC_VERSION = 'v2.4.1';
try{
  localStorage.setItem('hayatimiz_last_public_version', HO240F50_PUBLIC_VERSION);
  localStorage.setItem('ho240f14_current_site_version', HO240F50_PUBLIC_VERSION);
  if(typeof HO240F24_PUBLIC_VERSION_KEY !== 'undefined') localStorage.setItem(HO240F24_PUBLIC_VERSION_KEY, HO240F50_PUBLIC_VERSION);
  document.title = `Hayatımız Oyun - ${HO240F50_PUBLIC_VERSION}`;
}catch{}
try{ ho240f24Version = function(){ return HO240F50_PUBLIC_VERSION; }; }catch{}
try{
  const root = document.getElementById('root');
  if(root) root.dataset.mounted = '1';
  if(window.__HAYATIMIZ_BOOT_TIMER__) window.clearTimeout(window.__HAYATIMIZ_BOOT_TIMER__);
}catch{}
function ho240f50IsFormFetchAction(action){
  return [
    'ho240f42-refetch-release','ho240f41-refetch-release','ho240f42-find-covers','ho240f41-find-covers','ho240f11-find-covers','ho240f10-find-covers',
    'ho240f42-refetch-genres','ho240f41-refetch-genres','fix12-refetch-genres','fix10-refetch-genres',
    'ho240f42-refetch-description','fix12-refetch-story','fix10-refetch-story','fetch-game-story','fetch-game-story-edit',
    'ho240f42-fetch-all','auto-game-meta','auto-game-meta-edit','estimate-playlist-episodes','estimate-playlist-episodes-edit'
  ].includes(String(action||''));
}
function ho240f50ActiveForm(actionEl){
  return actionEl?.closest?.('form')
    || document.querySelector('.ho240f47ModalBackdrop #gameEditForm')
    || document.querySelector('.ho240f49ModalBackdrop #gameEditForm')
    || document.getElementById('gameEditForm')
    || document.getElementById('gameAddForm')
    || document.querySelector('form.gameForm');
}
async function ho240f50HandleFormAction(action, actionEl){
  const form = ho240f50ActiveForm(actionEl);
  if(!form) return setToast('Önce oyun formunu aç.');
  try{ if(typeof ho240f26MarkStableScroll === 'function') ho240f26MarkStableScroll(); }catch{}
  window.__HO240F50_SUPPRESS_FORM_RENDER__ = true;
  try{
    if(action === 'estimate-playlist-episodes' || action === 'estimate-playlist-episodes-edit'){
      if(typeof estimatePlaylistEpisodes === 'function') await estimatePlaylistEpisodes(form);
      else setToast('Playlist bölüm çekme sistemi hazır değil.');
      return true;
    }
    if(action.includes('find-covers')){
      if(typeof ho240f49StableCovers === 'function') await ho240f49StableCovers(form);
      else if(typeof ho240f42FindCovers === 'function') await ho240f42FindCovers(form);
      else setToast('Kapak çekme sistemi hazır değil.');
      return true;
    }
    if(action.includes('refetch-release')){
      if(typeof ho240f49StableRelease === 'function') await ho240f49StableRelease(form);
      else if(typeof ho240f42RefetchRelease === 'function') await ho240f42RefetchRelease(form);
      else setToast('Çıkış tarihi çekme sistemi hazır değil.');
      return true;
    }
    if(action.includes('refetch-genres')){
      if(typeof ho240f49StableGenres === 'function') await ho240f49StableGenres(form);
      else setToast('Tür çekme sistemi hazır değil.');
      return true;
    }
    if(action.includes('description') || action.includes('story')){
      if(typeof ho240f49StableDescription === 'function') await ho240f49StableDescription(form);
      else setToast('Açıklama çekme sistemi hazır değil.');
      return true;
    }
    if(action === 'ho240f42-fetch-all' || action === 'auto-game-meta' || action === 'auto-game-meta-edit'){
      if(typeof ho240f49StableFetchAll === 'function') await ho240f49StableFetchAll(form);
      else {
        if(typeof ho240f49StableCovers === 'function') await ho240f49StableCovers(form);
        if(typeof ho240f49StableRelease === 'function') await ho240f49StableRelease(form);
        if(typeof ho240f49StableGenres === 'function') await ho240f49StableGenres(form);
    ...
    console.error('FIX50 form action hatası:', error);
    setToast('Buton işlemi hata verdi: ' + (error?.message || error));
    return true;
  }finally{
    window.__HO240F50_SUPPRESS_FORM_RENDER__ = false;
  }
  return false;
}
try{
  window.HO240F50_HANDLE_FORM_ACTION = ho240f50HandleFormAction;
  window.HO240F50_IS_FORM_FETCH_ACTION = ho240f50IsFormFetchAction;
}catch{}
try{
  document.addEventListener('click', function(event){
    ...
    if(!actionEl) return;
    const action = actionEl.dataset.action || '';
    if(!ho240f50IsFormFetchAction(action)) return;
    if(!actionEl.closest?.('#gameAddForm,#gameEditForm,.ho240f47ModalBackdrop,.ho240f49ModalBackdrop,.ho240f46GameForm,.fix6GameForm')) return;
    event.preventDefault();
    event.stopPropagation();
    if(event.stopImmediatePropagation) event.stopImmediatePropagation();
    ho240f50HandleFormAction(action, actionEl);
  }, true);
  document.addEventListener('submit', function(event){
    const form = event.target;
    if(!form?.matches?.('#gameAddForm,#gameEditForm,.gameForm')) return;
    const submitter = event.submitter;
    if(submitter?.dataset?.action && ho240f50IsFormFetchAction(submitter.dataset.action)){
      event.preventDefault();
      event.stopPropagation();
      if(event.stopImmediatePropagation) event.stopImmediatePropagation();
      ho240f50HandleFormAction(submitter.dataset.action, submitter);
    }
  }, true);
}catch(error){ console.warn('FIX50 form button guard kurulamadı:', error); }
try{
  const __ho240f50PrevRender = typeof render === 'function' ? render : null;
  render = function(){
    if(window.__HO240F50_SUPPRESS_FORM_RENDER__) return;
    return __ho240f50PrevRender ? __ho240f50PrevRender() : undefined;
  };
}catch(error){ console.warn('FIX50 render guard kurulamadı:', error); }
try{
  const __ho240f50PrevBind = typeof bind === 'function' ? bind : null;
  bind = function(){
    if(__ho240f50PrevBind) __ho240f50PrevBind();
    document.querySelectorAll('#gameAddForm [data-action],#gameEditForm [data-action],.ho240f47ModalBackdrop [data-action],.ho240f49ModalBackdrop [data-action]').forEach(btn=>{
      if(ho240f50IsFormFetchAction(btn.dataset.action)) btn.setAttribute('type','button');
    });
  };
}catch(error){ console.warn('FIX50 bind guard kurulamadı:', error); }
try{
  topbar = function(){
    const role = normalizeRole(state.session?.role);
    const unread = userNotifications().filter(n=>!state.notificationSeen[n.id] && n.id!=='empty').length;
    return html`<header class="topbar v220Topbar v220TopbarClean fix6Topbar fix43Topbar fix45Topbar fix48Topbar fix49Topbar fix50Topbar">
      <button class="brand cleanBtn compactBrand topbarBrandChip fix43Brand fix45Brand fix48Brand" data-page="Ana Sayfa"><div class="mark">🎮</div><div><b>Hayatımız Oyun</b><span>${esc(HO240F50_PUBLIC_VERSION)}</span></div></button>
      <label class="search v220Search fix4Search fix6Search fix43Search fix45Search fix48Search">🔎 <input id="searchInput" value="${esc(state.query)}" placeholder="Oyun, seri veya etiket ara..." /><kbd>Ctrl K</kbd></label>
      <div class="topActions v220UserActions fix6TopActions fix43Actions fix45Actions fix48Actions">
        <button class="miniBtn fix45PublicCta" data-page="Oyun İstekleri">Oyun İste</button>
        <button class="miniBtn fix45PublicCta" data-page="Hata Bildir">Hata Bildir</button>
...
}catch(error){ console.warn('FIX50 topbar kurulamadı:', error); }
try{
  if(Array.isArray(VERSION_NOTES_ARCHIVE) && !VERSION_NOTES_ARCHIVE.some(n=>n.version===HO240F50_INTERNAL_VERSION)){
    VERSION_NOTES_ARCHIVE.unshift({
      version:HO240F50_INTERNAL_VERSION,
      title:'v2.4.1 Schema ve Form Buton Fix',
      summary:'Supabase schema.sql JSONB hatası düzeltildi. Oyun ekle/düzenle ekranında kapak, çıkış tarihi, tür, açıklama ve tüm bilgileri çekme butonları sayfa yenilemeden forma işler.',
      image:'previews/hayatimiz-oyun-v241-fix50-schema-form-buton.png',
      written:'FIX50 ile site_runtime_config kayıtları JSONB formatına alındı. Form içi veri çekme butonları erken yakalama katmanıyla submit/reload davranışından ayrıldı ve kayıt yine yalnızca Kaydet/Güncelle ile yapılır.'
    });
  }
  localStorage.setItem('hayatimiz_last_fix_version', HO240F50_INTERNAL_VERSION);
  if(window.__HAYATIMIZ_BOOT_TIMER__) window.clearTimeout(window.__HAYATIMIZ_BOOT_TIMER__);
  const root = document.getElementById('root');
  if(root){ root.dataset.mounted='1'; if(!root.innerHTML.trim()) render(); }
}catch(error){ console.warn('FIX50 final render atlandı:', error); }

/* FIX51: Oyun ekle/düzenle form submit ve çekme butonları tam stabil + detaylı hikaye çekme */
const HO240F51_INTERNAL_VERSION = 'v2.4.1 FIX 51';
const HO240F51_PUBLIC_VERSION = 'v2.4.1';
try{
  localStorage.setItem('hayatimiz_last_public_version', HO240F51_PUBLIC_VERSION);
  localStorage.setItem('ho240f14_current_site_version', HO240F51_PUBLIC_VERSION);
  if(typeof HO240F24_PUBLIC_VERSION_KEY !== 'undefined') localStorage.setItem(HO240F24_PUBLIC_VERSION_KEY, HO240F51_PUBLIC_VERSION);
  document.title = `Hayatımız Oyun - ${HO240F51_PUBLIC_VERSION}`;
}catch{}
try{ ho240f24Version = function(){ return HO240F51_PUBLIC_VERSION; }; }catch{}
const HO240F51_FORM_FETCH_ACTIONS = [
  'ho240f42-refetch-release','ho240f41-refetch-release','ho240f42-find-covers','ho240f41-find-covers','ho240f11-find-covers','ho240f10-find-covers',
  'ho240f42-refetch-genres','ho240f41-refetch-genres','fix12-refetch-genres','fix10-refetch-genres','v222-refetch-genres',
  'ho240f42-refetch-description','fix12-refetch-story','fix10-refetch-story','v222-refetch-story','fetch-game-story','fetch-game-story-edit','ho240f51-detailed-story',
  'ho240f42-fetch-all','auto-game-meta','auto-game-meta-edit','estimate-playlist-episodes','estimate-playlist-episodes-edit','toggle-technical-episodes'
];
function ho240f51IsGameForm(form){ return !!(form?.matches?.('#gameAddForm,#gameEditForm,.gameForm') || form?.closest?.('#gameAddForm,#gameEditForm,.gameForm')); }
function ho240f51Form(el){ return el?.closest?.('form') || document.querySelector('.ho240f47ModalBackdrop #gameEditForm') || document.querySelector('.ho240f49ModalBackdrop #gameEditForm') || document.getElementById('gameEditForm') || document.getElementById('gameAddForm') || document.querySelector('form.gameForm'); }
function ho240f51Stop(event){ try{ event.preventDefault(); event.stopPropagation(); if(event.stopImmediatePropagation) event.stopImmediatePropagation(); }catch{} }
function ho240f51Set(form, name, value){ if(form?.elements?.[name] && value !== undefined && value !== null) form.elements[name].value = String(value); }
function ho240f51NormList(value){ return String(value||'').split(',').map(x=>x.trim()).filter(Boolean); }
function ho240f51ToggleCsvValue(value, tag){
  const list = ho240f51NormList(value);
  const key = String(tag||'').toLocaleLowerCase('tr-TR');
  const exists = list.some(x=>x.toLocaleLowerCase('tr-TR')===key);
  return exists ? list.filter(x=>x.toLocaleLowerCase('tr-TR')!==key).join(', ') : [...list, tag].join(', ');
}
function ho240f51Capture(form){
  try{ if(typeof ho240f41Capture === 'function') ho240f41Capture(form); else if(typeof ho240f10CaptureForm === 'function') ho240f10CaptureForm(form); else if(typeof saveGameDraftFromForm === 'function') saveGameDraftFromForm(form); }catch{}
}
function ho240f51Patch(form, patch={}){
  if(!form) return;
  const lockedTitle = String(form.elements?.title?.value || '').trim();
  Object.entries(patch||{}).forEach(([k,v])=>{
    if((k==='title' || k==='name') && lockedTitle) return;
    ho240f51Set(form, k, v);
  });
  if(lockedTitle && form.elements?.title) form.elements.title.value = lockedTitle;
  ho240f51Capture(form);
}
function ho240f51ActiveSaveButton(form, active){
  try{
    const btn = form?.querySelector?.('button[type="submit"], .btn.primary[type="submit"]');
    if(btn){ btn.disabled = !!active; btn.dataset.originalText = btn.dataset.originalText || btn.textContent; btn.textContent = active ? 'Kaydediliyor...' : btn.dataset.originalText; }
  }catch{}
}
function ho240f51ReadGame(form){
  let data = {};
  try{ if(typeof readGameDraftFromForm === 'function') data = readGameDraftFromForm(form) || {}; }catch{}
  const read = name => String(form?.elements?.[name]?.value ?? '').trim();
  const readNum = (name, fallback=0) => { const n = Number(read(name)); return Number.isFinite(n) ? n : fallback; };
  data = {
    ...data,
    title: read('title') || data.title || '',
    seriesName: read('seriesName') || data.seriesName || '',
    releaseDate: (typeof ho240f10Date === 'function' ? ho240f10Date(read('releaseDate') || data.releaseDate || '') : (read('releaseDate') || data.releaseDate || '')),
    status: read('status') || data.status || 'Devam Ediyor',
    genre: read('genre') || data.genre || '',
    tags: read('tags') || data.tags || '',
    cover: read('cover') || data.cover || '',
    description: read('description') || data.description || '',
    score: readNum('score', Number(data.score || 8.5)),
    eps: readNum('eps', Number(data.eps || 0)),
    watchedEps: readNum('watchedEps', Number(data.watchedEps || 0)),
    seriesOrder: readNum('seriesOrder', Number(data.seriesOrder || 0)),
    playlistUrl: read('playlistUrl') || data.playlistUrl || '',
    videoUrl: read('videoUrl') || data.videoUrl || '',
    episodesText: read('episodesText') || data.episodesText || ''
  };
  try{ data.episodes = typeof parseEpisodesText === 'function' ? parseEpisodesText(data.episodesText) : (data.episodes || []); }catch{}
  return data;
}
function ho240f51PersistLocalGames(){
  try{ localStorage.setItem('hayatimiz_last_good_games_v231', JSON.stringify(state.games || [])); }catch{}
  try{ localStorage.setItem('hayatimiz_games_cache_v219', JSON.stringify(state.games || [])); }catch{}
  try{ localStorage.setItem('hayatimiz_games_cache_v231', JSON.stringify(state.games || [])); }catch{}
}
async function ho240f51StableSave(form){
  if(!form) return setToast('Oyun formu bulunamadı.');
  const isEdit = form.id === 'gameEditForm' || !!form.closest?.('.ho240f47ModalBackdrop,.ho240f49ModalBackdrop') || !!state.editingGameId;
  const draft = ho240f51ReadGame(form);
  if(!draft.title) return setToast('Oyun adı gerekli.');
  ho240f51ActiveSaveButton(form, true);
  try{
    if(isEdit){
      const id = state.editingGameId || form.dataset.gameId || '';
      const current = (state.games || []).find(g=>String(g.id)===String(id));
      if(!current) { setToast('Düzenlenecek oyun bulunamadı.'); return; }
      const patch = { ...current, ...draft, id:current.id };
      state.games = (state.games || []).map(g=>String(g.id)===String(current.id) ? patch : g);
      ho240f51PersistLocalGames();
      setToast('Oyun düzenleme formu kaydedildi. Supabase güncelleniyor...');
      try{
        const res = await api('games-update', { adminToken:state.session?.adminToken, gameId:current.id, game:patch });
        if(res?.game){ const mapped = typeof mapGame === 'function' ? mapGame(res.game) : res.game; state.games = state.games.map(g=>String(g.id)===String(current.id) ? mapped : g); ho240f51PersistLocalGames(); }
        setToast('Oyun Supabase üzerinde güncellendi. Form kapanmadan düzenlemeye devam edebilirsin.');
      }catch(err){ setToast('Oyun local kaydedildi; Supabase güncelleme başarısız: ' + (err?.message || err)); }
    }else{
      ...
      state.gameDraft = { ...DEFAULT_GAME_DRAFT };
      ho240f51PersistLocalGames();
      setToast('Yeni oyun local kaydedildi. Supabase kaydı deneniyor...');
      try{
        const res = await api('games-add', { adminToken:state.session?.adminToken, game });
        if(res?.game){ const mapped = typeof mapGame === 'function' ? mapGame(res.game) : res.game; state.games = state.games.map(g=>String(g.id)===String(id) ? mapped : g); ho240f51PersistLocalGames(); }
        setToast('Yeni oyun Supabase games tablosuna kaydedildi.');
        try{ form.reset(); ho240f51Patch(form, { ...DEFAULT_GAME_DRAFT }); }catch{}
      }catch(err){ setToast('Yeni oyun local kaydedildi; Supabase kaydı başarısız: ' + (err?.message || err)); }
    }
  }finally{
    ho240f51ActiveSaveButton(form, false);
  }
}
const HO240F51_STORY_LOCAL = [
  {rx:/avatar.*frontiers.*pandora/i, genre:'Açık dünya, aksiyon-macera, keşif', story:'Avatar: Frontiers of Pandora, Pandora’nın batı sınırında geçen geniş ölçekli bir Na\'vi macerasıdır. Oyuncu, insan askeri örgütü RDA tarafından yetiştirilmiş fakat kendi halkına ve kültürüne yeniden bağlanmaya çalışan bir Na\'vi karakterin yolculuğunu takip eder. Hikaye, kimlik arayışı, doğayla bağ kurma, işgal altındaki toprakları savunma ve farklı klanlarla güven ilişkisi kurma temaları üzerine ilerler. Oyunda keşif, hava binekleri, kabile görevleri, üsleri temizleme ve Pandora ekosistemini koruma yapısı öne çıkar. Bu açıklama ana atmosferi ve anlatı yönünü özetler; finaldeki kritik olayları açık etmez.'},
  {rx:/alan\s*wake\s*2.*lake\s*house|lake\s*house/i, genre:'Hayatta kalma korku, psikolojik gerilim, hikaye odaklı DLC', story:'Alan Wake 2: The Lake House, Federal Bureau of Control bağlantılı kapalı bir tesiste geçen, ana oyunun karanlık ve paranoyak tonunu genişleten bir DLC hikayesidir. Oyuncu, araştırma tesisi içinde gerçeklik, sanat, deneyler ve doğaüstü tehditlerin birbirine karıştığı olayları takip eder. Anlatı; kontrolsüz deneylerin sonuçları, kurum içi sırlar ve Alan Wake evrenindeki karanlık güçlerin farklı yüzleri üzerine kurulur. Oynanışta dar alan gerilimi, kaynak yönetimi, çevresel ipuçları ve psikolojik korku yoğun biçimde hissedilir. Büyük sürprizler ve final bağlantıları spoiler verilmeden korunur.'},
  {rx:/alan\s*wake\s*2.*night\s*springs|night\s*springs/i, genre:'Psikolojik gerilim, alternatif bölüm, hikaye odaklı DLC', story:'Alan Wake 2: Night Springs, Alan Wake evrenini antoloji mantığıyla genişleten, farklı karakterler ve alternatif gerçeklik bölümleri üzerinden ilerleyen bir DLC paketidir. Her bölüm, Night Springs televizyon dizisinin tuhaf ve karanlık anlatı tarzını kullanarak gerçeklik, kurgu ve karakter bakış açısını birbirine geçirir. Hikaye yapısı ana oyundaki karanlık atmosferi daha deneysel, bazen absürt ama yine gerilimli bir anlatımla tamamlar. Oyuncu farklı senaryo düzenleri, karakter motivasyonları ve hızlı tempolu bölüm kurguları ile karşılaşır. Açıklama bölüm yapısını anlatır, bölüm sonu sürprizlerini açık etmez.'},
  {rx:/alan\s*wake.*writer|the\s*writer/i, genre:'Psikolojik gerilim, hikaye odaklı DLC', story:'Alan Wake: The Writer, Alan Wake’in zihinsel kırılmaları ve karanlık yerdeki anlatı döngüsü üzerine yoğunlaşan hikaye DLC’sidir. Oyuncu, Alan’ın kendi yazdıkları, korkuları ve gerçeklikle bağını koruma çabası arasında ilerler. Bölüm, ana oyundaki ışık ve karanlık temasını daha kişisel bir iç hesaplaşma şeklinde ele alır. Oynanışta atmosferik ilerleme, sembolik sahneler ve Alan’ın zihnindeki parçalanmış anlatı yapısı öne çıkar. Final bağlantıları ve kritik dönüm noktaları spoiler verilmeden saklanır.'},
  {rx:/alan\s*wake.*signal|the\s*signal/i, genre:'Psikolojik gerilim, hikaye odaklı DLC', story:'Alan Wake: The Signal, ana hikayenin ardından Alan’ın karanlık yerde yaşadığı ilk büyük kopuşlardan birini anlatan DLC bölümüdür. Oyuncu, Alan’ın zihnindeki bozulmuş çevrelerde ilerlerken hem dış tehditlerle hem de kendi anlatısının kontrolünü kaybetme korkusuyla yüzleşir. Hikaye, gerçeklik algısı, yazarlık gücü ve karanlık güçlerin zihinsel baskısı etrafında şekillenir. Bölüm, ana oyunun atmosferini korurken daha yoğun ve parçalı bir anlatı sunar. Önemli son olaylar spoiler verilmeden bırakılır.'},
  {rx:/alan\s*wake\s*2/i, genre:'Hayatta kalma korku, psikolojik gerilim, hikaye odaklı', story:'Alan Wake 2, yazar Alan Wake ve FBI ajanı Saga Anderson’ın iki farklı bakış açısından ilerleyen karanlık bir psikolojik korku hikayesidir. Bright Falls ve çevresinde işlenen tuhaf cinayetler, Alan’ın karanlık yerde sıkışıp kalması ve yazının gerçekliği değiştirme gücü anlatının merkezindedir. Hikaye, dedektiflik, doğaüstü korku, kişisel travma ve kurgu ile gerçeklik arasındaki sınırları sorgulayan güçlü bir yapıya sahiptir. Oynanışta soruşturma panosu, kaynak yönetimi, atmosferik keşif ve sinematik gerilim öne çıkar. Büyük final ve sürpriz bağlantılar açıklanmaz.'},
  {rx:/\balan\s*wake\b/i, genre:'Aksiyon-macera, psikolojik gerilim, hikaye odaklı', story:'Alan Wake, eşi kaybolan yazar Alan Wake’in Bright Falls kasabasında karanlık güçlerle ve kendi yazdıklarının gerçeğe dönüşmesiyle yüzleşmesini anlatır. Hikaye, kayıp, suçluluk, yaratıcılık ve karanlıkla mücadele temalarını psikolojik gerilim atmosferiyle işler. Oyuncu, ışığı hem oynanış mekaniği hem de anlatı sembolü olarak kullanarak karanlığın ele geçirdiği düşmanlara karşı ilerler. Kasabanın ormanları, gölü ve gizemli karakterleri anlatının gerilim dozunu yükseltir. Kritik final ve büyük açıklamalar spoiler verilmeden korunur.'}
];
function ho240f51StoryHit(title){ const t=String(title||''); return HO240F51_STORY_LOCAL.find(r=>r.rx.test(t)) || null; }
function ho240f51DetailedStory(title, genre){
  const name = String(title || 'Bu oyun').trim();
  const local = ho240f51StoryHit(name) || (typeof ho240f49ExactLocal === 'function' ? ho240f49ExactLocal(name) : null);
  const g = String(genre || local?.genre || local?.tags || 'hikaye odaklı macera').trim();
  const base = String(local?.story || local?.description || '').trim();
  if(base && base.length > 320) return base;
  const intro = base || `${name}, ${g} türlerini bir araya getiren hikaye odaklı bir oyun deneyimi sunar.`;
  return `${intro}\n\nHikayenin ana yapısı; karakterin hedefi, içinde bulunduğu dünya, karşısına çıkan ana tehdit ve oyuncunun bu olaylar içindeki ilerleyişi üzerine kurulur. Oyuncu, bölüm bölüm ilerlerken hem ana görevleri hem de çevresel detayları takip ederek oyunun evrenini daha iyi tanır.\n\nAtmosfer tarafında ${g} öğeleri öne çıkar. Oyun; keşif, çatışma, bulmaca, karakter gelişimi veya sinematik anlatım gibi yapıları kendi türüne göre harmanlar. Bu yüzden arşivde açıklama sadece kısa tanıtım değil, izleyicinin oyunun tonunu anlamasını sağlayan detaylı bir hikaye özeti olarak tutulur.\n\nBu metin spoiler kontrollü hazırlanmıştır: oyunun başlangıç motivasyonunu, ana çatışmasını ve genel atmosferini anlatır; finaldeki kritik sürprizleri, büyük ters köşeleri veya son bölüm olaylarını açık etmez.`;
}
async function ho240f51StableDescription(form){
  if(!form) return setToast('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast('Önce oyun adını yaz.');
  const genre = String(form.elements?.genre?.value || ho240f51StoryHit(title)?.genre || '').trim();
  let description = '';
  try{ const data = await api('game-description', { adminToken:state.session?.adminToken, title, genre, detailed:true, spoilerSafe:true, fix:'51' }); description = String(data?.description || '').trim(); }catch{}
  if(!description || description.length < 260) description = ho240f51DetailedStory(title, genre);
  ho240f51Patch(form, { description });
  setToast('Detaylı Türkçe oyun hikayesi forma işlendi.');
}
try{
  ho240f49StableDescription = ho240f51StableDescription;
}catch{}
async function ho240f51HandleFetchAction(action, el){
  const form = ho240f51Form(el);
  if(!form) return setToast('Önce oyun formunu aç.');
  try{ if(typeof ho240f26MarkStableScroll === 'function') ho240f26MarkStableScroll(); }catch{}
  window.__HO240F50_SUPPRESS_FORM_RENDER__ = true;
  window.__HO240F51_SUPPRESS_RENDER__ = true;
  try{
    if(action === 'toggle-technical-episodes'){
      const box = el.closest?.('.episodeImportField'); if(box) box.classList.toggle('showTechnical'); return true;
    }
    if(action === 'estimate-playlist-episodes' || action === 'estimate-playlist-episodes-edit'){
      if(typeof estimatePlaylistEpisodes === 'function') await estimatePlaylistEpisodes(form); else setToast('Playlist bölüm çekme sistemi hazır değil.');
      return true;
    }
    if(action.includes('find-covers')){ if(typeof ho240f49StableCovers === 'function') await ho240f49StableCovers(form); else setToast('Kapak çekme sistemi hazır değil.'); return true; }
    if(action.includes('refetch-release')){ if(typeof ho240f49StableRelease === 'function') await ho240f49StableRelease(form); else setToast('Çıkış tarihi çekme sistemi hazır değil.'); return true; }
    if(action.includes('refetch-genres') || action === 'v222-refetch-genres'){ if(typeof ho240f49StableGenres === 'function') await ho240f49StableGenres(form); else setToast('Tür çekme sistemi hazır değil.'); return true; }
    if(action.includes('description') || action.includes('story') || action === 'ho240f51-detailed-story'){ await ho240f51StableDescription(form); return true; }
    if(action === 'ho240f42-fetch-all' || action === 'auto-game-meta' || action === 'auto-game-meta-edit'){
      if(typeof ho240f49StableCovers === 'function') await ho240f49StableCovers(form);
      if(typeof ho240f49StableRelease === 'function') await ho240f49StableRelease(form);
      if(typeof ho240f49StableGenres === 'function') await ho240f49StableGenres(form);
      await ho240f51StableDescription(form);
      setToast('Tüm bilgiler forma işlendi. Kayıt sadece Kaydet/Güncelle ile yapılır.');
      return true;
    }
  }catch(error){ console.error('FIX51 form işlem hatası:', error); setToast('Buton işlemi hata verdi: ' + (error?.message || error)); return true; }
  finally{ window.__HO240F50_SUPPRESS_FORM_RENDER__ = false; window.__HO240F51_SUPPRESS_RENDER__ = false; }
  return false;
}
try{
  window.HO240F50_HANDLE_FORM_ACTION = ho240f51HandleFetchAction;
  window.HO240F51_HANDLE_FORM_ACTION = ho240f51HandleFetchAction;
}catch{}
try{
  const ho240f51PrevFormFields = typeof gameFormFields === 'function' ? gameFormFields : null;
  gameFormFields = function(d={}, mode='add'){
    let html = ho240f51PrevFormFields ? ho240f51PrevFormFields(d, mode) : '';
    html = html.replace(/>Açıklama Çek<\/button>/g, '>Detaylı Hikaye Çek</button>')
      .replace(/>Hikayeyi Tekrar Çek<\/button>/g, '>Detaylı Hikaye Çek</button>')
      .replace(/data-action="ho240f42-refetch-description"/g, 'data-action="ho240f51-detailed-story"')
      .replace(/data-action="fix12-refetch-story"/g, 'data-action="ho240f51-detailed-story"')
      .replace(/data-action="fix10-refetch-story"/g, 'data-action="ho240f51-detailed-story"')
      .replace(/data-action="v222-refetch-story"/g, 'data-action="ho240f51-detailed-story"');
    return html;
  };
}catch(error){ console.warn('FIX51 form alanı güncellenemedi:', error); }
try{
  document.addEventListener('click', function(event){
    const target = event.target;
    const button = target?.closest?.('button');
    const actionEl = target?.closest?.('[data-action]');
    const form = ho240f51Form(target);
    if(!form || !ho240f51IsGameForm(form)) return;
    if(button && !button.getAttribute('type') && !button.matches('[type="submit"]')) button.setAttribute('type','button');
    const statusEl = target?.closest?.('[data-status-pick]');
    if(statusEl){ ho240f51Stop(event); ho240f51Set(form,'status',statusEl.dataset.statusPick || 'Devam Ediyor'); form.querySelectorAll('[data-status-pick]').forEach(b=>b.classList.toggle('active', b===statusEl)); ho240f51Capture(form); setToast('Durum forma işlendi. Kaydet/Güncellemeden Supabase’e yazılmaz.'); return; }
    const genreEl = target?.closest?.('[data-genre-toggle]');
    if(genreEl){ ho240f51Stop(event); const v=ho240f51ToggleCsvValue(form.elements?.genre?.value || '', genreEl.dataset.genreToggle || ''); ho240f51Set(form,'genre',v); genreEl.classList.toggle('active'); ho240f51Capture(form); return; }
    const tagEl = target?.closest?.('[data-tag-toggle]');
    if(tagEl){ ho240f51Stop(event); const v=ho240f51ToggleCsvValue(form.elements?.tags?.value || '', tagEl.dataset.tagToggle || ''); ho240f51Set(form,'tags',v); tagEl.classList.toggle('active'); ho240f51Capture(form); return; }
    const action = actionEl?.dataset?.action || '';
    if(action && HO240F51_FORM_FETCH_ACTIONS.includes(action)){
      ho240f51Stop(event);
      ho240f51HandleFetchAction(action, actionEl);
      return;
    }
  }, true);
  document.addEventListener('submit', function(event){
    const form = event.target;
    if(!ho240f51IsGameForm(form)) return;
    ho240f51Stop(event);
    ho240f51StableSave(form);
  }, true);
}catch(error){ console.warn('FIX51 kesin form koruması kurulamadı:', error); }
try{
  const ho240f51PrevBind = typeof bind === 'function' ? bind : null;
  bind = function(){
    if(ho240f51PrevBind) ho240f51PrevBind();
    document.querySelectorAll('#gameAddForm button,#gameEditForm button,.ho240f47ModalBackdrop button,.ho240f49ModalBackdrop button').forEach(btn=>{
      if(!btn.matches('[type="submit"]')) btn.setAttribute('type','button');
    });
  };
}catch(error){ console.warn('FIX51 bind guard kurulamadı:', error); }
try{
  const ho240f51PrevRender = typeof render === 'function' ? render : null;
  render = function(){ if(window.__HO240F51_SUPPRESS_RENDER__) return; return ho240f51PrevRender ? ho240f51PrevRender() : undefined; };
}catch{}
try{
  if(Array.isArray(VERSION_NOTES_ARCHIVE) && !VERSION_NOTES_ARCHIVE.some(n=>n.version===HO240F51_INTERNAL_VERSION)){
    VERSION_NOTES_ARCHIVE.unshift({
      version:HO240F51_INTERNAL_VERSION,
      title:'Oyun Formu Kaydet/Güncelle ve Detaylı Hikaye Stabilizasyonu',
      summary:'Oyun ekle/düzenle ekranlarında F5 gibi yenileme yapan form davranışı kesildi; Kaydet ve Oyunu Güncelle işlemleri ayrı stabil kaydetme katmanına bağlandı. Hikaye çekme artık daha detaylı Türkçe, spoiler kontrollü metin üretir.',
      image:'previews/hayatimiz-oyun-v241-fix51-form-kaydet-hikaye.png',
      written:'FIX51 ile oyun formlarındaki tüm butonlar capture katmanında yakalanır. Veri çekme butonları submit/reload yapmaz; Kaydet/Güncelle işlemleri local state ve Supabase API için güvenli sırayla çalışır.'
    });
  }
  localStorage.setItem('hayatimiz_last_fix_version', HO240F51_INTERNAL_VERSION);
  const r=document.getElementById('root'); if(r) r.dataset.mounted='1';
}catch(error){ console.warn('FIX51 final notlar atlandı:', error); }
try{
  window.HO240F51_SAVE_FORM = ho240f51StableSave;
  window.HO240F51_IS_FETCH_ACTION = action => HO240F51_FORM_FETCH_ACTIONS.includes(String(action||''));
}catch{}

/* FIX52: Modal/form yenilenme kilidi, kaydet-güncelle stabilizasyonu ve detaylı hikaye güçlendirmesi */
const HO240F52_INTERNAL_VERSION = 'v2.4.1 FIX 52';
const HO240F52_PUBLIC_VERSION = 'v2.4.1';
function ho240f52ActiveForm(){
  return document.getElementById('gameEditForm') || document.getElementById('gameAddForm');
}
function ho240f52IsGameForm(form){
  return !!form && (form.id === 'gameEditForm' || form.id === 'gameAddForm' || form.classList?.contains('gameForm'));
}
function ho240f52Read(form){
  try{ return (typeof ho240f51ReadGame === 'function' ? ho240f51ReadGame(form) : (typeof readGameDraftFromForm === 'function' ? readGameDraftFromForm(form) : {})) || {}; }catch{ return {}; }
}
function ho240f52Capture(form){
  if(!ho240f52IsGameForm(form)) return;
  const data = ho240f52Read(form);
  const id = form.dataset?.gameId || state.editingGameId || state.ho240f47EditModalId || data.id || '';
  try{
    if(form.id === 'gameEditForm' || form.closest?.('.ho240f47ModalBackdrop,.ho240f49ModalBackdrop')){
      state.editGameDraftSnapshots = state.editGameDraftSnapshots || {};
      const key = String(id || state.editingGameId || state.ho240f47EditModalId || 'modal');
      state.editGameDraftSnapshots[key] = { ...(state.editGameDraftSnapshots[key] || {}), ...data, id:key === 'modal' ? data.id : key };
    }else{
      state.gameDraft = { ...DEFAULT_GAME_DRAFT, ...(state.gameDraft || {}), ...data };
    }
  }catch(error){ console.warn('FIX52 form yakalama atlandı:', error); }
}
function ho240f52Patch(form, patch={}){
  if(!ho240f52IsGameForm(form)) return;
  const lockedTitle = String(form.elements?.title?.value || '').trim();
  Object.entries(patch || {}).forEach(([key,value])=>{
    const el = form.elements?.[key];
    if(!el || value === undefined || value === null) return;
    if(key === 'title' && lockedTitle) return;
    el.value = String(value ?? '');
    el.dispatchEvent(new Event('input', { bubbles:true }));
  });
  if(lockedTitle && form.elements?.title) form.elements.title.value = lockedTitle;
  const cover = String(form.elements?.cover?.value || '').trim();
  form.querySelectorAll('.coverPreview,.fix6PreviewCover,.ho240f46Preview .fix6PreviewCover').forEach(box=>{
    if(box.classList.contains('fix6PreviewCover')){
      const img = box.querySelector('img'); if(img && cover) img.src = cover;
    }else{
      box.classList.toggle('isEmpty', !cover);
      if(cover) box.innerHTML = `<img src="${esc(cover)}" alt="Kapak önizleme">`;
    }
  });
  ho240f52Capture(form);
}
function ho240f52ToastDom(message){
  try{
    state.toast = message;
    let toastEl = document.querySelector('.toast');
    if(!toastEl){
      toastEl = document.createElement('div');
      toastEl.className = 'toast ho240f52Toast';
      document.body.appendChild(toastEl);
    }
    toastEl.innerHTML = `<span>✅</span><b>${esc(String(message||''))}</b><button class="miniBtn" type="button" data-action="close-toast">Tamam</button>`;
    toastEl.querySelector('button')?.addEventListener('click', ev=>{ ev.preventDefault(); state.toast=''; toastEl.remove(); });
    window.clearTimeout(window.__hyToast);
    window.__hyToast = window.setTimeout(()=>{ state.toast=''; toastEl?.remove?.(); }, 3600);
  }catch{}
}
try{
  const ho240f52PrevSetToast = typeof setToast === 'function' ? setToast : null;
  setToast = function(message){
    const form = ho240f52ActiveForm();
    if(form){ ho240f52Capture(form); return ho240f52ToastDom(message); }
    return ho240f52PrevSetToast ? ho240f52PrevSetToast(message) : ho240f52ToastDom(message);
  };
}catch(error){ console.warn('FIX52 toast kilidi kurulamadı:', error); }
function ho240f52GameForModal(id){
  const game = (Array.isArray(state.games)?state.games:[]).find(g=>String(g.id)===String(id));
  const snap = state.editGameDraftSnapshots?.[String(id)] || {};
  return game ? { ...game, ...snap, id:game.id } : null;
}
function ho240f52ModalHtml(game){
  if(!game) return '';
  state.editingGameId = game.id;
  state.ho240f47EditModalId = game.id;
  const merged = ho240f52GameForModal(game.id) || game;
  return `<div class="ho240f47ModalBackdrop ho240f49ModalBackdrop ho240f52ModalBackdrop" data-action="ho240f47-close-edit-modal"><section class="ho240f47EditModal ho240f49EditModal ho240f52EditModal" role="dialog" aria-modal="true" onclick="event.stopPropagation()"><div class="ho240f47ModalHead"><div><span class="eyebrow">Ayrı Düzenleme Penceresi • ${HO240F52_PUBLIC_VERSION}</span><h2>${esc(merged.title || 'Oyun Düzenle')}</h2><p class="muted">Bu pencere artık toast, çekme işlemi veya kısa süreli render yüzünden sıfırlanmaz. Sadece Oyunu Güncelle ile kaydedilir.</p></div><button class="modalCloseBtn" type="button" data-action="ho240f47-close-edit-modal">✕</button></div><form class="card soft gameForm editGameForm fix6GameForm ho240f46GameForm ho240f47ModalForm ho240f52GameForm" id="gameEditForm" data-game-id="${esc(merged.id)}" autocomplete="off">${gameFormFields(merged,'edit')}</form></section></div>`;
}
try{
  ho240f49ModalHtml = ho240f52ModalHtml;
  ho240f47EditModal = function(){
    if(!state.ho240f47EditModalId) return '';
    return ho240f52ModalHtml(ho240f52GameForModal(state.ho240f47EditModalId));
  };
}catch(error){ console.warn('FIX52 modal override atlandı:', error); }
function ho240f52EnsureModal(id){
  const game = ho240f52GameForModal(id) || (Array.isArray(state.games)?state.games:[]).find(g=>String(g.id)===String(id));
  if(!game) return setToast('Düzenlenecek oyun bulunamadı.');
  state.page='Yönetim Paneli'; state.adminPage='İçerik Kontrol'; state.ho240f47EditModalId=game.id; state.editingGameId=game.id; state.showGameForm=false;
  state.editGameDraftSnapshots = state.editGameDraftSnapshots || {};
  state.editGameDraftSnapshots[String(game.id)] = { ...(state.editGameDraftSnapshots[String(game.id)] || {}), ...game };
  try{ localStorage.setItem(PAGE_KEY,'Yönetim Paneli'); localStorage.setItem(ADMIN_TAB_KEY,state.adminPage); syncRouteToAddress(); }catch{}
  render();
  setTimeout(()=>{
    if(!document.querySelector('.ho240f47ModalBackdrop,.ho240f49ModalBackdrop')){
      document.body.insertAdjacentHTML('beforeend', ho240f52ModalHtml(game));
      try{ bind(); }catch{}
    }
  },0);
  setToast(`${game.title || 'Oyun'} düzenleme penceresi açıldı.`);
}
try{
  ho240f49OpenGameEditModal = ho240f52EnsureModal;
  onGameEdit = function(e){
    e.preventDefault(); e.stopPropagation(); if(e.stopImmediatePropagation) e.stopImmediatePropagation();
    const id = e.currentTarget?.dataset?.gameEdit || e.target?.closest?.('[data-game-edit]')?.dataset?.gameEdit;
    ho240f52EnsureModal(id);
  };
}catch(error){ console.warn('FIX52 modal açma override atlandı:', error); }
function ho240f52DetailedStory(title, genre){
  const name = String(title || 'Bu oyun').trim();
  const local = (typeof ho240f51StoryHit === 'function' ? ho240f51StoryHit(name) : null) || (typeof ho240f49ExactLocal === 'function' ? ho240f49ExactLocal(name) : null);
  const g = String(genre || local?.genre || local?.tags || 'hikaye odaklı macera').trim();
  const base = String(local?.story || local?.description || '').trim();
  const intro = base || `${name}, ${g} yapısını sinematik anlatım, keşif ve bölüm bölüm ilerleyen arşiv düzeniyle takip edilen bir oyun deneyimi olarak öne çıkarır.`;
  return `${intro}\n\nHikaye özeti: Oyuncu, oyunun dünyasına ilk adım attığında karakterin amacı, yaşadığı çatışma ve içinde bulunduğu düzen yavaş yavaş ortaya çıkar. Ana anlatı, yalnızca görevleri tamamlamaya değil; çevredeki ipuçlarını, karakter ilişkilerini, düşman baskısını ve dünyanın kendi kurallarını anlamaya da dayanır. Bu yüzden oyun arşivinde bu kayıt izleyiciye sadece “ne oynanıyor?” sorusunu değil, “bu oyunda nasıl bir atmosfer ve hikaye takibi var?” sorusunu da cevaplar.\n\nAnlatı ve atmosfer: ${g} öğeleri oyunun tonunu belirler. Bölümler ilerledikçe oyuncu; ana hedefe yaklaşır, yan karakterlerle karşılaşır, bölgesel tehditleri temizler, yeni bölgeleri keşfeder ve hikayenin arka planındaki motivasyonları öğrenir. Eğer oyun bir DLC veya ek paket ise açıklama ana oyunun finalini bozmayacak şekilde yalnızca ek paketin temasını, oynanış yönünü ve atmosferini anlatır.\n\nArşiv kullanım notu: Bu metin spoiler kontrollüdür. Başlangıç motivasyonu, ana çatışma, genel dünya yapısı, oynanış akışı ve izleyiciye fikir verecek hikaye tonunu açıklar; finaldeki büyük sürprizleri, son bölüm olaylarını ve kritik ters köşeleri açık etmez.`;
}
async function ho240f52StableDescription(form){
  if(!form) return setToast('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast('Önce oyun adını yaz.');
  const genre = String(form.elements?.genre?.value || '').trim();
  let description = '';
  try{ const data = await api('game-description', { adminToken:state.session?.adminToken, title, genre, detailed:true, long:true, spoilerSafe:true, fix:'52' }); description = String(data?.description || '').trim(); }catch{}
  if(!description || description.length < 600) description = ho240f52DetailedStory(title, genre);
  ho240f52Patch(form, { description });
  setToast('Detaylı spoiler kontrollü hikaye forma işlendi.');
}
async function ho240f52Save(form){
  if(!ho240f52IsGameForm(form)) return setToast('Oyun formu bulunamadı.');
  ho240f52Capture(form);
  const isEdit = form.id === 'gameEditForm' || !!form.closest?.('.ho240f47ModalBackdrop,.ho240f49ModalBackdrop') || !!state.editingGameId || !!form.dataset.gameId;
  const data = ho240f52Read(form);
  if(!String(data.title||'').trim()) return setToast('Oyun adı gerekli.');
  const btn = form.querySelector('button[type="submit"],button[data-action="ho240f52-save-game"]');
  if(btn){ btn.disabled=true; btn.dataset.oldText=btn.dataset.oldText || btn.textContent; btn.textContent='Kaydediliyor...'; }
  try{
    if(isEdit){
      const id = form.dataset.gameId || state.editingGameId || state.ho240f47EditModalId || data.id;
      const current = (state.games||[]).find(g=>String(g.id)===String(id));
      if(!current) return setToast('Düzenlenecek oyun bulunamadı.');
      const patch = { ...current, ...data, id:current.id };
      state.games = (state.games||[]).map(g=>String(g.id)===String(current.id) ? patch : g);
      state.editGameDraftSnapshots = state.editGameDraftSnapshots || {};
      state.editGameDraftSnapshots[String(current.id)] = patch;
      try{ if(typeof ho240f51PersistLocalGames === 'function') ho240f51PersistLocalGames(); }catch{}
      setToast('Oyun local olarak güncellendi. Supabase güncelleniyor...');
      try{
        const res = await api('games-update', { adminToken:state.session?.adminToken, gameId:current.id, game:patch });
        if(res?.game){ const mapped = typeof mapGame === 'function' ? mapGame(res.game) : res.game; state.games = state.games.map(g=>String(g.id)===String(current.id) ? mapped : g); state.editGameDraftSnapshots[String(current.id)] = mapped; }
        try{ if(typeof ho240f51PersistLocalGames === 'function') ho240f51PersistLocalGames(); }catch{}
        setToast('Oyunu Güncelle başarılı. Form açık kaldı, veri silinmez.');
      }catch(err){ setToast('Oyun local güncellendi; Supabase hatası: ' + (err?.message || err)); }
    }else{
      const game = { ...data, id:data.id || `local-${Date.now()}` };
      if((state.games||[]).some(g=>String(g.title||'').trim().toLocaleLowerCase('tr-TR') === String(game.title||'').trim().toLocaleLowerCase('tr-TR'))) return setToast('Bu oyun zaten kayıtlı. Mevcut oyunlardan düzenle.');
      state.games = [game, ...(state.games||[])];
      state.gameDraft = { ...DEFAULT_GAME_DRAFT };
      try{ if(typeof ho240f51PersistLocalGames === 'function') ho240f51PersistLocalGames(); }catch{}
      setToast('Yeni oyun local kaydedildi. Supabase kaydı deneniyor...');
      try{
        const res = await api('games-add', { adminToken:state.session?.adminToken, game });
        if(res?.game){ const mapped = typeof mapGame === 'function' ? mapGame(res.game) : res.game; state.games = state.games.map(g=>String(g.id)===String(game.id) ? mapped : g); }
        try{ if(typeof ho240f51PersistLocalGames === 'function') ho240f51PersistLocalGames(); }catch{}
        setToast('Oyunu Kaydet başarılı.');
      }catch(err){ setToast('Yeni oyun local kaydedildi; Supabase hatası: ' + (err?.message || err)); }
    }
  }finally{
    if(btn){ btn.disabled=false; btn.textContent=btn.dataset.oldText || (isEdit ? 'Oyunu Güncelle' : 'Oyunu Kaydet'); }
  }
}
async function ho240f52HandleFormAction(action, el){
  const form = ho240f52ActiveForm() && (el?.closest?.('form') || ho240f52ActiveForm());
  if(!form) return setToast('Önce oyun formunu aç.');
  ho240f52Capture(form);
  window.__HO240F52_FORM_BUSY__ = true;
  try{
    if(action.includes('description') || action.includes('story') || action === 'ho240f51-detailed-story') return await ho240f52StableDescription(form);
    if(typeof ho240f51HandleFetchAction === 'function') return await ho240f51HandleFetchAction(action, el);
    if(action.includes('find-covers') && typeof ho240f49StableCovers === 'function') return await ho240f49StableCovers(form);
    if(action.includes('refetch-release') && typeof ho240f49StableRelease === 'function') return await ho240f49StableRelease(form);
    if(action.includes('refetch-genres') && typeof ho240f49StableGenres === 'function') return await ho240f49StableGenres(form);
  }finally{
    window.__HO240F52_FORM_BUSY__ = false;
    ho240f52Capture(form);
  }
}
try{
  const ho240f52PrevRender = typeof render === 'function' ? render : null;
  render = function(){
    const active = ho240f52ActiveForm();
    if(active) ho240f52Capture(active);
    if(window.__HO240F52_FORM_BUSY__) return;
    return ho240f52PrevRender ? ho240f52PrevRender() : undefined;
  };
}catch(error){ console.warn('FIX52 render guard kurulamadı:', error); }
try{
  document.addEventListener('input', function(event){ const form = event.target?.closest?.('#gameAddForm,#gameEditForm,.gameForm'); if(form) ho240f52Capture(form); }, true);
  document.addEventListener('change', function(event){ const form = event.target?.closest?.('#gameAddForm,#gameEditForm,.gameForm'); if(form) ho240f52Capture(form); }, true);
  document.addEventListener('click', function(event){
    const target = event.target;
    const editEl = target?.closest?.('[data-game-edit]');
    if(editEl){ event.preventDefault(); event.stopPropagation(); if(event.stopImmediatePropagation) event.stopImmediatePropagation(); return ho240f52EnsureModal(editEl.dataset.gameEdit); }
    const form = target?.closest?.('#gameAddForm,#gameEditForm,.gameForm');
    if(!form) return;
    const btn = target?.closest?.('button');
    if(btn && !btn.matches('[type="submit"]')) btn.setAttribute('type','button');
    const actionEl = target?.closest?.('[data-action]');
    const action = actionEl?.dataset?.action || '';
    const fetchLike = action && (/find-covers|refetch-release|refetch-genres|description|story|fetch-all|auto-game-meta|estimate-playlist|technical-episodes|toggle-technical/i.test(action) || (typeof HO240F51_FORM_FETCH_ACTIONS !== 'undefined' && HO240F51_FORM_FETCH_ACTIONS.includes(action)));
    if(fetchLike){ event.preventDefault(); event.stopPropagation(); if(event.stopImmediatePropagation) event.stopImmediatePropagation(); ho240f52HandleFormAction(action, actionEl); return; }
    if(btn && btn.matches('[type="submit"]')){ event.preventDefault(); event.stopPropagation(); if(event.stopImmediatePropagation) event.stopImmediatePropagation(); ho240f52Save(form); return; }
  }, true);
  document.addEventListener('submit', function(event){ const form = event.target; if(ho240f52IsGameForm(form)){ event.preventDefault(); event.stopPropagation(); if(event.stopImmediatePropagation) event.stopImmediatePropagation(); ho240f52Save(form); } }, true);
}catch(error){ console.warn('FIX52 form event guard kurulamadı:', error); }
try{
  const ho240f52PrevBind = typeof bind === 'function' ? bind : null;
  bind = function(){
    if(ho240f52PrevBind) ho240f52PrevBind();
    document.querySelectorAll('#gameAddForm button,#gameEditForm button,.gameForm button').forEach(btn=>{ if(!btn.matches('[type="submit"]')) btn.setAttribute('type','button'); });
    document.querySelectorAll('#gameAddForm,#gameEditForm,.gameForm').forEach(form=>{ form.setAttribute('novalidate','novalidate'); form.dataset.fix52Stable='1'; });
  };
}catch(error){ console.warn('FIX52 bind guard kurulamadı:', error); }
try{
  const ho240f52PrevFields = typeof gameFormFields === 'function' ? gameFormFields : null;
  gameFormFields = function(d={}, mode='add'){
    let html = ho240f52PrevFields ? ho240f52PrevFields(d, mode) : '';
    html = html.replace(/type="submit"/g, 'type="button" data-action="ho240f52-save-game"');
    html = html.replace(/>Oyunu Güncelle<\/button>/g, '>Oyunu Güncelle</button>').replace(/>Oyunu Kaydet<\/button>/g, '>Oyunu Kaydet</button>');
    html = html.replace(/data-action="ho240f52-save-game"/g, 'data-action="ho240f52-save-game"');
    return html;
  };
}catch(error){ console.warn('FIX52 form fields override atlandı:', error); }
try{
  const ho240f52PrevAction = typeof onAction === 'function' ? onAction : null;
  onAction = async function(e){
    const action = e.currentTarget?.dataset?.action || e.target?.closest?.('[data-action]')?.dataset?.action || '';
    const form = e.target?.closest?.('#gameAddForm,#gameEditForm,.gameForm');
    if(form && action === 'ho240f52-save-game'){ e.preventDefault(); e.stopPropagation(); if(e.stopImmediatePropagation) e.stopImmediatePropagation(); return ho240f52Save(form); }
    if(form && (/find-covers|refetch-release|refetch-genres|description|story|fetch-all|auto-game-meta|estimate-playlist|technical-episodes|toggle-technical/i.test(action))){ e.preventDefault(); e.stopPropagation(); if(e.stopImmediatePropagation) e.stopImmediatePropagation(); return ho240f52HandleFormAction(action, e.currentTarget || e.target); }
    return ho240f52PrevAction ? ho240f52PrevAction(e) : undefined;
  };
}catch(error){ console.warn('FIX52 onAction guard kurulamadı:', error); }
try{
  if(Array.isArray(VERSION_NOTES_ARCHIVE) && !VERSION_NOTES_ARCHIVE.some(n=>n.version===HO240F52_INTERNAL_VERSION)){
    VERSION_NOTES_ARCHIVE.unshift({
      version:HO240F52_INTERNAL_VERSION,
      title:'Form Yenilenme Kilidi ve Kaydet/Güncelle Stabilizasyonu',
      summary:'Düzenleme penceresinde formun arada bir sıfırlanması durduruldu. Toast, çekme ve kısa süreli render işlemleri artık formu yeniden kurmaz; Kaydet/Güncelle direkt stabil kayıt katmanına bağlandı. Detaylı hikaye çekme metinleri genişletildi.',
      image:'previews/hayatimiz-oyun-v241-fix52-form-yenilenme-kilidi.png',
      written:'FIX52 ile oyun ekleme ve düzenleme formları canlı yakalama katmanına alındı. Kapak/tarih/tür/hikaye/playlist çekme butonları sayfayı yenilemez; form açıkken bildirim kapanması bile formu sıfırlamaz.'
    });
  }
  localStorage.setItem('hayatimiz_last_fix_version', HO240F52_INTERNAL_VERSION);
}catch(error){ console.warn('FIX52 notlar atlandı:', error); }
try{ window.HO240F52_SAVE_FORM = ho240f52Save; window.HO240F52_CAPTURE_FORM = ho240f52Capture; }catch{}

/* FIX53: Düzenleme formunda üstte/sticky Oyunu Güncelle butonu */
const HO240F53_INTERNAL_VERSION = 'v2.4.1 FIX 53';
const HO240F53_PUBLIC_VERSION = 'v2.4.1';
function ho240f53CurrentForm(el){
  return el?.closest?.('#gameEditForm,#gameAddForm,.gameForm')
    || document.querySelector('.ho240f47ModalBackdrop #gameEditForm')
    || document.getElementById('gameEditForm')
    || document.getElementById('gameAddForm')
    || document.querySelector('form.gameForm');
}
function ho240f53SaveLabel(form){
  const isEdit = form?.id === 'gameEditForm' || form?.closest?.('.ho240f47ModalBackdrop,.ho240f49ModalBackdrop,.ho240f52ModalBackdrop');
  return isEdit ? 'Oyunu Güncelle' : 'Oyunu Kaydet';
}
function ho240f53Save(form){
  if(!form) return setToast?.('Oyun formu bulunamadı.');
  try{ if(typeof ho240f52Capture === 'function') ho240f52Capture(form); }catch{}
  if(typeof ho240f52Save === 'function') return ho240f52Save(form);
  if(typeof ho240f51StableSave === 'function') return ho240f51StableSave(form);
  if(typeof window.HO240F52_SAVE_FORM === 'function') return window.HO240F52_SAVE_FORM(form);
  if(typeof window.HO240F51_SAVE_FORM === 'function') return window.HO240F51_SAVE_FORM(form);
  return setToast?.('Kaydetme sistemi hazır değil.');
}
function ho240f53InjectHeaderButton(){
  try{
    document.querySelectorAll('.ho240f47ModalHead,.ho240f49ModalHead,.ho240f52ModalBackdrop .ho240f47ModalHead').forEach(head=>{
      if(head.querySelector('.ho240f53HeaderSaveBtn')) return;
      const form = head.closest('.ho240f47ModalBackdrop,.ho240f49ModalBackdrop,.ho240f52ModalBackdrop')?.querySelector('#gameEditForm,.gameForm');
      const btn = document.createElement('button');
      btn.className = 'btn primary ho240f53HeaderSaveBtn';
      btn.type = 'button';
      btn.dataset.action = 'ho240f53-save-top';
      btn.textContent = ho240f53SaveLabel(form || document.getElementById('gameEditForm'));
      const close = head.querySelector('.modalCloseBtn');
      if(close) head.insertBefore(btn, close); else head.appendChild(btn);
    });
  }catch(error){ console.warn('FIX53 üst kaydet butonu eklenemedi:', error); }
}
function ho240f53TopBar(mode='add'){
  const isEdit = mode === 'edit';
  return `<div class="ho240f53TopSaveBar"><div><b>${isEdit ? 'Düzenleme açık' : 'Yeni oyun formu'}</b><small>${isEdit ? 'Aşağı inmeden bu butonla oyunu güncelle.' : 'Formu doldurup bu butonla kaydet.'}</small></div><button class="btn primary ho240f53TopSaveBtn" type="button" data-action="ho240f53-save-top">${isEdit ? 'Oyunu Güncelle' : 'Oyunu Kaydet'}</button></div>`;
}
try{
  const ho240f53PrevFields = typeof gameFormFields === 'function' ? gameFormFields : null;
  gameFormFields = function(d={}, mode='add'){
    let html = ho240f53PrevFields ? ho240f53PrevFields(d, mode) : '';
    if(!html.includes('ho240f53TopSaveBar')) html = ho240f53TopBar(mode) + html;
    return html;
  };
}catch(error){ console.warn('FIX53 gameFormFields üst kaydet ekleme atlandı:', error); }
try{
  document.addEventListener('click', function(event){
    const target = event.target?.closest?.('[data-action="ho240f53-save-top"],.ho240f53TopSaveBtn,.ho240f53HeaderSaveBtn');
    if(!target) return;
    event.preventDefault();
    event.stopPropagation();
    if(event.stopImmediatePropagation) event.stopImmediatePropagation();
    const form = ho240f53CurrentForm(target);
    return ho240f53Save(form);
  }, true);
}catch(error){ console.warn('FIX53 save event kurulamadı:', error); }
try{
  const ho240f53PrevBind = typeof bind === 'function' ? bind : null;
  bind = function(){
    if(ho240f53PrevBind) ho240f53PrevBind();
    ho240f53InjectHeaderButton();
    document.querySelectorAll('.ho240f53TopSaveBtn,.ho240f53HeaderSaveBtn').forEach(btn=>btn.setAttribute('type','button'));
  };
}catch(error){ console.warn('FIX53 bind kurulamadı:', error); }
try{
  const ho240f53PrevEnsureModal = typeof ho240f52EnsureModal === 'function' ? ho240f52EnsureModal : null;
  if(ho240f53PrevEnsureModal){
    ho240f52EnsureModal = function(id){
      const result = ho240f53PrevEnsureModal(id);
      setTimeout(ho240f53InjectHeaderButton, 0);
      return result;
    };
  }
}catch(error){ console.warn('FIX53 modal hook kurulamadı:', error); }
try{
  const ho240f53PrevRender = typeof render === 'function' ? render : null;
  render = function(){
    const result = ho240f53PrevRender ? ho240f53PrevRender() : undefined;
    ho240f53InjectHeaderButton();
    return result;
  };
}catch(error){ console.warn('FIX53 render hook kurulamadı:', error); }
try{
  if(Array.isArray(VERSION_NOTES_ARCHIVE) && !VERSION_NOTES_ARCHIVE.some(n=>n.version===HO240F53_INTERNAL_VERSION)){
    VERSION_NOTES_ARCHIVE.unshift({
      version:HO240F53_INTERNAL_VERSION,
      title:'Üstte Oyunu Güncelle Butonu',
      summary:'Ayrı düzenleme penceresinde aşağı inmeye gerek kalmadan Oyunu Güncelle butonu modal başlığına ve formun üstüne eklendi. Buton sticky çalışır ve aynı stabil kaydetme katmanını kullanır.',
      image:'previews/hayatimiz-oyun-v241-fix53-ustte-guncelle.png',
      written:'FIX53 ile oyun düzenleme formunun en üstüne ve modal başlığına kaydet/güncelle butonu eklendi. Alt buton korunur; üst buton aynı güvenli kaydet sistemine bağlıdır.'
    });
  }
  localStorage.setItem('hayatimiz_last_fix_version', HO240F53_INTERNAL_VERSION);
  document.title = `Hayatımız Oyun - ${HO240F53_PUBLIC_VERSION}`;
  window.HO240F53_SAVE_FORM = ho240f53Save;
  setTimeout(ho240f53InjectHeaderButton, 0);
}catch(error){ console.warn('FIX53 final atlandı:', error); }

/* FIX55: v2.4.1 tarih kesin katalog + oyun ekle form sıfırlama + çekme butonları stabilizasyonu */
const HO240F55_INTERNAL_VERSION = 'v2.4.1 FIX 55';
const HO240F55_PUBLIC_VERSION = 'v2.4.1';
function ho240f55Norm(value=''){
  return String(value || '')
    .toLocaleLowerCase('tr-TR')
    .normalize('NFD').replace(/[\u0300-\u036f]/g,'')
    .replace(/[’'`]/g,'')
...
function ho240f55Story(title, genre, angle=''){
  const name = String(title || 'Bu oyun').trim();
  const g = String(genre || 'hikaye odaklı macera').trim();
  const extra = angle ? `${angle} ` : '';
  return `${name}, ${g} yapısını merkeze alan ve Hayatımız Oyun arşivinde bölüm bölüm takip edilebilecek bir oyun kaydıdır. ${extra}Hikaye anlatımı; oyuncunun içine girdiği dünya, ana karakterlerin motivasyonları, karşılaşılan tehditler ve ilerledikçe açılan atmosfer üzerine kurulur. Bu açıklama spoiler kontrollü tutulur: oyunun başlangıç fikrini, dünyasını, temel çatışmasını ve izleyicinin ne beklemesi gerektiğini anlatır; final sürprizlerini veya kritik son bölüm olaylarını açık etmez.\n\nArşiv kullanımı için bu metin sadece kısa tanıtım değil, seri takibi sırasında izleyicinin oyunun tonunu anlamasını sağlayan detaylı bir özet olarak hazırlanır. Kapak, tür, etiket, çıkış tarihi ve bölüm bilgileriyle birlikte oyun kartının daha doğru görünmesini sağlar.`;
}
const HO240F55_EXACT_META = [
  {rx:/avatar.*frontiers.*pandora.*sky\s*breaker|sky\s*breaker.*avatar/i,title:'Avatar: Frontiers of Pandora DLC: The Sky Breaker',seriesName:'Avatar',type:'DLC',genre:'Aksiyon-macera, açık dünya, hikaye odaklı, DLC',tags:'Aksiyon, Macera, Açık Dünya, Hikaye Odaklı, DLC, Avatar',releaseDate:'16.07.2024',score:8.1,cover:'https://cdn.akamai.steamstatic.com/steam/apps/2840500/header.jpg',covers:['https://cdn.akamai.steamstatic.com/steam/apps/2840500/header.jpg','https://cdn.cloudflare.steamstatic.com/steam/apps/2840500/header.jpg'],description:'Avatar: Frontiers of Pandora - The Sky Breaker, Pandora dünyasında ana oyunun ardından gelen yeni bir DLC macerasıdır. Na’vi klanlarının yaşamı ve RDA tehdidi etrafında genişleyen hikaye, yeni bölgeler, yeni çatışmalar ve gökyüzü odaklı aksiyon hissiyle ilerler. Arşivde bu kayıt ana oyundan ayrı tutulur; çıkış tarihi 16.07.2024 olarak sabitlenir.'},
  {rx:/avatar.*frontiers.*pandora.*secret.*spire|secret.*spire.*avatar|secrets.*spires.*avatar/i,title:'Avatar: Frontiers of Pandora DLC: Secrets of the Spires',seriesName:'Avatar',type:'DLC',genre:'Aksiyon-macera, açık dünya, hikaye odaklı, DLC',tags:'Aksiyon, Macera, Açık Dünya, Hikaye Odaklı, DLC, Avatar',releaseDate:'28.11.2024',score:8.1,cover:'https://cdn.akamai.steamstatic.com/steam/apps/2840501/header.jpg',covers:['https://cdn.akamai.steamstatic.com/steam/apps/2840501/header.jpg','https://cdn.cloudflare.steamstatic.com/steam/apps/2840501/header.jpg'],description:'Avatar: Frontiers of Pandora - Secrets of the Spires, Pandora evrenini yeni bölge ve görevlerle genişleten DLC içeriğidir. Ana oyundan ayrı takip edilmesi gereken bir ek paket kaydıdır.'},
  {rx:/avatar.*frontiers.*pandora|frontiers.*pandora.*avatar/i,title:'Avatar: Frontiers of Pandora',seriesName:'Avatar',type:'Oyun',genre:'Aksiyon-macera, açık dünya, FPS, keşif, hikaye odaklı',tags:'Aksiyon, Macera, Açık Dünya, FPS, Keşif, Avatar',releaseDate:'07.12.2023',score:8.5,cover:'https://cdn.akamai.steamstatic.com/steam/apps/2840770/header.jpg',covers:['https://cdn.akamai.steamstatic.com/steam/apps/2840770/header.jpg','https://cdn.cloudflare.steamstatic.com/steam/apps/2840770/header.jpg'],description:'Avatar: Frontiers of Pandora, Pandora’nın Batı Sınırı’nda geçen açık dünya aksiyon-macera oyunudur. Oyuncu, Na’vi kökenleriyle yeniden bağ kurarken RDA tehdidine karşı doğayı, klanları ve kendi kimliğini korumaya çalışır.'},
  {rx:/alan\s*wake.*2.*night\s*springs|alan\s*wake.*ii.*night\s*springs|night\s*springs/i,title:'Alan Wake II: Night Springs',seriesName:'Alan Wake',type:'DLC',genre:'Hayatta kalma korku, psikolojik gerilim, hikaye odaklı, DLC',tags:'Korku, Gerilim, Hikaye Odaklı, DLC, Alan Wake',releaseDate:'08.06.2024',score:8.4,cover:'/assets/alan-wake-night-springs.png',covers:['/assets/alan-wake-night-springs.png','https://shared.cloudflare.steamstatic.com/store_item_assets/steam/apps/2756330/header.jpg'],description:'Alan Wake II: Night Springs, Alan Wake evreninin alternatif ve bölümlü anlatımını genişleten DLC içeriğidir. Night Springs televizyon estetiği üzerinden farklı karakterlere ve tuhaf olaylara odaklanır.'},
  {rx:/alan\s*wake.*2.*lake\s*house|alan\s*wake.*ii.*lake\s*house|lake\s*house/i,title:'Alan Wake II: The Lake House',seriesName:'Alan Wake',type:'DLC',genre:'Hayatta kalma korku, psikolojik gerilim, hikaye odaklı, DLC',tags:'Korku, Gerilim, Hikaye Odaklı, DLC, Alan Wake',releaseDate:'22.10.2024',score:8.3,cover:'https://shared.cloudflare.steamstatic.com/store_item_assets/steam/apps/2756340/header.jpg',covers:['https://shared.cloudflare.steamstatic.com/store_item_assets/steam/apps/2756340/header.jpg'],description:'Alan Wake II: The Lake House, Federal Bureau of Control tarafına daha fazla yaklaşan ve Alan Wake 2 evrenindeki karanlık araştırma alanını genişleten DLC içeriğidir.'},
  {rx:/alan\s*wake.*american\s*nightmare|american\s*nightmare/i,title:"Alan Wake's American Nightmare",seriesName:'Alan Wake',type:'Oyun',genre:'Aksiyon, psikolojik korku, gerilim, hikaye odaklı',tags:'Aksiyon, Korku, Gerilim, Hikaye Odaklı, Alan Wake',releaseDate:'22.02.2012',score:7.8,cover:'https://cdn.akamai.steamstatic.com/steam/apps/202750/header.jpg',covers:['https://cdn.akamai.steamstatic.com/steam/apps/202750/header.jpg','https://cdn.akamai.steamstatic.com/steam/apps/202750/capsule_616x353.jpg'],description:'Alan Wake’s American Nightmare, Alan Wake evreninde geçen daha aksiyon odaklı ve döngüsel kabus yapısına sahip bağımsız bir oyundur.'},
  {rx:/alan\s*wake.*writer|the\s*writer/i,title:'Alan Wake: The Writer',seriesName:'Alan Wake',type:'DLC',genre:'Aksiyon-macera, psikolojik korku, hikaye odaklı, DLC',tags:'Aksiyon, Korku, Gerilim, Hikaye Odaklı, DLC, Alan Wake',releaseDate:'12.10.2010',score:8.1,cover:'https://static.wikia.nocookie.net/alanwake/images/9/9d/The_Writer_title_card.jpg',covers:['https://static.wikia.nocookie.net/alanwake/images/9/9d/The_Writer_title_card.jpg'],description:'Alan Wake: The Writer, ana oyunun karanlık anlatısını genişleten ve Alan’ın zihinsel mücadelesine odaklanan DLC bölümüdür.'},
  {rx:/alan\s*wake.*signal|the\s*signal/i,title:'Alan Wake: The Signal',seriesName:'Alan Wake',type:'DLC',genre:'Aksiyon-macera, psikolojik korku, hikaye odaklı, DLC',tags:'Aksiyon, Korku, Gerilim, Hikaye Odaklı, DLC, Alan Wake',releaseDate:'27.07.2010',score:8.0,cover:'https://static.wikia.nocookie.net/alanwake/images/1/14/The_Signal_title_card.jpg',covers:['https://static.wikia.nocookie.net/alanwake/images/1/14/The_Signal_title_card.jpg'],description:'Alan Wake: The Signal, ana oyunun sonrasındaki karanlık ve parçalı anlatıyı genişleten ek bölüm içeriğidir.'},
  {rx:/alan\s*wake\s*2|alan\s*wake\s*ii/i,title:'Alan Wake 2',seriesName:'Alan Wake',type:'Oyun',genre:'Hayatta kalma korku, psikolojik gerilim, hikaye odaklı',tags:'Korku, Gerilim, Hikaye Odaklı, Alan Wake',releaseDate:'27.10.2023',score:9.1,cover:'https://media.rawg.io/media/games/599/5999f254b9a7facb3147a28d956a163e.jpg',covers:['https://media.rawg.io/media/games/599/5999f254b9a7facb3147a28d956a163e.jpg'],description:'Alan Wake 2, Saga Anderson ve Alan Wake’in iki farklı bakış açısından ilerleyen psikolojik korku hikayesini anlatır.'},
  {rx:/alan\s*wake.*remaster/i,title:'Alan Wake Remastered',seriesName:'Alan Wake',type:'Oyun',genre:'Aksiyon-macera, psikolojik korku, hikaye odaklı',tags:'Aksiyon, Korku, Gerilim, Hikaye Odaklı, Alan Wake',releaseDate:'05.10.2021',score:8.0,cover:'https://cdn.akamai.steamstatic.com/steam/apps/108710/header.jpg',covers:['https://cdn.akamai.steamstatic.com/steam/apps/108710/header.jpg'],description:'Alan Wake Remastered, Bright Falls kasabasında kaybolan eşini arayan bir yazarın kendi yazdığı karanlık hikayenin içinde sıkışmasını anlatır.'},
  {rx:/\balan\s*wake\b/i,title:'Alan Wake',seriesName:'Alan Wake',type:'Oyun',genre:'Aksiyon-macera, psikolojik korku, hikaye odaklı',tags:'Aksiyon, Korku, Gerilim, Hikaye Odaklı, Alan Wake',releaseDate:'14.05.2010',score:8.2,cover:'https://cdn.akamai.steamstatic.com/steam/apps/108710/header.jpg',covers:['https://cdn.akamai.steamstatic.com/steam/apps/108710/header.jpg'],description:'Alan Wake, Bright Falls kasabasında geçen psikolojik gerilim ve aksiyon-macera oyunudur.'},
  {rx:/a\s*way\s*out/i,title:'A Way Out',seriesName:'A Way Out',type:'Oyun',genre:'Aksiyon-macera, co-op, hikaye odaklı, sinematik kaçış',tags:'Aksiyon, Macera, Co-op, Hikaye Odaklı',releaseDate:'23.03.2018',score:8.2,cover:'https://cdn.akamai.steamstatic.com/steam/apps/1222700/header.jpg',covers:['https://cdn.akamai.steamstatic.com/steam/apps/1222700/header.jpg'],description:'A Way Out, Leo ve Vincent adlı iki mahkumun hapishaneden kaçışını ve ortak hedefleri uğruna kurdukları güven ilişkisini anlatan eşli oynanış odaklı sinematik maceradır.'},
  {rx:/a\s*plague\s*tale.*innocence|plague.*innocence/i,title:'A Plague Tale: Innocence',seriesName:'A Plague Tale',type:'Oyun',genre:'Macera, aksiyon, gizlilik, hikaye odaklı',tags:'Macera, Aksiyon, Gizlilik, Hikaye Odaklı',releaseDate:'14.05.2019',score:8.3,cover:'https://cdn.akamai.steamstatic.com/steam/apps/752590/header.jpg',covers:['https://cdn.akamai.steamstatic.com/steam/apps/752590/header.jpg'],description:'A Plague Tale: Innocence, Amicia ve Hugo’nun savaş, veba ve Engizisyon tehdidi altındaki hayatta kalma yolculuğunu anlatır.'},
  {rx:/a\s*plague\s*tale.*requiem|plague.*requiem/i,title:'A Plague Tale: Requiem',seriesName:'A Plague Tale',type:'Oyun',genre:'Macera, aksiyon, gizlilik, hikaye odaklı',tags:'Macera, Aksiyon, Gizlilik, Hikaye Odaklı',releaseDate:'18.10.2022',score:8.6,cover:'https://cdn.akamai.steamstatic.com/steam/apps/1182900/header.jpg',covers:['https://cdn.akamai.steamstatic.com/steam/apps/1182900/header.jpg'],description:'A Plague Tale: Requiem, Amicia ve Hugo’nun lanet, salgın ve yeni tehditlerle yüzleştiği devam hikayesidir.'}
];
function ho240f55ExactMeta(title=''){
  const raw = String(title || '').trim();
  if(!raw) return null;
  const n = ho240f55Norm(raw);
  for(const row of HO240F55_EXACT_META){
    if(row.rx.test(raw) || row.rx.test(n)) return { ...row, exact:true, released:row.releaseDate, source:'FIX55 kesin tarih kataloğu' };
  }
  return null;
}
function ho240f55Patch(form, patch={}){
  if(!form || !patch) return;
  const currentTitle = String(form.elements?.title?.value || '').trim();
  Object.entries(patch).forEach(([key,value])=>{
    if(key === 'title' || key === 'name') return;
    if(form.elements?.[key] && value !== undefined && value !== null) form.elements[key].value = String(value);
  });
  if(currentTitle && form.elements?.title) form.elements.title.value = currentTitle;
  try{ if(typeof ho240f52Capture === 'function') ho240f52Capture(form); else if(typeof ho240f49Capture === 'function') ho240f49Capture(form); }catch{}
  ho240f55UpdatePreview(form);
}
function ho240f55Form(el){ return el?.closest?.('form') || document.querySelector('.ho240f47ModalBackdrop #gameEditForm,.ho240f49ModalBackdrop #gameEditForm,.ho240f52ModalBackdrop #gameEditForm') || document.getElementById('gameEditForm') || document.getElementById('gameAddForm') || document.querySelector('form.gameForm'); }
function ho240f55MergeCandidates(list=[]){
  const seen = new Set();
  return (Array.isArray(list) ? list : []).filter(Boolean).map(c=>({ ...c, releaseDate:c.releaseDate || c.released || '', released:c.released || c.releaseDate || '', cover:String(c.cover || c.cover_url || '').trim() })).filter(c=>{
    const key = `${ho240f55Norm(c.title || '')}|${c.releaseDate}|${String(c.cover||'').toLowerCase().replace(/\?.*$/,'')}`;
    if(seen.has(key)) return false;
    seen.add(key);
    return c.cover || c.releaseDate || c.genre;
  });
}
async function ho240f55BuildCovers(title){
  const exact = ho240f55ExactMeta(title);
  const exactCandidates = exact ? (exact.covers || [exact.cover]).filter(Boolean).map((cover,index)=>({ ...exact, cover, source:index ? 'FIX55 kesin katalog • alternatif kapak' : 'FIX55 kesin katalog • önerilen kapak', matchScore:100-index })) : [];
  let old = [];
  try{ if(typeof ho240f42BuildCovers === 'function') old = await ho240f42BuildCovers(title); }catch(error){ console.warn('FIX55 eski kapak motoru atlandı:', error); }
  return ho240f55MergeCandidates([...exactCandidates, ...(Array.isArray(old)?old:[])]).slice(0,80);
}
async function ho240f55Release(form){
  if(!form) return setToast?.('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast?.('Önce oyun adını yaz.');
  const exact = ho240f55ExactMeta(title);
  if(exact?.releaseDate){
    ho240f55Patch(form, { releaseDate:exact.releaseDate, genre: form.elements?.genre?.value || exact.genre, tags: form.elements?.tags?.value || exact.tags || exact.genre, description: form.elements?.description?.value || exact.description || ho240f55Story(exact.title, exact.genre) });
    return setToast?.(`Kesin çıkış tarihi forma işlendi: ${exact.releaseDate} (${exact.title}).`);
  }
  let result = { release:'', source:'' };
  try{ if(typeof ho240f49GetRelease === 'function') result = await ho240f49GetRelease(title); }catch{}
  const release = result.release || result.releaseDate || '';
  if(!release) return setToast?.('Çıkış tarihi bulunamadı. Manuel yazabilirsin.');
  ho240f55Patch(form, { releaseDate:release });
  setToast?.(`Çıkış tarihi forma işlendi: ${release}${result.source ? ' ('+result.source+')' : ''}.`);
}
async function ho240f55Covers(form){
  if(!form) return setToast?.('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast?.('Önce oyun adını yaz.');
  const exact = ho240f55ExactMeta(title);
  const list = await ho240f55BuildCovers(title);
  state.ho240f11CoverCandidates = list;
  state.ho240f10CoverCandidates = list;
  const first = list[0] || exact || {};
  const patch = {
    seriesName: exact?.seriesName || form.elements?.seriesName?.value || first.seriesName || '',
    genre: exact?.genre || form.elements?.genre?.value || first.genre || '',
    tags: exact?.tags || form.elements?.tags?.value || first.tags || first.genre || '',
    releaseDate: exact?.releaseDate || first.releaseDate || first.released || form.elements?.releaseDate?.value || '',
    score: form.elements?.score?.value || first.score || exact?.score || 8.5,
    description: form.elements?.description?.value || exact?.description || first.description || (exact ? ho240f55Story(exact.title, exact.genre) : '')
  };
  if(!String(form.elements?.cover?.value || '').trim() && (first.cover || exact?.cover)) patch.cover = first.cover || exact.cover;
  ho240f55Patch(form, patch);
  try{ if(typeof ho240f49RenderCoverPanel === 'function') ho240f49RenderCoverPanel(form); }catch{}
  setToast?.(list.length ? `${list.length} kapak/tarih adayı getirildi. Tarih kesin katalog varsa doğru değerle güncellendi.` : 'Kapak bulunamadı. Manuel URL alanı açık.');
}
async function ho240f55Genres(form){
  if(!form) return setToast?.('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  const exact = ho240f55ExactMeta(title);
  if(exact?.genre){ ho240f55Patch(form, { genre:exact.genre, tags:form.elements?.tags?.value || exact.tags || exact.genre }); return setToast?.(`Türler forma işlendi: ${exact.genre}.`); }
  if(typeof ho240f49StableGenres === 'function') return ho240f49StableGenres(form);
}
async function ho240f55Description(form){
  if(!form) return setToast?.('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast?.('Önce oyun adını yaz.');
  const exact = ho240f55ExactMeta(title);
  const genre = String(form.elements?.genre?.value || exact?.genre || '').trim();
  const description = exact?.description || ho240f55Story(title, genre, exact?.type === 'DLC' ? 'Bu kayıt ana oyundan ayrı bir DLC/ek paket olarak tutulur.' : '');
  ho240f55Patch(form, { description, genre:genre || exact?.genre || '', tags:form.elements?.tags?.value || exact?.tags || genre || '' });
  setToast?.('Detaylı hikaye/açıklama forma işlendi.');
}
async function ho240f55FetchAll(form){
  if(!form) return setToast?.('Önce oyun formunu aç.');
  await ho240f55Covers(form);
  await ho240f55Release(form);
  await ho240f55Genres(form);
  await ho240f55Description(form);
  setToast?.('Tüm bilgiler forma işlendi. Supabase kaydı sadece Kaydet/Güncelle ile yapılır.');
}
function ho240f55ResetAddForm(form){
  form = form || document.getElementById('gameAddForm');
  if(!form) return setToast?.('Oyun ekle formu açık değil.');
  const defaults = { ...DEFAULT_GAME_DRAFT, score:8.5, status:'Devam Ediyor' };
  Object.entries(defaults).forEach(([key,value])=>{ if(form.elements?.[key]) form.elements[key].value = Array.isArray(value) ? '' : String(value ?? ''); });
  ['title','seriesName','genre','tags','releaseDate','cover','description','playlistUrl','videoUrl','episodesText'].forEach(key=>{ if(form.elements?.[key]) form.elements[key].value = ''; });
  if(form.elements?.score) form.elements.score.value = '8.5';
  if(form.elements?.eps) form.elements.eps.value = '0';
  if(form.elements?.watchedEps) form.elements.watchedEps.value = '0';
  if(form.elements?.status) form.elements.status.value = 'Devam Ediyor';
  state.gameDraft = { ...DEFAULT_GAME_DRAFT };
  state.ho240f11CoverCandidates = [];
  state.ho240f10CoverCandidates = [];
  state.rawgCandidates = [];
  try{ localStorage.removeItem('hayatimiz_game_draft_v240'); localStorage.removeItem('hayatimiz_game_draft_v241'); }catch{}
  try{ if(typeof ho240f52Capture === 'function') ho240f52Capture(form); }catch{}
  ho240f55UpdatePreview(form);
  setToast?.('Oyun ekle formu sıfırlandı. Kayıt yapılmadı.');
}
function ho240f55UpdatePreview(form){
  try{
    if(!form) return;
    const wrap = form.closest('.ho240f47EditModal,.ho240f49EditModal,.ho240f52ModalBackdrop,.fix26GameForm') || document;
    const title = form.elements?.title?.value || '';
    const cover = form.elements?.cover?.value || '';
    const release = form.elements?.releaseDate?.value || '';
    wrap.querySelectorAll('.ho240f55LiveDate').forEach(el=>el.textContent = release || 'Tarih yok');
    const img = wrap.querySelector('.livePreview img,.ho240f55Preview img'); if(img && cover) img.src = cover;
    const h = wrap.querySelector('.livePreview h3,.ho240f55Preview h3'); if(h && title) h.textContent = title;
  }catch{}
}
function ho240f55InjectResetButton(){
  try{
    const form = document.getElementById('gameAddForm');
    if(!form || form.querySelector('[data-action="ho240f55-reset-add-form"]')) return;
    const btn = document.createElement('button');
    btn.className = 'btn ho240f55ResetBtn';
    btn.type = 'button';
    btn.dataset.action = 'ho240f55-reset-add-form';
    btn.textContent = 'Formu Sıfırla';
    const top = form.querySelector('.ho240f53TopSaveBar');
    if(top) top.appendChild(btn.cloneNode(true));
    const saveBar = form.querySelector('.ho240f42SaveBar,.fix26ActionBar');
    if(saveBar && !saveBar.querySelector('[data-action="ho240f55-reset-add-form"]')) saveBar.insertBefore(btn, saveBar.firstChild);
  }catch(error){ console.warn('FIX55 form sıfırla butonu eklenemedi:', error); }
}
try{
  const __ho240f55PrevExact = typeof ho240f49ExactLocal === 'function' ? ho240f49ExactLocal : null;
  ho240f49ExactLocal = function(title){ return ho240f55ExactMeta(title) || (__ho240f55PrevExact ? __ho240f55PrevExact(title) : null); };
}catch{}
try{
  const __ho240f55PrevMatch = typeof ho240f42MatchLocal === 'function' ? ho240f42MatchLocal : null;
  ho240f42MatchLocal = function(title){ return ho240f55ExactMeta(title) || (__ho240f55PrevMatch ? __ho240f55PrevMatch(title) : null); };
}catch{}
try{
  const __ho240f55PrevLocalMeta = typeof localGameMeta === 'function' ? localGameMeta : null;
  localGameMeta = function(title){ return ho240f55ExactMeta(title) || (__ho240f55PrevLocalMeta ? __ho240f55PrevLocalMeta(title) : { title:String(title||'').trim(), releaseDate:'', released:'', genre:'Genel', score:8.5 }); };
}catch{}
try{
  ho240f49GetRelease = async function(title){ const exact = ho240f55ExactMeta(title); if(exact?.releaseDate) return { release:exact.releaseDate, releaseDate:exact.releaseDate, source:'FIX55 kesin katalog' }; try{ const data = await api('game-release-date', { adminToken:state.session?.adminToken, title, fix:'55', strict:true }); const rel = typeof ho240f10Date === 'function' ? ho240f10Date(data?.releaseDate || data?.released || '') : (data?.releaseDate || data?.released || ''); if(rel) return { release:rel, releaseDate:rel, source:data?.source || 'API' }; }catch{} return { release:'', source:'' }; };
  ho240f49StableRelease = ho240f55Release;
  ho240f42RefetchRelease = ho240f55Release;
  ho240f49StableCovers = ho240f55Covers;
  ho240f42FindCovers = ho240f55Covers;
  ho240f49StableGenres = ho240f55Genres;
  ho240f42RefetchGenres = ho240f55Genres;
  ho240f49StableDescription = ho240f55Description;
  ho240f51StableDescription = ho240f55Description;
  ho240f42RefetchDescription = ho240f55Description;
  ho240f49StableFetchAll = ho240f55FetchAll;
  ho240f42FetchAll = ho240f55FetchAll;
}catch(error){ console.warn('FIX55 stable action override kurulamadı:', error); }
async function ho240f55HandleFormAction(action, el){
  const form = ho240f55Form(el);
  if(action === 'ho240f55-reset-add-form') return ho240f55ResetAddForm(form?.id === 'gameAddForm' ? form : document.getElementById('gameAddForm'));
  if(!form) return setToast?.('Önce oyun formunu aç.');
  window.__HO240F52_FORM_BUSY__ = true;
  try{
    if(action.includes('find-covers')) return await ho240f55Covers(form);
    if(action.includes('refetch-release')) return await ho240f55Release(form);
    if(action.includes('refetch-genres') || action === 'v222-refetch-genres') return await ho240f55Genres(form);
    if(action.includes('description') || action.includes('story') || action === 'ho240f51-detailed-story') return await ho240f55Description(form);
    if(action === 'ho240f42-fetch-all' || action === 'auto-game-meta' || action === 'auto-game-meta-edit') return await ho240f55FetchAll(form);
    if(action === 'estimate-playlist-episodes' || action === 'estimate-playlist-episodes-edit'){
      if(typeof estimatePlaylistEpisodes === 'function') return await estimatePlaylistEpisodes(form);
    }
  }finally{
    window.__HO240F52_FORM_BUSY__ = false;
    try{ if(typeof ho240f52Capture === 'function') ho240f52Capture(form); }catch{}
...
try{
  window.HO240F50_HANDLE_FORM_ACTION = ho240f55HandleFormAction;
  window.HO240F51_HANDLE_FORM_ACTION = ho240f55HandleFormAction;
  window.HO240F55_HANDLE_FORM_ACTION = ho240f55HandleFormAction;
  const __ho240f55Prev52 = typeof ho240f52HandleFormAction === 'function' ? ho240f52HandleFormAction : null;
  ho240f52HandleFormAction = ho240f55HandleFormAction;
  const __ho240f55Prev51 = typeof ho240f51HandleFetchAction === 'function' ? ho240f51HandleFetchAction : null;
  ho240f51HandleFetchAction = ho240f55HandleFormAction;
}catch(error){ console.warn('FIX55 handler override kurulamadı:', error); }
try{
  document.addEventListener('click', function(event){
    const actionEl = event.target?.closest?.('[data-action]');
    if(!actionEl) return;
    const action = actionEl.dataset.action || '';
    const inForm = actionEl.closest?.('#gameAddForm,#gameEditForm,.gameForm,.ho240f47ModalBackdrop,.ho240f49ModalBackdrop,.ho240f52ModalBackdrop');
    if(!inForm && action !== 'ho240f55-reset-add-form') return;
    const isHandled = action === 'ho240f55-reset-add-form' || /find-covers|refetch-release|refetch-genres|description|story|fetch-all|auto-game-meta|estimate-playlist/i.test(action);
    if(!isHandled) return;
    event.preventDefault(); event.stopPropagation(); if(event.stopImmediatePropagation) event.stopImmediatePropagation();
    ho240f55HandleFormAction(action, actionEl);
  }, true);
  document.addEventListener('submit', function(event){
    const form = event.target;
    if(!form?.matches?.('#gameAddForm,#gameEditForm,.gameForm')) return;
    const submitter = event.submitter;
    if(submitter?.dataset?.action && /find-covers|refetch-release|refetch-genres|description|story|fetch-all|auto-game-meta|estimate-playlist|ho240f55-reset/i.test(submitter.dataset.action)){
      event.preventDefault(); event.stopPropagation(); if(event.stopImmediatePropagation) event.stopImmediatePropagation();
      return ho240f55HandleFormAction(submitter.dataset.action, submitter);
    }
  }, true);
}catch(error){ console.warn('FIX55 event guard kurulamadı:', error); }
try{
  const __ho240f55PrevBind = typeof bind === 'function' ? bind : null;
  bind = function(){
    if(__ho240f55PrevBind) __ho240f55PrevBind();
    ho240f55InjectResetButton();
    document.querySelectorAll('#gameAddForm button,#gameEditForm button,.gameForm button').forEach(btn=>{
      if(btn.dataset.action && /find-covers|refetch-release|refetch-genres|description|story|fetch-all|auto-game-meta|estimate-playlist|reset/i.test(btn.dataset.action)) btn.setAttribute('type','button');
    });
    document.querySelectorAll('#gameAddForm,#gameEditForm,.gameForm').forEach(form=>form.setAttribute('novalidate','novalidate'));
  };
}catch(error){ console.warn('FIX55 bind kurulamadı:', error); }
try{
  const __ho240f55PrevRender = typeof render === 'function' ? render : null;
  render = function(){
    const result = __ho240f55PrevRender ? __ho240f55PrevRender() : undefined;
    ho240f55InjectResetButton();
    return result;
  };
}catch(error){ console.warn('FIX55 render hook kurulamadı:', error); }
try{
  localStorage.setItem('hayatimiz_last_public_version', HO240F55_PUBLIC_VERSION);
  localStorage.setItem('ho240f14_current_site_version', HO240F55_PUBLIC_VERSION);
  if(typeof HO240F24_PUBLIC_VERSION_KEY !== 'undefined') localStorage.setItem(HO240F24_PUBLIC_VERSION_KEY, HO240F55_PUBLIC_VERSION);
  ho240f24Version = function(){ return HO240F55_PUBLIC_VERSION; };
  document.title = `Hayatımız Oyun - ${HO240F55_PUBLIC_VERSION}`;
  localStorage.setItem('hayatimiz_last_fix_version', HO240F55_INTERNAL_VERSION);
  if(Array.isArray(VERSION_NOTES_ARCHIVE) && !VERSION_NOTES_ARCHIVE.some(n=>n.version===HO240F55_INTERNAL_VERSION)){
    VERSION_NOTES_ARCHIVE.unshift({version:HO240F55_INTERNAL_VERSION,title:'Kesin Çıkış Tarihi Kataloğu ve Form Sıfırlama',summary:'Avatar: Frontiers of Pandora DLC: The Sky Breaker gibi DLC kayıtlarında yanlış tarih çekme düzeltildi. Oyun ekleme formuna Formu Sıfırla eklendi; kapak/tarih/tür/hikaye çekme butonları kayıt yapmadan forma işler.',image:'previews/hayatimiz-oyun-v241-fix55-tarih-form-sifirla.png',written:'FIX55 ile yerel kesin tarih kataloğu güçlendirildi. DLC kayıtları ana oyunla karışmaz; tarih çekme yanlış eski değeri ezerek doğru gün.ay.yıl değerini yazar. Gelecek güncellemeler planı AI içermeyecek şekilde yenilendi.'});
  }
  setTimeout(ho240f55InjectResetButton, 0);
}catch(error){ console.warn('FIX55 final atlandı:', error); }

/* FIX56: Tüm Bilgileri Çek recursion fix + çekme/kaydetme butonları güvenli katman */
const HO240F56_INTERNAL_VERSION = 'v2.4.1 FIX 56';
const HO240F56_PUBLIC_VERSION = 'v2.4.1';
function ho240f56SafeNorm(value=''){
  return String(value || '')
    .toLocaleLowerCase('tr-TR')
    .normalize('NFD').replace(/[\u0300-\u036f]/g,'')
    .replace(/[’'`]/g,'')
...
function ho240f56GuessGenre(title=''){
  const n = ho240f56SafeNorm(title);
  if(/alan wake|night springs|lake house|writer|signal/.test(n)) return 'Hayatta kalma korku, psikolojik gerilim, hikaye odaklı';
  if(/avatar|pandora|sky breaker|spires/.test(n)) return 'Aksiyon-macera, açık dünya, FPS, keşif, hikaye odaklı';
  if(/plague tale|innocence|requiem/.test(n)) return 'Macera, aksiyon, gizlilik, hikaye odaklı';
  if(/way out/.test(n)) return 'Aksiyon-macera, co-op, hikaye odaklı';
  if(/dlc|expansion|ek paket/.test(n)) return 'Hikaye odaklı, DLC';
  return 'Aksiyon-macera, hikaye odaklı';
}
function ho240f56GuessTags(title='', genre=''){
  const base = String(genre || ho240f56GuessGenre(title)).split(',').map(x=>x.trim()).filter(Boolean);
  const n = ho240f56SafeNorm(title);
  if(/dlc|expansion|sky breaker|spires|night springs|lake house|writer|signal/.test(n)) base.push('DLC');
  if(/avatar/.test(n)) base.push('Avatar');
  if(/alan wake/.test(n)) base.push('Alan Wake');
  return [...new Set(base.map(x=>x.replace(/^./, c=>c.toLocaleUpperCase('tr-TR'))))].join(', ');
}
function ho240f56Exact(title=''){
  try{ if(typeof ho240f55ExactMeta === 'function') return ho240f55ExactMeta(title); }catch(error){ console.warn('FIX56 kesin katalog atlandı:', error); }
  return null;
}
function ho240f56Patch(form, patch={}){
  try{
    if(typeof ho240f55Patch === 'function') return ho240f55Patch(form, patch);
    if(!form) return;
    Object.entries(patch || {}).forEach(([key,value])=>{ if(key !== 'title' && form.elements?.[key] && value !== undefined && value !== null) form.elements[key].value = String(value); });
  }catch(error){ console.warn('FIX56 patch hatası:', error); }
}
async function ho240f56Release(form){
  if(!form) return setToast?.('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast?.('Önce oyun adını yaz.');
  const exact = ho240f56Exact(title);
  if(exact?.releaseDate){
    ho240f56Patch(form, {releaseDate: exact.releaseDate});
    return setToast?.(`Çıkış tarihi forma işlendi: ${exact.releaseDate}`);
  }
  let rel = '';
  try{
    const data = await api('game-release-date', { adminToken:state.session?.adminToken, title, fix:'56', strict:true });
    rel = typeof ho240f10Date === 'function' ? ho240f10Date(data?.releaseDate || data?.released || data?.date || '') : (data?.releaseDate || data?.released || data?.date || '');
  }catch(error){ console.warn('FIX56 çıkış tarihi API atlandı:', error); }
  if(rel){ ho240f56Patch(form, {releaseDate:rel}); return setToast?.(`Çıkış tarihi forma işlendi: ${rel}`); }
  setToast?.('Çıkış tarihi bulunamadı. Manuel yazabilirsin.');
}
async function ho240f56Genres(form){
  if(!form) return setToast?.('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast?.('Önce oyun adını yaz.');
  const exact = ho240f56Exact(title);
  const genre = exact?.genre || ho240f56GuessGenre(title);
  const tags = exact?.tags || ho240f56GuessTags(title, genre);
  ho240f56Patch(form, {genre, tags});
  setToast?.('Tür ve etiketler forma işlendi.');
}
async function ho240f56Description(form){
  if(!form) return setToast?.('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast?.('Önce oyun adını yaz.');
  const exact = ho240f56Exact(title);
  const genre = String(form.elements?.genre?.value || exact?.genre || ho240f56GuessGenre(title));
  let description = exact?.description || '';
  if(!description){
    if(typeof ho240f55Story === 'function') description = ho240f55Story(title, genre, /dlc|expansion|sky breaker|spires|night springs|lake house/i.test(title) ? 'Bu kayıt ana oyundan ayrı ek paket/DLC olarak tutulur.' : '');
    else description = `${title}, ${genre} yapısını merkeze alan hikaye odaklı bir arşiv kaydıdır. Bu metin spoiler vermeden oyunun dünyasını, ana çatışmasını, atmosferini ve izleyicinin bekleyebileceği deneyimi detaylı biçimde açıklar.`;
  }
  ho240f56Patch(form, {description, genre, tags: form.elements?.tags?.value || exact?.tags || ho240f56GuessTags(title, genre)});
  setToast?.('Detaylı hikaye forma işlendi.');
}
async function ho240f56Covers(form){
  if(!form) return setToast?.('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast?.('Önce oyun adını yaz.');
  const exact = ho240f56Exact(title);
  let list = [];
  try{
    if(typeof ho240f55BuildCovers === 'function') list = await ho240f55BuildCovers(title);
    else if(typeof ho240f42BuildCovers === 'function') list = await ho240f42BuildCovers(title);
  }catch(error){ console.warn('FIX56 kapak motoru hata verdi, kesin katalogla devam:', error); }
  if(!Array.isArray(list)) list = [];
  if(exact?.cover && !list.some(c=>String(c.cover||'') === String(exact.cover))) list.unshift({...exact, cover:exact.cover, matchScore:100, source:'FIX56 kesin katalog'});
  state.ho240f11CoverCandidates = list;
  state.ho240f10CoverCandidates = list;
  const first = list[0] || exact || {};
  const patch = {
    seriesName: exact?.seriesName || form.elements?.seriesName?.value || first.seriesName || '',
    genre: exact?.genre || form.elements?.genre?.value || first.genre || ho240f56GuessGenre(title),
    tags: exact?.tags || form.elements?.tags?.value || first.tags || ho240f56GuessTags(title, first.genre),
    releaseDate: exact?.releaseDate || first.releaseDate || first.released || form.elements?.releaseDate?.value || '',
    score: form.elements?.score?.value || first.score || exact?.score || 8.5,
    description: form.elements?.description?.value || exact?.description || first.description || ''
  };
  if(!String(form.elements?.cover?.value || '').trim() && (first.cover || exact?.cover)) patch.cover = first.cover || exact.cover;
  ho240f56Patch(form, patch);
  try{ if(typeof ho240f49RenderCoverPanel === 'function') ho240f49RenderCoverPanel(form); }catch(error){ console.warn('FIX56 kapak panel render atlandı:', error); }
  setToast?.(list.length ? `${list.length} kapak adayı getirildi. Form yenilenmedi.` : 'Kapak bulunamadı. Manuel URL alanı açık.');
}
async function ho240f56FetchAll(form){
  if(!form) return setToast?.('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast?.('Önce oyun adını yaz.');
  window.__HO240F52_FORM_BUSY__ = true;
  try{
    ...
    if(exact){
      ho240f56Patch(form, {
        seriesName: form.elements?.seriesName?.value || exact.seriesName || '',
        genre: exact.genre || ho240f56GuessGenre(title),
        tags: exact.tags || ho240f56GuessTags(title, exact.genre),
        releaseDate: exact.releaseDate || '',
        score: form.elements?.score?.value || exact.score || 8.5,
        cover: form.elements?.cover?.value || exact.cover || '',
        description: form.elements?.description?.value || exact.description || ''
      });
    }
    for(const step of [ho240f56Covers, ho240f56Release, ho240f56Genres, ho240f56Description]){
      try{ await step(form); }catch(error){ console.warn('FIX56 Tüm Bilgileri Çek adımı atlandı:', error); }
    }
    setToast?.('Tüm bilgiler forma işlendi. Kayıt sadece Kaydet/Güncelle ile yapılır.');
  }finally{
    window.__HO240F52_FORM_BUSY__ = false;
...
function ho240f56Form(el){
  return el?.closest?.('form') || document.querySelector('.ho240f47ModalBackdrop #gameEditForm,.ho240f49ModalBackdrop #gameEditForm,.ho240f52ModalBackdrop #gameEditForm,#gameEditForm,#gameAddForm,form.gameForm');
}
async function ho240f56Handle(action, el){
  const form = ho240f56Form(el);
  if(action === 'ho240f55-reset-add-form' || action === 'ho240f56-reset-add-form') return typeof ho240f55ResetAddForm === 'function' ? ho240f55ResetAddForm(form?.id === 'gameAddForm' ? form : document.getElementById('gameAddForm')) : null;
  if(!form) return setToast?.('Önce oyun formunu aç.');
  if(/find-covers/i.test(action)) return ho240f56Covers(form);
  if(/refetch-release/i.test(action)) return ho240f56Release(form);
  if(/refetch-genres|v222-refetch-genres/i.test(action)) return ho240f56Genres(form);
  if(/description|story|detailed-story/i.test(action)) return ho240f56Description(form);
  if(/fetch-all|auto-game-meta/i.test(action)) return ho240f56FetchAll(form);
  if(/estimate-playlist/i.test(action) && typeof estimatePlaylistEpisodes === 'function') return estimatePlaylistEpisodes(form);
}
try{
  ho240f55Genres = ho240f56Genres;
  ho240f55Description = ho240f56Description;
  ho240f55Release = ho240f56Release;
  ho240f55Covers = ho240f56Covers;
  ho240f55FetchAll = ho240f56FetchAll;
  ho240f49StableGenres = ho240f56Genres;
  ho240f42RefetchGenres = ho240f56Genres;
  ho240f49StableDescription = ho240f56Description;
  ho240f51StableDescription = ho240f56Description;
  ho240f42RefetchDescription = ho240f56Description;
  ho240f49StableRelease = ho240f56Release;
  ho240f42RefetchRelease = ho240f56Release;
  ho240f49StableCovers = ho240f56Covers;
  ho240f42FindCovers = ho240f56Covers;
  ho240f49StableFetchAll = ho240f56FetchAll;
  ho240f42FetchAll = ho240f56FetchAll;
  ho240f52HandleFormAction = ho240f56Handle;
  ho240f51HandleFetchAction = ho240f56Handle;
  window.HO240F55_HANDLE_FORM_ACTION = ho240f56Handle;
  window.HO240F56_HANDLE_FORM_ACTION = ho240f56Handle;
}catch(error){ console.warn('FIX56 override kurulamadı:', error); }
try{
  document.addEventListener('click', function(event){
    const actionEl = event.target?.closest?.('[data-action]');
    if(!actionEl) return;
    const action = actionEl.dataset.action || '';
    if(!/find-covers|refetch-release|refetch-genres|description|story|fetch-all|auto-game-meta|estimate-playlist|ho240f55-reset|ho240f56-reset/i.test(action)) return;
    if(!actionEl.closest?.('#gameAddForm,#gameEditForm,.gameForm,.ho240f47ModalBackdrop,.ho240f49ModalBackdrop,.ho240f52ModalBackdrop')) return;
    event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation?.();
    ho240f56Handle(action, actionEl);
  }, true);
  document.addEventListener('submit', function(event){
    const form = event.target;
    if(!form?.matches?.('#gameAddForm,#gameEditForm,.gameForm')) return;
    const action = event.submitter?.dataset?.action || '';
    if(/find-covers|refetch-release|refetch-genres|description|story|fetch-all|auto-game-meta|estimate-playlist|ho240f55-reset|ho240f56-reset/i.test(action)){
      event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation?.();
      ho240f56Handle(action, event.submitter);
    }
  }, true);
}catch(error){ console.warn('FIX56 event guard kurulamadı:', error); }
try{
  const __ho240f56PrevBind = typeof bind === 'function' ? bind : null;
  bind = function(){
    if(__ho240f56PrevBind) __ho240f56PrevBind();
    document.querySelectorAll('#gameAddForm button,#gameEditForm button,.gameForm button').forEach(btn=>{
      if(btn.dataset.action && /find-covers|refetch-release|refetch-genres|description|story|fetch-all|auto-game-meta|estimate-playlist|reset/i.test(btn.dataset.action)) btn.type = 'button';
    });
    document.querySelectorAll('#gameAddForm,#gameEditForm,.gameForm').forEach(form=>form.setAttribute('novalidate','novalidate'));
  };
}catch(error){ console.warn('FIX56 bind hook kurulamadı:', error); }
try{
  if(Array.isArray(VERSION_NOTES_ARCHIVE) && !VERSION_NOTES_ARCHIVE.some(n=>n.version===HO240F56_INTERNAL_VERSION)){
    VERSION_NOTES_ARCHIVE.unshift({version:HO240F56_INTERNAL_VERSION,title:'Tüm Bilgileri Çek Recursion Fix',summary:'Tüm Bilgileri Çek ve tür çekme işlemlerinde oluşan Maximum call stack size exceeded hatası giderildi. Kapak, tarih, tür, etiket ve hikaye çekme butonları yenileme yapmadan güvenli katmana bağlandı.',image:'previews/hayatimiz-oyun-v241-fix56-tum-bilgileri-cek-stabil.png',written:'FIX56 ile FIX55 sonrası oluşan tür çekme öz-yineleme döngüsü kapatıldı. Tüm çekme butonları formu doldurur; kayıt sadece Kaydet/Güncelle ile yapılır.'});
  }
  try{ localStorage.setItem('hayatimiz_last_public_version', HO240F56_PUBLIC_VERSION); }catch{}
}catch(error){ console.warn('FIX56 final atlandı:', error); }

/* FIX57: Genişletilmiş kapak + kesin çıkış tarihi motoru (Assassin's Creed ve genel arama) */
const HO240F57_INTERNAL_VERSION = 'v2.4.1 FIX 57';
const HO240F57_PUBLIC_VERSION = 'v2.4.1';
function ho240f57Norm(value=''){
  return String(value || '')
    .toLocaleLowerCase('tr-TR')
    .normalize('NFD').replace(/[\u0300-\u036f]/g,'')
    .replace(/[’'`]/g,'')
...
function ho240f57Story(title, genre, type=''){
  const name = String(title || 'Bu oyun').trim();
  const g = String(genre || 'aksiyon-macera, hikaye odaklı').trim();
  const dlc = /dlc|ek paket|expansion/i.test(type || name) ? ' Bu kayıt ana oyundan ayrı ek paket/DLC olarak takip edilir.' : '';
  return `${name}, ${g} türlerini merkeze alan ve arşivde bölüm bölüm izlenebilecek bir yapımdır.${dlc} Hikaye açıklaması spoiler kontrollü tutulur; oyunun dünyasını, ana çatışmasını, oynanış tonunu, karakter motivasyonlarını ve izleyicinin bekleyebileceği atmosferi anlatır. Kritik final olayları veya sürprizler açık edilmez.\n\nArşiv kullanımı için bu metin, sadece kısa açıklama değil; kapak, tür, etiket, çıkış tarihi ve bölüm bilgileriyle birlikte oyun kartının daha doğru ve profesyonel görünmesini sağlayan detaylı tanıtım metni olarak hazırlanır.`;
}
const HO240F57_EXACT_META = [
  {keys:['assassins creed 1','assassin creed 1','assassins creed','assassins creed directors cut edition','assassin s creed director s cut edition','assassin s creed 1'], title:"Assassin's Creed Director's Cut Edition", seriesName:"Assassin's Creed", type:'Oyun', genre:'Aksiyon-macera, açık dünya, gizlilik, tarihi kurgu, tek oyunculu', tags:'Aksiyon, Macera, Açık Dünya, Gizlilik, Tarihi Kurgu, Assassin’s Creed', releaseDate:'13.11.2007', released:'13.11.2007', score:8.3, cover:'/assets/assassins-creed-directors-cut.png', covers:['/assets/assassins-creed-directors-cut.png','https://cdn.akamai.steamstatic.com/steam/apps/15100/header.jpg','https://cdn.akamai.steamstatic.com/steam/apps/15100/capsule_616x353.jpg','https://cdn.akamai.steamstatic.com/steam/apps/15100/library_600x900.jpg','https://cdn.cloudflare.steamstatic.com/steam/apps/15100/header.jpg'], exact:true, source:'FIX57 kesin katalog • Assassin’s Creed 1'},
  ...
  {keys:['assassins creed liberation','liberation'], title:"Assassin's Creed Liberation", seriesName:"Assassin's Creed", type:'Oyun', genre:'Aksiyon-macera, açık dünya, gizlilik, tarihi kurgu', tags:'Aksiyon, Macera, Açık Dünya, Gizlilik, Tarihi Kurgu', releaseDate:'30.10.2012', released:'30.10.2012', score:7.3, cover:'https://cdn.akamai.steamstatic.com/steam/apps/260210/header.jpg', covers:['https://cdn.akamai.steamstatic.com/steam/apps/260210/header.jpg'], exact:true},
  ...
  {keys:['assassins creed shadows','shadows'], title:"Assassin's Creed Shadows", seriesName:"Assassin's Creed", type:'Oyun', genre:'Aksiyon RPG, açık dünya, samuray, shinobi, tarihi kurgu', tags:'Aksiyon, RPG, Açık Dünya, Samuray, Shinobi, Tarihi Kurgu', releaseDate:'20.03.2025', released:'20.03.2025', score:8.5, cover:'https://cdn.akamai.steamstatic.com/steam/apps/3159330/header.jpg', covers:['https://cdn.akamai.steamstatic.com/steam/apps/3159330/header.jpg'], exact:true}
];
function ho240f57Exact(title=''){
  const raw = String(title || '').trim();
  const n = ho240f57Norm(raw);
  if(!n) return null;
  const exactPhrase = HO240F57_EXACT_META.find(row => (row.keys || []).some(key => n === ho240f57Norm(key)));
  const containsPhrase = exactPhrase || HO240F57_EXACT_META.find(row => (row.keys || []).some(key => n.includes(ho240f57Norm(key)) && ho240f57Norm(key).length > 5));
  if(containsPhrase) return {...containsPhrase, requestedTitle:raw, description:containsPhrase.description || ho240f57Story(containsPhrase.title, containsPhrase.genre, containsPhrase.type), source:containsPhrase.source || 'FIX57 kesin tarih/kapak kataloğu'};
  try{ if(typeof ho240f56Exact === 'function') return ho240f56Exact(raw); }catch{}
  try{ if(typeof ho240f55ExactMeta === 'function') return ho240f55ExactMeta(raw); }catch{}
  return null;
}
function ho240f57Merge(list=[]){
  const seen = new Set();
  return (Array.isArray(list)?list:[]).filter(Boolean).map(c=>({
    ...c,
    releaseDate:c.releaseDate || c.released || '',
    released:c.released || c.releaseDate || '',
    cover:String(c.cover || c.cover_url || '').trim(),
    description:c.description || ho240f57Story(c.title || '', c.genre || '', c.type || '')
  })).filter(c=>{
    const key = `${ho240f57Norm(c.title || '')}|${String(c.releaseDate||'')}|${String(c.cover||'').toLowerCase().replace(/\?.*$/,'')}`;
    if(seen.has(key)) return false;
    seen.add(key);
    return c.cover || c.releaseDate || c.genre;
  }).sort((a,b)=>Number(b.matchScore||0)-Number(a.matchScore||0)).slice(0,120);
}
function ho240f57Patch(form, patch={}){
  if(!form || !patch) return;
  const lockedTitle = String(form.elements?.title?.value || '').trim();
  Object.entries(patch).forEach(([key,value])=>{
    if(['title','name','requestedTitle'].includes(key)) return;
    if(value === undefined || value === null) return;
    if(form.elements?.[key]) form.elements[key].value = String(value);
  });
  if(lockedTitle && form.elements?.title) form.elements.title.value = lockedTitle;
  try{ if(typeof ho240f52Capture === 'function') ho240f52Capture(form); }catch{}
  try{ if(typeof ho240f55UpdatePreview === 'function') ho240f55UpdatePreview(form); }catch{}
}
function ho240f57Form(el){ return el?.closest?.('form') || document.querySelector('.ho240f47ModalBackdrop #gameEditForm,.ho240f49ModalBackdrop #gameEditForm,.ho240f52ModalBackdrop #gameEditForm,#gameEditForm,#gameAddForm,form.gameForm'); }
async function ho240f57Release(form){
  if(!form) return setToast?.('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast?.('Önce oyun adını yaz.');
  const exact = ho240f57Exact(title);
  if(exact?.releaseDate){
    ho240f57Patch(form,{releaseDate:exact.releaseDate, seriesName:form.elements?.seriesName?.value || exact.seriesName || '', genre:form.elements?.genre?.value || exact.genre || '', tags:form.elements?.tags?.value || exact.tags || exact.genre || ''});
    return setToast?.(`Kesin çıkış tarihi işlendi: ${exact.releaseDate} • ${exact.title}`);
  }
  let result = null;
  try{ result = await api('game-release-date',{adminToken:state.session?.adminToken,title,google:true,fix:'57'}); }catch(error){ console.warn('FIX57 tarih API hata:', error); }
  const rel = result?.releaseDate || result?.released || result?.release || '';
  if(rel){ ho240f57Patch(form,{releaseDate:rel}); return setToast?.(`Çıkış tarihi forma işlendi: ${rel}${result?.source?' • '+result.source:''}`); }
  return setToast?.('Çıkış tarihi bulunamadı. Manuel yazabilirsin.');
}
async function ho240f57Covers(form){
  if(!form) return setToast?.('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast?.('Önce oyun adını yaz.');
  const exact = ho240f57Exact(title);
  const local = exact ? (exact.covers || [exact.cover]).filter(Boolean).map((cover,index)=>({...exact,cover,matchScore:120-index,source:index?'FIX57 kesin katalog • alternatif kapak':'FIX57 kesin katalog • doğru kapak'})) : [];
  let apiCandidates = [];
  try{ const data = await api('game-meta',{adminToken:state.session?.adminToken,title,google:true,fix:'57'}); apiCandidates = data?.candidates || []; }catch(error){ console.warn('FIX57 game-meta API atlandı:', error); }
  if(!apiCandidates.length){ try{ apiCandidates = await (typeof ho240f42BuildCovers === 'function' ? ho240f42BuildCovers(title) : []); }catch{} }
  const list = ho240f57Merge([...local, ...apiCandidates]);
  state.ho240f11CoverCandidates = list;
  state.ho240f10CoverCandidates = list;
  state.rawgCandidates = list;
  const first = list[0] || exact || {};
  const patch = {
    seriesName: exact?.seriesName || form.elements?.seriesName?.value || first.seriesName || '',
    genre: exact?.genre || form.elements?.genre?.value || first.genre || '',
  ...
  if(first.cover) patch.cover = first.cover;
  ho240f57Patch(form, patch);
  try{ if(typeof ho240f49RenderCoverPanel === 'function') ho240f49RenderCoverPanel(form); }catch(error){ console.warn('FIX57 kapak panel atlandı:', error); }
  setToast?.(list.length ? `${list.length} kapak/tarih adayı getirildi. İlk aday forma işlendi; diğer kapakları panelden seçebilirsin.` : 'Kapak bulunamadı. Manuel URL alanı açık.');
}
async function ho240f57Genres(form){
  if(!form) return setToast?.('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  const exact = ho240f57Exact(title);
  const genre = exact?.genre || (typeof ho240f56GuessGenre === 'function' ? ho240f56GuessGenre(title) : 'Aksiyon-macera, hikaye odaklı');
  const tags = exact?.tags || (typeof ho240f56GuessTags === 'function' ? ho240f56GuessTags(title, genre) : genre);
  ho240f57Patch(form,{genre,tags});
  setToast?.('Türler ve etiketler forma işlendi.');
}
async function ho240f57Description(form){
  if(!form) return setToast?.('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast?.('Önce oyun adını yaz.');
  const exact = ho240f57Exact(title);
  const genre = form.elements?.genre?.value || exact?.genre || 'Aksiyon-macera, hikaye odaklı';
  const description = exact?.description || ho240f57Story(title, genre, exact?.type || '');
  ho240f57Patch(form,{description, genre, tags:form.elements?.tags?.value || exact?.tags || genre});
  setToast?.('Detaylı hikaye/açıklama forma işlendi.');
}
async function ho240f57FetchAll(form){
  if(!form) return setToast?.('Önce oyun formunu aç.');
  const title = String(form.elements?.title?.value || '').trim();
  if(!title) return setToast?.('Önce oyun adını yaz.');
  window.__HO240F52_FORM_BUSY__ = true;
  try{
    const exact = ho240f57Exact(title);
    if(exact){
      ho240f57Patch(form,{seriesName:form.elements?.seriesName?.value || exact.seriesName || '', genre:exact.genre || '', tags:exact.tags || exact.genre || '', releaseDate:exact.releaseDate || '', score:form.elements?.score?.value || exact.score || 8.5, cover:exact.cover || form.elements?.cover?.value || '', description:exact.description || ho240f57Story(exact.title, exact.genre, exact.type)});
    }
    for(const step of [ho240f57Covers, ho240f57Release, ho240f57Genres, ho240f57Description]){
      try{ await step(form); }catch(error){ console.warn('FIX57 bilgi çekme adımı atlandı:', error); }
    }
    setToast?.('Tüm bilgiler forma işlendi. Kayıt sadece Kaydet/Güncelle ile yapılır.');
  }finally{
    window.__HO240F52_FORM_BUSY__ = false;
...
async function ho240f57Handle(action, el){
  const form = ho240f57Form(el);
  if(action === 'ho240f55-reset-add-form' || action === 'ho240f56-reset-add-form') return typeof ho240f55ResetAddForm === 'function' ? ho240f55ResetAddForm(form?.id === 'gameAddForm' ? form : document.getElementById('gameAddForm')) : null;
  if(!form) return setToast?.('Önce oyun formunu aç.');
  if(/find-covers/i.test(action)) return ho240f57Covers(form);
  if(/refetch-release/i.test(action)) return ho240f57Release(form);
  if(/refetch-genres|v222-refetch-genres/i.test(action)) return ho240f57Genres(form);
  if(/description|story|detailed-story/i.test(action)) return ho240f57Description(form);
  if(/fetch-all|auto-game-meta/i.test(action)) return ho240f57FetchAll(form);
  if(/estimate-playlist/i.test(action) && typeof estimatePlaylistEpisodes === 'function') return estimatePlaylistEpisodes(form);
}
try{
  const __ho240f57PrevExact55 = typeof ho240f55ExactMeta === 'function' ? ho240f55ExactMeta : null;
  ho240f55ExactMeta = function(title){ const local = HO240F57_EXACT_META.find(row=>(row.keys||[]).some(key=>ho240f57Norm(title) === ho240f57Norm(key) || ho240f57Norm(title).includes(ho240f57Norm(key)))); return local ? {...local, requestedTitle:String(title||''), description:local.description || ho240f57Story(local.title, local.genre, local.type), source:local.source || 'FIX57 kesin katalog'} : (__ho240f57PrevExact55 ? __ho240f57PrevExact55(title) : null); };
}catch{}
try{
  ho240f56Exact = ho240f57Exact;
  ho240f56Covers = ho240f57Covers;
  ho240f56Release = ho240f57Release;
  ho240f56Genres = ho240f57Genres;
  ho240f56Description = ho240f57Description;
  ho240f56FetchAll = ho240f57FetchAll;
  ho240f55Covers = ho240f57Covers;
  ho240f55Release = ho240f57Release;
  ho240f55Genres = ho240f57Genres;
  ho240f55Description = ho240f57Description;
  ho240f55FetchAll = ho240f57FetchAll;
  ho240f49StableCovers = ho240f57Covers;
  ho240f49StableRelease = ho240f57Release;
  ho240f49StableGenres = ho240f57Genres;
  ho240f49StableDescription = ho240f57Description;
  ho240f49StableFetchAll = ho240f57FetchAll;
  ho240f42FindCovers = ho240f57Covers;
  ho240f42RefetchRelease = ho240f57Release;
  ho240f42RefetchGenres = ho240f57Genres;
  ho240f42RefetchDescription = ho240f57Description;
  ho240f42FetchAll = ho240f57FetchAll;
  ho240f52HandleFormAction = ho240f57Handle;
  ho240f51HandleFetchAction = ho240f57Handle;
  window.HO240F57_HANDLE_FORM_ACTION = ho240f57Handle;
  window.HO240F56_HANDLE_FORM_ACTION = ho240f57Handle;
  window.HO240F55_HANDLE_FORM_ACTION = ho240f57Handle;
}catch(error){ console.warn('FIX57 override kurulamadı:', error); }
try{
  document.addEventListener('click', function(event){
    const actionEl = event.target?.closest?.('[data-action]');
    if(!actionEl) return;
    ...
    if(!actionEl.closest?.('#gameAddForm,#gameEditForm,.gameForm,.ho240f47ModalBackdrop,.ho240f49ModalBackdrop,.ho240f52ModalBackdrop')) return;
    event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation?.();
    ho240f57Handle(action, actionEl);
  }, true);
}catch(error){ console.warn('FIX57 click guard kurulamadı:', error); }
try{
  const __ho240f57PrevBind = typeof bind === 'function' ? bind : null;
  bind = function(){
    if(__ho240f57PrevBind) __ho240f57PrevBind();
    document.querySelectorAll('#gameAddForm button,#gameEditForm button,.gameForm button').forEach(btn=>{
      if(btn.dataset.action && /find-covers|refetch-release|refetch-genres|description|story|fetch-all|auto-game-meta|estimate-playlist|reset/i.test(btn.dataset.action)) btn.type = 'button';
    });
  };
}catch(error){ console.warn('FIX57 bind atlandı:', error); }
try{
  if(Array.isArray(VERSION_NOTES_ARCHIVE) && !VERSION_NOTES_ARCHIVE.some(n=>n.version===HO240F57_INTERNAL_VERSION)){
    VERSION_NOTES_ARCHIVE.unshift({version:HO240F57_INTERNAL_VERSION,title:'Kapak ve Çıkış Tarihi Motoru Genişletildi',summary:'Assassin’s Creed 1 / Director’s Cut için doğru kapak ve 13.11.2007 çıkış tarihi kesin kataloğa eklendi. Kapak, tarih, tür ve tüm bilgileri çekme butonları oyun adını bozmadan forma işler.',image:'previews/hayatimiz-oyun-v241-fix57-kapak-tarih-motoru.png',written:'FIX57 ile kapak/tarih motoru genişletildi. Assassin’s Creed serisi için kesin tarih-kapak kayıtları eklendi; API tarafında Steam, RAWG ve internet/Google tarzı görsel araması yedek olarak kullanılır. AI/deploy/redeploy sistemi eklenmedi.'});
  }
  localStorage.setItem('hayatimiz_last_public_version', HO240F57_PUBLIC_VERSION);
  localStorage.setItem('hayatimiz_last_fix_version', HO240F57_INTERNAL_VERSION);
  document.title = `Hayatımız Oyun - ${HO240F57_PUBLIC_VERSION}`;
}catch(error){ console.warn('FIX57 final atlandı:', error); }
