...
const FIX_NAME = 'v2.1.8 - Yetki Paneli ve Kullanıcı Yönetimi';
...
const DEFAULT_NOTES = [
  ...
  {id:'v218-yetki-paneli-kullanici-yonetimi',version:'v2.1.8',title:'👑 Yetki Paneli ve Kullanıcı Yönetimi',summary:'Kullanıcılar ve Yetkiler ekranı profesyonel hale getirildi. Kayıtlı kullanıcılar, Supabase yetki kayıtları, rol özetleri, arama/filtreleme, Türkçe rol seçimi, kullanıcı ekleme ve yetki kaydetme akışı güçlendirildi.',status:'Tamamlandı'},
...
function home(){
  ...
  return layout(`<section class="homeV214" id="top"><section class="homeV214Hero"><div class="homeV214Glow"></div><div class="homeV214Copy"><span class="badge green">🛠️ ${VERSION} FIX • Ana sayfa takvim ve hikaye düzeltmesi</span><h1>Hayatımız Oyun arşivi daha profesyonel, daha anlaşılır ve izlemeye hazır.</h1><p>Seri adı çekme, hikaye metni, ana sayfa yayın takvimi ve alfabetik harfe git düzeni güçlendirildi. Yönetim ve rehber bağlantıları düzenli şekilde korunur.</p><form class="homeV214Search" data-search-form><span>🔎</span><input name="q" placeholder="Oyun, seri, tür veya etiket ara..."><button class="btn primary" type="submit">Arşivde Ara</button></form><div class="homeV214Actions"><a class="btn primary" href="/oyun-arsivi?sort=az">🎮 Oyun Arşivi</a><a class="btn secondary" href="/seriler?sort=az">🎬 Seriler</a><a class="btn secondary" href="/site-rehberi">📘 Site Rehberi</a><a class="btn secondary" href="/yayin-takvimi">📅 Yayın Takvimi</a></div></div><aside class="homeV214Feature"><div class="featurePoster"><img src="${esc(featured.cover||'/assets/hayatimiz-kapak.png')}" onerror="this.src='/assets/hayatimiz-kapak.png'" alt="${esc(featured.title)}"><span class="pill ${statusClass(featured.status)}">${esc(featured.status||'Arşiv')}</span></div><h2>${esc(featured.title)}</h2><p>${esc(shortStoryForGame(featured))}</p><div class="progressMini"><i><span style="width:${featuredPct}%"></span></i><small>${featuredWatched}/${featuredTotal||0} bölüm • %${featuredPct} takip</small></div><div class="cardActions"><a class="miniBtn primary" href="/izle?id=${encodeURIComponent(featured.id)}">▶️ İzle</a><a class="miniBtn" href="/oyun-detay?id=${encodeURIComponent(featured.id)}">📖 Detay</a></div></aside></section><section class="homeV214Stats"><article><b>${games.length}</b><span>🎮 Oyun</span><small>Kalıcı arşiv</small></article><article><b>${series.size}</b><span>🎬 Seri</span><small>Doğru seri adı</small></article><article><b>${episodeTotal}</b><span>▶️ Bölüm</span><small>${watchedTotal} izlendi</small></article><article><b>%${controlScore}</b><span>📡 Site Sağlığı</span><small>Güvenli mod</small></article><article><b>${notes.length}</b><span>📝 Not</span><small>Güncelleme merkezi</small></article></section><section class="homeV214Quick">${oldHomeAction('/oyun-arsivi?sort=az','🔤','A-Z Oyunlar','A Harfinde Başlayan Oyunlar dahil')}${oldHomeAction('/seriler?sort=az','🎬','Seriler','Seri içi oyun sırası')}${oldHomeAction('/koleksiyonlar','🗂️','Koleksiyonlar','Seri ve tür grupları')}${oldHomeAction('/status','📡','Site Durumu','Bakım ve veri sağlığı')}${oldHomeAction('/yayin-takvimi','📅','Yayın Takvimi','Public yayın planı')}${isYönetim()?oldHomeAction('/yonetim','🛡️','Yönetim Paneli','Admin araçları'):oldHomeAction('/giris-yap','🔐','Giriş Yap','Hesabına giriş yap')}</section>${calendarHomeBlock(events)}<section class="homeV214Grid"><div class="homeV214Main"><div class="sectionHead"><div><h2>✨ Öne Çıkan Oyunlar</h2><p>Hikaye, seri adı ve izleme bağlantıları düzeltilmiş profesyonel kartlar.</p></div><a class="btn secondary" href="/oyun-arsivi?sort=az">Tümünü Gör</a></div><div class="homeV214Cards">${latestGames.length?latestGames.map(oldHomeTile).join(''):'<div class="empty">Henüz oyun yok.</div>'}</div><div class="sectionHead"><div><h2>🎬 Devam Eden Seriler</h2><p>Seriler doğru adla çekilir ve tüm seri izleme bağlantısı korunur.</p></div><a class="btn secondary" href="/seriler?sort=az">Serileri Aç</a></div><div class="homeV214Continue">${continued.length?continued.map(oldContinueCard).join(''):'<div class="empty">Devam eden seri bulunmadı.</div>'}</div></div><aside class="homeV214Rail"><article class="homeV214Panel"><h3>📊 Arşiv Özeti</h3>${statusBar('Devam Eden',active,games.length,'green')}${statusBar('Tamamlanan',completed,games.length,'green')}${statusBar('Plan/Yakında',planned,games.length,'amber')}<p class="muted">${genres.size} kategori • ${model.collections.size} koleksiyon</p></article><article class="homeV214Panel"><h3>📝 Son Güncelleme</h3><span class="pill green">${esc(latestNote.version||VERSION)}</span><h4>${esc(latestNote.title||'Güncelleme')}</h4><p class="muted">${esc(latestNote.summary||'')}</p></article><article class="homeV214Panel"><h3>🎬 Popüler Seriler</h3>${topSeries.length?topSeries.map(([name,rows])=>`<a class="homeSeriesLine" href="/seriler?sort=az#seri-harf-${encodeURIComponent(alphaKey(name))}"><b>${esc(name)}</b><span>${rows.length} oyun</span></a>`).join(''):'<p class="muted">Henüz seri yok.</p>'}</article></aside></section></section>`);
}
