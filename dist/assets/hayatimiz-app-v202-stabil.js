...
const NOTES_KEY = 'hayatimiz_update_notes_local_v202';
const LEGACY_NOTES_KEYS = ['hayatimiz_update_notes_local_v254f8','hayatimiz_update_notes_local_v251','hayatimiz_update_notes_local'];
const MAINTENANCE_KEY = 'hayatimiz_maintenance_cache_stable';

const DEFAULT_GAMES = [
  {id:'alan-wake-remastered',title:'Alan Wake Remastered',status:'Devam Eden',genre:'Korku',seriesName:'Alan Wake',cover:'/assets/alan-wake-night-springs.png',releaseDate:'2010',description:'Korku ve hikaye odaklı yayın arşivi.'},
  {id:'assassins-creed-directors-cut',title:'Assassin’s Creed Director’s Cut',status:'Tamamlanan',genre:'Aksiyon',seriesName:'Assassin’s Creed',cover:'/assets/assassins-creed-directors-cut.png',releaseDate:'2008',description:'Tamamlanan seri arşivi ve bölüm takibi.'},
  {id:'hayatimiz-oyun-arsiv',title:'Hayatımız Oyun Arşivi',status:'Yakında',genre:'YouTube Arşivi',seriesName:'Genel Arşiv',cover:'/assets/hayatimiz-kapak.png',releaseDate:'2026',description:'YouTube playlist, bölüm ve oyun koleksiyonu merkezi.'}
];
const DEFAULT_NOTES = [
...
function nav(){
  ...
  return `<aside class="side"><div class="brand"><span class="logo">🎮</span><span><b>Hayatımız <em>Oyun</em></b><small>${VERSION} • stabil çalışan site</small></span></div><nav class="nav">${items.map(([href,label])=>`<a class="${active(href)?'active':''}" href="${href}">${label}</a>`).join('')}</nav><button data-action="hard-refresh">Sayfayı Yenile</button><div class="sideFooter"><b class="okText">Boş ekran fix aktif</b><br>Kırık eski açılış dosyası devre dışı. Bu sürüm doğrudan çalışan stabil kabukla açılır.</div></aside>`;
}
function layout(content){ return `<main class="app">${nav()}<section class="main">${content}</section></main>`; }
function hero(title, text, badge='v2.0.2 • stabil'){
...
function gameCard(g){
  const cls = String(g.status).toLowerCase().includes('tamam') ? 'green' : String(g.status).toLowerCase().includes('yak') ? 'amber' : '';
  return `<article class="gameCard"><img src="${esc(g.cover)}" alt="${esc(g.title)}" onerror="this.src='/assets/hayatimiz-kapak.png'"><div class="body"><span class="pill ${cls}">${esc(g.status)}</span><h3>${esc(g.title)}</h3><p>${esc(g.description)}</p><p class="muted" style="margin-top:10px">${esc(g.genre)} ${g.seriesName?'• '+esc(g.seriesName):''} ${g.releaseDate?'• '+esc(g.releaseDate):''}</p></div></article>`;
}
function home(){
  ...
  return layout(`${hero('Hayatımız Oyun Arşivi','Ana sayfa artık siyah/boş ekranda kalmaz. Eski dolu v2.0.0 tabanındaki oyun arşivi, admin bağlantıları, takvim, notlar ve bakım modu stabil kabukla açılıyor.')}
  <section class="stats"><article class="stat"><b>${games.length}</b><span>Oyun</span></article><article class="stat"><b>${series.size}</b><span>Seri</span></article><article class="stat"><b>${genres.size}</b><span>Kategori</span></article><article class="stat"><b>${VERSION}</b><span>Sürüm</span></article></section>
  <div class="sectionHead"><div><h2>Öne Çıkan Oyunlar</h2><p>Kayıtlı oyun yoksa güvenli demo kartları görünür.</p></div><a class="btn secondary" href="/oyun-arsivi">Tümünü Gör</a></div><section class="grid">${games.slice(0,3).map(gameCard).join('')}</section>
  <section class="panels"><div class="panel"><h2>Son Güncelleme Notları</h2><div class="miniList">${notes.slice(0,4).map(n=>`<article class="note"><span class="pill green">${esc(n.version||VERSION)}</span><h3>${esc(n.title)}</h3><p>${esc(n.summary||n.description||'')}</p></article>`).join('')}</div></div><div class="panel"><h2>Yaklaşan Yayınlar</h2><div class="miniList">${events.length?events.slice(0,4).map(e=>`<article class="event"><span class="pill">${esc(e.date||'Tarih yok')}</span><h3>${esc(e.title||'Yayın')}</h3><p>${esc(e.time||'20:00')} • ${esc(e.note||e.type||'')}</p></article>`).join(''):'<div class="empty">Henüz takvim kaydı yok.</div>'}</div></div></section>`);
}
function archive(){
  ...
  return layout(`<div class="sectionHead"><div><h2>Oyun Arşivi</h2><p>Arama, durum filtresi ve kart görünümü stabil çalışır.</p></div><a class="btn primary" href="/yonetim/oyun-ekle">Yeni Oyun Ekle</a></div><form class="toolbar" data-search-form><input class="input" name="q" style="max-width:360px" placeholder="Oyun, tür, seri ara" value="${esc(q)}"><select name="status" style="max-width:220px">${statuses.map(s=>`<option ${s===f?'selected':''}>${esc(s)}</option>`).join('')}</select><button class="btn primary" type="submit">Filtrele</button></form>${filtered.length?`<section class="grid">${filtered.map(gameCard).join('')}</section>`:'<div class="empty">Bu filtreyle oyun bulunamadı.</div>'}`);
}
function seriesPage(){
...
function admin(){
  ...
  return layout(`<div class="sectionHead"><div><h2>Yönetim Paneli</h2><p>Admin panel artık siyah ekranda kalmadan açılır.</p></div><span class="badge">${VERSION}</span></div><section class="adminGrid">${cards.map(([href,t,d])=>`<a class="adminCard" href="${href}"><b>${esc(t)}</b><span>${esc(d)}</span></a>`).join('')}</section><section class="panels"><div class="panel"><h2>Sistem Durumu</h2><p class="okText">Ana sayfa, yönetim, oyun ekleme, takvim, notlar ve bakım modu stabil kabukta aktif.</p><p class="muted">Supabase bağlı değilse kayıtlar tarayıcı localStorage içinde korunur.</p></div><div class="panel"><h2>Sonraki Fix</h2><p>v2.0.3: Oyun ekle/düzenle akışını RAWG ve YouTube alanlarıyla ayrıntılı güçlendirme.</p></div></section>`);
}
function gameForm(){
  return layout(`<div class="sectionHead"><div><h2>Oyun Ekle</h2><p>Form kaydedince oyun arşivine eklenir. Veri localStorage içinde saklanır.</p></div></div><form class="panel formGrid" data-game-form><label class="field">Oyun Adı<input class="input" name="title" required placeholder="Örn: Alan Wake 2"></label><label class="field">Durum<select name="status"><option>Devam Eden</option><option>Tamamlanan</option><option>Yakında</option><option>Ara Verildi</option></select></label><label class="field">Tür / Kategori<input class="input" name="genre" placeholder="Korku, Aksiyon, RPG"></label><label class="field">Seri Adı<input class="input" name="seriesName" placeholder="Örn: Alan Wake"></label><label class="field">Çıkış Tarihi<input class="input" name="releaseDate" placeholder="2023"></label><label class="field">Kapak URL<input class="input" name="cover" placeholder="https://... veya /assets/hayatimiz-kapak.png"></label><label class="field full">YouTube Playlist URL<input class="input" name="youtubePlaylistUrl" placeholder="https://youtube.com/playlist?list=..."></label><label class="field full">Açıklama<textarea name="description" placeholder="Oyun açıklaması"></textarea></label><div class="full actions"><button class="btn primary" type="submit">Oyunu Kaydet</button><a class="btn secondary" href="/oyun-arsivi">Arşive Dön</a></div></form>`);
}
...
function bind(){
  ...
  document.body.addEventListener('submit', e=>{
    ...
    const gf=e.target.closest('[data-game-form]'); if(gf){ e.preventDefault(); const fd=new FormData(gf); const rows=loadGames(); const title=fd.get('title'); rows.unshift(normalizeGame({id:slugify(title),title,status:fd.get('status'),genre:fd.get('genre'),seriesName:fd.get('seriesName'),cover:fd.get('cover')||'/assets/hayatimiz-kapak.png',releaseDate:fd.get('releaseDate'),description:fd.get('description'),youtubePlaylistUrl:fd.get('youtubePlaylistUrl')},0)); saveGames(rows); toast('Oyun kaydedildi.'); setRoute('/oyun-arsivi'); return; }
...
window.addEventListener('error', ev=>{ console.error('v2.0.2 hata:', ev.message); const root=document.getElementById('root'); if(root && (root.textContent||'').trim().length<40) root.innerHTML=home(); });
