-- Hayatımız Oyun v2.5.4 FIX7 - Temiz Admin Takvim / Notlar / Oyun Ekle Kesin Fix
...
select
  'v2.5.4 FIX7',
  'Temiz Takvim / Notlar / Oyun Ekle Kesin Fix',
  'Önceki v2.5.4 patch zinciri temizlendi; yayın takvimi kaybolma, oyun ekle boş sayfa ve güncelleme notu yazma sorunları düzeltildi.',
  'Açılış öncesi tek stabil admin katmanı bırakıldı.',
  'Yayın takvimi kapak ve tür çeker, güncelleme notları eklenip düzenlenebilir, oyun ekleme boş sayfaya düşmez.',
  'published',
  true,
  false,
  now(),
  now()
where not exists (select 1 from public.site_update_notes where version = 'v2.5.4 FIX7');

insert into public.site_runtime_config (key, value, updated_at)
values
(
  'update_notes_center',
  jsonb_build_object('enabled', true, 'version','v2.5.4 FIX7', 'show_on_homepage', false, 'admin_page_only', true, 'editable', true, 'clean_layer', true, 'updated_at', now()),
  now()
),
(
  'calendar_events_standard',
  jsonb_build_object('enabled', true, 'version','v2.5.4 FIX7', 'mode','manual', 'auto_create', false, 'auto_cover_from_games', true, 'clean_layer', true, 'updated_at', now()),
  now()
),
(
  'schema_version',
  jsonb_build_object('version','v2.5.4 FIX7', 'status','Hayatımız Oyun v2.5.4 FIX7 schema hazır. Temiz admin takvim, notlar ve oyun ekleme kesin fix uygulandı.', 'updated_at', now()),
  now()
)
...
select 'v2.5.4 FIX7', 'Temiz admin takvim, notlar ve oyun ekleme kesin fix.', now(), now()
where not exists (select 1 from public.site_schema_versions where version = 'v2.5.4 FIX7');

select 'Hayatımız Oyun v2.5.4 FIX7 schema hazır. Temiz admin takvim, notlar ve oyun ekleme kesin fix uygulandı.' as status;
