-- Hayatımız Oyun v2.1.9 - Yayın Otomasyonu + Bildirim Merkezi
...
insert into public.site_runtime_config(key,value,updated_at)
values ('schema_version','v2.1.9',now())
on conflict (key) do update set value=excluded.value, updated_at=now();
...
select 'Hayatimiz Oyun v2.1.9 schema hazir. Bildirim merkezi, yorumlar ve toplu islem gecmisi aktif.' as status;
