...
values ('schema_version', jsonb_build_object('version','v2.1.7','applied_at', now()), now())
...
select 'Hayatimiz Oyun v2.1.7 schema hazir. Kalici bolum senkronizasyonu, seri siralama ve profesyonel arayuz aktif.' as status;
...
-- v2.1.7 kalıcı bölüm senkronizasyonu ve seri yönetimi
...
insert into public.site_runtime_config (key, value, updated_at)
values ('schema_version', jsonb_build_object('version','v2.1.7','applied_at', now(), 'note', 'Kalıcı bölüm senkronizasyonu aktif'), now())
on conflict (key) do update set value = excluded.value, updated_at = now();

insert into public.site_update_notes (version, title, summary, note, image_url, status, updated_at)
values ('v2.1.7', 'Gelişmiş Otomasyon + Kalıcı Bölüm Senkronizasyonu', 'Bölüm izlendi durumu Supabase’e kalıcı kaydedilir, playlist import değişen/yeni bölümleri karşılaştırır ve Seriler kategorisi profesyonel hale gelir.', 'v2.1.7 ile bölüm bazlı izlendi durumu tek tek kalıcı kaydedilir. Playlistten çekilen videolar eski kayıtla karşılaştırılır. Seri izleme ekranına otomatik sonraki bölüme geçme eklendi. Mobil alt menü, gelişmiş sağlık paneli, kırık kapak/boş hikaye/eksik seri sırası kontrolleri ve yönetim panelindeki sıraya göre Seriler sayfası eklendi.', 'previews/hayatimiz-oyun-v217-kalici-bolum-senkronizasyonu-preview.png', 'published', now())
