-- Hayatımız Oyun v2.5.4 FIX8 - Tam stabil site schema
...
(
  'schema_version',
  jsonb_build_object('version','v2.5.4 FIX8', 'status','Tam stabil site fix schema hazır.', 'updated_at', now()),
  now()
),
(
  'update_notes_center',
  jsonb_build_object('enabled', true, 'version','v2.5.4 FIX8', 'editable', true, 'stable_layer', true, 'updated_at', now()),
  now()
),
(
  'calendar_events_standard',
  jsonb_build_object('enabled', true, 'version','v2.5.4 FIX8', 'mode','manual', 'auto_create', false, 'updated_at', now()),
  now()
)
...
select
  ...
  'FIX8 ile tek stabil admin katmanı eklendi; site boş/siyah ekrana düşmeyecek şekilde güvenli hale getirildi.',
  ...
  now(),
  now()
where not exists (select 1 from public.site_update_notes where version = 'v2.5.4 FIX8');

insert into public.site_schema_versions (version, note, created_at, updated_at)
select 'v2.5.4 FIX8', 'Tam stabil site schema: eksik tablolar, takvim video URL, not alias, bakım koruması.', now(), now()
where not exists (select 1 from public.site_schema_versions where version = 'v2.5.4 FIX8');

select 'Hayatımız Oyun v2.5.4 FIX8 schema hazır. Mevcut bakım/oyun/kullanıcı kayıtları korunur.' as status;
