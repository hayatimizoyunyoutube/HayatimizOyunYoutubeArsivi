...
insert into public.site_runtime_config (key,value,updated_at)
values ('schema_version', jsonb_build_object('version','v2.2.1','applied_at', now(), 'note', 'v2.2.1 plan uygulandı: istek/hata durum yönetimi, çözüm notu, takvim görünüm geçişleri, sürükle-bırak seri sıralama, görünüm modları ve spoilersız hikaye çekme'), now())
on conflict (key) do update set value=excluded.value, updated_at=now();
...
select 'Hayatimiz Oyun v2.2.1 schema hazir. Plan uygulamasi aktif.' as status;
