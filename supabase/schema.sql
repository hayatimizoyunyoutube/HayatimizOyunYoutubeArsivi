...
select 'Hayatimiz Oyun v2.4.4 schema hazir. Magaza kaynak kontrolu, sosyal medya ve YouTube tablo senkronizasyonu aktif.' as status;
...
insert into public.site_runtime_config(key, value, updated_at) values (
  ...
  jsonb_build_object(
    ...
    'progress', 0,
    'version', 'v2.4.6',
    'notesText', 'v2.4.6 - Seri İzleme Ekranı ve Profesyonel Bakım Merkezi: Seri izleme ekranı, bakım modu ve güncelleme notları profesyonelleştirildi.\nv2.4.5 - Profesyonel Oyun Formu ve Bakım Merkezi: Oyun formu, YouTube tablosu ve toplu silme eklendi.\nv2.4.4 - Mağaza, Sosyal Medya ve YouTube Doğrulama: Mağaza kaynak kontrolü ve sosyal medya yönetimi eklendi.\nv2.4.3 - İlk Kullanım Rehberi: Site tanıtım rehberi ve hikaye açıklaması iyileştirildi.\nv2.4.2 - Yayın Öncesi Test Merkezi: Açılışa hazır/hazır değil kontrol merkezi eklendi.'
  ),
  now()
)
on conflict (key) do update set value = excluded.value, updated_at = now();
