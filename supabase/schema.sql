...
values (
  ...
  jsonb_build_object(
    'version','v2.4.9 FIX3',
    'status','Hayatımız Oyun v2.5.2 FIX10 schema hazır. Ana güncelleme ve Duyuru Merkezi görünür hale getirildi.',
    'updated_at',now()
  ),
  now()
)
...
select 'Hayatımız Oyun v2.5.2 FIX10 schema hazır. Ana güncelleme ve Duyuru Merkezi görünür hale getirildi.' as status;
...
-- v2.4.9 FIX6 - Harfe Git kesin profesyonel panel
create table if not exists public.site_schema_versions (
  id bigserial primary key,
  version text,
  note text,
...
delete from public.site_schema_versions where version = 'v2.4.9 FIX6';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.4.9 FIX6',
  'Seriler ve videolar Harfe Git paneli A-B-C-D sıralı profesyonel chip düzene alındı.',
  now(),
  now()
);
...
delete from public.site_update_notes where version = 'v2.4.9 FIX6';
insert into public.site_update_notes (version, title, summary, note, description, status, created_at, updated_at)
values (
  'v2.4.9 FIX6',
  'Harfe Git Kesin Düzeltme',
  'Seriler ve videolar sayfasındaki eski harf grubu alanı kaldırıldı.',
  'A-B-C-D sıralı profesyonel chip panel eklendi; harfe tıklayınca ana sayfaya atma engellendi.',
  'Videolar ve seriler aynı sayfa içinde alfabetik filtrelenir.',
  'Tamamlandı',
  now(),
  now()
);
...
-- v2.4.9 FIX7 - DLC yazısı kaldırma ve kapak düzeni
create table if not exists public.site_schema_versions (
  id bigserial primary key,
  version text,
  note text,
...
delete from public.site_schema_versions where version = 'v2.4.9 FIX7';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.4.9 FIX7',
  'DLC yazıları kaldırıldı; seri içi küçük kapak ve oyun listesi düzeni iyileştirildi.',
  now(),
  now()
);
...
delete from public.site_update_notes where version = 'v2.4.9 FIX7';
insert into public.site_update_notes (version, title, summary, note, description, status, created_at, updated_at)
values (
  'v2.4.9 FIX7',
  'DLC Yazısı ve Kapak Düzeni',
  'Seri ve video kartlarında DLC yazıları kaldırıldı; seri içi kapak listesi temizlendi.',
  'Küçük kapaklar daha düzgün sığacak şekilde ayarlandı. DLC rozeti ve metinleri gösterilmez.',
  'Serideki oyunlar listesi daha okunaklı ve kompakt hale getirildi.',
  'Tamamlandı',
  now(),
  now()
);
...
delete from public.site_schema_versions where version = 'v2.4.9 FIX8';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.4.9 FIX8',
  'Kapak galerisine Tüm Kayıtları Sil ve RAWG Oyunlarını Çek araçları eklendi.',
  now(),
  now()
);
...
delete from public.site_update_notes where version = 'v2.4.9 FIX8';
insert into public.site_update_notes (version, title, summary, note, description, status, created_at, updated_at)
values (
  'v2.4.9 FIX8',
  'Kapak Galerisi RAWG ve Temizleme',
  'Kapak seçiciye Tüm Kayıtları Sil ve RAWG Oyunlarını Çek butonları eklendi.',
  'Tüm Kayıtları Sil sadece kapak galeri sonuçlarını temizler. RAWG Oyunlarını Çek oyun adına göre en doğru 6 kapak önerisini getirir.',
  'Seçilen kapak canlı önizlemeye işlenir; kalıcı kayıt için Kaydet/Güncelle gerekir.',
  'Tamamlandı',
  now(),
  now()
);
...
delete from public.site_schema_versions where version = 'v2.4.9 FIX9';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.4.9 FIX9',
  'RAWG/Google/Steam kapak sonuçları oyun adına göre sıkı filtrelenir; yanlış oyun/DLC kapakları elenir.',
  now(),
  now()
);
...
delete from public.site_update_notes where version = 'v2.4.9 FIX9';
insert into public.site_update_notes (version, title, summary, note, description, status, created_at, updated_at)
values (
  'v2.4.9 FIX9',
  'RAWG Doğru Kapak Filtresi',
  'RAWG/Google/Steam kapakları oyun adına göre sıkı filtrelenir.',
  'Alan Wake 2 gibi oyunlarda farklı seri, remastered veya DLC kapakları elenir; güvenilir sonuç yoksa yanlış kapak otomatik basılmaz.',
  'Oyun adında sayı varsa sonuç başlığında aynı sayı aranır. Ana oyunlarda DLC/episode sonuçları filtrelenir.',
  'Tamamlandı',
  now(),
  now()
);
...
-- v2.5.0 - Supabase veri senkronizasyonu ve puan seçimi
create table if not exists public.site_schema_versions (
  id bigserial primary key,
  version text,
  note text,
...
delete from public.site_schema_versions where version = 'v2.5.0';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.0',
  'Supabase veri senkronizasyon merkezi ve otomatik/manuel puan seçimi eklendi.',
  now(),
  now()
);
...
delete from public.site_update_notes where version = 'v2.5.0';
insert into public.site_update_notes (version, title, summary, note, description, status, created_at, updated_at)
values (
  'v2.5.0',
  'Supabase Veri Senkronizasyonu',
  'Oyunlar, seriler, videolar, playlistler, bakım modu ve güncelleme notları için senkronizasyon merkezi eklendi.',
  'Demo veri ile gerçek veri ayrımı görünür hale getirildi. Oyun formuna otomatik/manuel puan seçimi eklendi.',
  'Bakım modu mevcut Supabase değeri korunur; schema çalışınca overwrite edilmez. Manuel puan seçildiğinde yazılan puan otomatik çekme işlemlerinde silinmez.',
  'Tamamlandı',
  now(),
  now()
);
...
-- Bakım modu mevcutsa korunur; sadece kayıt hiç yoksa varsayılan eklenir.
insert into public.site_runtime_config (key, value, updated_at)
select
  'maintenance_mode',
  jsonb_build_object(
  ...
  now()
where not exists (
  select 1 from public.site_runtime_config where key = 'maintenance_mode'
);
...
values (
  ...
  jsonb_build_object(
    'version','v2.5.0',
    'status','Supabase senkronizasyon merkezi hazır.',
    'demo_data_separated', true,
    'manual_score_supported', true,
    'updated_at',now()
  ),
  now()
)
...
delete from public.site_schema_versions where version = 'v2.5.0 FIX1';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.0 FIX1',
  'Supabase Verilerini Yenile işlemi güvenli hale getirildi; boş/eksik veriyle kapak, seri, bölüm ve puan alanları ezilmez.',
  now(),
  now()
);
...
delete from public.site_update_notes where version = 'v2.5.0 FIX1';
insert into public.site_update_notes (version, title, summary, note, description, status, created_at, updated_at)
values (
  'v2.5.0 FIX1',
  'Güvenli Supabase Yenileme',
  'Supabase Verilerini Yenile işlemi kapak, seri ve kayıtları bozmayacak şekilde güvenli hale getirildi.',
  'Yenilemeden önce yerel yedek alınır. Supabase boş/eksik veri döndürürse mevcut kayıtlar korunur.',
  'Kapak, seri, bölüm, playlist, puan ve açıklama alanları boş veriyle ezilmez. Yönetim paneline Son Yerel Yedeği Geri Al butonu eklendi.',
  'Tamamlandı',
  now(),
  now()
);
...
values (
  ...
  jsonb_build_object(
    'enabled', true,
    'version','v2.5.0 FIX1',
    'description','Boş veya eksik Supabase verisi mevcut oyun kayıtlarını ezmez.',
    'updated_at',now()
  ),
  now()
)
...
-- v2.5.0 FIX2 - Manuel puan stabil düzeltmesi
create table if not exists public.site_schema_versions (
  id bigserial primary key,
  version text,
  note text,
...
delete from public.site_schema_versions where version = 'v2.5.0 FIX2';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.0 FIX2',
  'Manuel puan alanı stabil hale getirildi; kapak, Steam, tüm bilgileri çek ve kaydet işlemlerinde manuel puan korunur.',
  now(),
  now()
);
...
delete from public.site_update_notes where version = 'v2.5.0 FIX2';
insert into public.site_update_notes (version, title, summary, note, description, status, created_at, updated_at)
values (
  'v2.5.0 FIX2',
  'Manuel Puan Stabil',
  'Manuel puan yazma alanı stabil hale getirildi.',
  'Manuel kullan seçildiğinde yazılan puan score alanına canlı işlenir. Kapak seçimi, Steam Kontrol, Tüm Bilgileri Çek ve Kaydet/Güncelle işlemlerinde korunur.',
  'Puan alanındaki disabled/readonly kilitleri otomatik kaldırılır; canlı önizleme manuel puan değişince güncellenir.',
  'Tamamlandı',
  now(),
  now()
);
...
delete from public.site_schema_versions where version = 'v2.5.0 FIX3';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.0 FIX3',
  'Çift manuel puan panelleri kaldırıldı; tek yazılabilir ve stabil manuel puan paneli eklendi.',
  now(),
  now()
);
...
delete from public.site_update_notes where version = 'v2.5.0 FIX3';
insert into public.site_update_notes (version, title, summary, note, description, status, created_at, updated_at)
values (
  'v2.5.0 FIX3',
  'Tek Manuel Puan Paneli',
  'Çift görünen manuel puan alanları kaldırıldı.',
  'Tek, profesyonel ve yazılabilir manuel puan paneli eklendi. Manuel puan score alanına canlı işlenir.',
...
delete from public.site_schema_versions where version = 'v2.5.1';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values ('v2.5.1','Güncelleme Notları Merkezi eklendi. Kullanıcı modalı/sayfası ve admin not yönetimi hazırlandı.',now(),now());
...
delete from public.site_update_notes where version = 'v2.5.1';
insert into public.site_update_notes (version, title, summary, note, description, status, pinned, created_at, updated_at)
values ('v2.5.1','Güncelleme Notları Merkezi','Kullanıcı tarafına profesyonel güncelleme notları modalı ve sayfası eklendi.','Admin panelinden not ekleme, düzenleme, silme, sabitleme ve kullanıcı önizlemesi yapılabilir.','Güncelleme notları merkezi mobil, masaüstü ve Vercel yayını için hazırlandı. Boş veri kontrolleri ve arama alanı eklendi.','Tamamlandı',true,now(),now());
...
insert into public.site_runtime_config (key, value, updated_at)
values ('schema_version',jsonb_build_object('version','v2.5.1','status','Hayatımız Oyun v2.5.2 FIX10 schema hazır. Ana güncelleme ve Duyuru Merkezi görünür hale getirildi.','updated_at',now()),now())
on conflict (key) do update set value = excluded.value, updated_at = now();

insert into public.site_runtime_config (key, value, updated_at)
values ('update_notes_center',jsonb_build_object('enabled',true,'version','v2.5.1','modal_enabled',true,'admin_enabled',true,'next_version','v2.5.2','updated_at',now()),now())
on conflict (key) do update set value = excluded.value, updated_at = now();
...
-- Bu bölüm mevcut verileri ezmez; eksik tablo/kolonları ekler.
...
-- Games tablosunda kalıcı puan ve seri sırası için güvenli kolonlar
...
-- Eski kolon adı varsa yeni alanı boş bırakma
update public.games
set manual_score = score
where manual_score is null and score is not null;

update public.games
set score_source = 'manual'
where score is not null and (score_source is null or score_source = '');

-- Bakım modu: mevcut kayıt ASLA overwrite edilmez, sadece hiç yoksa varsayılan eklenir.
insert into public.site_runtime_config (key, value, updated_at)
select
  'maintenance_mode',
  jsonb_build_object(
...
-- Bakım ekranında gösterilecek güncel son notlar
insert into public.site_runtime_config (key, value, updated_at)
values (
    ...
    (
      select coalesce(jsonb_agg(
        jsonb_build_object(
          'version', version,
          'title', title,
          'summary', coalesce(summary, note, description, ''),
          'status', coalesce(status, 'Tamamlandı')
        )
        order by created_at desc
      ), '[]'::jsonb)
      from (
        select version, title, summary, note, description, status, created_at
...
-- Güncelleme notu
delete from public.site_update_notes where version = 'v2.5.1 FIX1';
insert into public.site_update_notes (version, title, summary, note, description, status, pinned, created_at, updated_at)
values (
  'v2.5.1 FIX1',
  'Supabase Kalıcı Kayıtlar',
  'Bakım modu, güncelleme notları, manuel puan ve seri sıralaması Supabase’e kalıcı kaydedilecek şekilde düzeltildi.',
  'Schema çalıştırıldığında mevcut bakım modu overwrite edilmez. Güncelleme notları bakım ekranına son 5 sürüm olarak gelir.',
  'Manuel puan score/manual_score alanlarında korunur. Seri sırası series_order alanıyla kullanıcı tarafında doğru gösterilir.',
  'Tamamlandı',
  true,
  now(),
  now()
...
delete from public.site_schema_versions where version = 'v2.5.1 FIX1';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.1 FIX1',
  'Bakım modu, güncelleme notları, manuel puan ve seri sıralaması için kalıcı Supabase tablo/kolonları güçlendirildi.',
  now(),
  now()
);
...
values (
  ...
  jsonb_build_object(
    'enabled', true,
    'maintenance_mode', 'site_runtime_config.maintenance_mode',
    'update_notes', 'site_update_notes + site_runtime_config.update_notes_center',
    'manual_score', 'games.score + games.manual_score + games.score_source',
    'series_order', 'games.series_order',
    'version','v2.5.1 FIX1',
    'updated_at',now()
  ),
  now()
...
-- Bakım modu mevcutsa dokunma, sadece yoksa varsayılan ekle.
insert into public.site_runtime_config (key, value, updated_at)
select
  'maintenance_mode',
  jsonb_build_object(
...
delete from public.site_update_notes where version = 'v2.5.1 FIX2';
insert into public.site_update_notes (version, title, summary, note, description, status, pinned, created_at, updated_at)
values (
  'v2.5.1 FIX2',
  'Yönetim Paneli Stabilizasyonu',
  'Yönetim panelini karıştıran otomatik paneller kaldırıldı.',
  'Manuel puan paneli sadeleştirildi. Bakım modu, güncelleme notları, puan ve seri sırası için kalıcı tablo desteği korundu.',
  'Otomatik floating butonlar, senkron paneli ve admin not paneli artık yönetim ekranını birbirine sokmaz.',
  'Tamamlandı',
  true,
  now(),
  now()
...
delete from public.site_schema_versions where version = 'v2.5.1 FIX2';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.1 FIX2',
  'Yönetim paneli stabilizasyonu eklendi; kalıcı Supabase kayıt desteği korunur.',
  now(),
  now()
);
...
values (
  ...
  'system',
  'Bildirim ve Duyuru Merkezi eklendi',
  'Yeni duyurular, bakım bildirimleri, yayın haberleri ve seri/bölüm duyuruları artık tek panelden takip edilebilir.',
  'published',
  true,
  now(),
  now()
...
delete from public.site_update_notes where version = 'v2.5.2';
insert into public.site_update_notes (version, title, summary, note, description, status, pinned, created_at, updated_at)
values (
  'v2.5.2',
  'Bildirim ve Duyuru Merkezi',
  'Kullanıcı zil paneli, okundu/okunmadı sistemi ve admin duyuru yönetimi eklendi.',
  'Bakım, yayın, yeni seri, yeni bölüm ve sistem duyuruları ayrı tiplerde tutulur.',
  'Bildirim paneli mobil/masaüstü uyumlu hale getirildi. Supabase için site_notifications tablosu ve notifications_center runtime kaydı eklendi.',
  'Tamamlandı',
  true,
  now(),
  now()
...
delete from public.site_schema_versions where version = 'v2.5.2';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.2',
  'Bildirim ve Duyuru Merkezi eklendi; site_notifications tablosu ve notifications_center ayarı hazırlandı.',
  now(),
  now()
);
...
values (
  ...
  jsonb_build_object(
    'enabled', true,
    'version','v2.5.2',
    'types', jsonb_build_array('maintenance','release','series','episode','system'),
    'read_state', 'localStorage + future user profile',
    'updated_at', now()
  ),
  now()
)
...
delete from public.site_update_notes where version = 'v2.5.2 FIX1';
insert into public.site_update_notes (version, title, summary, note, description, status, pinned, created_at, updated_at)
values (
  'v2.5.2 FIX1',
  'Panel ve Bakım Notları Stabilizasyonu',
  'Yönetim panelinde karışan güncelleme/bildirim panelleri temizlendi; bakım modunda son sürüm notları kullanılacak.',
  'Güncelleme notları paneli ve duyuru admin paneli otomatik olarak yönetim paneline basılmayacak.',
  'Bakım modu güncelleme notları site_update_notes içindeki son sürümlerden güncellenir. Mevcut maintenance_mode ayarı overwrite edilmez.',
  'Tamamlandı',
  true,
  now(),
  now()
...
-- Bakım moduna son 5 güncelleme notunu yaz, ancak enabled/message/progress gibi mevcut ayarları bozma.
insert into public.site_runtime_config (key, value, updated_at)
values (
  'maintenance_update_notes',
  jsonb_build_object(
...
-- maintenance_mode varsa sadece updateNotes alanını güncelle; diğer ayarları sıfırlama.
...
-- maintenance_mode hiç yoksa varsayılanı sadece bir kere oluştur.
insert into public.site_runtime_config (key, value, updated_at)
select
  'maintenance_mode',
  jsonb_build_object(
...
delete from public.site_schema_versions where version = 'v2.5.2 FIX1';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.2 FIX1',
  'Yönetim paneli panel karmaşası temizlendi; bakım modu güncelleme notları son sürüme bağlandı.',
  now(),
  now()
);
...
delete from public.site_update_notes where version = 'v2.5.2 FIX2';
insert into public.site_update_notes (version, title, summary, note, description, status, pinned, created_at, updated_at)
values (
  'v2.5.2 FIX2',
  'Güncelleme Notları Stabil Aktif',
  'Güncelleme notları devre dışı bırakılmadan ayrı stabil panel olarak yeniden aktif edildi.',
  'Panel otomatik olarak yönetim paneline basılmaz; ayrı butonla açılır ve yönetim panelini karıştırmaz.',
  'Not ekleme, düzenleme, silme, arama ve bakım modu güncelleme notlarını son 5 sürümden güncelleme işlemleri aktiftir.',
  'Tamamlandı',
  true,
  now(),
  now()
...
values (
  ...
  jsonb_build_object(
    ...
    'version','v2.5.2 FIX2',
    'stable_panel', true,
    'auto_admin_panel_disabled', true,
    'open_with_button', true,
    'updated_at', now()
  ),
  now()
)
...
delete from public.site_schema_versions where version = 'v2.5.2 FIX2';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.2 FIX2',
  'Güncelleme notları ayrı stabil panel olarak yeniden aktif edildi; bakım modu notları son sürüme bağlandı.',
  now(),
  now()
);
...
delete from public.site_update_notes where version = 'v2.5.2 FIX3';
insert into public.site_update_notes (version, title, summary, note, description, status, pinned, created_at, updated_at)
values (
  'v2.5.2 FIX3',
  'Video İzle Sistemi Stabilizasyonu',
  'Videolar sayfasındaki İzle butonları tüm kart ve link tiplerinde stabil çalışacak şekilde düzeltildi.',
  'YouTube video ve playlist linkleri normalize edilir; data-open-url, data-video-url, href, episode linkleri ve playlistUrl alanları yakalanır.',
  'Router engeline takılmadan yeni sekmede açılır. Link yoksa kullanıcıya uyarı gösterilir.',
  'Tamamlandı',
  true,
  now(),
  now()
...
delete from public.site_schema_versions where version = 'v2.5.2 FIX3';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.2 FIX3',
  'Video İzle sistemi stabil hale getirildi.',
  now(),
  now()
);
...
values (
  ...
  jsonb_build_object(
    'enabled', true,
    'version','v2.5.2 FIX3',
    'supports', jsonb_build_array('videoUrl','playlistUrl','episode.videoUrl','data-open-url','data-video-url','youtube-id','playlist-id'),
    'updated_at', now()
  ),
  now()
)
...
-- v2.5.2 FIX4 - site_notifications UUID id fix
-- Hata düzeltmesi:
-- Supabase'te site_notifications.id kolonu uuid ise 'v252-release' metin değeri hata veriyordu.
-- Bu bölüm geçerli UUID kullanır ve mevcut bakım/güncelleme ayarlarını sıfırlamaz.
...
-- Yeni kurulumlarda id UUID olur. Var olan tablo UUID ise alttaki INSERT sorunsuz çalışır.
create table if not exists public.site_notifications (
  id uuid primary key default gen_random_uuid(),
  type text default 'system',
  title text,
...
-- UUID id ile güvenli bildirim kaydı
insert into public.site_notifications (id, type, title, message, status, pinned, created_at, updated_at)
values (
  '00000000-0000-4000-8000-000000002522'::uuid,
  'system',
  'Bildirim ve Duyuru Merkezi eklendi',
  'Yeni duyurular, bakım bildirimleri, yayın haberleri ve seri/bölüm duyuruları artık tek panelden takip edilebilir.',
  'published',
...
delete from public.site_update_notes where version = 'v2.5.2 FIX4';
insert into public.site_update_notes (version, title, summary, note, description, status, pinned, created_at, updated_at)
values (
  'v2.5.2 FIX4',
  'Schema UUID Bildirim Fix',
  'site_notifications id alanı UUID olduğu için metin id hatası düzeltildi.',
  'v252-release gibi metin id yerine geçerli UUID kullanıldı.',
  'Supabase SQL artık invalid input syntax for type uuid hatası vermez. Mevcut bakım modu ve güncelleme notları sıfırlanmaz.',
  'Tamamlandı',
  true,
  now(),
  now()
...
delete from public.site_schema_versions where version = 'v2.5.2 FIX4';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.2 FIX4',
  'site_notifications UUID id hatası düzeltildi.',
  now(),
  now()
);
...
-- v2.5.2 FIX5 - Site içi video izleme ve ana sayfa not barı düzeltmesi
...
delete from public.site_update_notes where version = 'v2.5.2 FIX5';
insert into public.site_update_notes (version, title, summary, note, description, status, pinned, created_at, updated_at)
values (
  'v2.5.2 FIX5',
  'Site İçi Video İzleme ve Not Barı Düzeltmesi',
  'Güncelleme notları ana sayfadan kaldırıldı; videolar site içi player ekranında tek kez açılır.',
  'İzle butonu YouTube’da yeni sekme açmaz, site içinde iframe izleme ekranı açar.',
  'Güncelleme Notları sadece yönetim panelinde ayrı butonla açılır. Ana sayfa görüntüsü bozulmaz.',
  'Tamamlandı',
  true,
  now(),
  now()
...
delete from public.site_schema_versions where version = 'v2.5.2 FIX5';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.2 FIX5',
  'Site içi video izleme ve ana sayfa güncelleme notları barı düzeltildi.',
  now(),
  now()
);
...
values (
  ...
  jsonb_build_object(
    'enabled', true,
    'version','v2.5.2 FIX5',
    'mode','internal_player',
    'external_youtube_new_tab', false,
    'double_open_guard', true,
    'updated_at', now()
  ),
  now()
)
...
-- v2.5.2 FIX6 - Tek bildirim ikonu düzeltmesi
...
delete from public.site_update_notes where version = 'v2.5.2 FIX6';
insert into public.site_update_notes (version, title, summary, note, description, status, pinned, created_at, updated_at)
values (
  'v2.5.2 FIX6',
  'Tek Bildirim İkonu',
  'Çift görünen bildirim ikonları temizlendi; sadece üst menüde tek bildirim ikonu kalacak.',
  'Sol tarafta/floating olarak çıkan ikinci bildirim ikonu kaldırıldı.',
  'Okunmamış bildirim sayısı üst menüdeki tek ikona taşındı. Bildirim paneli aynı ikon üzerinden açılır.',
  'Tamamlandı',
  true,
  now(),
  now()
...
delete from public.site_schema_versions where version = 'v2.5.2 FIX6';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.2 FIX6',
  'Tek bildirim ikonu düzeltmesi eklendi.',
  now(),
  now()
);
...
delete from public.site_update_notes where version = 'v2.5.2 FIX7';
insert into public.site_update_notes (version, title, summary, note, description, status, pinned, created_at, updated_at)
values (
  'v2.5.2 FIX7',
  'Yönetim Paneli Buton ve Panel Temizliği',
  'Tekrarlanan paneller kaldırıldı, admin butonları izole edildi ve bakım notları güncel sürümlere bağlandı.',
  'Kapak Galerisi Araçları kutusunun defalarca görünmesi engellendi. Son 5 Notu Getir güncel notları çeker.',
  'Bakım modu kullanıcı önizlemesindeki son güncellemeler v2.5.2 FIX7-FIX3 sürümlerinden oluşturulur. Mevcut bakım ayarları sıfırlanmaz.',
  'Tamamlandı',
  true,
  now(),
  now()
...
-- Bakım notları: eski v2.4.7 notları yerine son 5 güncel sürümü kaydet.
insert into public.site_runtime_config (key, value, updated_at)
values (
  'maintenance_update_notes',
  jsonb_build_object(
    ...
    'notes',
      jsonb_build_array(
        jsonb_build_object('version','v2.5.2 FIX7','title','Yönetim Paneli Buton ve Panel Temizliği','summary','Tekrarlanan paneller kaldırıldı, admin butonları izole edildi ve bakım notları güncel sürümlere bağlandı.','status','Tamamlandı'),
        jsonb_build_object('version','v2.5.2 FIX6','title','Tek Bildirim İkonu','summary','Çift görünen bildirim ikonları temizlendi; sadece üst menüde tek bildirim ikonu kaldı.','status','Tamamlandı'),
        jsonb_build_object('version','v2.5.2 FIX5','title','Site İçi Video İzleme ve Not Barı Düzeltmesi','summary','Videolar site içi player ekranında açılır; güncelleme notları ana sayfadan kaldırıldı.','status','Tamamlandı'),
        jsonb_build_object('version','v2.5.2 FIX4','title','Schema UUID Bildirim Fix','summary','site_notifications UUID id hatası düzeltildi.','status','Tamamlandı'),
        jsonb_build_object('version','v2.5.2 FIX3','title','Video İzle Sistemi Stabilizasyonu','summary','Videolardaki İzle butonları tüm kart ve link tiplerinde stabil çalışacak şekilde düzeltildi.','status','Tamamlandı')
      ),
    'updated_at', now()
  ),
  now()
...
-- maintenance_mode mevcut ayarlarını bozma; sadece updateNotes alanını güncelle.
update public.site_runtime_config
set
  value = jsonb_set(
    coalesce(value, '{}'::jsonb),
...
delete from public.site_schema_versions where version = 'v2.5.2 FIX7';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.2 FIX7',
  'Yönetim paneli buton/panel temizliği ve bakım güncelleme notları düzeltildi.',
  now(),
  now()
);
...
delete from public.site_update_notes where version = 'v2.5.2 FIX8';
insert into public.site_update_notes (version, title, summary, note, description, status, pinned, created_at, updated_at)
values (
  'v2.5.2 FIX8',
  'Güncelleme Notları ve Bildirim Paneli Stabil',
  'Yanlış admin panelleri güncelleme notlarından temizlendi, bildirim panelinin açılıp kapanması engellendi.',
  'Kapak Galerisi Araçları gibi kartlar artık Güncelleme Notları paneline basılmaz.',
  'Bildirim paneli admin temizleme kodları yüzünden kapanmaz. Bakım notları son sürümlere bağlanır.',
  'Tamamlandı',
  true,
  now(),
  now()
...
values (
  ...
  jsonb_build_object(
    ...
    'notes',
      jsonb_build_array(
        jsonb_build_object('version','v2.5.2 FIX8','title','Güncelleme Notları ve Bildirim Paneli Stabil','summary','Yanlış paneller güncelleme notlarından temizlendi, bildirim panelinin kapanması engellendi.','status','Tamamlandı'),
        jsonb_build_object('version','v2.5.2 FIX7','title','Yönetim Paneli Buton ve Panel Temizliği','summary','Tekrarlanan paneller kaldırıldı, admin butonları izole edildi ve bakım notları güncel sürüme bağlandı.','status','Tamamlandı'),
        jsonb_build_object('version','v2.5.2 FIX6','title','Tek Bildirim İkonu','summary','Çift görünen bildirim ikonları temizlendi; sadece üst menüde tek bildirim ikonu kaldı.','status','Tamamlandı'),
        jsonb_build_object('version','v2.5.2 FIX5','title','Site İçi Video İzleme ve Not Barı Düzeltmesi','summary','Videolar site içi player ekranında açılır; güncelleme notları ana sayfadan kaldırıldı.','status','Tamamlandı'),
        jsonb_build_object('version','v2.5.2 FIX4','title','Schema UUID Bildirim Fix','summary','site_notifications UUID id hatası düzeltildi.','status','Tamamlandı')
      ),
...
delete from public.site_schema_versions where version = 'v2.5.2 FIX8';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.2 FIX8',
  'Güncelleme notları panelinden yanlış kartlar temizlendi; bildirim panelinin kapanması engellendi.',
  now(),
  now()
);
...
-- v2.5.2 FIX9 - Temiz Güncelleme Notları ve admin bar düzeltmesi
...
delete from public.site_update_notes where version = 'v2.5.2 FIX9';
insert into public.site_update_notes (version, title, summary, note, description, status, pinned, created_at, updated_at)
values (
  'v2.5.2 FIX9',
  'Temiz Güncelleme Notları ve Admin Bar Fix',
  'Ana sayfadaki FIX bilgi barı kaldırıldı, Güncelleme Notları paneli sadece sürüm notlarını gösterecek şekilde temizlendi.',
  'Kapak Galerisi Araçları, RAWG ve Tüm Kayıtları Sil kartlarının Güncelleme Notları paneline karışması engellendi.',
  'Güncelleme Notları butonu sadece gerçek yönetim panelinde görünür. Eski debug/status barları devre dışı bırakıldı.',
  'Tamamlandı',
  true,
  now(),
  now()
...
values (
  ...
  jsonb_build_object(
    ...
    'notes',
      jsonb_build_array(
        jsonb_build_object('version','v2.5.2 FIX9','title','Temiz Güncelleme Notları ve Admin Bar Fix','summary','Ana sayfadaki FIX bilgi barı kaldırıldı, Güncelleme Notları paneli sadece sürüm notlarını gösterecek şekilde temizlendi.','status','Tamamlandı'),
        jsonb_build_object('version','v2.5.2 FIX8','title','Güncelleme Notları ve Bildirim Paneli Stabil','summary','Yanlış paneller güncelleme notlarından temizlendi, bildirim panelinin kapanması engellendi.','status','Tamamlandı'),
        jsonb_build_object('version','v2.5.2 FIX7','title','Yönetim Paneli Buton ve Panel Temizliği','summary','Tekrarlanan paneller kaldırıldı, admin butonları izole edildi ve bakım notları güncel sürüme bağlandı.','status','Tamamlandı'),
        jsonb_build_object('version','v2.5.2 FIX6','title','Tek Bildirim İkonu','summary','Çift görünen bildirim ikonları temizlendi; sadece üst menüde tek bildirim ikonu kaldı.','status','Tamamlandı'),
        jsonb_build_object('version','v2.5.2 FIX5','title','Site İçi Video İzleme ve Not Barı Düzeltmesi','summary','Videolar site içi player ekranında açılır; güncelleme notları ana sayfadan kaldırıldı.','status','Tamamlandı')
...
delete from public.site_schema_versions where version = 'v2.5.2 FIX9';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.2 FIX9',
  'Ana sayfadaki FIX barı kaldırıldı; Güncelleme Notları paneli sadece sürüm notlarını gösterecek şekilde temizlendi.',
  now(),
  now()
);
...
values (
  ...
  'Bu ana güncelleme artık Güncelleme Notları panelinde en üstte görünür.',
...
delete from public.site_update_notes where version = 'v2.5.2 FIX10';
insert into public.site_update_notes (version, title, summary, note, description, status, pinned, created_at, updated_at)
values (
  'v2.5.2 FIX10',
  'Ana Güncelleme ve Duyuru Merkezi Görünür',
  'v2.5.2 ana güncellemesi ve Duyuru Merkezi yönetim panelinde görünür hale getirildi.',
  'Güncelleme Notları paneli v2.5.2 ana güncellemesini en üstte gösterir.',
  'Yönetim paneline ayrı Duyuru Merkezi butonu eklendi; duyuru ekleme/düzenleme/silme/sabitleme görünür hale getirildi.',
  'Tamamlandı',
  true,
  now(),
  now()
...
values (
  ...
  'system',
  'v2.5.2 Bildirim ve Duyuru Merkezi yayında',
  'Duyurular, bakım mesajları, yayın haberleri, yeni seri ve yeni bölüm bildirimleri artık tek merkezden yönetilir.',
  'published',
  true,
  now(),
  now()
...
values (
  ...
  jsonb_build_object(
    'enabled', true,
    'version','v2.5.2 FIX10',
    'main_update_visible', true,
    'main_update_version', 'v2.5.2',
    'show_on_homepage', false,
    'admin_button_only', true,
    'updated_at', now()
  ),
  ...
  jsonb_build_object(
    ...
    'notes',
      jsonb_build_array(
        ...
        jsonb_build_object('version','v2.5.2 FIX10','title','Ana Güncelleme ve Duyuru Merkezi Görünür','summary','v2.5.2 ana güncellemesi ve Duyuru Merkezi yönetim panelinde görünür hale getirildi.','status','Tamamlandı'),
        jsonb_build_object('version','v2.5.2 FIX9','title','Temiz Güncelleme Notları ve Admin Bar Fix','summary','Ana sayfadaki FIX bilgi barı kaldırıldı, Güncelleme Notları paneli sadece sürüm notlarını gösterecek şekilde temizlendi.','status','Tamamlandı'),
        jsonb_build_object('version','v2.5.2 FIX8','title','Güncelleme Notları ve Bildirim Paneli Stabil','summary','Yanlış admin panelleri güncelleme notlarından temizlendi, bildirim panelinin kapanması engellendi.','status','Tamamlandı'),
        jsonb_build_object('version','v2.5.2 FIX7','title','Yönetim Paneli Buton ve Panel Temizliği','summary','Tekrarlanan paneller kaldırıldı, admin butonları izole edildi ve bakım notları güncel sürüme bağlandı.','status','Tamamlandı')
      ),
    'updated_at', now()
  ),
...
delete from public.site_schema_versions where version = 'v2.5.2 FIX10';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.2 FIX10',
  'v2.5.2 ana güncellemesi ve Duyuru Merkezi yönetim panelinde görünür hale getirildi.',
