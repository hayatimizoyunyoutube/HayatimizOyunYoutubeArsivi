...
values (
  ...
  jsonb_build_object(
    'version','v2.4.9 FIX3',
    'status','Hayatımız Oyun v2.5.2 FIX8 schema hazır. Güncelleme notları ve bildirim paneli stabil hale getirildi.',
    'updated_at',now()
  ),
  now()
)
...
select 'Hayatımız Oyun v2.5.2 FIX8 schema hazır. Güncelleme notları ve bildirim paneli stabil hale getirildi.' as status;
...
insert into public.site_runtime_config (key, value, updated_at)
values ('schema_version',jsonb_build_object('version','v2.5.1','status','Hayatımız Oyun v2.5.2 FIX8 schema hazır. Güncelleme notları ve bildirim paneli stabil hale getirildi.','updated_at',now()),now())
on conflict (key) do update set value = excluded.value, updated_at = now();
