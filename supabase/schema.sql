...
-- v2.2.0 FIX 11 - 4 kolon arsiv/seri, profesyonel form, bakım görselleri
alter table if exists public.games add column if not exists updated_at timestamptz default now();
alter table if exists public.site_runtime_config add column if not exists updated_at timestamptz default now();

insert into public.site_runtime_config (key,value,updated_at)
values ('schema_version', jsonb_build_object('version','v2.2.0 FIX 11','applied_at', now(), 'note', '4 kolon oyun arsivi/seriler, alfabetik harfe git, bos oyun ekleme formu, modal duzenleme ve profesyonel bakim/istek/hata gorunumu aktif'), now())
on conflict (key) do update set value=excluded.value, updated_at=now();

insert into public.site_update_notes (version,title,summary,note,status,created_at)
values ('v2.2.0 FIX 11','4 Kolon Arşiv + Profesyonel Form + Bakım Görsel Fix','Oyun Arşivi ve Seriler 4 kolon profesyonel yapıya alındı; Oyun Arşivi alfabetik sıralama ve Harfe Git kazandı; Oyun Ekle boş form açıyor; Mevcut Oyun Düzenle ayrı pencerede açılıyor; Oyun İste, Hata Bildir ve Bakım Modu logo/kapak görselleriyle yenilendi.','Bu fix yeni büyük güncelleme değildir. Güncellemelere geçmeden önce arşiv/seri görünümü, oyun formu, düzenleme penceresi, bakım modu ve kullanıcı bildirim ekranları stabilize edilmiştir.','published',now())
on conflict (version,title) do update set summary=excluded.summary, note=excluded.note, status='published';

select 'Hayatimiz Oyun v2.2.0 FIX 11 schema hazir. 4 kolon arsiv, alfabetik harfe git, modal duzenleme ve profesyonel bakim gorunumu aktif.' as status;
