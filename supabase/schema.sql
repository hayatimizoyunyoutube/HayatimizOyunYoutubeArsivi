-- Hayatımız Oyun v2.5.4 FIX3 - Açılış Öncesi Son Kontrol
...
select
  'v2.5.4 FIX3',
  'Açılış Öncesi Son Kontrol',
  'Boş sayfa koruması güçlendirildi, direkt yönetim URL önceliği düzeltildi ve gereksiz panel kalıntıları temizlendi.',
  'Güncelleme Notları ve Yayın Takvimi sayfaları direkt URL ile de panel sekmesiyle de boş kalmaz.',
  'Yayın takvimi manuel kayıt sistemiyle devam eder; otomatik sahte kayıt oluşturmaz.',
  'published',
  true,
  false,
  now(),
  now()
where not exists (select 1 from public.site_update_notes where version = 'v2.5.4 FIX3');

insert into public.site_runtime_config (key, value, updated_at)
values
('schema_version', jsonb_build_object('version','v2.5.4 FIX3','status','Hayatımız Oyun v2.5.4 FIX3 schema hazır. Açılış öncesi son kontrol tamamlandı.','updated_at',now()), now()),
('update_notes_center', jsonb_build_object('enabled',true,'version','v2.5.4 FIX3','show_on_homepage',false,'admin_page_only',true,'final_check',true,'updated_at',now()), now()),
('calendar_events_standard', jsonb_build_object('enabled',true,'version','v2.5.4 FIX3','mode','manual','auto_create',false,'suggestion_only',true,'final_check',true,'updated_at',now()), now())
on conflict (key) do update set value = excluded.value, updated_at = now();

insert into public.site_schema_versions (version, note, created_at, updated_at)
select 'v2.5.4 FIX3', 'Açılış öncesi son kontrol tamamlandı.', now(), now()
where not exists (select 1 from public.site_schema_versions where version = 'v2.5.4 FIX3');

select 'Hayatımız Oyun v2.5.4 FIX3 schema hazır. Açılış öncesi son kontrol tamamlandı.' as status;
