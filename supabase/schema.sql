-- Hayatımız Oyun v2.1.5 Gelişmiş Arşiv + Profesyonel UI schema
...
insert into public.site_update_notes (version, title, summary, note, image_url, status)
select 'v2.1.5', 'Gerçek Storage + Gelişmiş İzleme', 'Profil fotoğrafı gerçek Storage yüklemeye bağlandı; seriyi izle ekranı, bölüm takibi ve aktif harf görünümü eklendi.', 'v2.1.5 ile profile-photos bucket, manuel/izlenen bölüm ayrımı, seri sıra no, YouTube izleme detay ekranı, güncelleme notu düzenle/sil API uçları ve admin kapak oranı düzeltmesi tamamlandı.', 'previews/hayatimiz-oyun-v215-storage-izleme-preview.png', 'published'
where not exists (select 1 from public.site_update_notes where version='v2.1.5' and title='Gerçek Storage + Gelişmiş İzleme');
