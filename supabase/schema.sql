-- Hayatımız Oyun v2.5.2 FIX12 - Güvenli Supabase schema/migration
-- Mevcut oyunları, kapakları, bakım modu açık/kapalı durumunu, mesajı, yüzdeyi ve tahmini açılışı SIFIRLAMAZ.
-- Yalnızca gerekli kolonları/ayar kayıtlarını ekler ve updateNotes alanını günceller.
...
delete from public.site_update_notes where version in ('v2.5.2','v2.5.2 FIX12');
...
(
  'v2.5.2 FIX12',
  'Final Temiz Panel, Seri Sıralama ve Kalıcı Bakım',
  'Eski karışan paneller kapatıldı; Duyuru Merkezi, Güncelleme Notları, seri sıralaması ve bakım kayıtları stabil hale getirildi.',
  'Kapak Galerisi Araçları sadece Oyun Ekle/Düzenle formunda çalışır. Ana sayfada Güncelleme Notları barı görünmez.',
  'Seri sıralaması series_order/seriesOrder değerleriyle gösterilir. maintenance_mode mevcut ayarları korunur.',
  'Tamamlandı',
  true,
  now(),
  now()
...
values (
  '00000000-0000-4000-8000-000000002512'::uuid,
  'system',
  ...
  jsonb_build_object(
    ...
    'notes',
      jsonb_build_array(
        ...
        jsonb_build_object('version','v2.5.2 FIX12','title','Final Temiz Panel, Seri Sıralama ve Kalıcı Bakım','summary','Eski karışan paneller kapatıldı; Duyuru Merkezi, Güncelleme Notları, seri sıralaması ve bakım kayıtları stabil hale getirildi.','status','Tamamlandı'),
        jsonb_build_object('version','v2.5.2 FIX11','title','Temiz Yönetim, Seri Sıralama ve Kalıcı Bakım','summary','Eski patch katmanları temizlendi; yönetim paneli ve bakım modu güvenli hale getirildi.','status','Tamamlandı'),
...
delete from public.site_schema_versions where version = 'v2.5.2 FIX12';
insert into public.site_schema_versions (version, note, created_at, updated_at)
values (
  'v2.5.2 FIX12',
  'Final temiz panel, seri sıralaması ve kalıcı bakım modu düzeltildi.',
  now(),
  now()
);
...
values (
  ...
  jsonb_build_object(
    'version','v2.5.2 FIX12',
    'status','Hayatımız Oyun v2.5.2 FIX12 schema hazır. Final temiz panel, seri sıralama ve kalıcı bakım düzeltildi.',
    'updated_at',now()
  ),
  now()
)
...
select 'Hayatımız Oyun v2.5.2 FIX12 schema hazır. Final temiz panel, seri sıralama ve kalıcı bakım düzeltildi.' as status;
