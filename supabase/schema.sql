...
-- V2.5.1 Fix 3: Açılış öncesi boş kategoriler gizlendi, ana menü/admin panel anlaşılır kategori yapısına alındı.

-- V2.5.1 Fix 4: Beğenilen UI'ye göre ana sayfa profesyonel kart/hero yapısına, admin panel ise solda sabit dashboard yapısına güncellendi.

-- V2.5.1 Fix 5: Beğenilen profesyonel admin tone arayüzüne göre site ve yönetim paneli güncellendi; sol menü sabitlikten çıkarıldı, dashboard yeniden tasarlandı.


-- V2.7.0 Fix 4 - Temiz UI/Admin rebuild ve açılış öncesi Supabase güncellemesi
alter table site_settings add column if not exists current_version text default 'V2.7.0 Fix 4';
alter table site_settings add column if not exists final_release_version text default 'V3.0.0';
alter table site_settings add column if not exists maintenance_progress integer default 78;
alter table site_settings add column if not exists opening_message text default 'Hayatımız Oyun V3.0.0 açılışına hazırlanıyor.';
update site_settings set current_version='V2.7.0 Fix 4', final_release_version='V3.0.0', updated_at=now() where id=1;

create table if not exists game_requests (
  id bigserial primary key,
  kind text default 'oyun_iste',
  title text not null,
  type text default 'Oyun İste',
...
do $$ begin
  create policy "Public insert game requests" on game_requests for insert with check (true);
exception when duplicate_object then null; end $$;
do $$ begin
  create policy "Public read game requests" on game_requests for select using (true);
exception when duplicate_object then null; end $$;

insert into update_notes(version,type,title,body,public_visible) values
('V2.7.0 Fix 4','Fix','Temiz yönetim paneli ve site düzeni','Önceki fixlerde biriken eski panel kalıntıları temiz override katmanıyla kapatıldı; üst menü, yönetim paneli, profil, oyun ekleme ve YouTube araçları yeniden düzenlendi.', true),
('V2.7.0 Fix 4','Admin','YouTube ve otomatik kapak araçları geri bağlandı','Kanal oynatma listelerini listeleme, tümünü batch çekme, playlistten oyun oluşturma, RAWG kapak/hikaye çekme ve hata onarım paneli tek admin yapısına alındı.', true),
('V2.7.0 Fix 4','Supabase','Açılış öncesi schema güncellendi','site_settings V2.7.0 Fix 4 / V3.0.0 açılış bilgisiyle güncellendi; kullanıcı oyun isteği ve hata bildirimi için game_requests tablosu eklendi.', true)
on conflict do nothing;

-- V2.7.0 Fix 5: Menü tek satır, profesyonel yönetim paneli, profil fotoğraf dosya yükleme ve admin araçları onarımı.
