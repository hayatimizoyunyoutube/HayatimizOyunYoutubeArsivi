-- v2.1.5 Kurulum Sırası Temiz Fix: RLS ayarları 02-SUPABASE-RLS-GUVENLIK.sql dosyasındadır.
-- Hayatımız Oyun v2.1.4.4 tek Supabase schema
...
-- v2.1.4.4: Kullanıcının panelden eklediği notları silmez. Aynı version+title varsa tekrar eklemez.
...
select 'Hayatimiz Oyun v2.1.4.4 schema hazir. Sonra GitHub temiz kurulum ve Vercel Clear Build Cache yap.' as status;

-- v2.1.4.4 ek güvenli kolonlar ve hazır modüller
alter table public.games add column if not exists tags text;
alter table public.games add column if not exists rawg_slug text;
...
-- v2.1.3 ek alanlar ve tablolar
alter table if exists public.games add column if not exists episode_titles jsonb default '[]'::jsonb;
alter table if exists public.games add column if not exists favorite_count integer default 0;
...
values ('v2.1.3', 'Storage + Güncelleme Notları + Seri Fix', 'Kapak yükleme, not düzenleme/silme, bakım ekranı notları, eksik meta raporu ve seri/alfabetik görünüm eklendi.', 'v2.1.3 planındaki maddeler tamamlananlara taşındı.', 'published', now())
