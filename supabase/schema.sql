...
-- FIX 3: Eski public.site_settings id primary key hatasına takılmamak için yeni güvenli tablo kullanılır.
create table if not exists public.site_runtime_config (
  key text primary key,
  value jsonb not null default '{}'::jsonb,
  updated_at timestamptz default now()
...
insert into public.site_runtime_config (key, value, updated_at) values
  ('current_version', '{"version":"v2.1.3-fix-5","name":"Kurucu Yetki + Bakım Kilidi + İşlevli Butonlar"}'::jsonb, now()),
  ('safe_fallback', '{"enabled":true,"mode":"local-json"}'::jsonb, now()),
  ('auto_fetch', '{"youtube":false,"supabase":true,"manual":true}'::jsonb, now()),
  ('register_target', '{"table":"public.site_users","enabled":true}'::jsonb, now()),
  ('maintenance_mode', '{"enabled":false,"message":"Hayatımız Oyun kısa süreli bakımda."}'::jsonb, now())
on conflict (key) do update set value = excluded.value, updated_at = now();
...
-- v2.1.3 Fix 4 admin navigation notu
insert into public.update_notes (version, title, items, date)
select 'v2.1.3 Fix 4', 'Yönetim Paneli buton/sekme fix', '["Admin sekmesi Genel Bakışa geri atmaz", "Panel içi butonlar ayrı sekmeleri açar", "Aktif sekme localStorage ile korunur"]'::jsonb, '2026-05-23'
where not exists (select 1 from public.update_notes where version = 'v2.1.3 Fix 4' and title = 'Yönetim Paneli buton/sekme fix');


-- v2.1.3 Fix 5 rol/yetki düzeltmesi
-- Eski admin rolü yeni sistemde yonetici olarak okunur. Kurucu için role='kurucu' yaz.
alter table public.site_users add column if not exists role text default 'user';
alter table public.site_users add column if not exists is_active boolean default true;
alter table public.site_users add column if not exists banned_at timestamptz;
alter table public.site_users add column if not exists ban_reason text;
...
insert into public.update_notes (version, title, items, date)
select 'v2.1.3 Fix 5', 'Kurucu yetki, bakım kilidi ve işlevli buton fix', '["Admin rolü kaldırıldı; kurucu/yönetici/moderatör/editör rolleri eklendi", "Supabase role=yonetici/admin olan hesap artık kullanıcı gibi görünmez", "Bakım modu giriş yapmayan ve normal kullanıcılara global gösterilir", "Export ve kontrol butonları gerçek işlem yapar"]'::jsonb, '2026-05-23'
where not exists (select 1 from public.update_notes where version = 'v2.1.3 Fix 5');
